...
11. 创建或更新 Argument 页，frontmatter 写入 `citation`，正文 `## 来源` 列出 `[[<论文命名>]]`；若有图像型材料，在对应论证位置写图片占位，图片路径使用第 4 步确定的最终 `<论文命名>`。
...
- 图片占位只写图号或表号、名称和预计嵌入链接；不要替用户重画图。
...
> [!example]- 图片占位
> 图X：名称  
> ![](https://img.mylikemie.icu/sources/<论文命名>/figures/<论文命名>_FigX_Descriptive_Name.png)
```
...
> [!example]- 图片占位
> 图X-X：名称  
> ![](https://img.mylikemie.icu/books/<book-folder>/figures/Figure_X-X_Descriptive_Name.png)
```

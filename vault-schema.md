...
9. 新建条目：只读取对应 `wiki/templates/template-*.md` → 按模板逻辑组织内容，先主题后时间；所有新建条目必须在当前 Argument 页至少一句话提及并链接。
10. 实证研究必须更新或新建至少一个 Method 条目，在 `## 使用此方法的研究` 加入一条方法案例，并链接当前 Argument。
11. 创建或更新 Argument 页，frontmatter 写入 `citation`，正文 `## 来源` 列出 `[[<论文命名>]]`；若有图像型材料，在对应论证位置写图片占位，图片路径使用第 4 步确定的最终 `<论文命名>`，且占位图片统一使用 `.jpg` 后缀。
...
- 遇到 figure、图片、流程图、模型图、截图表格、扫描表格等图像型材料时，不读取独立 schema；先判断图片主要服务于文献整体论证还是某个具体 wiki 条目。服务于整体论证时，写在当前 Argument 的对应论证位置；服务于具体 Concept / Theory / Method / Fact / Person 条目时，写在对应条目的相关小节中，并在 Argument 页只用文字概述或链接该条目。
- 文本表格、可复制表格、HTML 表格或可转写表格，只要对理解论证、概念、数据或分类有价值，应尽量直接读取并整理为 Markdown 表格；不要因为表格不是图像就省略。
- 图片占位必须跟随正文自然叙述放在最相关的段落之后，不要集中堆在章节、页面或条目开头；如果同一段论证涉及多张图，按正文提及顺序分别插入。
- 图片占位只写图号或表号、名称和预计嵌入链接；不要替用户重画图；占位使用 Markdown 图片嵌入 `![](...)`。若已上传图片显示过小，优先检查图片文件尺寸或站点样式，不要改成纯文本路径。占位图片链接统一使用 `.jpg` 后缀，即使原始材料或旧示例使用 `.png` / `.svg`。

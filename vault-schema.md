...
- 遇到 figure、图片、流程图、模型图、截图表格、扫描表格等图像型材料时，不读取独立 schema。
- 图片主要服务于文献整体论证时，放在当前 Argument 的对应位置；主要服务于具体 Concept / Theory / Method / Fact / Person 时，放在对应条目中，并在 Argument 页简要提及或链接该条目。
- 图片占位跟随正文叙述放在最相关段落之后，不堆在开头；使用 Markdown 嵌入 `![](...jpg)`，不要包在任何注释中。
- 文本表格、可复制表格、HTML 表格或可转写表格，只要有理解价值，应整理为 Markdown 表格。

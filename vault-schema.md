...
5. 判断条目是否已存在：
   - 读取 `wiki/index.md` 作为 Dataview 自动索引 / 快速检索入口，用于快速识别可能已存在的条目
   - `wiki/index.md` 不手动维护，不作为人工清单；若 Dataview 结果不足或存在歧义，再按条目类型搜索对应正式文件夹：
     Concept → `wiki/concepts/`
     Theory → `wiki/theories/`
     Policy / Event / Fact → `wiki/facts/`
     Person → `wiki/persons/`
     Method → `wiki/methods/`
...
10. 不再手动更新 wiki/index.md；新条目通过 Dataview 自动出现在索引页中
```
...
**关于 `wiki/index.md`：**
- `wiki/index.md` 是 Dataview 自动索引与导航入口
- 可以读取它用于快速检索已有条目，减少逐文件夹搜索成本
- 不再手动向其中添加新条目
- 若 index 中结果不足或存在歧义，再搜索对应类型文件夹确认

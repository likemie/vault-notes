---
title: Research Informed Practice Site
aliases:
  - 研究知情实践网站
summary: "英国政府推出的研究知情实践网站，为教育实践者提供研究主题与证据资源，原隶属于儿童、学校与家庭部（DCSF），后因部门改组而废弃"
type: fact
subtype: event
region: uk
fact_region: "uk"
fact_kind: "event"
fact_related_count: 3
fact_related_level: 0
fact_related_stars: "☆"
fact_related_color: "#fef3c7"
tags:
  - region/uk
  - theme/evidence-based-education
  - theme/research-practice-gap
related_concepts:
  - "[[Evidence-Based Education]]"
related_facts:
  - "[[EPPI-Centre]]"
related_arguments:
  - "[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch17]]"
confidence: medium
status: draft
created: 2026-07-14
updated: 2026-07-14
---

# Research Informed Practice Site

---

## 背景

> [!event-context] 机构创建与定位
> - **时间** 2000 年代
> - **隶属** 英国儿童、学校与家庭部（Department for Children, Schools and Families，DCSF）
> - **定位** 为教育实践者和学校提供可获取的研究证据与主题资源

研究知情实践网站（Research Informed Practice Site）是英国政府推动[[Evidence-Based Education|循证教育]]的一项具体措施，直接面向一线教师和学校管理者提供研究证据（pp.330–335）。

---

## 职能

网站汇集了按主题组织的研究证据资源，旨在降低教师获取学术研究的门槛，缩小研究与实践之间的鸿沟。在[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch17|Cohen et al. (2011, Ch17)]]列举的[[Evidence-Based Education|循证教育]]制度基础设施中，该网站是唯一由政府部门直接运营、面向一线实践者的平台。

---

## 后续发展

DCSF 于 2010 年改组为教育部（Department for Education，DfE），原网址 `www.standards.dcsf.gov.uk/research/themes` 随部门改组而废弃。相关职能并入英国教育部 GOV.UK 体系，但不再有独立的研究知情实践网站。

---

---

## 影响与评价

研究知情实践网站在 [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch17|Cohen et al. (2011, Ch17)]]列举的循证教育制度基础设施中是唯一由政府部门直接运营、面向一线教师的平台。与 [[EPPI-Centre]] 面向研究者、[[Education Endowment Foundation|EEF]] 面向资助决策不同，该网站的定位是降低教师获取研究证据的门槛。其随 DCSF 改组而消失的轨迹，也反映了英国教育行政体制频繁重组对证据基础设施稳定性的影响。

## 相关条目

> [!ref-table] 关联索引
> | 条目 | 关系 |
> |---|---|
> | [[Evidence-Based Education]] | 研究知情实践网站是循证教育的制度基础设施之一 |
> | [[EPPI-Centre]] | 同为英国循证教育的制度组成部分 |
> | [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch17\|Cohen et al. (2011, Ch17)]] | 第17章系统介绍该网站在循证教育制度网络中的角色与功能 |

---
title: Public Impact-Focused Research
aliases: ["PIR", "公共影响导向研究"]
summary: "美国公立与赠地大学协会（APLU）创建的倡议，旨在为各种社会响应型研究倡议建立统一的概念框架，推动大学扩展公共影响研究组合并识别和缓解相关障碍"
type: fact
subtype: policy
region: us
issuing_organization: "Association of Public and Land-grant Universities (APLU)"
tags:
  - region/us
  - policy/research-policy
  - theme/public-engagement
  - theme/research-impact
  - level/higher-education
related_concepts:
  - "[[Convergence Research]]"
  - "[[Responsible Innovation]]"
related_theories: []
related_methods: []
related_persons: []
related_facts: []
related_arguments:
  - "[[Argument_OxfordUIDP_2019_UIPartnerships]]"
confidence: medium
status: draft
created: 2026-06-10
updated: 2026-06-10
---

# Public Impact-Focused Research

---
## 背景

> [!info]
> 在美国，越来越多的社会响应型和影响驱动型研究倡议正在开展，但它们往往是各自独立的、各自使用自己的术语——例如围绕"大挑战"（grand challenges）、社区参与式研究（community-based participatory research）、[[Convergence Research|融合研究]]（convergence research）和高度整合的基础与响应研究（highly integrative basic and responsive research）。这种碎片化和术语不一致阻碍了系统性推广。[[Argument_OxfordUIDP_2019_UIPartnerships|(Oxford & UIDP, 2019, pp. 40-41)]]

---
## 政策文本摘要

> [!abstract]
> PIR 倡议旨在为各种社会响应型研究倡议创建一个总括性概念（overarching concept），推动一场战略性的转型运动。其核心目标：
> - 发展共同的术语和框架，使大学及其伙伴能够展示关键倡议及其社会影响
> - 鼓励大学扩展公共影响研究组合
> - 识别和传播最佳实践
> - 识别和缓解大学及伙伴开展此类活动的障碍

PIR 的核心主张是："基础"与"应用"研究的区分已经过时——问题常常不是研究是否会产生影响，而是何时产生影响。该概念试图传达：越来越多的基础和应用研究是与公众合作并为公众服务的。[[Argument_OxfordUIDP_2019_UIPartnerships|(Oxford & UIDP, 2019, pp. 40-41)]]

---
## 相关概念／政策

> [!tip]
> - [[Convergence Research]] — PIR 试图将融合研究等多种社会响应型倡议统一在一个框架下。
- [[Responsible Innovation]] — 两者共享"研究应服务于社会"的价值前提，但 PIR 侧重框架统一和制度推动，负责任创新侧重创新过程中的伦理参与。

...
| 贡献地图 | 裸 Markdown 表格（类型 / 条目 / 贡献 / 代表著作） |
...
- `[!person-profile]` 是入口模块，不写成长篇传记；三格固定回答”此人是谁（身份位置）、在知识网络中的节点功能（核心角色）、贡献在哪里（代表贡献，满行）”。
- `## 贡献地图` 直接用裸 Markdown 表格（类型 / 条目 / 贡献 / 代表著作），不加 callout 包装。只写有真实贡献的行，每行必须说明贡献关系，不只是列出关联条目。

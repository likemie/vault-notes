...
| Callout | 用途 |
|---|---|
| `[!person-profile]` | 人物档案四格，固定写身份位置、核心角色、代表贡献和阅读边界 |
...
- `[!person-profile]` 是入口模块，不写成长篇传记；四格固定回答”此人是谁（身份位置）、在知识网络中的节点功能（核心角色）、贡献在哪里、本文读哪一面”。
- `[!contribution-map]` 优先使用表格（维度 / 条目 / 贡献说明），而不是普通列表。每行必须说明此人与对应条目的贡献关系，不只是关联索引。
- `[!work-line]` 用于信息量有限时的著作线索，默认一书一条；著作 4 本以上时可在 callout 内改用表格（年份 / 著作 / 核心贡献 / 关联条目）。
- `[!thought-timeline]` 用于阶段发展清晰、资料充足时的分阶段思想发展；资料不足时降级为 `[!work-line]`，不强行分期。两者二选一，无需同时保留。

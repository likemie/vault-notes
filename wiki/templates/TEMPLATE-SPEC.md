...
- 只要与概念相关的研究包含样本、测量、频数、比例、均值、相关、回归、组间差异、显著性、置信区间、效应量或元分析结果，`## 实证数据` 就是必填章节，至少填写一张与数据类型相符的表格，不得仅在“相关研究”中概述结论。通用实证结果使用 `[!ref-table]-`，可比较效应量的一级研究同时使用 `[!effect-table]-`，元分析同时使用 `[!ma-table]-`。每行必须链接对应 Argument；原文未报告的指标写 `—`，不得自行推算。完全没有量化或实证数据时才可删除本节。

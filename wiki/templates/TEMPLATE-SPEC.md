...
- `## 贡献地图` 直接使用裸 Markdown 表格（类型 / 条目 / 贡献 / 代表著作），不加 callout 包装；只写有真实贡献的行，每行必须说明贡献关系，不只是列出关联条目。
...
- `## 核心思想` 使用 `[!claim]` 写可争辩的核心论断，用 `[!citation-card]-` 收录关键表述；不另列概念工具（贡献地图表格的"条目"列已覆盖）。

...
- 一阶元分析不使用通用 Argument 模板，改用 `template-argument-meta-analysis.md`；二阶元分析不使用通用 Argument 模板，改用 `template-argument-meta-meta-analysis.md`。
...
### Meta-analysis Argument

Meta-analysis Argument 用于直接合成原始研究的一阶元分析。它按证据综合报告流程组织，不使用通用 Argument 的“论证步骤一／二／三”。
...
- 使用 `template-argument-meta-analysis.md`；`subtype` 使用 `meta-analysis`。
- 必须填写“核心变量与调节因素操作化编码表”。每个实际类别单列一行，说明构念、类别边界、纳入／排除判据、测量或干预、编码值及代表案例。
...
### Meta-meta-analysis Argument

Meta-meta-analysis Argument 用于以一阶元分析为证据单位的二阶元分析、元—元分析或 Second-Order Meta-Analysis（SOMA）。
...
- 使用 `template-argument-meta-meta-analysis.md`；`subtype` 使用 `meta-meta-analysis`。
- 操作化编码表必须区分一阶元分析与原始研究两个证据层级，并交代一阶效应量提取口径、方法学质量、偏倚状态与原始研究重叠。
- 必须报告原始研究重叠的识别依据、重叠程度、排除／择一规则以及残余依赖的统计处理；原文未报告时明确写“未报告”，不能自行补造。
- 按“检索一阶元分析 → 二阶编码与质量评价 → 重叠审计 → 一阶效应量转换 → 二阶依赖模型 → 宏观效应与异质性 → 二阶元回归 → 偏倚和敏感性 → 宏观解释”的顺序报告。
- 同时报告一阶元分析数、二阶效应量数、去重后的原始研究数和参与者数；无法从原文确定去重数量时分别报告原文口径。
- 二阶调节分析必须保留集群数、自由度、小样本校正和置信区间信息，避免将低自由度下的数值差异写成稳定领域规律。

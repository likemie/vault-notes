...
- 论证步骤优先写成 `[!claim]` + `[!chain-link]` + 可选 `[!warrant]`：先给局部主张，再给证据到判断的链节，最后补足推理桥梁。
- 需要并列线索时使用 `[!line-a]`；需要对比或结构化展示数据时，在 callout 内部使用 Markdown 表格或卡片。
- `## 主要发现` 不超过四点；三点时按两短一长编排，更多细节放入数据卡、证据卡或正文小节。
- `## 主要发现` 后可用 `[!implication]` 收束理论、经验或研究意义。
...
- `## 核心命题` 组织机制、条件、结果或分类体系。
- `## 核心命题` 推荐使用 `[!claim]`、`[!warrant]` 和 `[!implication]` 区分命题、支撑理由和命题后果。
- `## 概念演变` 写接受史、扩展史、领域迁移或概念转向。
- `## 理论语境` 写概念与理论传统、研究范式或相关理论的关系。
- 概念或框架本身写入 `## 核心命题`、`## 核心框架` 或对应功能章节。

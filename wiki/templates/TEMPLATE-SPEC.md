...
- 信度列必须区分当前样本估计、测试手册数据和其他既有研究证据。原文没有明确说明 α、ω、ICC 或重测系数来自当前样本时，不得把它写成本研究样本的测量结果。
...
- 一项研究报告多个维度或题项的详细结果时，可在研究索引后以 `### Author（Year）` 增加一张分维度或分题项明细表。明细表只承载索引行无法清楚容纳的数据，不重复整行研究信息。

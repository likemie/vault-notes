...
- 只要与概念相关的研究包含样本、测量、频数、比例、均值、相关、回归、组间差异、显著性、置信区间、效应量、一阶元分析或二阶元分析结果，`## 实证数据` 就是必填章节，至少填写一张与数据类型相符的表格，不得仅在“相关研究”中概述结论。可比较效应量的原始研究使用 `[!effect-table]-`；一阶元分析与二阶元分析分别使用对应的 `[!ma-table]-`。只有这些专表都不适用时，才使用带 `concept-other-empirical-table-marker` 的 `[!ref-table]- 其他实证结果`，不得与专表并列造成重复。每行必须链接对应 Argument；原文未报告的指标写 `—`，不得自行推算。完全没有量化或实证数据时才可删除本节。
- 一阶元分析总体结果按结果变量逐行记录，至少包括效应指标与模型、汇总效应及其置信区间、异质性；原文报告预测区间或证据确定性时一并记录。
- 一阶元分析报告调节变量、亚组分析或元回归时，增加带 `concept-meta-moderator-table-marker` 的调节分析表，记录效应较大的条件或方向、条件效应或回归系数、正式交互或回归检验、是否预设及解释边界。不得仅凭一个亚组显著而另一个不显著断言存在条件差异。
- Concept 页不记录一阶或二阶元分析的发表偏倚、敏感性分析及详细质量评估，需要时回到对应 Argument 查阅。
- 二阶元分析使用独立的七列汇总表，总体结果与少数核心结果类型分别成行。表中合并报告证据规模（$M$：一阶元分析数；$E$：一阶汇总效应数；$k$：底层原始研究数；$N$：参与者数）、共同效应尺度与模型、二阶汇总效应及 95% CI、异质性／预测区间／正式组间检验和关键解释边界。只填写原文报告的规模数据；未能去重时不得自行推算唯一研究数或总样本。完整一阶元分析清单、检索过程、转换公式、软件参数、详细质量评分、重叠矩阵和全部调节项保留在 Argument，不默认复制到 Concept；只有重叠或依赖问题直接限制结果解释时才简要记录。

...
- 元分析与二阶元分析 Argument 必须在 `## 理论框架` 末尾保留并填写“核心变量与调节因素操作化编码表”；其他研究类型删除该模板专用节。每个实际编码类别单列一行，至少说明理论构念、操作化定义、类别边界、纳入／排除判据、典型测量或干预及代表案例，不能只罗列变量名称。
- 二阶元分析的操作化编码表还必须区分一阶元分析与原始研究两个证据层级，并交代一阶效应量提取口径、原始研究重叠识别与处理、方法学质量及偏倚编码规则。

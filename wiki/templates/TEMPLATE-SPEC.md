...
- `## 核心命题与机制` 优先使用 `[!proposition-chain]` 和 `[!mechanism-map]`。一个理论可以包含多个核心命题，必须先逐条写成完整、可判断真假的陈述，再为每条命题分别还原作者的“论证起点 → 中间推导 → 成立条件 → 推导结论”；不得用一条笼统的总链替代各命题自身的推导，也不得只列命题而不分析作者如何论证。延伸命题须注明源自哪条核心命题及其附加前提。每条抽象命题后尽量给教育研究例子或解释性补充。

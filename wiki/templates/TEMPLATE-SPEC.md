...
- `## 研究程序` 写可执行步骤，并按方法类型保留相关模块。质性方法保留材料、抽样、编码和可信度模块；量化方法保留数据结构、变量、模型、核心公式、数学原理、结果解读、诊断和软件实现模块；混合方法保留设计类型、先后顺序、整合点和整合逻辑模块。
- 复杂量化模型必须尽量写清楚公式体系、变量含义、数学原理、参数设定、权重来源、阈值或分类规则、结果解读、软件实现、敏感性和稳健性检查。典型对象包括综合评价模型、空间分析模型、面板模型、机器学习模型、效应量综合模型和耦合协调模型。
...
- `## 研究程序` 优先用 `[!proc]`；`## 资料与分析` 优先用 `[!method-panel]`；公式用 `[!formula]` 或 `[!formula-set]`；公式之后优先用 `[!math-principle]` 解释数学原理，用 `[!result-reading]` 解释结果读法，用 `[!software-impl]` 说明实现路径；阈值、指标和参数表用 `[!ref-table]`；`## 使用此方法的研究` 默认作为一句话索引。

...
- `## 方法定位` 说明认识论前提、方法层级、有效性标准和不能回答的问题。质性方法写研究者位置与可信度；量化方法写测量、模型选择和效度；复杂评价模型必须说明是否能支持因果推断。
...
- 复杂量化模型必须尽量写清楚公式体系、变量含义、数学原理、参数设定、权重来源、阈值或分类规则、结果解读、软件实现、敏感性和稳健性检查。面向新手时，不要把所有公式集中到一张大表；优先为每个关键公式单独写一个 `[!formula-step]`。
- `## 资料与分析` 是摘要表，不替代详细的质性、量化或复杂模型模块。若方法简单，可以只保留摘要表；若方法复杂，摘要表用于帮助读者快速定位。
- `## 适用场景` 写适合回答的问题类型、资料条件和不适合的问题；优先用 `[!method-fit]`。
- `## 局限性` 写偏误来源、适用边界、误用风险和补救方式；复杂量化方法必须避免把描述性指标、分类结果或空间相关解释成因果关系。
- `## 相关理论与方法` 同时放理论支撑和前置、替代、补充、子模块或扩展方法。
- `## 研究程序` 优先用 `[!proc]` 做流程总览；`## 资料与分析` 优先用 `[!method-panel]`；复杂量化公式优先用 `[!formula-step]`，把公式、数学原理、结果读法和注意事项放在同一卡片内；`[!formula-set]` 只作公式链总览；用 `[!software-impl]` 说明实现路径；阈值、指标和参数表用 `[!ref-table]`；`## 使用此方法的研究` 默认作为一句话索引。

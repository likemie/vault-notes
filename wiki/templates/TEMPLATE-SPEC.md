...
- 只有课程大纲、章节很多且信息非常规则时，才用 `[!ref-table]` 或普通表格承载章节路线。
- 主要关联条目只列 3–5 个最核心的 Concept / Theory / Method / Fact / Person。
- `## 章节概览` 每章使用 `[!chapter-learning]`、`[!knowledge-index]`、`[!learning-sequence]` 和必要的 `[!textbook-artifact]`，跟随教材自身知识推进逻辑，记录学习目标、知识索引、概念/理论/方法展开顺序，以及例子、表格、图片和练习如何帮助学习。

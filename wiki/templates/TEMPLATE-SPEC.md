...
- `## 核心命题与机制` 优先使用 `[!proposition-chain]` 和 `[!mechanism-map]`。一个理论可以包含多个核心命题；每条分别写“命题—推导—依据”，简要还原作者如何从理由推出结论。不得用一条笼统的总链替代各命题自身的推导，也不得只列命题而不分析作者如何论证。延伸命题须注明源自哪条核心命题。每条抽象命题后尽量给教育研究例子或解释性补充。

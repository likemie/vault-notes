...
- 元分析与二阶元分析均不使用通用 Argument 模板，统一改用 `template-argument-meta-analysis.md`，并按实际证据层级保留条件模块。
...
### Meta-analysis and Meta-meta-analysis Argument

该模板统一用于直接合成原始研究的元分析，以及以一阶元分析为主要证据单位的二阶元分析、元—元分析或 Second-Order Meta-Analysis。它按证据综合报告流程组织，不使用通用 Argument 的“论证步骤一／二／三”。
...
- 统一使用 `template-argument-meta-analysis.md`。直接合成原始研究时 `subtype` 使用 `meta-analysis`；以一阶元分析为主要证据单位时使用 `meta-meta-analysis`。
- “二阶元分析”描述证据层级，不自动等同于最新 SOMA，也不预设 CHE、RVE、多水平模型或任何特定统计路线。必须按原文术语、年代与实际计算方法记录，不用当前方法标准替换原文做法。
- 必须区分两张表：`[!coding-frame]` 用于结果变量和干预／暴露／预测因素的实质性操作化分类；`[!coding-protocol]` 用于研究标识、样本、设计、情境、效应量、质量与偏倚等数据提取字段。
- 原文已有操作化定义表、变量编码表、编码手册、研究特征表或数据提取表时，以原文表格为内容与分类权威：保留作者的类别、层级、字段、顺序、编码值和数值，仅为可读性调整列结构、中文译名、链接与图表解读。不得另造平行分类或重复制作同内容表格。
- 原文表格与模板骨架不一致时，优先适配原文：删除原文未使用的模板字段，保留原文特有字段；原文未报告的信息写“未报告”或省略，不能把模板建议伪装成作者实际程序。
- 原文将核心构念与研究特征放在同一张表时，可依据阅读需要拆为 `[!coding-frame]` 与 `[!coding-protocol]`，但必须保持原始字段映射清楚、数据完整且无重复；原文分别提供多张表时，原则上沿用其功能分工。
- `[!coding-frame]` 中每个实际结果类别与干预类别单列一行，说明构念、类别边界、纳入／排除判据、测量或操作规程及代表案例；不得用研究特征字段稀释核心结果与干预分类。
- `[!coding-protocol]` 只写证据单位层级的字段、允许值、混合／缺失信息处理和判定规程；结果与干预字段引用上方构念分类，不重复长篇定义。
- 按“检索与筛选 → 数据提取与质量 → 效应量与模型 → 总体效应与异质性 → 调节分析 → 偏倚与稳健性 → 实质解释”的顺序报告。
- 总体效应同时报告研究数、样本量、效应量、置信区间和异质性；原文报告预测区间时必须保留。
- 调节分析区分预设与探索性分析，不把效应量数值排序写成显著组间差异。
- 只有原文确实提出独立于统计结果的理论论证时，才在“结果解释与证据边界”下增加解释性论证小节。
- 以一阶元分析为证据单位时，操作化编码必须区分一阶元分析与底层原始研究；一阶效应量提取、综述质量、底层研究重叠、二阶抽样误差与依赖结构均为条件项，原文涉及才展开。
- 原文未识别底层研究重叠或未采用现代误差校正时，如实注明其报告状态或早期处理方式，不能补造 CCA、CHE、RVE 等程序。
- 二阶调节分析在原文提供时保留集群数、自由度、小样本校正和置信区间信息，避免将低自由度下的数值差异写成稳定领域规律。

---
title: TEMPLATE-SPEC
summary: "wiki/templates 主规范说明，定义条目类型骨架、字段标准、结构规范与写作要求"
type: template
tags:
  - wiki/template-spec
related_concepts:
...
- 人名第一次出现尽可能使用全名，中文正文采用“中文（英文）”格式，例如“哈罗德·诺亚（Harold Noah）”；后文再次出现时可自由处理，可以直接使用英文原名（如 Noah、Harold Noah 等），也可以使用中文名、姓氏或代称，无需重复标注英文或机械添加双语括号。APA citation 中的学者姓名保留英文原样，不进行汉化翻译。
...
- 减少使用破折号和不必要的引号，少用冒号，优先用短句、逗号或句号。
...
- 杜绝 AI 风格网红词汇与互联网黑话：严禁使用“跃迁”、“闭环”、“赋能”、“抓手”、“颗粒度”、“底层逻辑/底层基因”、“打法”、“破局”、“赛道”、“痛点”、“发力”、“助力/助推”等过度滥用的套话与黑话。使用质朴、严谨、具体的学术概念表达机制演变、逻辑关联与制度实践。
...
- **步骤标题主张化**：各级标题（`### 论证步骤` 与 `#### 子论点`）必须体现明确的学术主张和立场观点，直接亮明核心判断，严禁使用中立、模糊或纯描述性短语（如禁止单纯写“论证步骤一”或“历史背景”，必须直接亮明学术命题）。
- **内容围绕核心主张展开**：步骤下全部内容必须高度聚焦核心命题，系统展示证据支撑（一手文献、法案条款、实证数据等）与内在论证推导过程（思想史考证、因果机制推演、对质反驳等），杜绝机械直译或清单式平铺直叙。
- **避免大段裸文字，但保留过渡串联**：排版上杜绝数百字未加结构化的大块裸文字；同时严禁将所有文字全部塞入 Callout 导致盒子紧挨堆叠。在标题下方、不同 Callout 之间，均应保留 1–2 句精炼严谨的过渡性正文（裸文字）作为逻辑串联与推进纽带，形成“过渡铺垫 ➔ 核心组件承载 ➔ 递进深化”的优良学术阅读节奏。
- 逐步拆解论证，不跳过中间环节直接给结论。
- 抽象理论必须配例子；例子优先来自原文，原文没有时可用简短教育情境说明。
- `## 论证结构` 顶部可用 Mermaid 图呈现核心逻辑链；图后必须跟 `---` 分割线。
- `## 论证结构` 内根据论文实际论证方式综合运用多种 callout，例如 `[!claim]`、`[!warrant]`、`[!line-a]`、`[!contrast-table]`、`[!evidence-grid]`、`[!logic-map]`。`[!chain-link]` 仅用于 3–5 个链节、后项依赖前项、不可任意换序且共同指向一个结论的深层论证链；超过 5 项必须合并相邻且功能相近的链节，仍无法压缩时改用 `[!logic-map]` 总览和分段散文。普通“证据到判断”、并列发现、数据分解、时间顺序或章节内容摘要不得使用。单层证据如何支持判断优先用 `[!warrant]-`，多条并列证据用 `[!evidence-grid]`。
...
- 元分析与二阶元分析继续使用通用 `template-argument.md`；必须填写“核心变量与操作化编码表”，并按原文报告检索、效应量、异质性、调节分析及偏倚检验。
- “核心变量与操作化编码表”固定采用三列：变量维度与分类、理论构念与操作化定义、核心教学规程／典型测量工具／纳入案例。无教学干预时第三列不强行补写教学规程，只填写原文报告的测量工具、具体指标和代表性纳入研究。
- 需要并列线索时使用 `[!line-a]`；需要对比或结构化展示数据时，使用 `[!contrast-table]`、`[!evidence-grid]`、`[!ref-table]`、`[!meta-table]` 或普通 Markdown 表格。Argument 页的原文量化结果统一使用这些通用表格 callout；`[!ma-table]` 仅供 Concept 条目的 `## 实证数据` 使用，不得用于 Argument。
...
- `## 围绕概念形成的命题` 写可争辩命题，组织机制、条件、结果或分类体系。书写时须严格遵守以下规范：
  ...
  5. **命题聚焦理论推导与机制，严禁堆砌具体数据**：命题及学者观点块（`[!claim]`）的核心功能是提炼理论判定、因果机制、逻辑推导与质性解释，**严禁在命题陈述中放置具体的统计数据**（如样本量 $N$、原始研究数 $k$、效应量数值 $g/d$、组间检验量 $Q$ 或 $p$ 值等）。所有具体实证统计数据与亚组检验必须严格收拢在 `## 实证数据` 表格中，以保持理论断言的凝练性并彻底杜绝两节之间的内容冗余。
...
- 只要与概念相关的研究包含样本、测量、频数、比例、均值、相关、回归、组间差异、显著性、置信区间、效应量、一阶元分析或二阶元分析结果，`## 实证数据` 就是必填章节，至少填写一张与数据类型相符的表格，不得仅在“相关研究”中概述结论。可比较效应量的原始研究使用 `[!effect-table]-`；一阶元分析与二阶元分析分别使用对应的 `[!ma-table]-`。原文未报告可提取效应量，但提供频数、比例、均值、相关、回归系数、组间差异或检验统计量时，使用带 `concept-other-empirical-table-marker` 的 `[!ref-table]- 其他实证结果（无效应量）`。同一结果已有 $d$、$g$、OR、RR 或其他可比较效应量时不得在无效应量表重复。每行必须链接对应 Argument；原文未报告的指标写 `—`，不得自行推算。完全没有量化或实证数据时才可删除本节。
- 填写一阶或二阶元分析前，先判断当前 Concept 在该项证据中是干预因素（包括干预、暴露或预测因素）还是结果变量。角色不同的证据不得直接合并；同一 Concept 在不同研究中兼具两种角色时按角色分别成行。
- 一阶元分析使用总体结果表，至少记录原始研究数 $k$、参与者数 $N$、效应指标与模型、总体汇总效应及 95% CI、异质性／预测区间和关键解释边界。
- 一阶元分析的亚组结果使用独立可选表并采用互补维度：当前 Concept 是干预因素时记录结果变量亚组；当前 Concept 是结果变量时记录干预措施、项目模式或实施技术亚组。每个亚组同时报告原始研究数／样本量、效应及 95% CI 和正式组间检验；不得根据点估计排序或“一个显著、另一个不显著”推断亚组差异。
- Concept 页不记录一阶或二阶元分析的发表偏倚、敏感性分析及详细质量评估，需要时回到对应 Argument 查阅。
- 一阶与二阶元分析在 Concept 页默认报告汇总效应及 95% CI，不单列标准误。原文只报告 SE 而未报告 CI 时，可将 SE 写入效应单元格；仅用于逆方差加权或二阶计算的 SE 留在 Argument 方法部分。
- 二阶元分析使用独立的七列汇总表，只记录总体结果。表中合并报告证据规模（$M$：一阶元分析数；$E$：一阶汇总效应数；$k$：底层原始研究数；$N$：参与者数）、共同效应尺度与模型、二阶汇总效应及 95% CI、异质性／预测区间和关键解释边界。只填写原文报告的规模数据；未能去重时不得自行推算唯一研究数或总样本。完整一阶元分析清单、检索过程、转换公式、软件参数、详细质量评分、重叠矩阵和全部调节项保留在 Argument，不默认复制到 Concept；只有重叠或依赖问题直接限制结果解释时才简要记录。
- 二阶元分析的亚组结果使用独立可选表并采用互补维度：当前 Concept 是干预因素时记录结果变量亚组；当前 Concept 是结果变量时记录干预措施、项目模式或实施技术亚组。每个亚组同时报告效应、95% CI、证据数及正式组间检验。学科、学段、地区、年份、研究设计和质量等级等其他调节结果默认回到 Argument 查阅，除非该维度本身就是当前 Concept 的定义组成。不得根据点估计排序或“一个显著、另一个不显著”推断亚组差异。
- `## 应用案例` 默认用 callout 承载一句话索引，不展开成完整小节；详细内容回到对应 Argument。
- `[!citation-card]-` 或关键逐字引文优先集中到页面后部，不散落在各节。

### Theory

Theory 页写可作为理论框架、解释机制、分析视角、方法论框架或规范批判资源的理论、模型、框架或范式。Theory 条目要回答理论从何而来、提出者如何形成判断、后来者改变了什么、理论能够怎样组织对新现象的分析，以及哪些结论超出其解释能力。理论页不是概念定义或单篇文献摘要。
...
- summary 用一句话说明理论对象、核心判断或机制，以及可用于哪类教育研究问题；不堆代表人物和系统名称。
- ## 理论定位 使用 [!theory-position] 给出解释对象、理论问题、理论类型和知识位置；[!claim] 只概括最能组织分析的总体判断，不替代下文的命题与出处。
- ## 理论来源与形成 使用 [!theory-origin] 说明提出者与原始文本、原初问题、理论资源与材料、形成路径。要交代作者如何由既有理论、观察、案例、比较或推理得到关键区分与命题；不得把后来的应用写成原理论的提出过程。
- 后续发展使用 [!dev-timeline] 按原初版本、修订、扩展、经验检验和跨领域应用区分贡献。每个节点说明谁改动了什么，以及与前一版本的关系；单纯使用或重复验证不写成理论修订。原始文本未入库或未核对时，明确使用二手转引或标待核，不能把后续 Argument 当作已核对的原典。
- ## 关键概念与理论构件 使用 [!entry-map] 三列表格（构件 / 类型 / 在理论中的功能），只写理解命题和建立分析框架所必需的构件；类型可选概念、分类、机制、方法、框架。
- ## 核心命题 中每条命题使用一个 [!theory-proposition]，按“命题＋解释＋应用实例”写。标题写完整、可讨论的判断；解释用连贯文字说明含义、构件关系、依据与成立条件，并交代提出者、版本和来源；应用实例用一个具体教育情境说明命题怎样工作。优先采用原文或已处理研究的案例，自拟情境须标为“示例”，不能写成实证发现。不要把同一判断拆成前提、机制、条件、推论等多个碎片。命题之间确有复杂关系时可附 [!mechanism-map]-，图中关系须有文本解释和出处。
- 理论的本体论、认识论或规范立场会改变研究设计和结论判读时，按需使用 [!theory-stance]；不能直接推出的结论必须在 ## 局限性与适用边界 中说明，不因没有独立立场 callout 而省略。
- ## 转化为分析框架 是 Theory 页的主体。先用 [!theory-use] 确定适合的研究问题、分析对象与单位、证据需求和解释目标；再用 [!theory-framework] 建立四列映射：理论依据、分析维度与提问、可观察线索与材料、判读规则与边界。每一维度须追溯到命题或构件，区分分析维度、观察指标和资料来源，说明支持性模式、反例或替代解释。维度数量由理论决定，不能为凑表格而增加。
- 不同用途需要不同观察对象或判断规则时，可分别建立分析框架；规范理论、解释理论和批判视角不强行改写成同一种变量模型。按需使用 [!exegesis]- 展示多个维度如何共同分析同一现象，避免重复命题下的简短应用实例；跨维度路径清楚时可附 [!logic-map]-。
- ## 局限性与适用边界 使用 [!theory-boundary] 区分适用问题、成立条件、解释不足、研究转化困难和不能直接推出的判断。原作者自述、后续批评与本条目推断须标明归属。局限是理论解释或操作化能力的边界；学者之间尚有分歧的判断放在 ## 争议与批评，不以局限名义裁断争议。
- ## 争议与批评 保持现有 [!debates]、[!tension]、[!critique] 和 [!warning] 的用法，按争议焦点组织，不与局限性混写。
- ## 相关研究 使用 [!evidence-grid-a] 记录理论的应用、检验、修正或批评，并注明研究承担的具体作用；它是证据入口，不能替代理论来源或分析框架。
...
- 来源提供具体题项时，应在 `## 题项与作答方式` 中按维度分别建立 `###` 小节：
  - **Likert 与标准化答案题项规范**：凡是 Likert 等距量表或具备统一标准化作答选项与计分规则的题项组（如多点李克特等级、固定频度量表、统一二分类核查、统一位次排序等），一律采用 `[!seq-table]-` 自动序号表格呈现。
    - **折叠标题栏**：紧邻紫色 SEQ 徽章处使用简明**纯中文构念名称**（如 `组织行动者生态活跃度`），不加冗长英文括号，保持折叠条视觉清爽；
    - **选项与计分集中归仓**：在 Callout 内部首行集中声明 `**作答选项** ... · **计分规则** ...`，避免在每行单元格内机械重复空列与横杠；
    - **表头英文承接**：表格列首使用 `| 题项（<Construct English>） |`，前端由 CSS 注入 `# ` 渲染为 `# 题项（<Construct English>）`，既清晰引导下方的题项序号，又保留学术原版英文构念追溯；
    - **题项双行全宽排版**：题项单元格独占整表宽度，统一采用“英文原文 + `<br>` + **中文加粗权威译文**”；左侧序号由 CSS Counter 自动注入圆圈数字，Markdown 内无需维护“编号”列，亦无需在题项表内加入“来源”列（工具来源已在元数据与关联论证中统一定义）。
  - **异质选项题项**：仅当每个题项的作答选项或计分规则高度异质、各不相同时，才采用 `[!ref-table]-` 5 列表格（编号、题项、作答选项、计分、来源）。
...
- `## 核心思想` 使用 `[!claim]` 写可争辩的核心论断，用不折叠的 `[!citation-card]` 收录代表性思想引文；卡片标题直接采用具体思想主题（如“教育权作为国家公共资源”），严禁添加“关键表述一/二/三”、“关键表述：”等自我说明性标签与元叙述；不另列概念工具（条目关联表格的“条目”列已覆盖）。
- `## 影响路径` 使用 `[!influence-path]` 区分理论路径、方法路径、政策路径和跨国／跨领域传播；相关研究链接到讨论此人物或使用其思想的 Argument。
- `## 历史评价` 记录权威史学家、思想史家或后世学者对该人物的历史定位、学术定性与时代评价；直接使用不折叠的 `[!citation-card]` 关键引用卡片呈现权威定性引文与出处，标题采用“学者名论主题”，内文直接叙述观点，杜绝“关键史学定性”、“章作者指出”等自我说明性标签与元叙述；若涉及方法论或意识形态争议批评，应置于 `## 争议与批评`；无充分评价材料时可省略本节。

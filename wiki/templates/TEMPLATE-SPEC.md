...
- 信度列必须区分当前样本估计、测试手册数据和其他既有研究证据。无法确认 α、ω、ICC 或重测系数来自当前样本时，不得把它写成当前样本的测量结果。
...
- `## 使用该工具的研究` 使用 `[!ref-table]-`，每项研究单独一行，记录工具版本、样本与用途、信度、效度与可比性及实证结果，并链接对应 Argument。原文没有报告的项目写 `—`。

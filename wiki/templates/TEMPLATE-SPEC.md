...
- ## 核心命题与解释机制 中每条命题须是完整、可讨论的判断，并标明提出者、版本和来源。并列判断优先用 [!claim]；只有作者的论证确实存在必要前提与推导时才用 [!proposition-chain]。作者的理论命题、后续研究的实证发现和本条目导出的研究推论须分开。抽象命题后用简短情境说明；复杂机制可附 [!mechanism-map]-，图中关系须有文本解释和出处。
...
- 不同用途需要不同观察对象或判断规则时，可分别建立分析框架；规范理论、解释理论和批判视角不强行改写成同一种变量模型。研究示例使用 [!exegesis]-，展示从现象与问题到材料、综合判读和结论强度的完整过程；跨维度路径清楚时可附 [!logic-map]-。

...
- 表格用于需要逐行累计或横向比较的记录，包括维度、题项、信度、效度、测量不变性、使用该工具的研究和版本。表格每行只放一个题项、版本、结果或研究，不把解释性长段落塞进单元格。
...
- 跨语言、文化或群体使用时，在版本表记录版本差异，在测量属性中记录测量不变性或差异项功能。不同群体各自具有信度，不等于群体间分数可以直接比较。
- 来源提供具体题项时，应在 `## 题项与作答方式` 的题项表中尽可能逐题记录。每行写题号、原文或准确译文、所属维度、作答与计分规则及来源页码；不能确认的内容写 `—`，不得自行补写。
- `## 使用该工具的研究` 使用 `[!ref-table]-`，每项研究单独一行，记录工具版本、样本与情境、测量用途及该研究提供的测量或实证信息，并链接对应 Argument。

...
%% 每个维度建立一个独立的三级标题和题项表。按实际维度复制下面的完整单元；单维工具保留一个。
   规范：
   1. 凡是 Likert 量表或具备统一标准化作答选项与计分规则的题项组，统一采用 [!seq-table]- 自动序号表格 Callout 呈现：
      - 折叠条标题紧邻 SEQ 徽章，采用纯中文构念名称（如“循证组织文化与心态认同”），保持折叠栏视觉极简；
      - 在 Callout 内容顶部首行集中声明选项与计分（如“**作答选项** 5 点 Likert 标尺 ... · **计分规则** 正向计分”），彻底消除表内重复列；
      - 表头采用“| 题项（<Construct English>） |”，兼顾题项列表引导与原版英文构念追溯；
      - 题项独占全宽，采用“英文原文<br>**中文权威译文**”标准双行排版；左侧由 CSS Counter 自动注入圆圈序号，表内无需“编号”和“来源”列。
   2. 仅当各题选项高度异质（每题选项或分值完全不同）时，才使用 [!ref-table]- 5 列排版；访谈工具使用 [!interview-guide]；观察工具使用 [!observation-focus]。 %%

### <维度一>

> [!seq-table]- <构念名称（纯中文）>
> <span class="instrument-item-table-marker" aria-hidden="true"></span>
> **作答选项** 5 点 Likert 标尺（1 = 强烈不同意，5 = 强烈同意） · **计分规则** 正向连续计分
>
> | 题项（<Construct English>） |
> |---|
> | English item text<br>**题项中文权威译文** |

%% 若各题项作答选项或计分规则完全异质、每题各不相同时，备选 5 列结构如下：
> [!ref-table]- 题项
> <span class="instrument-item-table-marker" aria-hidden="true"></span>
>
> | 编号 | 题项 | 作答选项 | 计分 | 来源 |

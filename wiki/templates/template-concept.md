...
> | `[!citation-card]` | 关键逐字引文，附清晰归因；每节最多1–2个；统一移至页面末尾 | 页面末尾（相关案例之前）|
> | `[!tension]` | 理论争议、对立立场；两方均需充分呈现 | 争议与批评 |
...
> | `[!boundary]` | 边界划定：本概念不等于什么、适用范围之外是什么；可嵌套在定义节或 `[!line-a]` 内部 | 定义（边界子节）、核心命题内嵌 |
> | `[!contrast-table]` | 3–4 个概念/框架的多维度表格对比（`## 概念辨析` 专用） | 概念辨析 |
> | `[!col-l]` + `[!col-r]` | 两个并列子主题左右分栏，嵌套在父 callout 内自动并排；移动端堆叠 | 核心框架内两组对等概念并排 |
> | `[!evidence-grid-a]` / `[!evidence-grid-b]` | 多点并列证据，两色区分两组主题 | 应用案例、实证发现 |
> | `[!case]` | 独立政策或实证案例，与 `[!line-a]` 序号系统隔离；少于3个时直接用列表 | 应用案例 |
>
> **嵌套原则**：不超过2层嵌套；列表项内嵌套 callout 语法：`>    > [!type]`（外层 `>` + 3 空格 + 内层 `>`）。
> **可折叠规则**：任何 callout 加 `+`（默认展开可折叠）或 `-`（默认折叠），推荐用于大型 callout（`[!dev-timeline]+`、`[!stat-cards]-`）。
> **Mermaid 规则**：节点标签不使用 `
`，保持单行简短标签；流程图默认横版（`flowchart LR`）。
> **分割线语义**（`---`）：`##` 之前 → 模块级切割；`###` 之前 → 步骤间延续；`[!citation-card]` 块 → 用 `---` 与正文隔离。

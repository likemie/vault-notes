...
%% 规则：
   ...
   5. 命题聚焦理论推导、因果机制与观点辩护，严禁堆砌具体统计数据（如效应量数值 g/d、样本量 k/N、p 值等）；所有具体实证统计一律收拢在“## 实证数据”表格中记录，避免两节内容重复。 %%
...
%% 强制规则
   只要与本概念相关的研究包含样本、测量、频数、比例、均值、相关、回归、组间差异、显著性、置信区间、效应量、一阶元分析或二阶元分析结果，本节就不得删除，并且必须至少填写一张与数据类型相符的表格。
...
> |---|---|---|---|---|---|---|---|
> | [[Argument_Author_Year_Journal|Author (Year)]] | 组别、条件或干预 | 结果变量 | 各组 n 或总 N | 各组 M（SD）、比例或其他原始统计 | 原文报告的 d、g、OR、RR 或 — | CI、SE、检验量与 p 或 — | 设计类型及该结果能与不能说明什么 |
...
> |---|---|---|---|---|---|---|
> | [[Argument_Author_Year_Journal|Author (Year)]] | 干预因素／结果变量 | 干预因素条目填写结果变量亚组；结果变量条目填写干预措施／技术亚组 | 贡献该亚组的原始研究数 $k$ 与样本量 $N$ | 共同尺度下的亚组效应与 95% CI | $Q_b$、df、$p$，或正式元回归检验 | 亚组证据量、类别不平衡及能否作比较判断 |
...
> |---|---|---|---|---|---|---|
> | [[Argument_Author_Year_Journal|Author (Year)]] | 干预因素／结果变量 | 干预因素条目填写结果变量亚组；结果变量条目填写干预措施／技术亚组 | 贡献该亚组的一阶元分析数 $M$ 或一阶汇总效应数 $E$ | 共同尺度下的亚组效应与 95% CI | $Q_b$、df、$p$，或正式元回归检验 | 亚组证据量、类别不平衡及能否作比较判断 |
...
> |---|---|---|---|---|---|---|
> | [[Argument_Author_Year_Journal|Author (Year)]] | $N$、群体、地区或时期 | 调查、观察、纵向或其他设计 | 核心变量、测量或指标 | 频数、比例、均值、相关、回归系数、组间差异或检验统计量 | SE、CI、$p$ 或 — | 该结果能与不能说明什么 |

%% 原始研究报告组间比较、干预结果或效应量时使用 [!effect-table]，每个结果变量单独一行。优先保留原文报告的组别统计、效应量和显著性或不确定性；原文未报告的指标写 —，不得根据均值、标准差或样本量自行推算。
   填写一阶或二阶元分析前，先确定当前 Concept 在该项证据中是“干预因素”（包括干预、暴露或预测因素）还是“结果变量”。角色不同的研究不得直接合并；同一 Concept 兼具两种角色时按角色分别成行。
   一阶元分析主表只记录总体结果。一阶亚组独立成表并采用互补维度：当前 Concept 是干预因素时记录结果变量亚组；当前 Concept 是结果变量时记录干预措施、项目模式或实施技术亚组。必须同时报告亚组效应、证据规模及正式组间检验。
   二阶元分析主表只记录总体结果；证据规模合并写入一列，并区分一阶元分析数 $M$、一阶汇总效应数 $E$、底层原始研究数 $k$ 与参与者数 $N$。只填原文实际报告的数量，未去重时不自行推算唯一研究数或总样本。
   二阶亚组独立成表并采用互补维度：当前 Concept 是干预因素时，记录结果变量亚组；当前 Concept 是结果变量时，记录干预措施、项目模式或实施技术亚组。必须同时报告亚组效应及正式组间检验，不能依据点估计排序或“一个显著、另一个不显著”判定亚组差异。学科、学段、地区、年份、研究设计和质量等级等其他调节结果默认回到 Argument 查阅；只有该维度本身就是当前 Concept 的定义组成时才例外保留。
   Concept 页不记录一阶或二阶元分析的发表偏倚、敏感性分析及详细质量评估，需要时回到对应 Argument 查阅。二阶主表也不重复完整一阶元分析清单、检索过程、效应转换公式、软件参数、重叠矩阵或全部调节项；只有重叠或依赖问题直接限制结果解释时，才在“关键解释边界”中简要注明。
   一阶与二阶元分析表默认报告汇总效应及 95% CI，不单列标准误。原文只报告 SE 而未报告 CI 时，可在效应单元格中记录 SE；若 SE 仅用于逆方差加权或二阶计算，留在 Argument 的方法部分，不复制到 Concept。
   “其他实证结果（无效应量）”主要用于原文未报告可提取效应量，但提供频数、比例、均值、相关、回归系数、组间差异、检验统计量或显著性的数据。同一结果已有 $d$、$g$、OR、RR 或其他可比较效应量时，优先放入对应效应量表，不在本表重复。
   只有原始研究、一阶元分析和二阶元分析的效应量表都不适用时，才保留“其他实证结果（无效应量）”。删除其余不适用的表，不以整表 — 占位。 %%

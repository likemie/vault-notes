...
%% 强制规则
   只要与本概念相关的研究包含样本、测量、频数、比例、均值、相关、回归、组间差异、显著性、置信区间、效应量或元分析结果，本节就不得删除，并且必须至少填写一张与数据类型相符的表格。
...
%% 一级研究报告组间比较、干预结果或效应量时使用 [!effect-table]，每个结果变量单独一行。优先保留原文报告的组别统计、效应量和显著性或不确定性；原文未报告的指标写 —，不得根据均值、标准差或样本量自行推算。
   元分析总体结果每个结果变量单独一行。
   报告调节变量、亚组分析或元回归时保留调节分析表；报告敏感性分析或发表偏倚时保留稳健性与偏倚表。
   只有一级研究结果表和元分析表都不适用时，才保留“其他实证结果”。删除其余不适用的表，不以整表 — 占位。 %%

...
> [!example]
> 一位历史教师在准备关于法国大革命的课。她面临的不是一个讲什么内容的简单问题，而是三种维度的同时权衡：（1）Bildung 维度——学生通过理解大革命的起因和后果，应该形成对社会变革何以发生这一根本问题的判断力；（2）Erziehung 维度——学生在学习过程中需要被引导去关心不公正的社会结构如何被挑战这一问题，而不仅仅把革命当作记忆日期；（3）Didaktik 维度——应该先放罗伯斯庇尔的演讲视频，还是先让学生读一段当代目击者的日记？应该把三级会议和攻占巴士底狱放在一节课还是分两节？每一个选择都同时在三个维度上产生后果。
...
在 Jornitz & Klinge (2022) 的运用中，Gruschka 的教学理论被用作批判数字学习工具的分析框架。其批判逻辑是：数字学习工具通常只替代了 Didaktik 的部分功能（呈现内容、安排任务序列、提供反馈），但在这个过程中，它们同时暗中重新定义了 Bildung 和 Erziehung——将理解简化为完成任务，将朝向 Bildung 的定向简化为对奖励系统的动机依赖（Jornitz & Klinge, 2022, pp.240–243）。因为这个三元结构是不可分割的星座，对任何一个维度的技术干预都会波及全部三个维度。

...
> [!proposition-chain] 核心命题六｜活动理论移动协作学习框架（AT-MCSCL）的人机协同全要素调节机制
> - **前提一（生态中介[[Hypothesis|假设]]）** 在移动与智能技术支持协作学习（CSCL/MCSCL）中，技术的促学效果并非孤立的工具固有属性，而是由活动系统微观要素的结构咬合与宏观文化生态交互决定（Collis & Margaryan, 2004; Jonassen & Rohrer-Murphy, 1999）。
> - **前提二（全要素[[Operationalization|操作化]]架构）** AT-MCSCL 将活动理论的六要素系统操作化为六大关键调节维度：客体与产出分化、主体特征与学科情境、中介工具类型、人机劳动分工、教学规则与规程、共同体与宏观文化生态。[[Argument_Chen_Cheung_2025_ERR|(Chen & Cheung, 2025, pp. 3–7)]]
> - **推导（系统生态调节机制）** 人机协同学习的成效来自活动系统全要素的动态咬合与生态适应；任何孤立的技术引入若缺乏分工重组、规则创新与情境支持，必然导致系统失调，为智能时代教学干预提供了中层[[Analytic Framework|分析框架]]。
...
> ```mermaid
> flowchart LR
>     A1["活动系统 A<br>（既有教学制度与规范惯性）"] --> B["双系统边界相遇与矛盾激化<br>（工具与规则失调 · 分工失衡）"]
>     A2["活动系统 B<br>（新兴智能技术与外部改革）"] --> B
>     B --> C["边界跨越与对话协商<br>（跨越系统隔阂 · 识别核心张力）"]
>     C --> D["建构共享客体<br>（Shared Object：重塑共同目标）"]
>     D --> E["全要素扩展性改造<br>（工具·规则·分工·共同体协同升级）"]
>     D -.宏观权力不对等或认知外包.-> F["扩展受阻与异化<br>（表面合规 · 批判思维萎缩）"]
> ```
...
> > - **研究背景** 学界关于生成式 AI 对大学生促学成效大小存在显著分歧，既有实证常局限于单一工具评估或孤立学科，缺乏系统性的理论解释架构与大[[Sample Size Determination|样本量]]化综合。
> > - **六要素实证调节数据** 研究采用 AT-MCSCL 框架，对 57 项严格控制基线等效（$d < 0.25$）的[[Experimental Research|实验研究]]（$N = 12,236$）实施元分析编码，揭示出全要素实证调节规律：
> >   - **客体产出级差** [[Language Skills|语言技能]]获超大增益（$g^+=2.331$），[[Academic Achievement|学业成绩]]（$g^+=0.633$）与[[Higher-Order Thinking Skills|高阶思维]]（$g^+=0.580$）呈中度增益，而[[Metacognition|元认知]]未达统计显著（$g^+=0.078$），实证确立认知穿透力单调递减规律；
> >   - **主体特征调节** 大一新生处于大学过渡适应期，获益显著最高（$g^+=0.698$）；语言学科与大模型底层语言机制天然契合，增益最为突出（$g^+=1.782$）；
> >   - **中介工具模态** 文本、代码与视觉多模态均达中大促学效应且组间差异不显著，证实教学任务设计的核心地位远胜于平台技术表象；

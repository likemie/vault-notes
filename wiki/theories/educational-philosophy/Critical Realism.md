...
> EEF 宣称针对中一阅读困难学生的“Fresh Start”合成拼读项目带来了 $+0.24$ 标准差（SD）的效应量。但深读技术报告后会发现，由于治疗组的前测成绩远低于控制组，所谓的“进步”仅是糟糕[[Random Assignment|随机化]]（Random Assignment）带来的偏误。当研究者从中筛选出前测分数完全相同的学生子集进行匹配分析时，治疗组和控制组的平均进步幅度和后测成绩**几乎完全相同（效应量为 $+0.00$ SD）**。实验没有触及任何深层因果机制，仅在经验层面用数字黑箱包装了数据幻象。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 5]])

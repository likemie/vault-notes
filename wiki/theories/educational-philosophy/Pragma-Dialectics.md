...
> | 谬误即脱轨（Fallacy as Derailment） | 分类 | 理论将传统谬误类型学重新解释为对批判性讨论规则的违反——每种谬误本质上都是论证程序的"脱轨"（derailment），而非独立的形式瑕疵类别。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, p. 186)]] |
> | 战略运作（Strategic Maneuvering） | 概念 | 论证者在修辞有效性（说服听众）与辩证合理性（遵守讨论规则）之间维持微妙平衡的行为。这一概念将修辞学洞察纳入规范框架。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, p. 186)]] |

---

## 核心命题与机制

> [!proposition-chain] 核心命题一｜论证应被分析为批判性讨论中的言语行为序列，其质量取决于对分歧解决的贡献
> - **前提一** 论证发生在意见分歧的语境中，分歧双方（或多方）各自持有不同的立场（standpoints）。
...
> [!theory-use] 如何用于研究
> - **作为理论框架** 用于分析科学争议、政策辩论、课堂讨论或教科书论证的结构和质量。
> - **作为分析工具** 将论证段落重建为立场、前提、反驳和结论的序列，按规则逐项检查谬误（derailments）。
> - **作为批判视角** 揭示"看似合理"的论证如何通过模糊限制语（hedging）、概念偷换或框架预设来规避理性审查。
> - **报告方式** 呈现论证的分析性概览（analytic overview），标注每个言语行为在批判性讨论中的功能和偏离点。

---
title: Appraisal Theory
aliases:
  - Frijda's appraisal theory
  - appraisal theory of emotion
  - 情感评估理论
summary: "Frijda 提出的情绪理论，强调认知评估在情绪生成中的核心作用，将情绪过程分解为七个阶段，并认为文化参与每个阶段的调节"
type: theory
tags:
  - theme/emotion
  - theme/affect
  - theme/cognition
  - theory/appraisal
related_concepts:
  - "[[Pride in Learning]]"
related_theories:
  - "[[Cultural Models]]"
related_methods:
  - "[[Coding in Qualitative Research]]"
related_persons: []
related_facts: []
related_arguments:
  - "[[Argument_Li_2012_Cambridge]]"
confidence: medium
status: draft
created: 2026-05-23
updated: 2026-06-19
---

# Appraisal Theory

---

## 理论定位

> [!theory-position] 理论定位
> - **解释对象**：情绪的生成机制及其与认知的关系，以及文化对情绪体验的塑造。
> - **理论问题**：情绪是纯粹的物理生理反应还是受认知中介的过程？文化如何以及在哪个阶段参与情绪的调节？
> - **理论类型**：心理学机制模型与文化心理学分析框架。
> - **知识位置**：认知情绪心理学（Cognitive Psychology of Emotion）传统，代表人物为 Nico Frijda 与 Batja Mesquita。

> [!claim] 核心主张
> 情绪不是对外部事件的直接生理反应，而是经过认知评估中介的过程。个体在体验到任何情绪之前，必须先对事件进行评估以确定其对自身的关联性和重要性；此外，文化并非仅影响情绪的最终表达，而是全面参与情绪生成的七个阶段。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])

> [!citation-card] 关键表述
> 为了让我们体验到任何情绪，在情绪产生之前，我们会先用大脑对事件进行评估，以确定其对我们自身的关联性和重要性。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])
>
> In order for us to experience any emotion, we use our mind to appraise the event to determine its relevance and importance to ourselves before our emotions are generated.

---

## 核心命题与机制

> [!proposition-chain] 情绪生成的七阶段命题链
> - **前提一：认知评估中介**：情绪过程起源于对事件的心理表征和分类（编码），即评估“这对我意味着什么”。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])
> - **前提二：生理与行动倾向的关联**：评估结果直接触发相应的生理反应模式（如脸红、心跳加速）以及从行为库中选择下一步行动的准备状态。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])
> - **机制：文化全面调节**：文化并非只在最后的“表达调节”起作用，而是深入参与情绪生成的每一个环节（从前因事件的选择、事件编码、意义评估，到生理反应模式、行动准备、情绪行为和最终调节）。([[Argument_Li_2012_Cambridge|Li, 2012, p. 55–56]])
> - **结果判断：情绪体验的跨文化异质性**：由于不同文化拥有不同的[[Cultural Models|文化模型]]，各文化群体在情绪的激活频率、行动倾向选择和调节模式上存在系统性差异。([[Argument_Li_2012_Cambridge|Li, 2012, p. 55]])

> [!mechanism-map]- 情绪生成与文化调节机制图
> ```mermaid
> flowchart TD
>     E["1. 前因事件"] --> C["2. 事件编码"]
>     C --> A["3. 认知评估"]
>     A --> P["4. 生理反应模式"]
>     P --> AR["5. 行动准备"]
>     AR --> B["6. 情绪行为"]
>     B --> R["7. 情绪调节"]
>     
>     subgraph Cul["文化模型 (Cultural Models) 的全面调节"]
>         direction LR
>         M1["影响事件选择"] -.-> E
>         M2["规约分类标准"] -.-> C
>         M3["界定自我也关性"] -.-> A
>         M4["影响自主神经模式"] -.-> P
>         M5["限制备选行动库"] -.-> AR
>         M6["社会规范约束"] -.-> B
>         M7["引导应对策略"] -.-> R
>     end
> ```

> [!exegesis]- 教育研究中的情感社会化例子
> 在学习情境中，如果某种文化（如东亚儒家文化）强调对学习成就的[[Pride in Learning|骄傲]]需保持谦逊，而另一种文化（如美国个人主义文化）则大力鼓励表达骄傲，这就导致两类文化背景的学习者在“事件编码”和“情绪行为/调节”上产生完全不同的评估路径：前者可能将高分编码为“需继续努力的起点”并调节骄傲情绪，后者则将其评估为“个人能力的彰显”并积极表达。([[Argument_Li_2012_Cambridge|Li, 2012, p. 56]])

---

## 关键概念与理论构件

> [!theory-components] 理论构件
> - **评估 (Appraisal)**：理论的核心构件，指个体对事件之于自我的意义 and 重要性所做的认知评价过程。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])
> - **事件编码 (Event Coding)**：将前因事件按文化所认可的类别进行归类的心理表征过程。([[Argument_Li_2012_Cambridge|Li, 2012, p. 54]])
> - **[[Cultural Models|文化模型]]**：提供评估标准和分类图式，规范个体在各阶段的情绪反应和行为选择。([[Argument_Li_2012_Cambridge|Li, 2012, p. 55]])

---

## 认识论与方法含义

> [!theory-stance] 认识论与方法含义
> - **本体论**：情绪不是生理实体，而是以主客体交互的认知关系为前提的心理-社会建构。
> - **认识论**：反对情绪是外部刺激的直接反射（行为主义观），主张通过主观认知评估的逻辑来解释情绪的多样性。
> - **方法含义**：要求在研究情绪时，不能仅测量生理指标，必须结合[[Coding in Qualitative Research|质性编码]]、访谈或情境实验来测量个体的评估维度和文化图式。
> - **不能直接推出的东西**：不能由相同的外部物理刺激直接推导或预测个体的最终情绪体验，必须引入其中介评估和文化背景。

---

## 分析框架与使用方式

> [!theory-use] 如何用于研究
> - **作为理论框架**：可用于研究少数族裔学生、跨文化学习者在学校环境中的情感社会化（Affective Socialization）过程。
> - **作为分析工具**：可通过对母子对话或师生互动进行[[Coding in Qualitative Research|质性编码]]，追踪其在事件编码、评估和调节等七个阶段中表现出的文化特征。

---

## 适用边界

> [!theory-boundary] 适用边界
> - **适合解释**：社会文化情境下的复杂情绪生成、跨文化情感差异、课堂环境中的情感社会化。
> - **谨慎使用**：纯粹由低级神经回路驱动的瞬时生理应激反应（如惊吓反射）。
> - **不适合解释**：无意识或完全不涉及概念中介的病理性情感障碍。

---

## 发展脉络

> [!dev-timeline] 发展脉络
> - **1986 — Frijda (The Emotions)**：提出认知评估在情绪生成中的核心地位。
> - **1992 — Mesquita & Frijda (Psychological Bulletin)**：将文化维度全面引入情绪的七个生成阶段，建立文化情绪心理学分析框架。
> - **2012 — 教育研究应用 (Li, 2012)**：Li (2012) 将该理论引入对中西家庭母子互动和[[Parental Cognitive and Affective Socialization|情感社会化]]的对比分析中。

---

## 争议与批评

> [!tension] 争议焦点
> - **认知与情绪的先后顺序**：Zajonc 等学者批评认为，情感反应可以在不发生认知评估的情况下独立产生（即“情感在认知之先”）。
> - **生理通用性与文化建构的张力**：部分进化心理学家主张基本情绪具有跨文化一致的生理基础，质疑文化在情绪生成所有阶段中参与调节的程度。

---

## 相关研究

> [!evidence-grid-a] 相关研究索引
> - [[Argument_Li_2012_Cambridge]] — 通过对母子对话进行话语分析，使用该理论论证中西文化在儿童[[Parental Cognitive and Affective Socialization|情感社会化]]中如何全面塑造情绪生成的各阶段。

---

## 应用领域

> [!case] 应用领域索引
> - [[Mother-Child Conversation]] / [[MCC Discourse Analysis Examples]] — 在儿童情感社会化和母子话语研究中，作为解释情感对话如何传递文化评估规则的理论基础。

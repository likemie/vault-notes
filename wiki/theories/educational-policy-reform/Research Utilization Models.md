---
title: Research Utilization Models
aliases:
  - 研究利用模型
  - 研究使用模型
  - Models of Research Utilization
  - Weiss's Models of Research Utilization
  - 韦斯研究利用模型
summary: "由卡罗尔·韦斯开创并经公共政策与教育研究发展的理论模型谱系，系统阐明学术研究进入政策与实践的非线性多元机制（知识驱动、问题解决、互动、政治、战术、启蒙及社会智识事业），奠定工具性、概念性与象征性使用三元分类的基础。"
type: theory
theory_field: "educational-policy-reform"
theory_related_count: 54
theory_related_level: 6
theory_related_stars: "⭐⭐⭐⭐⭐⭐"
theory_related_color: "#fef3c7"
tags:
  - field/educational-policy-reform
  - theme/research-use
  - theme/evidence-informed-practice
  - theme/knowledge-mobilization
related_concepts:
  - "[[School Leadership]]"
  - "[[Technical Rationality]]"
  - "[[Policy Science in Comparative Education]]"
  - "[[Knowledge Mobilisation]]"
  - "[[Evidence-Based Education]]"
  - "[[Problem Solving]]"
  - "[[Problem Finding]]"
  - "[[Research Utilization]]"
  - "[[Construct]]"
  - "[[Conceptual, Instrumental, and Symbolic Use of Research]]"
  - "[[Research Literacy]]"
  - "[[Evidence-Informed Practice]]"
  - "[[Paradigm]]"
  - "[[Hypothesis]]"
  - "[[Blue Skies Research]]"
  - "[[Evaluation Research]]"
  - "[[Research Translation]]"
  - "[[Dialogue in Education]]"
  - "[[Research-Practice Partnership]]"
  - "[[Variable]]"
  - "[[Formative Assessment]]"
  - "[[Critical Pedagogy]]"
  - "[[Constructivist Paradigm]]"
  - "[[Theoretical Perspective]]"
  - "[[Literature Search]]"
  - "[[Knowledge Transfer]]"
  - "[[New Public Management]]"
  - "[[Homework]]"
  - "[[Epistemic Stances]]"
  - "[[Ontology]]"
  - "[[Epistemology]]"
  - "[[Knowledge-that and Know-how-to]]"
  - "[[Academic Achievement]]"
  - "[[Research Question]]"
  - "[[Questionnaire]]"
  - "[[Operationalization]]"
  - "[[Professional Judgment]]"
  - "[[Knowledge Production]]"
  - "[[Document]]"
related_theories:
  - "[[Ecosystem for Research-Engaged Schools]]"
  - "[[Theory of Planned Behavior]]"
  - "[[Complexity Theory]]"
related_methods:
  - "[[Framework Synthesis]]"
  - "[[Meta-analysis]]"
  - "[[Mixed Methods Research]]"
  - "[[Qualitative Interview]]"
  - "[[Coding in Qualitative Research]]"
related_instruments:
  - "[[Research Literacy Scale for Teachers]]"
related_persons:
  - "[[Carol Weiss]]"
related_facts:
  - "[[What Works Clearinghouse]]"
related_arguments:
  - "[[Argument_Hagevold_Forsstrom_Keles_2026_ERR]]"
  - "[[Argument_Nelson_2017_ER]]"
  - "[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch03]]"
  - "[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO]]"
confidence: high
status: draft
created: 2026-08-29
updated: 2026-08-29
---

# Research Utilization Models

---

## 理论定位

> [!theory-position] 理论定位
> - **解释对象** 学术研究与实证证据如何被政策制定者、教育行政人员、[[School Leadership|学校领导]]者与一线教师在不同情境下获取、解释、消化、转化、引用或策略性部署。
> - **理论问题** 破除“研究产生 $\rightarrow$ 直接转化为政策或教学行动”的线性[[Technical Rationality|技术理性]]迷思，解释为何高质量科研成果很少产生即刻的工具性应用，以及研究如何以非线性、扩散性与政治性的方式深层重构行动者的思维视界。
> - **理论类型** 中层机制模型群与分析类型学（Typological Framework）。
> - **知识位置** 所属学科为[[Policy Science in Comparative Education|政策科学]]（Policy Sciences）、[[Knowledge Mobilisation|知识动员]]与利用（Knowledge Mobilization and Utilization）、[[Evidence-Based Education|循证教育]]学；开创者为[[Carol Weiss]]（1979, 1980, 1991b），后续经 Pelz (1978)、Beyer & Trice (1982)、Nutley et al. (2007) 系统化。

> [!claim] 核心主张
> 研究并非通过直接的“证据输入 $\rightarrow$ 决策产出”工程式闭环进入实践，而是沿着知识驱动、[[Problem Solving|问题解决]]、持续互动、政治辩护、战术拖延、概念启蒙与社会智识涵育等多重路径展开；其中最深远的影响是通过“知识渗透”（knowledge creep）与“决策累积”（decision accretion）潜移默化地重塑实践者的概念视界与[[Problem Finding|问题界定]]方式。[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|(Hagevold et al., 2026, p. 3)]]

> [!citation-card]- 关键表述
> [[Research Utilization|研究利用]]并不总是意味着做出具体决策时的直接依据。更常见的情况是，研究的概念和理论逐渐扩散并渗透到决策者的日常思考中，以缓慢而累积的方式重构他们理解问题、定义优先级与评价备选方案的整体智识地图。[[Argument_Nelson_2017_ER|(Nelson & Campbell, 2017, p. 132)]]
>
> *Research use is not a single, monolithic act of direct implementation. Instead, it encompasses multiple diffuse pathways through which research evidence, theoretical perspectives, and empirical concepts percolate into the policy and professional discourse, gradually reshaping how problems are understood and framed.*

---

## 关键概念与理论构件

> [!entry-map]
>
> | 构件 | 类型 | 在理论中的功能 |
> |:-----|:-----|:--------------|
> | [[Research Utilization]] | 概念 | 核心统摄[[Construct\|理论构念]]，界定证据进入政策与教育实践的总体过程、范畴与测量维度。 |
> | [[Conceptual, Instrumental, and Symbolic Use of Research]] | 分类 | 核心三元类型学，将多重模型提炼为工具性（直接应用）、概念性（思维重塑）与象征性（政治辩护）三种基本形态。 |
> | [[Carol Weiss]] | 人物 | 理论开创者，提出经典的七种利用模型与知识渗透理论机制。 |
> | [[Research Literacy]] | 概念 | 实践者识别、解读、转化与批判性运用本理论模型所必需的专业能力与知觉行为控制构件。 |
> | [[Evidence-Informed Practice]] | 框架 | 研究利用模型在学校与教师层面落地为知证教学实践的系统性[[Paradigm\|范式]]。 |
> | [[Knowledge Mobilisation]] | 机制 | 促成研究成果跨越学术界与实践界边界流动、转化与共创的中间机制。 |

---

## 核心命题与机制

> [!proposition-chain] 核心命题一｜[[Research Utilization|研究利用]]存在七种质性相异的机制模型并可归纳为四大过程[[Paradigm|范式]]
> - **前提一** 传统政策与管理学默认“线性技术模型”（[[Technical Rationality|Technical-Rational Model]]），[[Hypothesis|假设]]只要研究质量足够高、结论足够明确，决策者就会据此直接制定政策或调整教学行为。[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch03|(Cohen et al., 2011, p. 43)]]
> - **前提二** 现实中的政策制定与教学实践高度受到价值冲突、政治博弈、制度惯性与认知负荷的制约，研究证据只是多元竞争信息流中的一种输入。[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|(Hagevold et al., 2026, p. 3)]]
> - **推导** Weiss (1979) 进而提出解构线性迷思的七种经典模型，并在 1991 年进一步提炼为四种过程范式：
>   ① **知识驱动模型（Knowledge-Driven Model）** [[Blue Skies Research|基础研究]] $\rightarrow$ [[Evaluation Research|应用研究]] $\rightarrow$ 技术开发 $\rightarrow$ 成果应用，适用于强自然科学或工程[[Research Translation|技术转化]]，但在复杂社会教育情境中往往受限；
>   ② **[[Problem Solving|问题解决]]模型（Problem-Solving Model）** 决策者面临明确待解难题 $\rightarrow$ 主动检索或委托针对性实证研究 $\rightarrow$ 研究提供最优方案 $\rightarrow$ 决策落地；
>   ③ **互动模型（Interactive Model）** 研究者、政策制定者、行政人员与一线教师处于非线性的多边[[Dialogue in Education|对话]]网络中，研究证据与实践者的经验智慧、政治判断相互激荡与调适；
>   ④ **政治模型（Political Model）** 决策方案已由意识形态或既得利益预先确定，研究被选择性引用作为佐证论点、打击异见或增强合法性的“政治弹药”；
>   ⑤ **战术模型（Tactical Model）** 政府或学校管理层以“正在委托专家开展深入研究”为由拖延决策、转移公众问责压力或展示虚假积极姿态；
>   ⑥ **启蒙模型（Enlightenment Model）** 研究不提供立竿见影的处方，而是像水滴石穿一样弥散扩散，逐渐改变实践者的概念框架与对常识的理解；
>   ⑦ **社会智识事业模型（Intellectual Enterprise Model）** 学术研究作为社会整体文化反思与思想演进的有机组成部分，广泛塑造时代精神与教育价值观。
>
> > [!framework-table]- 表：Weiss 研究利用七大模型核心特征与机制对比
> > | 模型名称 | 运作逻辑 | 研究在决策中的角色 | 典型教育场景 | 局限性与风险 |
> > |---|---|---|---|---|
> > | **知识驱动模型**<br>Knowledge-Driven | 基础研究 $\rightarrow$ 应用研究 $\rightarrow$ 成果转化 | 自主产生技术突破，驱动后续实践变革 | 学习科学实验室脑成像发现转化为数字化教学软件 | 忽视教育复杂社会属性与情境约束 |
> > | **问题解决模型**<br>Problem-Solving | 现实困境 $\rightarrow$ 检索/委托研究 $\rightarrow$ 直接采纳 | 决策者按需取用的“现成处方”与证据库存 | 针对特定年级阅读障碍委托针对性干预评估研究 | 假定问题与目标无争议，现实极易遇阻 |
> > | **互动模型**<br>Interactive | 多边网络平等对话、经验与证据互构 | 协同共创与探究伙伴（[[Research-Practice Partnership\|RPP]] 范式） | 大学研究者与学校教研组组建长期行动探究共同体 | 沟通成本高，需要极高的组织专业信任 |
> > | **政治模型**<br>Political | 立场先行 $\rightarrow$ 选择性引用研究为已有政策辩护 | 辩护武器与公关修辞弹药 | 政策制定者挑选支持特定考试分流方案的研究报告 | 证据被工具化操纵，扭曲学术求真本质 |
> > | **战术模型**<br>Tactical | 借口“开展深入研究”以拖延或推卸行政责任 | 拖延决策与转移问责的挡箭牌 | 面临争议性减负方案时宣布“设立专家组开展长周期调研” | 消耗公众信任，造成政策实践瘫痪 |
> > | **启蒙模型**<br>Enlightenment | 概念弥散渗透与决策累积，重塑常识与思维框架 | 慢[[Variable\|变量]]启蒙，水滴石穿改变行动者心智视界 | “[[Formative Assessment\|形成性评价]]”与“认知负荷”概念全面重塑日常教学常识 | 影响隐蔽漫长，难以开展即刻量化因果归因 |
> > | **社会智识事业模型**<br>Intellectual Enterprise | 学术研究作为社会文明反思与批判的公共资源 | 提供时代反思框架与广泛文化滋养 | [[Critical Pedagogy\|批判教育学]]与教育哲学对功利主义教育观的持续省思 | 远离具体操作指令，对微观实践缺乏直接指导 |

> [!proposition-chain] 核心命题二｜概念性启蒙通过“知识渗透”与“决策累积”发挥最深远的塑造作用
> - **前提一** 大量实证调查表明，决策者和教师极少能明确回忆起某项具体决策是基于哪一篇特定论文的哪组统计数据（[[Conceptual, Instrumental, and Symbolic Use of Research|工具性使用]]发生率低）。[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|(Kazancı Tınmaz & Sezgin, 2023, p. 3, p. 14)]]
> - **前提二** 决策者和教师却广泛运用了源自学术研究的概念（如“多元智能”、“认知负荷”、“[[Formative Assessment|形成性评价]]”、“[[Constructivist Paradigm|建构主义]]”）来思考与组织日常教学。[[Argument_Nelson_2017_ER|(Nelson & Campbell, 2017, p. 130)]]
> - **推导** Weiss (1980) 阐明了研究发挥影响的核心微观机制：
>   ① **知识渗透（Knowledge Creep）** 研究概念、概括性结论与[[Theoretical Perspective|理论视角]]通过期刊、培训、大众媒体与同行交流非正式地弥散开来，在未被明确察觉来源的情况下融入实践者的背景认知；
>   ② **决策累积（Decision Accretion）** 政策与教学决策通常不是在某一特定时刻做出的单一孤立抉择，而是在日常无数微小调整与常规重构中逐渐固化定型的，渗透进心智的知识在此过程中潜移默化地指导了选择取向。

> [!proposition-chain] 核心命题三｜研究利用的多维转化受制于知觉控制与专业实践介入方式
> - **前提一** 教师普遍对科研价值持有极高的认同感（启蒙模型带来的普遍价值共识），但将其转化为工具性教学行为或互动反思时面临显著的实践断层。[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|(Kazancı Tınmaz & Sezgin, 2023, p. 8)]]
> - **前提二** 工具性使用与深度反思需要跨越方法论门槛与信息过载，要求具备[[Literature Search|文献检索]]、统计图表解读与行动探究的操作技能。
>   > [!evidence-grid]- 证据
>   > - **结构方程与多[[Variable|变量]]分析证据** Kazancı Tınmaz & Sezgin (2023) 基于 568 名教师的实证检验表明，开展科研（$p < .001, \text{partial } \eta^2 = .056$）与追踪学术期刊（$p = .001, \text{partial } \eta^2 = .046$）分别对提升研究技能与驱动日常课堂使用产生最大主效应，证实做研究与读期刊是驱动研究利用从概念认同走向实践转化的双轨独立引擎。[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|(Kazancı Tınmaz & Sezgin, 2023, p. 13)]]
>   > - **协作[[Framework Synthesis|框架综合]]证据** [[Argument_Hagevold_Forsstrom_Keles_2026_ERR|Hagevold et al. (2026)]] 对 32 项质性与混合研究的框架综合发现，外在自上而下的绩效问责容易诱发表面应付的“象征性使用”，而只有在具备专业信任与同行协作的生态中，互动模型与整合使用才能真正发生。[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|(Hagevold et al., 2026, p. 18)]]
> - **推导** 研究利用并非由证据质量单向决定，而是取决于组织制度环境（信任 vs 问责）以及个体知觉行为控制（[[Research Literacy|研究素养]]与技能储备）的交互调控。

> [!proposition-chain]- 延伸命题
> - **推论一（源自核心命题一：互动模型的制度化演进）** 从 Weiss 的互动模型出发，当代教育改革演化出了“第四代研究使用”理论与[[Ecosystem for Research-Engaged Schools|研究参与学校生态系统模型]]（Ecosystem for Research-Engaged Schools），主张研究机构与中小学建立“研究者-实践者伙伴关系”（Researcher-Practitioner Partnerships, [[Research-Practice Partnership|RPP]]），从单向[[Knowledge Transfer|知识转移]]（Transfer）走向知识共创（Co-production）。[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|(Hagevold et al., 2026, p. 3)]]
> - **推论二（源自核心命题二：反向象征性异化推论）** 当[[New Public Management|新公共管理]]（NPM）与高利害问责机制将“证据本位”强行作为量化考核指标时，学校和教师为了展示合法性，极易倒退向政治模型与战术模型，产生仪式性的象征性使用（如言语上堆砌术语、机械填报量表），阻碍真实教学反思。[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|(Hagevold et al., 2026, p. 18)]]
> - **应用命题** 在教育政策与教师教育评价中，不得仅以“是否有具体证据被直接采纳（工具性指标）”作为衡量科研转化成效的唯一标准，必须将概念性理解深化、专业话语更新与探究文化塑造纳入多维评价体系。

> [!mechanism-map]- 机制图
> ```mermaid
> flowchart LR
>   A["学术研究证据与理论成果"] --> B{"传播转化路径"}
>   B --"直接匹配预设问题"--> C["问题解决 / 工具性使用<br>（直接调整政策或教学策略）"]
>   B --"多边网络持续对话"--> D["互动模型 / 协同共创<br>（经验与证据整合重构）"]
>   B --"弥散传播与非正式渗透"--> E["启蒙模型 / 概念性使用<br>（知识渗透与决策累积）"]
>   B --"既定利益与合法性需求"--> F["政治与战术模型 / 象征性使用<br>（辩护立场或推诿责任）"]
>   
>   C --> G["显性行为与制度产出"]
>   D --> G
>   E --> H["深层心智模型与话语更新"]
>   H -.长周期反哺.-> G
>   F --> I["形式化合规与合法性维持"]
> ```

> [!exegesis]- 教育研究例子
> **以学校推行形成性评价（Formative Assessment）改革为例说明三类模型的运作差异**
> 1. **工具性问题解决模型** 学校发现学生数学两极分化严重（明确问题），检索到 Black & Wiliam (1998) 的[[Meta-analysis|元分析]]结论与具体策略（研究输入），直接在下学期教案模板中强制加入“每堂课必须包含 2 次出堂测验卡”（直接工具性落地）。
> 2. **概念性启蒙渗透模型** 教师们在几年内陆续阅读了多篇讨论评估的通俗文章，潜移默化地改变了对考试的看法，认识到“测试不是为了给学生打等级，而是为了发现教学盲点”，从而在日常批改[[Homework|作业]]和课堂提问时更加关注学生的思维卡点，即使他们并未严格执行任何标准化的评估工具方案。
> 3. **象征性/战术政治模型** 为了应对上级督导评估中的“现代化教学”指标，学校在公开课和宣传展板上大量印刷“全面落实形成性评价”标语并装订厚重的档案盒，但教师常规授课依然完全延续传统讲授灌输，研究成果沦为应付检查的符号盾牌。

---

## 理论立场与使用方式

> [!theory-stance] [[Epistemic Stances|认识论立场]]
> - **[[Ontology|本体论]]** 将社会实践、教育政策与教学过程理解为充斥着多元价值竞争、不确定性与微观互动的复杂社会系统，否定将实践简化为纯粹机械输入-输出工程的确定论本体观。
> - **[[Epistemology|认识论]]** 秉持多元认识论（Pluralist Epistemology），认为学术研究提供的命题性知识（[[Knowledge-that and Know-how-to|propositional knowledge]]）并不天然高于实践者的情境性知识（Tacit & Contextual Knowledge），真正的有效利用是多元知识形态在情境中的重构与融合。
> - **方法含义** 倡导采用[[Mixed Methods Research|混合方法]]、[[Framework Synthesis|框架综合]]、质性追踪与多维度量表测评（如将[[Research Utilization|研究利用]]细分为意识、态度、技能、日常使用等多阶潜[[Variable|变量]]），避免用简单的“用/没用”二分法进行统计。
> - **不能直接推出的东西** 不能根据某一模型（如启蒙模型）自动证明具体教学改革必然提升学生[[Academic Achievement|学业成绩]]，也不能从理论上论证“[[Conceptual, Instrumental, and Symbolic Use of Research|工具性使用]]必然优于概念性使用”的规范性价值判断。

> [!theory-use] 如何用于研究
> - **作为理论框架** 用于组织研究使用、[[Knowledge Mobilisation|知识动员]]、循证实践或教育政策执行的[[Research Question|研究问题]]，构建“认知意向 $\rightarrow$ 机制分流 $\rightarrow$ 多维实践表现”的解释逻辑。[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|(Kazancı Tınmaz & Sezgin, 2023, pp. 3–5)]]
> - **作为分析工具** 将七种模型或三分类（工具、概念、象征）直接转化为[[Qualitative Interview|质性访谈]][[Coding in Qualitative Research|编码]]类目、观察量规或[[Questionnaire|问卷调查]]的分量表题项。
> - **作为批判视角** 揭示政策制定中将科研证据“武器化”为政治辩护或战术推诿的修辞把戏，批判唯[[Technical Rationality|技术理性]]的单向传播政策[[Hypothesis|假设]]。
> - **报告方式** 在写作中应将研究利用的各类形态置于具体制度与文化脉络中呈现，明确区分行动者的言语认同（概念/态度）与实际操作行为（技能/工具使用）。

> [!logic-map]- 分析流程图
> ```mermaid
> flowchart LR
>   Q["研究问题：证据如何影响实践？"] --> L["理论镜头：Weiss 研究利用模型谱系"]
>   L --> O["观察维度：获取途径 / 解释过程 / 转化方式 / 政治意图"]
>   O --> E["证据材料：政策文本 / 访谈话语 / 课堂行为 / 量表测量数据"]
>   E --> I["理论解释：识别工具性、概念性或象征性机制及其制约条件"]
> ```

---

## 适用边界

> [!theory-boundary] 适用边界
> - **适合解释** 教育政策制定、课程改革推行、学校管理决策、教师日常循证实践、教研组协作探究等涉及专业知识与行政/教学行动关系的微观、中观与宏观场景。
> - **谨慎使用** 在完全高度标准化的封闭操作技术领域（如纯硬件设施安装、基础信息网络搭建等无需主体价值调适的纯技术场景），模型中的启蒙与政治维度解释力相对减弱。
> - **不适合解释** 纯粹的基础认知心理学加工过程、无任何知识介入纯由偶发情绪驱动的非理性冲动行为。
> - **常见误用** 将描述性的七种模型硬性套用为“从低级到高级”的阶梯式进化等级；忽视情境约束，将未发生工具性采纳简单归咎于实践者“拒绝科学”或“素养不足”。

---

## 发展脉络

> [!dev-timeline] 发展脉络
> - **1979 — 经典奠基** [[Carol Weiss|Carol Weiss]] 发表标志性论文 *The Many Meanings of [[Research Utilization]]*，系统提出知识驱动、[[Problem Solving|问题解决]]、互动、政治、战术、启蒙及社会智识事业 7 种经典模型，打破[[Technical Rationality|技术理性]]垄断。
> - **1980 — 机制深化** Weiss 发表 *Knowledge Creep and Decision Accretion*，正式提出“知识渗透”与“决策累积”概念，为[[Conceptual, Instrumental, and Symbolic Use of Research|概念性使用]]奠定微观认知与社会学机制解释。
> - **1982 — 类型学提炼** Beyer & Trice 在组织研究中进一步将纷繁的利用形态凝练为**工具性（Instrumental）**、**概念性（Conceptual）**与**象征性（Symbolic）**三元分类，成为后续实证研究最通用的[[Operationalization|操作化]]工具。
> - **1991 — 过程模型重构** Weiss 在公共政策研究中提炼出研发、问题解决、互动与战术四类政策过程模型（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch03|Cohen et al., 2011, p. 43]]）。
> - **2007 — 跨部门体系化** Nutley, Walter & Davies 出版专著 *Using Evidence: How research can inform public services*，整合公共卫生、社会工作与教育研究中的证据利用机制，确立当代循证实践理论骨架。
> - **2023 — 测量操作化落地** Kazancı Tınmaz & Sezgin 将研究利用模型与[[Theory of Planned Behavior|计划行为理论]]深度融合，成功编制 20 题四维度[[Research Literacy Scale for Teachers|教师研究素养量表]]（RLS），实证揭示做研究与读期刊的双轨赋能路径（[[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|Kazancı Tınmaz & Sezgin, 2023]]）。
> - **2026 — 协作生态拓展** Hagevold et al. 在《Educational Research Review》发表综述，运用研究利用类型学剖析国际教师协作参与科研的深层整合与浅层合规机制（[[Argument_Hagevold_Forsstrom_Keles_2026_ERR|Hagevold et al., 2026]]）。

---

## 争议与批评

> [!debates] 理论争议
>
> > [!axis] 描述性分类 vs 规范性价值排序的界限争论
> > 部分[[Evidence-Based Education|循证教育]]倡导者主张只有带来可测量行为改变的“[[Conceptual, Instrumental, and Symbolic Use of Research|工具性使用]]”才是有效利用，将概念性使用视为无法量化的次优妥协；批判学者则指出概念性启蒙才是教育这一价值承载事业的核心所在。
> >
> > - **Nutley et al.（2007）** 强调三种使用形态在不同政策阶段各具不可替代的价值，工具性追求短期具体产出，而概念性决定长期改革方向，不应预设高低优劣。
> > - **Biesta（2007）** 批评将[[Research Utilization|研究利用]]窄化为工具性技术处方的做法剥夺了教师的[[Professional Judgment|专业判断力]]（Professional Judgment），强调启蒙与批判才是教育研究的本真价值。
>
> > [!axis] 线性传输[[Hypothesis|假设]]与复杂系统演化的[[Paradigm|范式]]对立
> > 传统循证政策制定者依然偏好“知识驱动”与“[[Problem Solving|问题解决]]”的线性逻辑，试图建立从国家证据中心（如 [[What Works Clearinghouse]]）直接下达操作指南的管道；[[Complexity Theory|复杂性理论]]学者则认为该构想在复杂教育生态中注定失灵。
> >
> > - **Hargreaves（1996）** 主张建立类似循证医学的[[Knowledge Production|知识生产]]与直接转化体系。
> > - **Greenhalgh & Wieringa（2011）** 批评单纯的知识传输（[[Knowledge Transfer]]）隐喻脱离实际，呼吁全面转向基于社会实践的知识流动与共创（[[Knowledge Mobilisation]]）。

> [!critique]- 批评索引
> - [[Argument_Hagevold_Forsstrom_Keles_2026_ERR|Hagevold et al. (2026)]] — 指出既有研究利用模型多聚焦于个体决策者的认知与态度，对“[[New Public Management|新公共管理]]”等宏观问责体制如何系统性诱发“象征性使用”和“合规表演”缺乏足够的制度权力分析。
> - [[Argument_Nelson_2017_ER|Nelson & Campbell (2017)]] — 指出早期的启蒙模型虽然概念解释力强，但在实证测评中由于其隐蔽与扩散性特征，极难进行量化因果归因与效果分离。

---

## 相关研究

> [!evidence-grid-a] 相关研究索引
> - [[Argument_Kazanci_Tinmaz_Sezgin_2023_SO|Kazancı Tınmaz & Sezgin (2023)]] — 将[[Research Utilization|研究利用]]模型与[[Theory of Planned Behavior|计划行为理论]]结合，开发并验证 20 题 4 维度的[[Research Literacy Scale for Teachers|教师研究素养量表]]，证实研究技能是连接积极态度与实际使用的关键知觉控制枢纽，且做研究与读期刊具有双轨赋能效应。
> - [[Argument_Hagevold_Forsstrom_Keles_2026_ERR|Hagevold et al. (2026)]] — 在关于教师协作参与科研的[[Framework Synthesis|框架综合]]中，运用 Weiss 的研究利用分类区分深度整合与浅层象征性合规，揭示信任与制度支持在促成互动模型中的决定性作用。
> - [[Argument_Nelson_2017_ER|Nelson & Campbell (2017)]] — 梳理 Weiss (1979) 以来研究利用[[Document|文献]]对国际[[Evidence-Informed Practice|证据知情实践]]（EIP）的奠基作用，探讨[[Knowledge Mobilisation|知识动员]]条件与系统性落地瓶颈。
> - [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch03|Cohen, Manion & Morrison (2011, Ch. 3)]] — 系统评述 Weiss (1991b) 的政策研究利用四模型（研发、[[Problem Solving|问题解决]]、互动、战术），探讨教育研究与政策制定之间的多重复杂关系。

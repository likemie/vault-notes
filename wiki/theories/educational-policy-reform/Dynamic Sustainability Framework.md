---
title: Dynamic Sustainability Framework
aliases:
  - 动态可持续性框架
  - DSF
summary: "由 David A. Chambers、Russell E. Glasgow 与 Kurt C. Stange（2013）提出的实施科学与系统变革理论，彻底颠覆干预成效随时间衰减的传统‘电压降’假设，主张干预措施、实践现场与宏观生态系统三者处于持续的自适应调适、双向学习与动态拟合之中，以实现长效可持续性。"
type: theory
theory_field: "educational-policy-reform"
theory_related_count: 30
theory_related_level: 3
theory_related_stars: "⭐⭐⭐"
theory_related_color: "#ede9fe"
tags:
  - theory/educational-policy-reform
  - theme/implementation-science
  - theme/evidence-informed-practice
  - theme/sustainability
related_concepts:
  - "[[Paradigm]]"
  - "[[Reflexivity]]"
  - "[[Problem Solving]]"
  - "[[Hypothesis]]"
  - "[[Programme Fidelity vs Adaptation]]"
  - "[[Phronesis]]"
  - "[[Metacognition]]"
  - "[[Business as Usual]]"
  - "[[Epistemic Stances]]"
  - "[[Ontology]]"
  - "[[Epistemology]]"
  - "[[Pragmatic Paradigm]]"
  - "[[Formative Assessment]]"
  - "[[Evidence-Based Education]]"
  - "[[Interaction Effect]]"
  - "[[Heterogeneity]]"
  - "[[Research Literacy]]"
related_theories:
  - "[[Quality Use of Research Evidence Framework]]"
  - "[[Cognitive Load Theory]]"
  - "[[Complexity Theory]]"
  - "[[Organizational Culture]]"
related_methods:
  - "[[Analytic Framework]]"
  - "[[Implementation and Process Evaluation]]"
  - "[[Design-Based Research]]"
  - "[[Mixed Methods Research]]"
  - "[[Coding in Qualitative Research]]"
  - "[[Meta-analysis]]"
  - "[[Correlational Research]]"
related_instruments: []
related_persons:
  - "[[Mark Rickinson]]"
related_facts: []
related_arguments:
  - "[[Argument_Rickinson_2022_ER]]"
confidence: high
status: draft
created: 2026-09-18
updated: 2026-09-18
---

# Dynamic Sustainability Framework

---

## 理论定位

> [!theory-position] 理论定位
> - **解释对象** 循证干预措施在现实学校或社会组织落地后，为何随时间推移常遭遇成效衰退、形骸化或被彻底废弃，以及如何通过自适应调适维持长效机制。
> - **理论问题** 颠覆传统实施科学将可持续性视为“静态方案维持度”的机械忠实度[[Paradigm|范式]]，回应干预方案与复杂动态情境脱节的理论困境。
> - **理论类型** 中层实施科学理论、自适应组织变革模型与动态系统[[Analytic Framework|分析框架]]。
> - **知识位置** 源自实施科学、系统论与健康服务研究；由戴维·钱伯斯（David A. Chambers）、拉塞尔·格拉斯哥（Russell E. Glasgow）与库尔特·斯坦格（Kurt C. Stange）于 2013 年在《Implementation Science》发表奠基论文，并由 Chambers & Norton（2016）进一步发展；后被 [[Mark Rickinson]] 等学者引入教育证据高质量使用研究，构成 [[Quality Use of Research Evidence Framework|QURE]] 框架中“适切调适（Appropriate Adaptation）”的核心理论支柱。

> [!claim] 核心主张
> 干预措施的可持续性不是被动维持初始实验方案的静态忠实度，而是干预措施、实践现场与更广泛宏观生态系统之间通过持续学习、[[Reflexivity|反思性]][[Problem Solving|问题解决]]与适切调适而达成“动态拟合”（Dynamic Fit）的演进过程。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, pp. 140, 143–144)]]

> [!citation-card] 动态可持续性的系统观
> 实施本质上是一个动态的、跨系统的双向适应过程；干预措施、实践组织与生态环境都在持续演化，长效可持续性要求各系统要素在持续学习与问题解决中不断优化拟合度。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, p. 140)]]
>
> *Sustainability is not a static endpoint of intervention maintenance, but an ongoing process of continuous learning, adaptation, and dynamic fit between the intervention, the practice setting, and the broader ecological system.*

---

## 关键概念与理论构件

> [!entry-map]
>
> | 构件 | 类型 | 在理论中的功能 |
> |:-----|:-----|:--------------|
> | 动态拟合（Dynamic Fit） | 核心机制 | 衡量干预措施的核心功能与组织特定情境、文化和资源之间的匹配与共生程度。 |
> | 电压降假说批判（Voltage Drop Critique） | 理论靶子 | 批判传统[[Paradigm\|范式]]将实施成效衰退归咎于实践者未严格按原样照搬的机械[[Hypothesis\|假设]]。 |
> | [[Programme Fidelity vs Adaptation\|忠实度与调适平衡]] | 概念边界 | 区分破坏干预机理的盲目漂移（Program Drift）与基于[[Phronesis\|实践智慧]]的适切调适（Appropriate Adaptation）。 |
> | 核心功能与适配形态（Core Functions vs Forms） | 结构构件 | 锁定干预发挥效能的底层因果逻辑（核心功能），同时允许具体实施步骤与工具形式随情境调整。 |
> | [[Implementation and Process Evaluation\|过程评价]] | 诊断方法 | 追踪组织日常运作中拟合度演进、反馈收集与循环改进的经验手段。 |

---

## 核心命题与机制

> [!proposition-chain] 核心命题一｜实践现场与外部系统处于持续变迁之中，静态复制固定干预方案必然导致系统不相容与成效衰减
> - **前提一** 学校、社区与宏观政策环境并非静态实验温室，师资流动、生源构成变化、政策考核更迭均使实践情境具有高度动态性。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, p. 140)]]
> - **前提二** 传统实施观将干预措施视作封闭“标准化药丸”，要求实施者无条件执行固定规程，剥夺了基层专业人员的情境裁量权。
> - **推导** 当外在环境改变而干预形态固化时，干预措施与组织日常生态必然发生摩擦排斥，最终沦为形式主义或被迫中止。

> [!proposition-chain] 核心命题二｜长效可持续性依赖干预措施、组织环境与宏观系统三者之间的动态拟合与双向自适应
> - **前提一** 高质量证据利用不是被动采纳，而是结合本土情境对干预方案展开“深思熟虑的参与和实施”。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, pp. 143–144)]]
>   > [!evidence-grid]- 证据
>   > - **跨领域综述验证** 卫生保健与教育领域综述表明，获得长期维系且成效显著的变革，均允许实践者根据学生特征与学校资源调整干预载体，同时坚守关键教学机制。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, p. 140)]]
> - **前提二** 组织自身需建立反馈学习结构，促使实践现场的组织流程、教师协作文化向干预需求发生适配性演化。
> - **推导** 可持续性是干预（Intervention）、组织（Practice Setting）与生态系统（Ecological System）相互学习、双向塑造的结果；适切调适增强了干预与本土情境的契合度，从而提升长期干预成效。

> [!proposition-chain] 核心命题三｜高质量适切调适必须以维系干预的核心功能为边界，警惕瓦解因果链条的无效漂移
> - **前提一** 干预的有效性取决于特定的理论作用机制（如支架式反馈、[[Metacognition|元认知]]唤醒）；若关键机制被剔除，干预便丧失科学依据。
> - **前提二** 适切调适强调“变其形而守其神”，即允许根据学情调整材料难度、课时节奏或载体，但严格保留促成改变的因果核心。
> - **推导** 评估调适质量的关键在于考察其实践逻辑是否符合证据的底层机理，而非表面形式的一致性。

> [!mechanism-map]- 动态可持续性拟合系统图
> ```mermaid
> flowchart TD
>   subgraph Macro ["宏观生态系统 (Ecological System)"]
>     P1["政策要求与拨款导向"]
>     P2["专业标准与学术共同体支持"]
>   end
> 
>   subgraph Meso ["实践现场组织 (Practice Setting)"]
>     S1["学校领导力与支持文化"]
>     S2["教师专业素养与协作日常"]
>     S3["资源配置与时间排程保障"]
>   end
> 
>   subgraph Micro ["干预措施 (Intervention)"]
>     I1["不变：底层因果核心功能 (Core Functions)"]
>     I2["可变：情境化实施形态 (Adaptable Forms)"]
>   end
> 
>   Macro <--> Meso
>   Meso <--> Micro
>   Micro -.动态反馈与持续改进.-> Meso
> ```

> [!exegesis]- 教育研究例子
> 某初中引入一项基于[[Cognitive Load Theory|认知负荷理论]]的实证数学分层教学方案。初始实验方案设计为每周三次课后一对一电脑辅导，但该校缺乏专属机房且课后师资紧张。学校未生硬照搬导致项目搁置，而是识别出该方案的核心功能在于“即时诊断性反馈与个性化变式练习”，随后将其形态调适为课内小组合作探究中的纸质微反馈单与同伴互评。这一调适既保留了因果核心，又高度契合学校日常运作节律，使干预方案历经多年仍稳固运行在[[Business as Usual|常态教学]]体系中。

---

## 理论立场与使用方式

> [!theory-stance] [[Epistemic Stances|认识论立场]]
> - **[[Ontology|本体论]]** 将组织、制度与实践变革视作开放、非线性的[[Complexity Theory|复杂适应系统]]（Complex Adaptive Systems），否定还原主义与确定性因果链。
> - **[[Epistemology|认识论]]** 秉持[[Pragmatic Paradigm|实用主义]]与情境化认识论；认为证据知识的效力是条件性、脉络化的，知识价值在与具体行动情境的持续互动中得以生成与确证。
> - **方法含义** 强调[[Formative Assessment|形成性评估]]、设计型研究（[[Design-Based Research]]）、[[Mixed Methods Research|混合方法]]过程追踪与实施科学评价，反对单一的事后断点测量。
> - **不能直接推出的东西** 不能直接断定一切现场随意变动皆具合理性；调适必须接受严谨的机理审查与效果监测。

> [!theory-use] 如何用于研究
> - **作为理论框架** 指导[[Evidence-Based Education|循证教育]]改革项目、课程创新方案的长期跟踪与可持续性评估。
> - **作为分析工具** 提炼“核心功能与可变形态[[Coding in Qualitative Research|编码]]矩阵”，分析实践者在实施过程中的具体调适行为及其对教学产出的[[Interaction Effect|调节效应]]。
> - **作为批判视角** 批判政策制定者对“机械忠实度”的教条考核，破除忽视学校[[Heterogeneity|异质性]]的“一刀切”政策问责。
> - **报告方式** 详细交代实施背景的演变轨迹、调适决策的具体动因与核心机制的保全程度。

---

## 适用边界

> [!theory-boundary] 适用边界
> - **适合解释** 中小学课程教学改革、全校性行为支持干预、区域教育创新项目的长期落地与制度化整合。
> - **谨慎使用** 涉及不可变更的法律强制底线或硬性安全规程（如实验室生化安全规范、未成年人强制报告制度）。
> - **不适合解释** 追求极端严格条件控制的实验室心理测量或纯理论概念演绎。
> - **常见误用** 将动态调适作为实践现场因执行不力、敷衍应付而导致方案彻底走样的免责借口。

---

## 发展脉络

> [!dev-timeline] 发展脉络
> - **2013–2016 — 框架创立与调适科学深化** Chambers 等人相继发表奠基论文并提出“调适组学”（Adaptome）概念，系统建构干预调适分类学与监测框架，直指传统“电压降”理论盲区。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, p. 140)]]
> - **2022 — 教育质量使用整合** [[Mark Rickinson]] 等人将 DSF 整合为 [[Quality Use of Research Evidence Framework|QURE]] 框架的关键构件，确立了“适切调适”作为深思熟虑使用证据的核心评价维度。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, pp. 140, 143–144)]]

---

## 争议与批评

> [!debates] 理论争议
>
> > [!axis] 核心功能界定的主观性难题
> > 批评者指出，在复杂的教育与社会干预中，往往极难清晰剥离出哪些部分属于不可变动的“核心功能”，哪些属于可自由变通的“表面形态”。
> >
> > - **实验方法学者（2015）** 认为模糊了核心与形式的界限会导致难以明确干预的真实有效成分。
> > - **Chambers 等人（2013）** 回应指出必须依赖扎实的干预作用机理研究与形成性[[Implementation and Process Evaluation|过程评估]]加以厘清。
>
> > [!axis] 调适自由度与评估科学性的平衡
> > 过度鼓励情境调适可能导致不同实施现场产出高度分化的干预版本，增加多中心综合评价与统计[[Meta-analysis|元分析]]的难度。
> >
> > - **定量评价研究者（2018）** 担忧[[Heterogeneity|异质性]]剧增削弱因果结论的外在效度。
> > - **Rickinson 等人（2022）** 强调教育实践以解决本土实际问题为第一要义，评估[[Paradigm|范式]]应当适应情境复杂性。[[Argument_Rickinson_2022_ER|(Rickinson et al., 2022a, p. 144)]]

> [!critique]- 批评索引
> - [[Argument_Rickinson_2022_ER|Rickinson et al., 2022a]] — 指出在教育领域推行 DSF 需同步建设教师的[[Research Literacy|研究素养]]与[[Reflexivity|反思性]][[Organizational Culture|组织文化]]，否则易滑入形式主义调适。

---

## 相关研究

> [!evidence-grid-a] [[Correlational Research|相关研究]]索引
> - [[Argument_Rickinson_2022_ER|Rickinson et al., 2022a]] — 采纳 DSF 构建 [[Quality Use of Research Evidence Framework|QURE]] 框架中“深思熟虑参与及适切调适”构件的核心理论基础。

---
title: Research Knowledge Mobilisation Model
aliases:
  - 研究知识动员模型
  - RKM
  - Levin's RKM Model
...
>
> *Knowledge mobilisation happens where two or more of these contexts interact. These contexts do not refer necessarily to structures or organisations but, rather, to functions. The model acknowledges that some people and groups can operate in more than one context, which is why the triangles in the representation overlap. The multiple organisational and personal connections between the contexts are represented by two-way arrows of different thicknesses, indicating the strength or the intensity of these relationships.*
...
> | 五阶段研究利用阶梯（Measurement Ladder） | 测量构件 | 提供从信息获取、领会理解、态度信念、行为改变到最终受益人产出的五级递进度量基准。 |
> | 机制—障壁诊断比（Diagnostic Ratio） | 诊断构件 | 测度生产、中介与使用各界面促成机制与阻碍障壁的相对密度，评估系统真实运作优先级。 |
...
> [!mechanism-map]- Levin 研究知识动员系统拓扑图
> ```mermaid
> flowchart TD
>   subgraph MacroSystem ["宏观社会大背景 (Larger Social Context: 政治 / 法律 / 文化 / 治理体制)"]
>     subgraph CoreMobilisation ["三大功能情境重叠交互域 (Knowledge Mobilisation)"]
>       P["研究生产功能<br>(Research Production)"]
>       U["研究使用功能<br>(Research Use)"]
>       M["研究中介功能<br>(Research Mediation)"]
>       
>       P <-->|"粗箭头 (高频互动)"| M
>       M <-->|"粗箭头 (长期共创)"| U
>       P <-.->|"细箭头 (直接弱联结)"| U
...
> - **作为阶段化监测标尺** 依托 Levin 的五阶段利用阶梯（获取 → 理解 → 态度 → 行为 → 最终产出），为教育科研政策评估提供多层级的效果追踪工具，避免将中介机构的受众覆盖量（初级阶梯）盲目等同于学生学习成效（终极阶梯）。
> - **作为中介机构功能设计指南** 指导智库、教师协会与跨界研究团队摆脱“单向论文推介”，转向在生产与使用重叠地带设计联合共创项目。[[Argument_Torres_2022_KMModels\|(Torres, 2022b)]]
> - **作为运作优先级评估器** 运用各情境的“机制—障壁比”，诊断全系统知识动员的结构性梗阻与政策盲区。[[Argument_Torres_2022_BarriersMechanisms\|(Torres, 2022a)]]
...
> [!debates] 理论争议
>
> > [!axis] 模型的高度抽象性与因果机制细化不足
> > 批评者指出 Levin 模型描绘了优雅的系统轮廓，但未充分解开重叠地带内部的微观因果“黑箱”。
> >
> > - **宏观系统论者** 赞许其抓住了全系统整体演进与功能弹性的本质，具备极高的政策概括力。
> > - **微观认知与行为学者** 指出模型未能详细说明行动者在重叠地带究竟经历何种心理转化（需补充 Langer 的 COM-B 机制或 Rickinson 的 QURE 质量维度，[[Argument_Torres_2022_KMModels\|Torres, 2022b]]）。

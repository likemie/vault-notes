---
title: Capability Approach in Education
aliases: ["可行能力路径", "capability approach", "Sen's capability approach"]
type: theory
tags: [capability-approach, higher-education, social-justice, philosophy]
related_concepts: ["[[University Rankings (Global)]]"]
related_theories: []
related_persons: ["[[Pepka Boyadjieva]]"]
related_arguments: ["[[Argument_Boyadjieva_2022_Springer]]"]
sources: ["books/Parreira do Amaral(Ed.)_2022_Springer/Ch8_Boyadjieva_2022.md"]
confidence: low
status: draft
created: 2026-05-11
updated: 2026-05-11
---

# Capability Approach in Education

## 核心主张

> [!tip]
> 可行能力路径（capability approach）由 Amartya Sen 和 Jean Drèze 系统阐述。该理论的核心主张是：发展的目标不应仅以资源或效用衡量，而应以扩展人们"实际享有的自由"（real freedoms that people enjoy）为评价标准。在教育领域，该路径为理解教育对个人自由的多重价值提供了一个超越纯经济回报的分析框架（Boyadjieva, 2022, p.128）。

---

## 核心命题

> [!abstract]
> Drèze & Sen（2002, pp.38–40）在教育语境中区分了教育对个人自由的五种不同价值（Boyadjieva, 2022, p.128）：

- **内在重要性（intrinsic importance）**：接受教育本身就是一种自由和能力的扩展，具有内在价值
- **工具性个人角色（instrumental personal roles）**：教育帮助个人获得更好的就业机会和收入
- **工具性社会角色（instrumental social roles）**：教育促进社会互动、公共事务参与和集体行动的效能
- **工具性过程角色（instrumental process roles）**：教育过程本身扩展了个人的社会网络和参与渠道
- **赋权与分配角色（empowerment and distributive roles）**：教育使个人能够更好地表达自身利益、参与决策并在社会分配中获得更公正的对待

---

## 发展脉络

> [!note]
> - 可行能力路径最早由 Amartya Sen 在 1980–1990 年代发展，作为对功利主义福利经济学的替代。
> - Drèze & Sen（2002）在 *India: Development and Participation* 中将该框架应用于教育领域，系统区分了教育的五种价值（Boyadjieva, 2022, p.128）。
> - Boyadjieva & Ilieva-Trichkova（2016）将可行能力路径与新制度主义视角结合，发展了高等教育的多维使命模型——区分个体/社会两个层次与工具性/内在性/转化-赋权性三个维度（Boyadjieva, 2022, pp.128–130）。

---

## 应用领域

> [!success]
> - [[University Rankings (Global)]] — Boyadjieva (2022) 以可行能力路径为基础构建多维使命模型，用于批判全球排名对高等教育公共价值的忽视
> - [[Pepka Boyadjieva]] 的高等教育多维使命模型——将五种教育价值重构为个体/社会×工具/内在/转化-赋权的矩阵框架

---

## 来源

- [[Ch8_Boyadjieva_2022]]

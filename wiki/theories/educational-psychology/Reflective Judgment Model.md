---
title: Reflective Judgment Model
summary: "King和Kitchener提出的关于个体如何理解认知过程并为劣构问题提供辩护的七阶段反思性判断模型。"
type: theory
theory_field: "educational-psychology"
theory_related_count: 5
theory_related_level: 0
theory_related_stars: ""
theory_related_color: "#e5e7eb"
aliases:
  - "反思性判断模型"
  - "reflective judgment model"
tags:
  - "theme/epistemological-beliefs"
related_concepts:
  - "[[Reflexivity]]"
  - "[[Hypothesis]]"
  - "[[Epistemology]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons:
  - "[[Jean Piaget]]"
related_facts: []
related_arguments:
  - "[[Argument_Hofer_1997_RER]]"
status: active
created: 2026-08-13
updated: 2026-08-13
---

# Reflective Judgment Model

## 理论定位

> [!theory-position] 
> - **解释对象** 个体对劣构问题（ill-structured problems）的推理与信念辩护方式。
> - **理论问题** 人们如何理解认知的过程，以及相应地如何为没有确定答案的问题辩护？
> - **理论类型** 基于[[Jean Piaget|皮亚杰]]发展论的七阶段认识认知（Epistemic cognition）模型。
> - **知识位置** 详细阐发了Perry图式的高阶阶段，区分了前反思、准反思与反思阶段。[[Argument_Hofer_1997_RER|(Hofer & Pintrich, 1997, p. 100)]]

## 核心命题与机制

### 反思性判断的七个阶段

> [!proposition-chain] 
> [[Reflexivity|反思性]]判断能力从前反思阶段、准反思阶段发展至反思阶段，知识本质从绝对确定演变为开放的概率性评估。

> [!chain-link]
> - **前提一** 发展是由认知失衡引发的同化与顺应，个体对知识的[[Hypothesis|假设]]不断进行结构性重组。
> - **机制一** 前反思阶段（Pre-reflective，阶段1-3）：认为知识是确定且绝对的，通过权威直接获得。
> - **机制二** 准反思阶段（Quasi-reflective，阶段4-5）：认识到确定性是不可能的，认为知识受限于认知者视角，每个人都有权保留己见，但尚不能有效协调证据与论点。
> - **结论一** 反思阶段（Reflective，阶段6-7）：知识是主动建构、受语境限制的，个体能够结合多种视角，运用批判性质询和概率性辩护来评估结论的合理性。[[Argument_Hofer_1997_RER|(Hofer & Pintrich, 1997, pp. 100-101)]]

## 适用边界

> [!theory-boundary] 
> - **适合** 解释个体（尤其是青少年到成年期）在面对复杂真实世界问题时的认知成熟度。
> - **谨慎** 高阶的[[Reflexivity|反思性]]判断（阶段6与7）极其罕见，通常仅在接受高级研究生教育的个体中出现。
> - **局限** 将信念的激活局限于“劣构问题”可能无法完全反映个体在日常教育环境中所抱持的普通[[Epistemology|认识论假设]]。[[Argument_Hofer_1997_RER|(Hofer & Pintrich, 1997, pp. 102-103)]]

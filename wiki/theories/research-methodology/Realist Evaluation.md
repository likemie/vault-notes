...
> - **理论问题**：批判实证主义将评估等同于单纯测量“干预是否有效”的扁平化经验规律（休谟因果观），回应“什么在什么情境中对谁有效，为什么，以及如何有效”的现实评估需求。
...
> - **前提二：推理与资源的整合**：干预项目本身不能直接“产生”变化，它只提供资源（自变量）；结果（因变量）必须经由人（受试者）的能动性与信念进行推理（中介变量）而产生。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 7]])
> - **机制：CMO 配置 (Context-Mechanism-Outcome Configuration)**：因果机制（Mechanism, M）被引入不同的社会背景（Context, C）中，会因为背景对机制的触发或抑制而导致完全不同的结果（Outcome, O）。即：\(C + M \rightarrow O\)。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 8; Rambla, 2022, p. 165]])
> - **结果判断：去情境化统计聚合的失效**：将不同情境的研究混为一谈并计算“平均效应量”（如在元分析中）是盲目的经验主义。这种“洗涤过程”抹杀了关键机制与背景，无法指导任何具体的教育改进。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 13]])
...
> EEF Toolkit 的元-元分析将教学助理（TA）项目评为“低影响、高成本”（效应量仅 $+0.08$），导致许多学校计划裁撤 TA。实在论评估则对这一结果进行解构，展现不同的 CMO 配置：
...
> - **结果 (Outcome)**：在特定背景中激活特定机制后产生的非线性涌现结果。
> - **实在论综合 (Realist Synthesis)**：一种系统综述方法，旨在通过探究不同研究中的 CMO 配置来综合出关于“干预如何工作”的中层理论，而非对效应量进行数学平均。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 14]])
...
> - **本体论**：深度实在论（Depth Realism）。世界是分层的，结构和机制是真实的，且其因果力量独立于我们的观察。
> - **认识论**：反实证主义/经验主义。知识是概念中介的；因果关系不等于恒常规则性，而是机制在开放系统中的复杂涌现。
> - **方法含义**：方法论多元主义（Methodological Pluralism）。结合质性观察、历史分析、行动研究和统计分析，重在追踪和验证机制的运作，而非单纯估计效应量。
...
> [!theory-use] 如何用于研究
> - **作为理论框架**：替代传统的 EBE “什么有效”框架，分析政策在不同地域、学校或班级中实施时产生的异质性结果。
> - **作为分析工具**：在评估具体教育项目（如项目制学习、合成拼读）时，构建 CMO 矩阵，编码并提取影响效果的深层组织和交互因素。
...
> - **2006 — Pawson (Evidence-based Policy: A Realist Guide)**：将实在论评估扩展到系统综述领域，提出“实在论综合（Realist Synthesis）”方法，系统批判经验主义循证政策。
> - **2018 — 教育研究批评 (Wrigley, 2018)**：Wrigley 在 BERJ 中将实在论评估和批判实在论作为解构 EBE 统计聚合（Hattie、EEF Toolkit）的主要方法论武器。
...
> - **对决策者的沟通障碍**：政策制定者更青睐直观、简化的效应量排行榜或“几个月额外进步”的数字，实在论评估所提供的复杂情境叙事难以提供瞬间的决策支持。([[Argument_Wrigley_2018_BERJ|Wrigley, 2018, p. 15]])
...
> - [[Argument_Wrigley_2018_BERJ|Wrigley (2018)]] — 用实在论评估的因果机制观，深度解构了 EEF Fresh Start 拼读项目和教学助理项目由于去情境化聚合导致的政策误导。
> - [[Argument_Rambla_2022_Springer|Rambla (2022)]] — 将实在论评估框架应用于剖析欧盟技能议程政策，分析特定政策干预在特定情境中激活的因果机制。

...
> [!exegesis]- 教育研究例子
> 一位研究者最初计划用[[Questionnaire|问卷调查]]教师的工作满意度（方法先行）。但通过概念框架的反思，她意识到满意度是一个多维[[Construct|构念]]（内在满足、外在条件、关系质量），单一的评分量表无法捕捉其复杂性。她调整了研究问题（从"教师满意度水平如何"变为"教师如何体验和建构他们的工作满意度"），并在方法上增加了半结构化访谈。同时，她意识到个人目标（希望研究结果能用于学校改进）要求她的研究在效度上必须获得参与者的认可。在研究的迭代过程中，她不断回到互动模型，检查五个领域是否仍然相互一致。这就是 Maxwell 模型在实际研究中的运作方式。

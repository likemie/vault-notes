...
>
> 该框架的适用范围不限于某一特定设计类型——它统一适用于[[Randomised Controlled Trials|随机对照试验]]（RCT）、[[Quasi-Experimental Designs|准实验设计]]、[[Single-Case Design|单一个案设计]]（[[Single-Case Design|SCD]]）等多种能够产生因果证据的设计（Hitchcock et al., 2015, p.462）。
>
...
> - **对实验设计的统一性**：该框架的关键洞察是不同实验设计（[[Randomised Controlled Trials|RCT]]、准实验、[[Single-Case Design|SCD]]）共享相同的因果推断逻辑——都通过排除替代解释来建立因果关系——因此可以被统一的理论框架所涵盖（Hitchcock et al., 2015, p.462）
...
> - **[[Single-Case Design|SCD]] 推广性的特殊偏见**：存在"一种普遍（且错误）的信念认为 [[Single-Case Design|SCD]] 证据不能推广"（Hitchcock et al., 2015, p.467）——这意味着框架在面对 SCD 时面临额外的社会认识论障碍，不仅仅是方法论障碍
...
> - [[Educational Evidence Clearinghouses]] — Wadhwa et al. (2024) 用[[Construct Validity in Quantitative Measurement|构念效度]]问题重新审视清算中心对"evidence-based"项目的认证实践，显示跨机构评级收敛不足会削弱该术语的实践效度（Wadhwa et al., 2024, p.26）

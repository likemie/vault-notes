...
Cartwright & Munro (2010) 提出，RCT 结果的外推需要经过三个不同层级的知识阶段。因为 RCT 的设计前提恰恰是"我们不知道所有重要的因果因素"——随机化用以控制未知因素——所以 RCT 结果本身不能直接外推到环境不同的新情境。外推需要额外的因果机制知识来弥补 RCT 的设计性无知。

> "The primary purpose of an RCT is to make a judgement on effectiveness where the contribution of other circumstantial factors are unknown... Since an RCT is necessary, that knowledge of generalisability to different circumstances is likely to be absent."（Cartwright and Munro, 2010, 引自 Cowen et al., 2015, p.10-11）

## 理论内部结构

### 阶段一：It-works-somewhere 主张
T（干预措施）在某种条件下对某人群引起 O（结果）（例如在研究人群 X 中，以方法 M 实施）。
- 这是 RCT 能直接提供的唯一主张
- 只告诉你"在某处有效"，不告诉你"为什么有效"或"在你这里是否有效"

> 例：一项 RCT 显示，在田纳西州的特定学校中，缩小班级规模提高了阅读成绩。但这只告诉你"在田纳西州那些学校有效"（Cowen et al., 2015, p.17-18）
...
### 阶段三：It-will-work-for-us 主张
T 将在人群 q 中以政策 P 规定的方式实施时引起 O（即可行性／有效性主张）。
- 这需要将因果机制知识与本地情境因素结合
...
- Cowen (2019) 在 *Educational Research and Evaluation* 中将此框架与 Tullock 官僚制理论结合，解释 EBE 的制度韧性

## 研究范式

- 认识论立场：科学实在论——承认因果机制真实存在且可被研究，但反对简单的实证主义归纳
- 分析方法：概念分析 + 案例研究 + 政策分析
- 不依赖定量数据，而是对 RCT 方法论的逻辑结构进行哲学剖析

## 争议与批评

- **过度复杂化**：有批评认为三阶段框架对从业者来说过于复杂，忙碌的教师和政策制定者需要简化的实施指南，而非哲学分析（编者类比）
- **并非反对 RCT**：Cartwright 明确表示该框架不是为了否定 RCT 的价值，而是为了正确使用 RCT。她承认有时大效应可以"压倒"其他不可观察的因素，使从业者即使在知识有限的情况下也能行动（Cowen et al., 2015, p.11）
...
- [[Evidence-Based Education]] — 该框架被用于批判纯 RCT 中心的证据观，论证本地知识和专业判断在教育证据实施中的不可替代性

...
summary: "研究者与实践者合作，通过计划—行动—观察—反思的螺旋循环过程（勒温提出，凯米斯改进）解决实际问题的研究范式，强调实践改进而非理论建构"
type: method
method_type: mixed
method_family: "mixed"
...
勒温首先提出"计划→行动→观察→反思"的基本螺旋循环模型，凯米斯在此基础上提出"计划→行动→观察→反思→再计划→……"的改进模型，强调行动研究是一个螺旋式加深的发展过程——每一个螺旋发展圈包含这四个环节，完成一圈的反思后进入下一圈的再计划，每一圈都在前一圈基础上深化。

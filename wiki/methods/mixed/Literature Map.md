...
aliases: ["文献地图", "文献图谱", "literature mapping", "literature diagram"]
...
文献地图的功能不仅是分类整理，更是一种**论证装置([[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022]])** 通过可视化展示"已有研究的版图是什么样的，我的研究将延伸版图的哪一部分"，它让研究的合法化空间变得直观可见——底部的"Need to Study"框与上方已有文献分支之间的连线，本身就是对"本研究为何必要"的视觉论证。
...
3. **在每个分类框中标注文献性质** 用标签描述该分类中研究的特征（如"结果""信任""组织支持"等）。
4. **在每个分类框中标注关键引文** 放入说明该分类内容的主要引文，使用适当的格式（如 [[APA Style|APA]] 格式）。使用当前和有代表性的文献引用。
...
> [!example] 文献地图示例结构
> 以 Janovec (2001) 的组织公平感（Procedural Justice）研究为例：
> - **顶层主题** 组织中的程序公平
...
> > [!example]- 公平文献地图
> > 
> > ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/Figure_2-1_Literature_Map_Example.jpg)

从这个示例中可以归纳几条设计原则([[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022]])：不同分支的展开程度不必均匀——取决于每个分支下已有文献的多少和研究者的探索深度；地图中应同时包含量化、质性和混合方法研究；某些分支比另一些更发达，这如实反映了不同子领域的研究积累差异；地图需要配合一段叙述性说明，让读者（尤其是第一次见到文献地图的人）理解顶层的广泛主题、搜索过的数据库、文献的划分逻辑，以及计划研究的定位与延伸方向。
...
> - **使用软件程序绘图** 可使用 Word 文档或 PowerPoint 幻灯片来绘制图形

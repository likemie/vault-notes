---
title: Typological Analysis
aliases:
  - 类型学分析
  - 类型分析
  - typological analysis method
summary: "按明确标准把质性数据分入组、子集或类别的分类过程，是次级编码与类型建构的基础"
type: method
...
> 1. **汇集数据** 刻意汇集参与者在某一议题上的全部数据——采用了什么策略。
> 2. **分解差异** 拆分并分离策略实例范围之间的变异。
> 3. **分类** 把这些变异归入集合与子集。
...
> [!method-fit] 适用判断
> - **适合使用** 需要按性质、行为、意义或参与方式对数据建立清晰分类体系的研究。
> - **谨慎使用** 分类标准不明确或类别间重叠时，须先厘清标准。
> - **不适合使用** 需要解释类别成因或建立因果的研究，类型学本身不提供解释。
...
> | 条目 | 类型 | 关系 |
> |:-----|:-----|:-----|
> | [[Coding in Qualitative Research\|质性编码]] | 前置方法 | 类型学是编码后的次级编码过程，把描述性编码汇总为子集。 |
> | [[Constant Comparison\|持续比较]] | 补充方法 | 通过比较新数据与既有类别，修正分类直至容纳所有数据。 |
> | [[Content Analysis\|内容分析]] | 相近方法 | 都涉及按标准分类；内容分析更强调穷尽且互斥的类别化与频次。 |
> | [[Analytic Induction\|分析归纳]] | 补充方法 | 从分类出发寻求类别间关系并生成理论。 |

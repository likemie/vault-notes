...
> [!proc] Tesch 系统编码八步规程（Tesch, 1990; Creswell & Creswell, 2022, Ch. 9）
...
> > 超越单纯的描述性归纳，提炼具有推论性与解释力的分析标签。例如将“实验”“控制变量”“测试”等描述性活动编码综合提炼为“像科学家一样工作”，或将教师“忽略破坏”“不予回应”等行为提炼为“教师放弃/否认”。
>
> > [!step] 阶梯三：主轴编码（Axial Coding）
> > 在开放编码完成碎化后，将数据围绕核心轴心范畴重新组合，建立范畴内部子群以及不同范畴之间的联结。主轴编码依据经典六要素范式模型分为两大分析向度（Buckley & Waring, 2009）：
> > - **前置脉络与诱发条件** 包括因果条件（诱发核心现象的前置诱因）、情境脉络（特定外部行动场域）与中介条件（促成或阻碍行动的宏微观结构条件）。
> > - **核心行动与衍生结果** 包括核心现象（处于中心位置的事件或行动模式）、行动与互动策略（应对现象采取的策略过程）与结果（策略直接导致的实质后果）。
>
> > [!step] 阶梯四：选择性编码（Selective Coding）
> > 识别并确立统摄所有已建范畴的单一核心范畴（core category），将其余范畴系统关联至核心范畴并接受经验数据验证。选择性编码遵循五步循环迭代：深度把握主要[[Story Line|故事线]]（story line）➔ 提炼核心范畴 ➔ 建立维度关联 ➔ 依据经验数据逐一验证 ➔ 填补范畴空缺以实现概念密度（conceptual density）与理论饱和。
...
> 鲍格丹与比克连（Robert Bogdan & Sari Biklen）系统梳理了质性研究中最常见的九类描述性编码家族，归纳为三大经验分析维度：[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch30|(Bogdan & Biklen, 1992; Cohen et al., 2011, pp. 560–561)]]
>
...
> | 关系层级 | 条目 | 类型 | 关系 |
> |:---|:-----|:-----|:-----|
> | **方法论基石** | [[Grounded Theory\|扎根理论]] | 理论基石 | 提供开放、主轴、选择性编码的完整方法论指引与理论饱和标准。 |
...
> | **质量控制与支撑工具** | [[Qualitative Codebook\|质性编码手册]] | 质量工具 | 规范编码定义、包容准则与正反样例以保障编码一致性。 |

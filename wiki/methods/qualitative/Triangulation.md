...
>   使用社会科学中三个主要分析层次中的多个层次：**个体层次**、**交互层次**（群体）和**集体层次**（组织、文化或社会）。Smith 进一步确定了七种可能层次：聚合/个体层次以及六个更全局的层次——群体分析（个体和群体的互动模式）、组织分析单位（具有个体不拥有的品质的单位）、制度分析（法律、政治、经济和家庭制度内部和之间的关系）、生态分析（关注空间解释）、文化分析（关注规范、价值观、实践、传统和意识形态）和社会分析（关注城市化和工业化等总体因素）。结合**多个分析层次**的研究更受青睐。
...
> > Silverman (1985) 认为三角验证概念本身是实证主义的——数据三角验证尤其暴露了这一点：[[Hypothesis|假设]]多重数据源（[[Concurrent Validity|共时效度]]）**优于**单一数据源或工具。假设单一单位总能测量多于一次，违反了互动论关于[[Emergence|涌现]]性、流动性、独特性和特定性的原则（Denzin, 1997: 320）。Denzin (1997) 回应了这一批评，坚称三角验证的效度不在于承诺达到客观真理，而在于对单一方法局限性的自觉。
> >
...
>
> > [!axis] 研究者三角验证的前提是否成立
> > Lincoln & Guba (1985: 307) 质疑研究者三角验证的[[Epistemology|认识论]]前提：假设一位研究者会**证实**另一位的发现——这是错误的、不可辩护的，尤其在质性[[Reflexivity|反思性]]探究中。两个理论永远不可能对正在研究的现象产生**足够完整**的解释。他们将三角验证重新定位为[[Interpretive Paradigm|自然主义探究]]中**数据的检查**，而[[Member Checking|成员核查]]则用作**对成员数据建构的检查（p. 315）**。
> >
> > - **Lincoln & Guba (1985)** 研究者三角验证的前提有问题——不同研究者可能且应该产生不同的解释。
> > - **L&G (1985: 315)** 提出替代性功能定位：三角验证是数据检查，成员核查是建构检查。
...
> - [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch19|Cohen et al. (2011, Ch. 19)]] — 在[[Virtual World|虚拟世界]]研究中，文字、聊天、语音和手势等不同沟通渠道的数据均可被记录和转录，通过时间戳（time-stamp）比较不同渠道的分析结果，实现数据三角验证（Martin & Vallance, 2008; Vallance & Wiz, 2008）。

...
>   使用社会科学中三个主要分析层次中的多个层次：**个体层次**、**交互层次**（群体）和**集体层次**（组织、文化或社会）。Smith 进一步确定了七种可能层次：聚合/个体层次以及六个更全局的层次——群体分析（个体和群体的互动模式）、组织分析单位（具有个体不拥有的品质的单位）、制度分析（法律、政治、经济和家庭制度内部和之间的关系）、生态分析（关注空间解释）、文化分析（关注规范、价值观、[[Praxis|实践]]、传统和意识形态）和社会分析（关注城市化和工业化等总体因素）。结合**多个分析层次**的研究更受青睐。

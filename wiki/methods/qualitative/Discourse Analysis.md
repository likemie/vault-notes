...
> [!method-scope] 方法范围
> - **研究对象** 日常谈话、课堂对话、文本材料、访谈转录——即作为情境化的话语实践的语言（Edwards, 1991）。
> - **问题类型** 语言如何执行社会行动；参与者如何通过话语建构共享的意义框架；话语中如何体现和再生产权力关系。
> - **分析单位** 话语序列、对话回合、言语行为、文本段落。
> - **输出形式** 对话语功能、意图和后果的分析性解释，附语言证据支持。

> [!citation-card]- 关键定义
> 话语分析研究者不将儿童的语言视为一个需要通过控制大量干扰变量来实验探索的内部认知世界的代表，而是将语言视为行动——视为**情境化的话语实践（situated discursive practice）**。（第22章, p.455）
>
...
> - **研究者角色** 研究者需要仔细阅读和解释文本材料，解释必须由语言证据支持；需要高度敏感于语言的细微差别（Coyle, 1995, p. 247）；需要保持反身性，因为研究者自身的话语也在建构"发现"。
...
> - **分析方法** 编码（早期阶段）、模式识别、言语行为功能分析、替代解释检验
> - **辅助技术** 转录、反身性记录、意识形态批判（批判理论传统）
...
> | 维度 | 信息 |
> |---|---|
> | 材料来源 | 自然发生的谈话（课堂对话、日常对话）、访谈转录、政策文本、书面材料 |
> | 抽样或选案 | 以话语事件或文本为单位；选择能体现研究问题的典型或关键话语情境 |
> | 研究者位置 | 研究者是话语的读者和解释者，但自身的阅读和解释也是话语实践，需要反身性审视 |
> | 资料边界 | 以话语序列或文本为边界；需说明为何选择这些话语材料而非其他 |

> [!proc] 话语分析程序
> 1. 收集自然发生的话语材料（录音、转录、文本），保留社会情境信息
> 2. 早期阶段使用编码为文本材料分配代码，发现话语中的模式和广泛领域（Parker, 1992; Potter & Wetherell, 1987）
> 3. 重新检视文本，发现话语的意图、功能和后果——检查言语行为功能（传递信息、说服、指责、谴责、鼓励等）
> 4. 寻求替代解释——考察话语中的变异程度，排除竞争性解释
> 5. 关注话语中的权力不对称和意识形态维度（批判理论传统）
...
> - **谨慎使用** 当需要大样本统计概括时；当研究问题更适合通过实验或调查回答时；当研究者的话语立场可能过度影响解释时。
> - **不适合使用** 需要标准化测量和因果推断的研究；将语言仅视为信息传递工具的研究。
...
> - **补救方式** 高度的反身性；寻求替代解释和变异检验；将话语分析与制度和物质条件分析结合。
...
> - [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch22]] — 本章系统介绍话语分析的定义、与哈贝马斯批判理论的关联、Edwards的课堂话语分析案例（温室对话、Edwards & Mercer的课堂研究），以及方法批评。

---
summary: "实践认识论分析（Practical Epistemological Analysis, PEA）由威克曼（Per-Olof Wickman）与厄斯特曼（Leif Östman）基于维特根斯坦语言游戏创立。该方法将认识论置于具体行动之中，通过编码立足点（Stand Fast）、认知裂隙（Gaps）、建立关联（Relations）与情境碰撞（Encounters），精准追踪师生在探究中何者被视为不证自明、何时遭遇理解断裂，以及如何调动媒介就地建构新意义。"
type: method
method_type: qualitative
method_family: "qualitative"
method_related_count: 31
method_related_level: 3
method_related_stars: "⭐⭐⭐"
method_related_color: "#dbeafe"
title: "Practical Epistemology Analysis"
aliases:
  - "PEA"
  - "实践认识论分析"
  - "Practical Epistemological Analysis"
tags:
  - paradigm/pragmatism
  - method/discourse-analysis
  - method/qualitative
  - theme/classroom-discourse
  - theme/epistemic-practices
related_concepts:
  - "[[Practical Epistemology]]"
  - "[[Pragmatic Paradigm]]"
  - "[[Epistemology]]"
  - "[[Scaffolding]]"
  - "[[Unit of Analysis]]"
  - "[[Construct]]"
  - "[[Rich and Thick Description]]"
  - "[[Academic Achievement]]"
  - "[[Reflexivity]]"
  - "[[Hypothesis]]"
  - "[[Student-Teacher Relationship]]"
  - "[[Operationalization]]"
  - "[[Epistemic Practices]]"
  - "[[Nature of Science]]"
  - "[[Scientific Explanation]]"
related_theories: []
related_methods:
  - "[[Discourse Analysis]]"
  - "[[Transcription in Qualitative Research]]"
  - "[[Coding in Qualitative Research]]"
  - "[[Ethnography]]"
  - "[[Peer Debriefing]]"
  - "[[Observation Method]]"
  - "[[Qualitative Observation]]"
  - "[[Effect Size]]"
  - "[[Sample Size Determination]]"
  - "[[Pre-test and Post-test]]"
  - "[[Triangulation]]"
  - "[[Questionnaire]]"
  - "[[Correlational Research]]"
related_instruments:
  - "[[Views of Nature of Science Questionnaire]]"
related_persons: []
related_facts: []
related_arguments:
  - "[[Argument_Kelly_Licona_2018_EpistemicPractices]]"
  - "[[Argument_Sandoval_2005_SE]]"
confidence: medium
status: draft
created: 2026-09-22
updated: 2026-09-22
---
# Practical Epistemology Analysis

---

## 定义

> [!def] 方法定义
> [[Practical Epistemology\|实践认识论]]分析（Practical Epistemological Analysis, PEA）是一种立足于维特根斯坦（Ludwig Wittgenstein）语言游戏理论与[[Pragmatic Paradigm\|实用主义]][[Epistemology\|认识论]]的质性微观[[Discourse Analysis\|话语分析]]方法。该方法由佩尔-奥洛夫·威克曼（Per-Olof Wickman, 2004）与莱夫·厄斯特曼（Leif Östman, 2014）创立，旨在通过微观考察课堂日常对话与操作性行动，追踪参与者如何就地（in situ）协商何者算作有效知识。PEA 将分析焦点从静态的心智信念转向动态的行动认识论，通过识别立足点（`Stand Fast`）、认知裂隙（`Gaps`）与关系建立（`Relations`），系统刻画学习者在面对反常或新现象时意义建构的具体推进轨迹。[[Argument_Kelly_Licona_2018_EpistemicPractices\|(Kelly & Licona, 2018, pp. 149–150)]]

> [!method-scope] 方法范围
> - **研究对象** 课堂教学互动中的口头交谈、具身手势、实验操作以及实物媒介（如地图、天球仪、仪器读数）。
> - **问题类型** 适合回答学习者如何在即时活动中产生理解障碍、如何调动先验经验建立新关联，以及特定[[Scaffolding\|教学支架]]如何影响意义生成。
> - **[[Unit of Analysis\|分析单位]]** 连续话语回合（turns of talk）、具身操作片段或师生围绕特定科学现象展开的微观交互事件。
> - **输出形式** 微观话语[[Transcription in Qualitative Research\|转录]][[Coding in Qualitative Research\|编码]]序列、认知裂隙产生与弥合的动态轨迹图、学生就地意义建构的质性机制阐释。

> [!citation-card] 论实践认识论分析的微观核心
> 实践认识论分析并不将科学认识论预设为一个既定的抽象实体，也不试图机械灌输给学生，而是聚焦于学生和教师理解现象与发展知识的日常实践。这种视角将认识论视为嵌入在持续的交流、行动与实践之中的动态过程。该方法考察参与者在对话中哪些概念是不证自明的立足点（stand fast），以及当学生遭遇挑战时所出现的认知裂隙（gaps），并进而分析学生如何通过建立新关联（relations）来跨越裂隙。（pp. 149–150）
>
> *Rather than consider the epistemology of science as a known entity, and seeking to inculcate students into this perspective, a practical epistemology analysis focuses rather on the everyday practices of students and teachers making sense of phenomena and developing knowledge. This view takes epistemology as situated “in on-going communication, action, and practice”... This approach considers those concepts that stand fast (assumed communal meaning) and the gaps (lack of initial understanding) that occur when students encounter challenges through talk or action. Students build relations with understood and effectively useful [[Construct\|constructs]] across gaps in understanding.*

---

## 方法定位

> [!method-position] [[Epistemology\|认识论]]与方法定位
> - **知识观** 知识不是储存在大脑中的静态命题信念，而是在具体社群中通过言语、工具与行动被提出、使用与检验的实践成就。
> - **研究者角色** 研究者采取“行动中的科学”（science-in-the-making）的[[Ethnography\|人种志]]观察立场，不预设绝对的规范性标准，而是关注参与者自身在互动中如何界定何者算作有效证据。
> - **有效性标准** 依托微观多模态[[Transcription in Qualitative Research\|转录]]的[[Rich and Thick Description\|厚描述]]、语料情境的完整还原、团队交叉[[Coding in Qualitative Research\|编码]]的一致性检验，以及同伴审议（[[Peer Debriefing]]）。
> - **不声称回答的问题** 不能直接测量学生去情境化的普遍心智发展阶段，亦不用于量化统计学生长期[[Academic Achievement\|学业成绩]]的净增长值。

> [!method-stack] 方法层级
> - **研究设计** 微观交互[[Ethnography\|人种志]]、课堂[[Discourse Analysis\|话语分析]]、实验室微观[[Observation Method\|观察研究]]。
> - **数据收集** 课堂多角度音视频录像、师生即时手势与操作记录、实验教具交互轨迹、学生生成性文本与图表。
> - **分析方法** 逐行多模态话语[[Coding in Qualitative Research\|编码]]、意义裂隙分类分析、关联建构路径追踪。
> - **辅助技术** 音视频切片转录软件（Transana, ELAN）、微观言语互动情境还原、手势与语调多模态标注。

---

## 研究程序

> [!proc] 通用程序
> 1. **确定微观互动事件** 选取师生或同伴围绕某一探究任务（如重力测量、振动规律、化学反应）展开的完整话语片段。
> 2. **进行多模态精细[[Transcription in Qualitative Research\|转录]]** 将口头言语、停顿、语调高低、身体手势、仪器操作与教具移动同步转录为对应行。
> 3. **执行 PEA 核心四元[[Coding in Qualitative Research\|编码]]** 逐句标定对话中的立足点（`Stand Fast`）、认知裂隙（`Gaps`）、建立关联（`Relations`）与情境碰撞（`Encounters`）。
> 4. **绘制意义建构轨迹** 梳理学生如何通过建立新关联填补裂隙，或者旧有关联为何失效从而引发新的认知裂隙。
> 5. **提炼教学互动机制** 分析教师介入话语（如元话语提示）或实物工具如何引导学生克服裂隙，形成结论性解释。

### 质性分析程序

> [!sample-panel] 材料与进入现场
> | 维度 | 信息 |
> |---|---|
> | **材料来源** | 科学探究课原生态录像转录文本、小组实验台微观录音、黑板板书与实验记录单。 |
> | **抽样或选案** | 目的抽样：选取包含反常数据争论、概念认知分歧或典型仪器操作困难的微观互动片断。 |
> | **研究者位置** | 非介入式[[Qualitative Observation\|田野观察]]者，保持对课堂微观话语权力和文化背景的敏锐[[Reflexivity\|反思性]]。 |
> | **资料边界** | 聚焦探究任务的关键交互阶段（通常为 5–30 分钟的高密度互动情境）。 |

> [!proc] 质性话语编码四阶段操作程序
> 1. **识别立足点（Stand Fast）** 寻找当前对话中被所有参与者共同默认、无需进一步解释或辩护的词汇、概念与常识性操作（例如学生讨论“物体往下掉”时，“往下”是不证自明的）。
> 2. **定位认知裂隙（Gaps）** 标定参与者遭遇困惑、产生分歧或无法用现有词汇解释观察结果的时刻（例如“如果地球是球形的，地球背面的人为什么不会掉下去”）。
> 3. **追踪关联建构（Relations）** 分析学生如何将立足点与新观察对象连接（例如通过旋转地球仪，指出“下”其实是指向“地心”，从而搭建新概念桥梁）。
> 4. **评估[[Epistemology\|认识论]]后果** 判断所建立的关联是成功推进了探究、维持了原有误解，还是催生了更深层次的探究问题。

---

## 核心要素

> [!feature] [[Practical Epistemology\|实践认识论]]分析的核心分析范畴
> - **立足点（Stand Fast）** 
>   在特定互动时刻被参与者理所当然接受、无需争论的基础概念与动作。它是所有意义建构的出发点与依托基石。
> - **认知裂隙（Gaps）** 
>   在探索与交流中由于观察到反常现象或遭遇他人质疑而出现的理解空白或意义断裂，分为概念裂隙与操作裂隙。
> - **建立关联（Relations）** 
>   学习者调动先验经验、具身动作或实物教具，将立足点与未知对象建立语义或因果连接的动态过程。
> - **情境碰撞（Encounters）** 
>   学习者与外部物质环境（实验仪器、样本、天球仪）或他人话语发生直接互动，促使潜在[[Hypothesis\|假设]]显性化。

---

## 适用场景

> [!method-fit] 适用判断
> - **适合使用** 
>   - 探究科学课堂中微观意义建构与概念转变的即时发生过程；
>   - 分析具身动作、物理教具与数字传感器在学生推理中的中介作用；
>   - 考察教师在师生对话中如何通过提问与元话语有效搭建[[Epistemology\|认识论]]支架。[[Argument_Kelly_Licona_2018_EpistemicPractices\|(Kelly & Licona, 2018, pp. 149–150)]]
> - **谨慎使用** 
>   - 教师主导的纯讲授式大班教学（学生自发言语极少，难以捕捉裂隙与关联建构）；
>   - 缺乏高质量录像与多模态[[Transcription in Qualitative Research\|转录]]条件的二手研究。
> - **不适合使用** 
>   - 试图评估教学干预对全校学生标准化测试成绩的宏观平均[[Effect Size\|效应量]]；
>   - 测量孤立个体心智中跨情境的抽象认知阶段。

---

## 局限性

> [!method-limits] 方法局限
> - **[[Transcription in Qualitative Research\|转录]]与[[Coding in Qualitative Research\|编码]]成本高昂** 微观多模态转录耗时巨大，通常仅能对数十分钟的关键课堂片段开展精细分析，[[Sample Size Determination\|样本量]]较为有限。
> - **情境高度特异性** 发现的裂隙弥合路径高度依赖具体的课堂情境、[[Student-Teacher Relationship\|师生关系]]与实验器材，难以直接进行大样本统计推广。
> - **对研究者洞察力依赖度高** 辨识何者真正“立足”（`Stand Fast`）需要研究者深入理解学科知识与儿童日常语言，容易受研究者主观解释偏误的影响。
> - **补救方式** 采用团队多编码员交叉检验、结合前[[Pre-test and Post-test\|后测]]文本进行[[Triangulation\|三角互证]]，并公开详尽转录文本片段以增强研究透明度与确认性。

---

## 相关理论与方法

> [!entry-map]
>
> | 条目 | 类型 | 关系 |
> |:-----|:-----|:-----|
> | [[Practical Epistemology]] | 理论基础 | 实践[[Epistemology\|认识论]]分析（PEA）是实践认识论（Practical Epistemology）在微观课堂话语中的具体质性分析方法[[Operationalization\|操作化]]。 |
> | [[Epistemic Practices]] | 分析对象 | PEA 为考察课堂中提出、沟通、评估与合法化主张的认识论实践提供了逐行微观[[Coding in Qualitative Research\|编码]]工具。 |
> | [[Discourse Analysis]] | 上位方法 | PEA 属于质性话语分析的专门分支，融合了语言哲学与课堂微观互动分析。 |
> | [[Views of Nature of Science Questionnaire]] | 互补工具 | [[Questionnaire\|问卷]]测量学生去情境化的显性[[Nature of Science\|科学本质]]信念，PEA 则捕捉学生在行动中就地展现的隐性实践认识论。 |

---

## 使用此方法的研究

> [!evidence-grid-a] [[Correlational Research\|相关研究]]索引
> - [[Argument_Kelly_Licona_2018_EpistemicPractices\|Kelly & Licona (2018)]] — 综述并运用 PEA 框架分析中小学物理、工程与科学课堂中的[[Epistemic Practices\|认识论实践]]与意义建构。
> - [[Argument_Sandoval_2005_SE\|Sandoval (2005)]] — 借鉴[[Practical Epistemology\|实践认识论]]视角分析学生在计算机探究环境中的[[Scientific Explanation\|科学解释]]与证据运用。

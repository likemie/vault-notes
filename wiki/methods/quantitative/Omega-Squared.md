...
> [!def] 方法定义
> **欧米茄平方（Omega-Squared, $\omega^2$）** 是在单因素或多因素[[Analysis of Variance|方差分析]]（Analysis of Variance, ANOVA）中用于度量分类[[Independent Variable|自变量]]（自变量处理或分组）解释连续[[Dependent Variable|因变量]]总变异比例的总体[[Effect Size|效应量]]估计量。由统计学家威廉·海斯（William L. Hays, 1963）系统提出，它通过从组间平方和中扣除由随机误差引起的变异，有效克服了传统样本决定系数 Eta-Squared（$\eta^2$）在小样本或多组设计中系统性高估总体效应的样本偏倚。[[Argument_Altrichter_2019_ZfB|(Altrichter et al., 2019, p. 27)]]

> [!method-scope] 方法范围
> - **研究对象** 实验设计、准实验研究或问卷调查中的组间均值比较数据（方差分析模型）。
> - **问题类型** 因果效应强度估计、组间差异实际重要性评估、总体方差贡献率测度。
> - **分析单位** 包含分类自变量分组标签与连续因变量得分的观测个体（如学生、教师、学校）。
...
> - **知识观** 属于频率学派参数统计与因果推断范式，主张不能仅依靠依赖于样本量的统计显著性检验（$p$ 值），必须通过报告对总体效应的无偏点估计来界定经验差异的实质科学价值。
> - **研究者角色** 研究者需检验方差分析的基本假设（正态性、方差齐性），在完全随机单因素、析因设计或重复测量设计中选取对应形态的 $\omega^2$ 计算公式，并合理解释效应量的理论与实践内涵。
> - **有效性标准** 统计结论效度（消除小样本正向偏倚）、跨研究可比性与元分析效应量汇总适切性。
> - **不声称回答的问题** $\omega^2$ 仅度量方差解释比例与关联强度，其数值高低本身不能证明自变量与因变量之间必然存在因果关系（需依赖严格的研究设计排查混杂偏误）。

> [!method-stack] 方法层级
> - **研究设计** 随机对照试验（RCT）、准实验组间比较、横截面分类调查设计。
> - **数据收集** 标准化测验成绩、李克特量表问卷调查、行为时间记录。
> - **分析方法** 单因素方差分析（One-way ANOVA）、多因素方差分析（Factorial ANOVA）。
> - **辅助技术** 
>   - Levene 方差齐性检验
>   - 事后多重比较（Post-hoc tests with Cohen's $d$ / Hedges' $g$）
>   - 效应量置信区间 Bootstrap 自助抽样法

---

## 统计原理与数学推导

> [!formula-step] 公式推导一　单因素方差分析中的 $\omega^2$ 解析式
...
> 在发表文献中，若仅报告了方差分析检验统计量 $F$ 值与自由度，可直接通过下式换算 $\omega^2$：
...
> 在多因素析因方差分析设计中，为了控制其他主效应与交互效应的变异干扰，通常计算偏欧米茄平方：
...
> [!method-heuristics] 效应量大小判定标准与指标选取规则
...
>
> | 研究文献 | 分析模型与变量 | $F$ 检验与样本量 | $\omega^2$ 统计结果 | 效应等级与实证解释 |
> |---|---|---|---|---|

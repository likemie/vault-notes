...
> 时间序列设计是对一个非随机取样的实验组（或控制组），在接受实验处理之前和之后，重复接受若干次测量，而非仅在处理前后各接受一次测量的[[Quasi-Experimental Designs|准实验设计]]（[[Argument_QiMei_2015_EducationalResearchMethods|齐梅, 2015]], Ch.4）。若前测每一次所得分数大致相同，但后测平均数高于前测，则表示该实验处理产生了正向效果。
...
基本模式为：O₁ O₂ O₃ X O₄ O₅ O₆（处理前多次测量 → 处理 → 处理后多次测量）。处理前的观察称重复前测（repeated pretests），处理后的观察称重复后测（repeated posttests）（[[Argument_QiMei_2015_EducationalResearchMethods|齐梅, 2015]], Ch.4）。
...
统计分析上，可以将两组各自的一系列时间前测成绩的平均数与一系列后测成绩的平均数加以比较，从成绩增减说明处理效果；也可以将两组之间的一系列时间前后测成绩相比较，判断两组接受不同处理所产生的效果（[[Argument_QiMei_2015_EducationalResearchMethods|齐梅, 2015]], Ch.4）。

...
> [!def] 方法定义
> **倾向得分匹配（Propensity Score Matching，简称 PSM）**是由保罗·罗森鲍姆与唐纳德·鲁宾（Rosenbaum & Rubin, 1983）奠定的经典[[Quasi-Experimental Designs|准实验]]因果推断方法。其核心思想是：利用处理前丰富且关键的可观测基线协变量向量 $\mathbf{X}_i$，通过概率回归模型将个体面临的多维混杂特征压缩为一维的条件参与概率标量——**倾向得分（Propensity Score）** $e(\mathbf{X}_i) = \Pr(Z_i = 1 \mid \mathbf{X}_i)$；进而在处理组与对照组之间寻找倾向得分高度相近的个体进行配对（Matching），以此模拟[[Randomised Controlled Trials|随机对照试验]]中两组基线特征期望相等的无偏状态，有效校正由于非随机选择产生的可观测选择偏误（Selection on Observables）。[[Argument_Edovald_Nevill_2021_ROE|(Edovald & Nevill, 2021, pp. 55–56)]]
...
> [!citation-card] 准实验匹配在学校体制抉择评估中的机制应用（Edovald & Nevill, 2021）
...
> - **知识观** 根植于反事实推理与鲁宾因果模型（Rubin Causal Model）。假定只要控制了影响处理选择与潜在线性产出的全部共同前置变量，非实验组别间的因果反事实即可通过概率加权或相近距离配对加以重构。
> - **核心识别假定**
...
> - **有效性标准** 内部效度取决于协变量的完备性与匹配后各维度标准化均值差（SMD $< 0.10$）；统计推断必须考虑配对依赖性。
...
> |---|---|---|---|---|
> | **倾向得分匹配（PSM）** | 压缩为单一概率标量 $[0, 1]$ | **极低**（彻底规避维数灾难） | 基于倾向得分绝对距离配对 | 协变量丰富、样本量充裕；但可能存在模型设定偏误。 |
...
> - **数据收集** 行政普查数据库（英国[[National Pupil Database|国家学生数据库]]）、学业前后测档案、问卷调查多维背景数据。
...
> [!proc] 通用程序
> 1. **协变量挑选与外生性审查** 广泛搜集在政策干预发生前已确定的基线变量（前测成绩、家庭经济背景、生源流动性等），严禁纳入受到干预结果反向污染的中介变量。
...
>   - 协变量向量 $\mathbf{X}_i$：个人人口学特征、先前学业成就、家庭社会经济地位、学校组织规模等。
>   - 结果变量 $Y_i$：后测成绩或学业增值。
...
>
> **这个公式在做什么** 计算处理组个体的实际产出 $Y_i$ 与其所配对的对照组加权反事实产出 $\sum_j w(i, j) Y_j$ 之间的差值，并在全体处理组样本上取平均，得到干预对实际参与者的净效应。
>
> **符号说明**
> - $N_T$：处于共同支撑集内的处理组有效样本量。
> - $T, C$：处理组与对照组配对样本集。
> - $w(i, j)$：对照个体 $j$ 赋予处理个体 $i$ 的匹配权重（在 1:1 近邻匹配中为 1，在核匹配中为核权重）。
>
> **结果怎么读** $\tau_{\text{ATT}} > 0$ 且置信区间不包含 0，表明接受政策干预的学校或学生在匹配后显著优于具有同等入选概率的对照群体。
>
> **注意事项** 估计值的有效性完全依赖于“无不可观测混杂”假定。在英国 EEF 的实证应用中，PSM 常被与[[Difference-in-Differences|双重差分法]]串联为匹配双重差分（PSM-DID），利用 DID 消除不随时间变化的未观测异质性（[[Argument_Edovald_Nevill_2021_ROE|Edovald & Nevill, 2021]]）。
...
> - **报告标准** 完整报告匹配前后各变量标准化偏差（SMD）、匹配丢弃样本量、共同支撑集重合比例及 ATT 点估计与稳健标准误。
...
> [!method-fit] 适用判断
> - **适合使用**
>   - 拥有海量行政普查数据库，且能够采集到极丰富基线学生与学校特征的大规模评估项目（[[Argument_Edovald_Nevill_2021_ROE|Edovald & Nevill, 2021]]）；
>   - 处理组样本量较小但存在庞大的未受干预“候选对照池”的情形；
>   - 评估自愿报名或按既有政策选择开展特定教改项目的真实成效。
> - **谨慎使用**
>   - 缺乏关键前置特征（如缺少历史成绩前测、家庭背景信息极度匮乏）；
>   - 处理组与对照池特征极其悬殊，导致共同支撑集范围极窄、匹配后丢弃绝大多数样本的研究。
> - **不适合使用**
>   - 处理分配高度由隐藏动机（如强烈进取心、未公开人际关系）决定的情境，此类隐性偏误无法被可观测变量捕捉。
...
> [!method-limits] 方法局限
> - **偏误来源**
>   - **不可观测混杂偏差（Hidden Bias）** PSM 最大的方法学软肋：它仅能平衡纳入模型的“可观测”变量，完全无法抵御未观测变量带来的内生选择性偏误。
...
> - **补救方式** 采用匹配双重差分法（PSM-DID），将横向可观测匹配与纵向时间差分双重结合；运用 Rosenbaum Bounds 计算隐藏偏误临界值 $\Gamma$。
...
> [!debates] 学术争议
>
> > [!axis] 可观测变量控制充分性 vs 隐性动机选择
> > 观察研究中能否通过增加控制变量彻底解决选择偏差。
> >
...
> > - **PSM 辩护** 指出在微观连续变量极多的复杂教育真实数据中，CEM 极易因空箱问题遭遇严重的样本脱落，而 PSM 依然是可行性最强且经过充分实证检验的基准范式。
...
> | [[Randomised Controlled Trials]] | 对标黄金标准 | PSM 通过在观察数据中构造平衡性以逼近 RCT 的反事实对照机制。 |
> | [[National Pupil Database]] | 数据基础 | 英国支撑学校与学生层面 PSM 协变量精准匹配的国家级微观纵向大数据库。 |
> | [[Researching School Choices]] | 实践载体 | EEF 设立的依托 PSM 等准实验评估宏观学校办学选择的重大资助流。 |

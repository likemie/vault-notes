...
aliases:
  - 整群抽样
  - cluster sample
summary: "当总体大且分散时，选择地理或组织上聚集的群组并测试群内所有成员的概率抽样方法，以行政可行性换取可能的群内同质性偏差"
type: method
...
> [!def] 方法定义
> 整群抽样（Cluster Sampling）是当总体大且广泛分散时，选择特定数量的地理或组织上聚集的群组（如学校、社区），测试这些群中所有成员的[[Random Sampling|概率抽样]]方法。它解决了简单随机抽样在行政上的不可行性——如调查全国学生体能水平，随机选取学生并四处奔波测试完全不切实际。
...
> [!method-fit] 适用判断
> - **适合使用**：总体大且地理分散、逐一接触个体行政上不可行。
> - **谨慎使用**：群内同质性高时——少数群可能不代表总体的多样性。
> - **不适合使用**：追求高精度个体层面估计时。

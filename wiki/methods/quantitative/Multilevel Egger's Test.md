...
> ```mermaid
> flowchart LR
>   A["输入效应量 z_ij 与标准误 SE_ij<br/>SE_ij = √(V_ij)"] --> B["构建三水平元回归模型<br/>z_ij = β_0 + β_SE · SE_ij + ζ_(3)j + ζ_(2)ij + ε_ij"]
>   B --> C["检验斜率系数 β_SE<br/>若 p < 0.05 证实存在小研究偏倚"]
>   C --> D["提取回归截距 β_0 (SE → 0)<br/>即为偏倚校正后的真实效应量"]
>   D --> E["应用 RVE CR2 小样本调整<br/>计算截距的稳健 95% CI 与 PI"]
>   E --> F["反向转换<br/>r_adj = tanh(β_0) 输出最终汇报值"]
> ```

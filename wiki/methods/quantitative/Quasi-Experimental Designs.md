...
反事实可行性也说明，证据审查不能只看“研究设计是否严格”。还需要报告对照条件的[[Logic Model|逻辑模型]]、使用强度、证据支持程度和实施质量，否则实践者很难判断研究效果是否能[[Transfer Translation Transformation|迁移]]到本地学校（Ross & Morrison, 2021, p.112）。

## 适用场景

> [!success]
> QED 适用于教育项目需要达到 ESSA “中等证据”标准的场景。学校、学区或项目开发者若无法提供 [[Randomised Controlled Trials|RCT]] 证据，但能够提供 QED 证据，就可能进入 ESSA 的第二层证据支持（Ross & Morrison, 2021, p.109）。

## 局限性

> [!warning]
> QED 和 [[Randomised Controlled Trials|RCT]] 一样，若只报告对学生成就的统计显著效果，仍可能遗漏实践者真正需要的信息，如实施要求、用户满意度、成本、情境限制和项目使用强度。对低强度补充项目或复杂改革而言，这类信息会直接影响学校是否采用和持续使用项目（Ross & Morrison, 2021, pp.110-112, 120-121）。

> [!warning]
> QED 的“中等证据”地位不等于所有 QED 证据都同样可用。若比较组反事实较弱、[[Implementation Fidelity|实施忠实度]]不清楚、结果测量过窄，研究仍可能给实践者留下过度乐观或难以[[Transfer Translation Transformation|迁移]]的效果判断（Ross & Morrison, 2021, pp.112, 120-121）。
...
> - [[Implementation Fidelity]] — QED 的结果解释需要知道项目是否以足够质量和强度实施，以及对照条件是否足够可信（Ross & Morrison, 2021, p.112）。

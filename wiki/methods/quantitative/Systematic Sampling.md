...
> [!def] 方法定义
> 系统抽样（Systematic Sampling）是[[Random Sampling|简单随机抽样]]的变体，以系统而非随机方式从总体名单中选取受试者。从随机起点开始，按固定频率间隔依次选取，直至达到所需样本量。频率间隔 $f = N / sn$（$N$ 为总体人数，$sn$ 为所需样本人数）。
...
> 2. 从随机样本量表中确定所需样本人数 $sn$。
> 3. 计算频率间隔 $f = N / sn$。
> 4. 随机选择起始点（第 1 到第 $f$ 名之间）。
> 5. 从起始点开始，每隔 $f$ 人选取一人，直至达到样本量。
...
> [!method-fit] 适用判断
> - **适合使用**：有完整且随机排列的总体名单、需要简化操作的大规模概率抽样。
> - **谨慎使用**：名单排序存在规律性时。

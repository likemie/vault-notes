...
> [!def] 方法定义
> 因子混合模型（Factor Mixture Modeling, FMM）结合了因子分析降维测量的优势与潜在类别/剖面分析探究异质性的功能。它利用个体在多个连续的潜在因子（潜变量）上的表现特征，将其划分至具备不同特征轮廓（profile）的未知类群中。[[Argument_Greene_2010_JEP|(Greene et al., 2010, p. 239)]]

> [!method-scope] 方法范围
> - **研究对象** 具备多维测量结构并假设存在群体异质性分类的数据。
...
> [!method-limits] 方法局限
> - **偏误来源** 对起点的初值较为敏感，极大可能面临局部极值和不收敛问题，通常需要充足的样本量；另外潜在类群的实质解释依赖研究者的理论预设，有主观色彩。
...
> - [[Argument_Greene_2010_JEP]] — 该研究通过因子混合模型，将大样本学生在认识论维度上的测量得分有效聚类为了理论预设的四个阶段组。

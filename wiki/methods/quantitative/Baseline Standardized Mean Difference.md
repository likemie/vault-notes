...
> |---|---|---|---|---|
> | **$|d| \le 0.05$** | **基线高度等价**<br>（High Equivalence） | 组间差异微弱，属于完全[[Random Sampling\|随机抽样]]误差范围；[[Pre-test and Post-test\|后测]]分析无需加入协[[Variable\|变量]]。 | **无条件采纳** | 组间无系统性选择偏倚，[[Internal Validity\|内部效度]]达到准随机对照水平。 |
> | **$0.05 < |d| \le 0.25$** | **基线有条件等价**<br>（Conditional Equivalence） | 初级研究必须在统计分析中将前测成绩作为[[Covariate Adjustment\|协变量控制]]（如 [[Analysis of Covariance\|ANCOVA]] 或回归）。 | **有条件采纳**（报告校正结果者纳入，未校正则排除） | 组间存在轻中度初始偏差，但在线性假定范围内可通过协方差统计模型消除混杂。 |

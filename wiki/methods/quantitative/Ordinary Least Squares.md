...
> [!def] 方法定义
> **普通最小二乘法（Ordinary Least Squares，简称 OLS）**是统计学、计量经济学与实证教育研究中最核心、应用最广泛的参数化线性回归估计方法。该方法以代数投影原理为基础，通过最小化样本观测值因变量与模型线性预测值之间的残差平方和（Sum of Squared Residuals, SSR），求解使误差达到全局最小的回归参数向量 $\hat{\boldsymbol{\beta}}$。在满足高斯-马尔可夫定理（Gauss-Markov Theorem）的古典线性假定下，OLS 估计量是所有线性无偏估计量中方差最小的最佳线性无偏估计（Best Linear Unbiased Estimator, BLUE）。在[[Randomised Controlled Trials|随机对照试验]]（RCT）中，引入基线前测的 OLS 模型（即[[Analysis of Covariance|协方差分析]] ANCOVA 回归形式）能够大幅吸收个体基线能力变异，是[[Covariate Adjustment|协变量调整]]的主流基准。[[Argument_Edovald_Nevill_2021_ROE|(Edovald & Nevill, 2021, pp. 52–53)]]

> [!method-scope] 方法范围
> - **研究对象** 连续型因变量与一组解释变量（连续、离散或分类虚拟变量）之间的线性相关或条件均值关系。
> - **问题类型** 线性关联检验、条件均值预测、边际效应估计、因果推断协变量控制与平均处理效应评估。
> - **分析单位** 学生个体、教师、学校、地区或年份横截面数据。
...
> - **知识观** 建立在条件期望函数（Conditional Expectation Function, CEF）线性逼近论之上。假定复杂的现实因果与关联模式可通过自变量的线性组合进行局部最优逼近；在因果推断视角下，若满足条件独立性假定（CIA），回归系数可被赋予净因果边际贡献的解释。
...
> - **有效性标准** 估计量的无偏性取决于外生性假定；有效性（BLUE）取决于球形扰动；假设检验有效性依赖残差正态性或渐进正态性。
> - **不声称回答的问题** 不能在存在遗漏变量混杂或逆向因果的非实验数据中直接宣称因果；不能在未校正群聚相关的多层嵌套数据中直接套用常规标准误。

> [!contrast-table] OLS vs 加权/广义最小二乘（WLS/GLS） vs 多层线性模型（HLM）
> | 比较维度 | 普通最小二乘法（OLS） | 加权/广义最小二乘（WLS/GLS） | 多层线性模型（HLM） |
> |---|---|---|---|
> | **误差结构假定** | 同方差、无序列相关（球形扰动） | 允许异方差或已知相关矩阵（非球形） | 显式分解为组内误差与组间多重随机效应 |
> | **异方差应对** | 需外挂怀特（White/HC）稳健标准误 | 逆方差加权消除异方差（如元回归） | 在随机效应方程中建模方差异质性 |
> | **群聚相关处理** | 常规标准误严重低估，需 CRSE 校正 | 依赖块对角工作协方差矩阵（如 GEE） | 模型内自然估计 ICC 并校正标准误 |
...
> [!method-stack] 方法层级
> - **研究设计** 实验研究（协变量调整回归）、观察性截面调查、断点回归局部线性拟合（RDD）、双重差分双向固定效应（TWFE）。
> - **数据收集** 学业测验考分、调查问卷连续量表总分、国家学生普查行政数据库（NPD）。
> - **分析方法** 简单一元线性回归、多元线性回归、协方差分析模型（ANCOVA）、调节效应与交互项模型。
...
> [!proc] 通用程序
> 1. **明确模型设定与变量界定** 界定因变量 $Y$、核心自变量 $T$（干预状态或政策暴露）以及控制协变量集合 $\mathbf{X}$。
...
> 5. **稳健推断与标准误校正** 若存在异方差则汇报 Huber-White 稳健标准误；若存在班级或学校嵌套，则采用群聚稳健标准误（CRSE）或转入多层线性模型。
...
> - **核心变量**
>   - 因变量 $Y_i$：后测标准化考试成绩或连续型测量指标。
>   - 处理变量 $T_i$：实验组与对照组分配指示变量（0/1）。
>   - 基线前测 $Y_{\text{pre},i}$：干预实施前采集的同构学业成绩（核心协变量）。
...
> [!formula-step] 公式步骤　试验协变量调整回归（ANCOVA 模型）
...
>
> **这个公式在做什么** 在随机对照试验中，通过纳入基线前测成绩 $Y_{\text{pre},i}$ 与背景协变量 $\mathbf{Z}_i$，大幅吸收后测结果的未解释变异，压缩处理效应 $\tau$ 的抽样方差。
>
> **符号说明**
...
>
> **数学直觉** 在教育测验中，基线前测成绩与后测成绩通常具有极高相关性（$r \approx 0.70 \sim 0.85$），可解释后测方差的 50% 到 70%。纳入前测能够将误差方差从 $\sigma_Y^2$ 压缩至 $(1 - r^2)\sigma_Y^2$，使标准误收缩 30% 到 50%，等效于成倍扩充了有效样本量。
>
> **结果怎么读** $\tau$ 的点估计值即为调整后的干预净效应；在完全随机分配下，$\tau$ 与无协变量模型的点估计在渐进意义上趋于一致，但其置信区间大幅缩窄，显著提高检验灵敏度。
>
...
>   4. 效应量换算：根据全样本合并对照组标准差计算标准化 Hedges' $g$。
...
> [!method-fit] 适用判断
> - **适合使用**
>   - 随机对照试验中控制基线前测成绩以提升统计功效的协变量调整分析（ANCOVA）；
>   - 探索连续型教育结果变量与学生、家庭背景变量之间的多元线性相关性；
>   - 准实验设计中作为倾向得分匹配（PSM）前后的回归调整基准或断点回归（RDD）的局部线性估计器。
> - **谨慎使用**
>   - 存在显著学校或班级嵌套结构的数据（必须外挂聚类稳健标准误，否则标准误严重失真）；
>   - 解释变量之间存在高多重共线性的情形（导致单项系数方差膨胀、正负号反转）。
> - **不适合使用**
>   - 因变量为离散二分类或有序多分类变量（应转向 Logistic 回归或 Probit 回归）；
>   - 缺乏外生分配机制且存在不可观测混杂因素的横截面因果推断（因内生性偏误无法成立因果解释）。
...
> [!method-limits] 方法局限
> - **群聚数据标准误严重低估** OLS 建立在各观测误差项独立同分布的严苛假定上。教育现场中学生嵌套于学校，同校学生的同质性导致误差项正自相关；直接套用传统 OLS 会将集群共有的变异错误记为独立自由度，导致标准误被大幅低估 30% 至 60%，制造大量虚假的统计显著性（Type I Error）。
> - **极端值与杠杆点敏感性** 残差平方和的二次惩罚机制赋予远离拟合平面的极端离群点与高杠杆点过高的权重，单个异常点即可显著扭曲回归斜率。
> - **函数形式误设偏误** 当变量间存在明显的非线性关系（如阈值效应或加速递减）时，强行采用线性 OLS 会导致系统性的残差结构化偏误。
...
> [!debates] 学术争议
>
> > [!axis] OLS 聚类稳健标准误 vs 多层线性模型（HLM）
...
> > - **教育统计学派立场** 认为单纯靠 CRSE 给 OLS 贴补丁无法替代多层模型。教育系统具有不可约减的层级生态，HLM 不仅能纠正标准误，更能显式分解校内与校际方差、估计群内相关系数（ICC），并允许探究跨层级交互作用，具有更丰富的实体教育解释力。肖等学者（Xiao et al., 2016）在 NPD 复算中证实，两类模型在效应量点估计和区间覆盖上存在显著分歧，推动了循证教育界对建模标准的审慎反思（[[Argument_Edovald_Nevill_2021_ROE|Edovald & Nevill, 2021]]）。
...
> | 条目 | 类型 | 关系 |
> |:-----|:-----|:-----|
> | [[Simple Difference in Means Model]] | 前置/基准方法 | 无协变量时的 OLS 特例，两者构成方差吸收与功效提升的鲜明对比。 |
> | [[Hierarchical Linear Model]] | 进阶模型 | 突破 OLS 独立同分布假设、显式建模层级嵌套结构的现代高级回归模型。 |
> | [[Covariate Adjustment]] | 核心技术 | OLS 在 RCT 中作为吸收基线变异、校正偶然不平衡的主要统计载体。 |
> | [[Analysis of Covariance]] | 等价形式 | OLS 纳入类别处理变量与连续前测变量时的经典方差分析表达形式。 |
> | [[Standard Error]] | 推断基础 | OLS 参数推断的核心指标，在嵌套数据中易发生系统性低估。 |
> | [[Statistical Significance]] | 决策指标 | OLS 模型设定与标准误校正直接左右的统计显著性结论。 |
> | [[Model Dependency]] | 风险现象 | 解释了为什么 OLS 与简单均值模型、多层模型之间的选择会引发效应量漂移。 |
...
> - [[Argument_Edovald_Nevill_2021_ROE|Edovald & Nevill (2021)]] — 综述了齐·肖（ZhiMin Xiao）、史蒂夫·希金斯（Steve Higgins）与阿迪特·卡西姆（Adetayo Kasim）利用英格兰 NPD 数据开展的 17 项试验复算，证实了基线前测 OLS 回归在方差吸收与功效提升方面的核心价值，同时也警示了未校正聚类结构的 OLS 会诱发严重的标准误低估与伪显著性，促成了英国试验统计指南的确立。

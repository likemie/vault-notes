...
> 匹配隐含[[Counterfactual|反事实]][[Causality|因果推断]]的认识论立场：如果能找到在处理前协[[Variable|变量]]分布相同的两组主体，则两组结果的差异可归因于处理本身。这要求"自然进行的等效于在给定一组协变量条件下的随机实验"([[Argument_Berk_2011_ER|Berk, 2011, p.198]])——一个需要由过去研究和理论来论证的强假设。
...
>
> 3. **评估样本平衡**：检验匹配后两组在协变量上的平衡程度。最近的进展已改善了进行匹配、评估样本平衡和确定对遗漏协变量的敏感性的方法（Rosenbaum, 2010: Part II）([[Argument_Berk_2011_ER|Berk, 2011, p.198]])
>
> 4. **敏感性测试**：评估结果对未观测协变量（遗漏变量）的敏感性。"敏感性测试可以有所帮助，但除了在极端情况下，很少能提供决定性证据"([[Argument_Berk_2011_ER|Berk, 2011, p.198]])
...
> | 对软件依赖 | 较低 | 高——"点点点按按按的统计软件包使因果建模看似容易"([[Argument_Berk_2011_ER\|Berk, 2011, p.197]]) |
...
> "样本中的平衡并不意味着未观测协[[Variable|变量]]也存在平衡"([[Argument_Berk_2011_ER|Berk, 2011, p.198]])。匹配仅能基于已观测和已包含的协变量建立可比性——如果关键混杂因素未被测量或未包含在匹配中，结果的因果解释仍然脆弱。对这一假设的辩护"始于过去的研究和理论"([[Argument_Berk_2011_ER|Berk, 2011, p.198]])。


> [!warning] 需要满足与随机实验相同的假设
> 匹配不能豁免随机实验所需的关键假设。例如，"同样必须满足无干扰（no interference）的要求"——即 [[Stable Unit Treatment Value Assumption|SUTVA]] 假设([[Argument_Berk_2011_ER|Berk, 2011, p.199]])。


> [!warning] 协变量交互效应
> "与协变量的交互效应最好通过事后分层（post stratification）来解决，而非因果模型"([[Argument_Berk_2011_ER|Berk, 2011, p.199]])。这意味着匹配后的分析仍需要方法论上的谨慎。


> [!warning] Berk 的错误使用警告
> [[Argument_Berk_2011_ER|Berk (2011, p.198)]] 指出匹配虽然比[[Causal Modeling|因果建模]]更稳健，但"研究者的错误仍然过于常见"——方法本身并不保证正确执行。
...
- [[Causal Modeling]] — [[Argument_Berk_2011_ER|Berk (2011)]] 将匹配定位为因果建模的替代路径，认为其较少依赖不可检验模型假设

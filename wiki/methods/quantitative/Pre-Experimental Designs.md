...
> [!info]
> 前实验设计（Pre-Experimental Designs）是[[Experimental Research|实验研究]]中内部效度最低的一类设计。这类设计研究单一组并实施干预，不设对照组与实验组进行比较，或使用的比较组不等价（Creswell & Creswell, 2022, Ch8）。
...
> [!warning]
> - [[Internal Validity|内部效度]]极低：缺乏随机分配、对照组和／或前测，几乎无法排除替代性因果解释。
...
> [!tip]
> - [[Quasi-Experimental Designs]] — 准实验设计增加了对照组和时间序列观测，内部效度高于前实验设计。

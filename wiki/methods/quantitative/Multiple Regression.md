...
> [!info]
> 多元回归（Multiple Regression）是将 Pearson 相关扩展到两个或以上预测[[Variable|变量]]（predictor variables）的推断统计方法。它用于评估多个预测变量（自变量）与一个结果变量（因变量）之间的关系，提供整体模型拟合指标（R² 和 F 统计量）以及每个预测变量的相对贡献（b 系数和 t 统计量）。多元回归揭示了众多变量中哪一个变量对结果的相对预测力最强（[[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022]], Ch8）。
...
> - **t 统计量** — 检验每个单独预测变量的贡献是否显著（[[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022]], Ch8）。

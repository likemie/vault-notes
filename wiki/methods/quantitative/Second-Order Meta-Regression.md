...
>   B --> C["步骤 3：基于 CHE 协方差拟合<br/>(GLS 广义加权最小二乘估计)"]
>   C --> D["步骤 4：计算 RVE 三明治协方差<br/>(应用 CR2 杠杆值小样本校正)"]
>   D --> E["步骤 5：多参数联合 Wald 检验<br/>(基于 Hotelling T² 输出稳健 p 值)"]
>   E --> F["步骤 6：计算二阶伪判定系数 R²<br/>(评估宏观特征对异质性的解释率)"]
> ```

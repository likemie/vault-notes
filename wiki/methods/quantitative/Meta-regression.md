...
> [!def] 方法定义
> 元回归（Meta-regression）是在[[Meta-analysis|元分析]]框架下，以各纳入初级研究的[[Effect Size|效应量]]为[[Dependent Variable|因变量]]、研究层面的特征（调节[[Variable|变量]]）为[[Independent Variable|自变量]]，通过加权回归分析检验调节变量对效应量[[Heterogeneity|异质性]]解释力的统计方法体系。它将元分析的调节效应检验从离散分类子组比较推广到连续数值变量及多元复合模型，依据各研究的抽样精度（抽样方差倒数）与研究间真实异质性方差进行广义逆方差加权估计。[[Argument_Greene_2018_JEP|(Greene et al., 2018, pp. 1098–1102)]]; [[Argument_Lei_Ding_Chiu_2026_ERR|(Lei et al., 2026, pp. 5–6, 10–11)]]

> [!method-scope] 方法范围
> - **研究对象** 跨原始研究的效应量矩阵及其对应的样本特征、测量工具信效度、干预参数、宏观社会经济背景及文献发表年份等协变量。
> - **问题类型** 机制解释、异质性来源探索与因果调节识别：哪些连续型特征（如干预时长、量表信度、人均 GDP）或离散特征系统性预测效应量大小？
> - **分析单位** 独立实证研究（Study）或独立样本（Sample），每条数据对应一项纳入元分析的效应量估计值。
> - **输出形式** 回归截距 $\beta_0$、斜率系数 $\beta_j$、标准误、95% [[Confidence Interval|置信区间]]（CI）、模型整体检验量 $F$ 或 $Q_{\text{model}}$、残余异质性 $\tau^2_{\text{residual}}$ 及拟 $R^2$ 解释比例。
...
> - **知识观** [[Positivism|实证主义]]后设整合：将单项实证研究视为带有[[Sampling Error|抽样误差]]与情境约束的数据点，通过跨研究的多元回归建模提取超越单一情境的稳健调节规律。
> - **研究者角色** 预先依据实质理论设定候选调节变量，对文献纳入标准、效应量提取口径和变量编码信度负责，避免无理论指导的事后逐步数据挖掘。
> - **有效性标准** 统计结论效度（检验功效高度依赖纳入研究数 $k$，经验要求每个自变量至少对应 10 项独立研究）与模型设定准确性。
> - **不声称回答的问题** 元回归揭示的是研究层面的生态关联（Ecological Association），不能直接推断个体层面的微观因果中介，不可犯[[Ecological Fallacy|生态谬误]]。

> [!contrast-table] 经典一阶元回归 vs 二阶元回归 vs 分类亚组检验
> | 比较维度 | 经典一阶元回归（[[Meta-regression]]） | 二阶元回归（[[Second-Order Meta-Regression]]） | 分类亚组检验与[[Pairwise Wald Tests|成对 Wald 检验]] |
> |---|---|---|---|
> | **分析单位** | 原始实证研究（Primary Studies） | 一阶元分析汇总效应量集群（Meta-Analysis Clusters） | 原始实证研究按属性划分的离散子群 |
> | **自变量形态** | 连续型变量（如信度、人均 GDP、年份）或哑变量 | 宏观理论构念与跨领域特征指标 | 离散分类变量（如学段、干预形态、学科） |
> | **依赖性处理** | 假定各研究抽样独立或采用简单多水平结构 | 采用 CHE 块对角协方差与 RVE 经验三明治估计 | 假定各亚组效应量独立或分层建模 |
...
> [!proc] 通用程序
> 1. 明确理论假设：预设哪些连续型或分类型研究特征（如量表信度、人均 GDP、发表年份、干预时长）可能系统性调节效应量大小。
> 2. 系统检索与文献筛选，提取各研究的效应量指标（如 Hedge's $g$、Pearson $r$、Fisher's $z$）及其抽样方差 $v_i$，并对调节变量进行标准化编码。
> 3. 计算合并效应与异质性检验，评估总异质性 $Q_{\text{total}}$ 与研究间方差 $\tau^2$ 是否显著，确立开展元回归的统计前提。
> 4. 构建加权随机效应元回归模型，估计截距 $\beta_0$、偏回归系数 $\beta_j$、置信区间及模型解释方差比例 $R^2_{\text{analog}}$。
> 5. 进行模型诊断：检验残余异质性 $Q_{\text{residual}}$、多重共线性、高杠杆异常点及发表偏倚影响。
...
> - **变量或指标** 
>   - 因变量：效应量估计值 $\hat{\theta}_i$（如 Hedges' $g$、Pearson $r$ 或 Fisher's $z$）。
>   - 自变量：研究特征连续变量（如人均 GDP_Z 分数、发表年份、量表信度）或分类虚拟变量。
...
>
> **这个公式在做什么** 以第 $i$ 项研究的效应量估计值 $\hat{\theta}_i$ 为因变量，通过 $p$ 个研究特征调节变量 $x_{i1}, \dots, x_{ip}$ 预测效应量大小，并同时分解研究间真实异质性误差 $u_i$ 与研究内抽样误差 $\epsilon_i$。
>
> **符号说明**
...
>
> **这个公式在做什么** 以图形组织器干预的效应量 Hedge's $g$ 为因变量，以纳入国家的人均国内生产总值 Z 分数（$\text{GDP\_Z}$）与文献发表年份（$\text{PublishYear}$）为连续型协变量，检验宏观经济技术投入与教学法演进对微观干预成效的跨层调节效应。
>
> **符号说明**
> - $\hat{g}_i$ 第 $i$ 项研究报告的图形组织器对高阶思维干预效应量 Hedge's $g$
...
>
> **这个公式在做什么** 以各研究报告的认识论认知与学业成就相关系数 $r$ 为因变量，以问卷内部一致性信度（Cronbach's $\alpha$）为预测变量，检验测量误差对可观测效应量的系统性衰减机制。
>
> **符号说明**
> - $\hat{r}_i$ 第 $i$ 项研究报告的相关系数
> - $\text{Reliability}_i$ 认识论问卷的内部一致性信度系数（0 到 1）
> - $\beta_1$ 信度元回归斜率估计值（Greene et al. 报告为 $b = .300, p < .001$）
>
> **数学直觉** 经典测量理论表明测量误差会导致相关系数发生统计衰减。元回归实证验证了这一规律：问卷信度越高，可观测到的效应量越大；若信度为 0，效应量归零；若信度达 1.0，理论真实相关性可达 $r = .300$。
>
...
>     summary(res_mod)
>     ```
> - **实现流程** 数据清洗 -> 效应量转换与方差提取 -> 连续调节变量标准化与中心化 -> REML 模型估计 -> 残差诊断与气泡图（Bubble Plot）绘制。
> - **报告标准** 完整报告样本量 $k$、截距与斜率系数 $B$、标准误 SE、95% CI、$p$ 值、模型 $F$ 检验量、残差异质性 $\tau^2_{\text{residual}}$ 及解释方差比例 $R^2$。
...
> [!method-fit] 适用判断
> - **适合使用** 纳入元分析的研究间存在显著异质性（$I^2 > 25\%$ 或 $Q$ 检验 $p < .05$），且研究者希望在连续维度上检验假设的调节变量（如样本年龄、干预时长、国家人均 GDP、发表年份、量表信度）。[[Argument_Greene_2018_JEP|(Greene et al., 2018)]]; [[Argument_Lei_Ding_Chiu_2026_ERR|(Lei et al., 2026)]]
> - **谨慎使用** 纳入研究数量较少（$k < 10$）时模型自由度不足；调节变量间存在高度多重共线性时系数难以分离；探索性质的元回归需明确声明。
> - **不适合使用** 旨在推断个体层面的微观因果中介机制；原始文献中关键调节变量大面积缺失且无法补齐。
...
> [!method-limits] 方法局限
> - **偏误来源** 生态谬误（研究层面的变量关联无法直接推导至学生个体）；混杂变量遗漏（由于 $k$ 有限无法同时控制所有潜在混杂因素）；发表偏倚与小样本效应干扰。
> - **适用边界** 需要充足的独立研究数量；模型结论受制于原始研究报告的质量（Garbage in, garbage out）。
> - **误用风险** 在高维调节变量空间中进行无理论指导的逐步回归，导致假阳性率激增。
> - **补救方式** 预注册调节假设；采用置换检验（Permutation test）校正小样本推断；结合散点气泡图直观展示效应量与调节变量的关系。
...
> | 条目 | 类型 | 关系 |
> |:-----|:-----|:-----|
> | [[Meta-analysis]] | 前置方法 | 元回归依附于元分析框架，以元分析提取的效应量与抽样方差作为建模输入。 |
> | [[Pairwise Wald Tests]] | 互补方法 | 元回归处理连续型协变量与多变量线性趋势，成对 Wald 检验处理离散多亚组的事后成对级差推断。 |
> | [[Second-Order Meta-Regression]] | 进阶方法 | 在二阶元分析多层嵌套与文献重叠数据下，运用 CHE 与 RVE 检验宏观调节效应的高阶形态。 |
> | [[Moderator Analysis]] | 关联方法 | 元回归是调节分析的高阶形态，弥补了传统子组方差分析无法处理连续变量的缺陷。 |
> | [[Heterogeneity]] | 诊断基础 | 研究间异质性是实施元回归的前提依据，元回归的核心目标即为解释此异质性。 |
> | [[Effect Size]] | 因变量 | 各项研究的效应量点估计构成元回归模型的响应变量。 |
...
> - [[Argument_Greene_2018_JEP|Greene et al. (2018)]] 在认识论认知元分析中运用元回归检验问卷信度对效应量的预测力，发现信度回归系数达 $b = .300$。
> - [[Argument_Park_2026_TSC|Park et al. (2026)]] 在三水平元分析框架下运用元回归检验参与者年龄、学段、地理区域、发表年份及创造力与批判性思维测量类型对相关量的调节作用，并检验两类测量类型的交互项（$Q_M(1) = 6.524, p = .011$）。

...
> 2. 系统检索与文献筛选，提取各研究的效应量指标（如 Hedges' $g$、Pearson $r$、Fisher's $z$）及其抽样方差 $v_i$，并对调节变量进行标准化编码。
...
>
> **这个公式在做什么** 以图形组织器干预的效应量 Hedges' $g$ 为因变量，以纳入国家的人均国内生产总值 Z 分数（$\text{GDP\_Z}$）与文献发表年份（$\text{PublishYear}$）为连续型协变量，检验宏观经济技术投入与教学法演进对微观干预成效的跨层调节效应。
>
> **符号说明**
> - $\hat{g}_i$ 第 $i$ 项研究报告的图形组织器对高阶思维干预效应量 Hedges' $g$

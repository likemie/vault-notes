...
> [!citation-card] 广义估计方程在教育试验审计中的半参数稳健性（[[Argument_Edovald_Nevill_2021_ECNUROE|Edovald & Nevill, 2021]]; Xiao et al., 2016）
> 在杜伦大学学者针对英国 [[Education Endowment Foundation|EEF]] 早期 17 项大规模教育 [[Randomised Controlled Trials|RCT]] 开展的四模型并行复算中，广义估计方程（GEE）作为应对学校群聚偏误的重要备选模型接受了全面审计。复算显示，GEE 与[[Hierarchical Linear Model|多层线性模型]]（HLM）均能有效吸收校内组内相关性，避免了标准 [[Ordinary Least Squares|OLS]] 产生的虚假显著性膨胀；由于 GEE 聚焦于“干预对全系统平均学生的净效益”，无需对学校间未观测[[Heterogeneity|异质性]]施加严格的正态分布[[Hypothesis|假设]]，因而在集群规模充足的教育试验中展现出极佳的边际效应稳健性。[[Argument_Edovald_Nevill_2021_ECNUROE|(Edovald & Nevill, 2021, pp. 52–53)]]
>
...
> - **有效性标准** [[Internal Validity|内部效度]]取决于外生性设定；统计推断效度依赖三明治方差估计量的大样本渐进正态性（要求独立集群数 $J \ge 30 \sim 50$）。
> - **不声称回答的问题** GEE 无法直接输出特定单个学校或班级的随机效应方差分量，亦不能像 [[Hierarchical Linear Model|HLM]] 那样直接提供[[Intraclass Correlation Coefficient|组内相关系数]]（ICC）的解析方差比例，不适合用于纯粹探究学校间组织[[Heterogeneity|异质性]]的机制研究。

> [!contrast-table] GEE vs 多层线性模型（HLM） vs [[Ordinary Least Squares|OLS]] 聚类稳健[[Standard Error|标准误]]
...
> | **参数解释** | 全样本群体的平均边际效应 | 控制了特定学校潜[[Variable\|变量]]后的条件效应 | 全样本平均效应 |
> | **非线性连接** | 完美支持二元（Logit）、计数（Poisson） | 非线性时需复杂的数值积分（GLMM） | 线性模型有效，非线性下易受限 |
> | **对误设的稳健性** | 极高（工作矩阵误设仍能保证无偏一致） | 较低（随机效应分布误设可能污染参数） | 极高（仅依赖独立同分布外生性） |

> [!method-stack] 方法层级
> - **研究设计** [[Cluster Randomized Trials|集群随机对照试验]]（Cluster [[Randomised Controlled Trials|RCT]]）、纵向多轮追踪调查、多阶段[[Stratified Sampling|分层抽样]]评估。
...
> [!proc] 通用程序
> 1. **定义边际均值模型与连接函数** 依据[[Dependent Variable|因变量]]类型（连续、二元、计数）选定适宜的连接函数 $g(\cdot)$（如恒等连接、Logit 连接或 Log 连接）。
> 2. **界定集群结构与独立聚类单位** 明确高层级独立集群标识符（如学校 ID 或学区 ID），确认集群间彼此独立。
> 3. **选取工作相关矩阵（Working Correlation Matrix）** 结合数据生成机制预设相关结构（教育集群试验通常选用可交换相关结构 Exchangeable，[[Hypothesis|假设]]同校任意两名学生相关度相同）。
> 4. **执行准似然迭代求解** 利用广义估计方程算法迭代计算回归参数向量 $\hat{\boldsymbol{\beta}}$。
> 5. **提取并汇报稳健[[Standard Error|标准误]]** 无论模型基于工作相关矩阵输出的模型标准误为何，最终结论必须汇报基于[[Robust Variance Estimation|三明治估计量]]的经验稳健标准误（Robust Standard Errors）。

### 量化分析模块

> [!method-stack] 数据、[[Variable|变量]]与模型
> - **数据结构** 包含 $J$ 个独立学校集群（$j = 1, \dots, J$），第 $j$ 所学校包含 $n_j$ 名学生（$i = 1, \dots, n_j$），每个观测拥有[[Independent Variable|预测变量]]向量 $\mathbf{X}_{ij}$ 与因变量 $Y_{ij}$。
> - **核心变量**
>   - 因变量 $Y_{ij}$：[[Pre-test and Post-test|后测]]连续学业考分或及格达标二元哑变量。
...
>
> **数学直觉** GEE 是加权广义[[Ordinary Least Squares|最小二乘法]]向非线性非独立数据的精妙拓展。它利用工作相关矩阵 $\mathbf{R}_j(\boldsymbol{\alpha})$ 为同校学生的残差赋予恰当的非对角协方差权重，使估计方程在数学期望上恒等于零，从而提纯出全系统的无偏边际因果效应。
>
...
> - $\text{Var}_{\text{robust}}(\hat{\boldsymbol{\beta}})$：最终用于假设检验与构建[[Confidence Interval|置信区间]]的稳健协方差矩阵。
>
...
> [!method-fit] 适用判断
> - **适合使用**
>   - 学校或班级集群数量充足（$J \ge 40$）的大规模整群[[Randomised Controlled Trials|随机对照试验]]（[[Cluster Randomized Trials|Cluster RCT]]）；
>   - 评估干预对全系统平均学生的净成效（如国家教育部门关心的整体普惠政策增益）；
>   - [[Dependent Variable|因变量]]为非正态分布（如学生是否及格的二元达标率、违纪行为发生次数的计数[[Variable|变量]]）的嵌套数据；
...
> - **不适合使用**
>   - [[Central Question|核心研究问题]]聚焦于解构各学校自身[[Organizational Culture|组织文化]]差异与特定学校[[Heterogeneity|异质性]]截距的情境（应选用[[Hierarchical Linear Model|多层线性模型]] HLM）；
>   - 意在检验复杂跨层级随机斜率[[Interaction Effect|交互作用]]（Cross-level Random Slope Interaction）的研究。
...
> [!method-limits] 方法局限
> - **小集群三明治方差向下偏倚** 当集群数量不足 30 所时，经验三明治方差对真实方差的估计存在系统性低估，导致[[Type I and Type II Errors|第一类错误]]（假阳性）虚高。
> - **缺失数据处理严苛** GEE 默认仅在数据完全随机缺失（Missing Completely at Random, MCAR）时保持无偏；若数据属于随机缺失（MAR），必须引入逆概率加权（IPW-GEE），否则估计量可能产生偏误（相比之下，全参数 [[Hierarchical Linear Model|HLM]] 在 MAR 条件下依然无偏）。
...
> >
> > - **边际模型（GEE）阵营** 强调政策制定的公共属性。宏观教育决策者关心的是“推行该教学法能让全系统平均学童提高多少分”，而非某个特定学校潜[[Variable|变量]]是多少。GEE 无需强加正态随机效应假定，对工作矩阵误设具有免疫力，是最诚实稳健的政策评估工具。
> > - **多层模型（[[Hierarchical Linear Model|HLM]]）阵营** 强调教育实践的生态分层属性。批评 GEE 将宝贵的学校间方差视作无用噪声（Nuisance），丢弃了理解学校组织差异与跨层级教学机制的机会。肖等学者（Xiao et al., 2016）利用 NPD 归档数据的复算表明，在教育试验样本充足且正态性良好的场景下，两类模型的点估计极为接近，共同构成了捍卫实验公信力的双重防火墙（[[Argument_Edovald_Nevill_2021_ECNUROE|Edovald & Nevill, 2021]]）。
...
> | [[Simple Difference in Means Model]] | 基准模型 | 零协[[Variable\|变量]]时的最简比较模型，与 GEE 构成控制方差与群聚特征的演进阶梯。 |
> | [[Intraclass Correlation Coefficient]] | 关联参数 | GEE 可交换工作相关矩阵中的关联系数参数对应概念。 |
> | [[Cluster Randomized Trials]] | 核心应用场景 | GEE 作为解决整群依赖、校正[[Standard Error\|标准误]]的核心分析工具所服务的试验形态。 |
> | [[Model Dependency]] | 风险现象 | 解释了四模型复算中因模型底层哲学差异而引发的[[Effect Size\|效应量]]敏感性。 |

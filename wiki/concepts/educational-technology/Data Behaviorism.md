...
> [!abstract]
> - **绕过意义建构**：算法不需要理解它处理的数据"意味着"什么——它只需要识别数据中的统计模式。人类的转录（将经验[[Transfer Translation Transformation|转化]]为语言）、解释（赋予经验以含义）和符号化（将含义固化为社会共识）——这些是人类[[Knowledge Production|知识生产]]的核心环节——在数据行为主义中被全部跳过（Rouvroy, 2013, p.143, cited in Jornitz & Klinge, 2022, p.234）

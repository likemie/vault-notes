...
> [!warning]
> Jornitz & Klinge（2022）从学校学习软件的角度，分析了学习分析数据如何通过仪表盘可视化被呈现和消费，以及 Gamification 元素如何与学习分析的技术逻辑耦合。

### 可视化作为预先取代判断的机制

大多数数字学习平台通过仪表盘呈现关键数据——由底层数据挖掘系统的算法决定数据处理和计算方式，教师、学生和家长只能通过仪表盘的呈现间接操作（Williamson, 2016, cited in Jornitz & Klinge, 2022, p.243）。仪表盘通过可识别的颜色提示引导注意力——红色=最差，绿色=最好——颜色的象征意义预先取代了复杂的判断过程。

一项关于健康测量的研究表明，即便测量专家已对结果进行了口头情境化解释，被红色标示的测量输出（在绿-黄-红色谱中）仍使部分参与者经历了数周的焦虑（Klinge et al., 2020, cited in Jornitz & Klinge, 2022, pp.243–244）。这表明可视化的情感冲击力可能压倒了对测量本身的理性评估——在教育语境中，同样的机制可能导致学生、教师和家长过度信任仪表盘的颜色判断，而忽视度量背后的计算逻辑。

以德国阅读平台 Antolin 的测验结果页面为例：颜色、图表、正确／错误答案数量和"表现百分比"共同构成直观可理解的视觉呈现。但百分比如何计算并不清楚——实际上两个度量系统（累计负分与表现百分比）在数学上相互矛盾，但视觉呈现掩盖了这一逻辑不一致。学生和教师逐渐被"条件化"以适应这种表现可视化——无论度量方式如何，信息始终是"表现应该达到 100%，在一个完全绿色的圆圈中"（Jornitz & Klinge, 2022, pp.244–245）。

### Gamification 与学习分析的耦合

学习分析的量化输出——积分、百分比、正确率——为 [[Gamification]] 提供了数据基础。在 bettermarks 等数学学习平台中，Gamification 元素（金币、星星）与学习分析数据（任务完成百分比）并列展示，但黄色和大尺寸的 Gamification 符号在视觉上比百分比数字更加醒目。从教育的角度看，这套系统创造了一种新的"货币"——按游戏逻辑而非教学逻辑构造，但通过仪表盘的视觉权威获得了教育正当性（Jornitz & Klinge, 2022, pp.242–243）。

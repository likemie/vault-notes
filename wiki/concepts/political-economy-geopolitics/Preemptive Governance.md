...
先发制人治理的核心特征是治理时间的**前置**（preemption）。传统教育治理在以下时间点运作([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.221–224]])：
...
先发制人治理的技术基础是"成功相关性"（correlations of success）分析——[[Learning Analytics|学习分析]]和教育数据挖掘技术使大学能够识别最可能导致学业成功或失败的学生特征([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, p.220]])。Antunes（2010）的 ASAP 分类器（ASAP classifiers）是典型案例——能够自动预测学生成功或失败概率的算法，以便在"最早阶段"实施对策([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, p.224]])。

### 从排斥到自我选择

先发制人治理的关键机制不是直接的筛选淘汰，而是自我选择（self-selection）。OSA 被明确宣称为"不是入学考试，不影响录取"——其治理效果恰恰依赖这种非强制性：学生在获得"有条件的匹配"或"不匹配"的反馈后，自行决定调整选择或弥补不足([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.222–223]])。这正是 [[Governmentality|治理术]] 的典型运作——通过塑造自我理解来引导行为，而非通过强制禁止。

### 数字差异驱动的优化

先发制人治理的具体操作依赖于 [[Digital Self|数字自我]]-[[Digital Optimum|数字最优]]差异机制：算法生成一个"完美起点"的理想形象（数字最优），OSA 数据建构个体的统计画像（数字自我），两者之间的差距驱动学生进行弥补性自我优化——上语言班、补修先修课程、调整专业选择([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, p.226]])。详见 [[Digital Self#与数字最优的差异]]。
...
1. **宏观层**：[[Competitiveness|高等教育竞争力]] 作为地缘政治装置——通过排名和指标化将大学重塑为竞争者([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.219–220]])
2. **中观层**：大学作为 [[Stakeholder University|利益相关者大学]] 转型为多行动者连接平台——需要管理学生成功的可预测性以维持排名和声誉([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.221–222]])
3. **微观层**：先发制人治理通过 [[Online Self-Assessment|OSA]] 和 [[Learning Analytics]] 在个体层面运作——构建 [[Digital Self|数字自我]]、生成 [[Digital Optimum|数字最优]]、通过差异驱动自我优化([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.224–226]])
4. **主体层**：治理产出 [[Promising Student|有前景的学生]]——主动自我评估、持续自我优化、将学业成功视为个人风险管理([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, pp.221, 226]])
...
> - **教育开放性的丧失**：当治理逻辑预设"学生的未来可以被入场数据预测"，教育的根本开放性——成长、转变、不可预测的发展——被系统性地排除在治理视野之外([[Argument_Thompson_2022_Promising_Student|Thompson et al., 2022, p.227]])

---
title: Governing by Numbers
aliases:
  - 数字治理
  - 以数字治理
  - governance by numbers
summary: "Grek (2009) 提出的概念，指通过量化指标、国际排名和数据比较实现跨国教育治理的软性机制，PISA 是这一治理模式的典型工具"
type: concept
domain: "political-economy-geopolitics"
related_count: 21
related_level: 2
related_stars: "⭐⭐"
related_color: "#99f6e4"
tags:
  - governance-by-numbers
  - pisa
  - soft-governance
  - oecd
  - region/global
  - theme/global-governance
related_concepts:
  - "[[Policy Brokerage]]"
  - "[[Transfer Translation Transformation]]"
  - "[[Evidence-Based Education]]"
  - "[[Hypothesis]]"
  - "[[Soft Power]]"
  - "[[Relational Space]]"
  - "[[Technical Rationality]]"
  - "[[Paradigm]]"
  - "[[Champ]]"
  - "[[Variable]]"
  - "[[Document]]"
related_theories:
  - "[[Governing at a Distance]]"
  - "[[Governmentality]]"
  - "[[Convergence Theory in Comparative Education]]"
  - "[[Institutional Isomorphism]]"
related_methods:
  - "[[Effect Size]]"
  - "[[Systematic Review]]"
related_instruments: []
related_persons:
  - "[[Gita Steiner-Khamsi]]"
related_facts:
  - "[[PISA]]"
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Steiner-Khamsi_2024_CE]]"
  - "[[Argument_Li_2025_HSSC]]"
confidence: medium
status: draft
created: 2026-07-05
updated: 2026-09-11
---

# Governing by Numbers

---

## 定义

> [!def] 核心定义
> 数字治理（Governing by Numbers）指通过量化指标、国际排名和数据比较来实现跨国教育治理的软性机制。该概念由 Grek (2009) 奠基，最初指大型国际评估（如 [[PISA]]）提供的跨国可比数据使政策制定者能够在全球尺度上比较教育系统，用指标制定政策并形成远距离指导；Steiner-Khamsi 等（2024）进一步提出“数字治理 2.0”（Governance by numbers 2.0），指出在数字经济与信息过载时代，治理重点已从单纯发布测验排名深化为通过[[Policy Brokerage|政策中介]]筛选、包装与[[Transfer Translation Transformation|转译]]海量证据，以此在多重不确定性下维系软治理。

> [!concept-lens] 概念透镜
> - **含义** 教育治理的权力运作不再主要依赖强制或立法，而是通过数据生产、排名比较、指标标准化以及政策中介的证据包装来引导各国教育政策向特定方向靠拢。
> - **用途** 用于分析国际组织（尤其是 [[OECD]] 与世界银行）如何通过评估工具以“软治理”方式重塑各国教育政策议程，以及量化数据如何在政治决策中获得合法性。
> - **边界** 不等于传统自上而下的科层治理或法律强制——数字治理依赖的是信息的权威性和比较压力，而非直接命令；亦非中立的科学传播，数据生产与中介转译本身包含政治性的分类、标准化与排他性网络逻辑。

> [!citation-card] 数字治理 2.0 的研究议程
> 数字治理 2.0 比较研究计划基于多学科（历史、政治学、跨学科政策研究）的解释框架，提出一种跨国的、关系性的调查方法，聚焦全球/国家的关联，考察国际组织如何将政策中介发展为全球治理工具，以及主权国家政府在政策周期各阶段的策略性应对。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, p. 538)]]
>
> *The authors make the case for a comparative research programme – tentatively labeled 'Governance by Numbers 2.0' – that is informed by a multidisciplinary (history, political science, interdisciplinary policy studies) interpretive framework and that advances a transnational, relational method of inquiry which draws attention to the global/national nexus.*

> [!boundary]- 概念边界
> - 不等于 [[Governing at a Distance|远处治理]]（Governing at a Distance） — 远处治理侧重于福柯式的[[Governmentality|治理术]]和超越地理空间的调控形态；数字治理更具体聚焦于量化指标、统计技术、数据库与评测排名的数字媒介机制。
> - 不等于 [[Evidence-Based Education|循证教育]]（Evidence-Based Education） — 循证教育侧重于教育决策与实践对实证科研结果的理想化学术依托；数字治理则揭示了数字与证据在现实政治博弈中被武器化、符号化与中介包装的权力运作。

---

## 概念辨析

> [!contrast-table] 概念辨析
> | 维度 | 数字治理 1.0（指标与排名驱动） | 数字治理 2.0（[[Policy Brokerage\|政策中介]]与工具打包） | 传统国家科层治理 |
> |------|-------------------------------|----------------------------------|----------------|
> | **治理媒介** | 跨国大型测验（如 [[PISA]]）去情境化排名与发榜冲击 | 复合工具打包（评测+同行审查+外向借鉴+自指方案库） | 法律法规、自上而下行政命令与财政拨款调控 |
> | **权力形态** | 基于同侪竞争与公开排名的软性规范压力 | 知识武器化、认知框架锁定与中介权威背书 | 民族国家主权之下的法定科层强制力与行政惩戒 |
> | **信息生态[[Hypothesis\|假设]]** | 假设信息匮乏或分散，需要基准测试填补空白 | 确认信息过载与证据过剩，决策者极度依赖筛选[[Transfer Translation Transformation\|转译]] | 依靠法定科层通道进行自上而下的政策指令下达 |
> | **国际组织关系** | 视单一国际组织为独立的跨国评估机构 | 视五大巨头为嵌入治理复合体的利基竞争与同盟者 | 主权国家政府为主导，排斥或从属于外部条约体系 |

---

## 核心要素

> [!feature] 核心要素
> - **量化数据的政治权威** [[PISA]] 等数据被分析和解读后为教育政策制定和改革所用。国际组织通过发布测评结果和全球排名，为各国政策制定者提供远距离指导（Remote Guidance），用以验证既有改革的合法性或驱动后续政策（Gillis et al., 2016）。
> - **去情境化与普遍性外观** 跨国测验受益于其去情境化和普遍性特征，使不同政治、文化和历史背景的教育体系可以在同一标尺上被排列和比较，为循证政策制定披上理性外衣。
> - **信息过量下的[[Policy Brokerage|政策中介]]化（2.0 阶段特征）** 在数字化时代的研究证据过剩与信息孤岛环境下，更多知识并未减少政策歧义；数字治理演进为依托[[Policy Brokerage|政策中介]]机构选择性筛选科研发现并将其[[Transfer Translation Transformation|转译]]为定向政策证据。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 538–540)]]
> - **以硬事实施展软权力（[[Soft Power]] by Hard Facts）** 国际组织将核心中介工具与差异化知识产品打包（如[[OECD|经合组织]]的 PISA 绑定国别经验对照，世界银行的基准数据库绑定自指性实践仓库），通过指标问责施展软权力。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 539, 544)]]

> [!logic-map]- 数字治理 1.0 向 2.0 演进逻辑图
> ```mermaid
> flowchart TD
>     subgraph V1["数字治理 1.0 (2000s)"]
>         PISA["大规模跨国评测 (PISA)"]
>         Rank["全球公开排行榜与发榜冲击"]
>         Peer["同行声誉压力与远处治理"]
>         PISA --> Rank --> Peer
>     end
>     subgraph V2["数字治理 2.0 (2020s)"]
>         Overload["信息过载与证据过剩生态"]
>         Complex["全球教育治理复合体 (Big Five)"]
>         Brokerage["政策中介：工具打包、自指性方案库与经验对照"]
>         Weapon["知识武器化：组织利基防御与排他权力同盟"]
>         Overload --> Brokerage
>         Complex --> Weapon
>         Brokerage <--> Weapon
>     end
>     Peer ==> Overload
> ```

---

## 围绕概念形成的命题

---

### 命题一　跨国量化评估构筑起无需强制法律的远距离软治理机制

> [!concept-lens] 软治理与排名问责
> 探讨国际组织如何将教育系统表现转化为跨国数据，从而行使软性权力。

> [!claim] Grek (2009)
> **信息生产与软性权力机制** [[PISA]] 代表的数字治理机制通过将教育系统表现量化为跨国可比的排名和数据，使 [[OECD]] 能够在不对各国施加法律强制的情况下引导全球教育改革方向。

> [!claim] Li et al.
> **数字治理的双向互动机制** PISA 与国家教育改革之间存在双向互动：各国政策制定者利用 PISA 数据来验证和推动国内已有的改革议程，同时又因国际排名压力而调整政策方向以回应外部审视。[[Argument_Li_2025_HSSC|(Li et al., 2025)]]

---

### 命题二　数字治理 2.0 将治理中枢拓展至政策中介的证据筛选与周期博弈

> [!concept-lens] 信息过载与政策周期过滤
> 探讨从 1.0 的测验排名推进至 2.0 阶段的[[Policy Brokerage|政策中介]]竞争与国家政策周期的多阶段过滤。

> [!claim] [[Argument_Steiner-Khamsi_2024_CE|Steiner-Khamsi et al. (2024)]]
> **中介证据包装与行政出台阶段的政治过滤** 在信息过载与证据过剩的数字经济下，数字治理进入 2.0 阶段；国际组织不再仅靠单次发榜制造冲击，而是通过长期扮演政策中介，将测评工具与辅助知识打包以确立组织利基。在国家政策周期中，跨国中介证据在议程设置阶段被广泛动员，但在行政部门颁布法令的法定决策阶段，主权国家官僚往往展现出强烈的自主筛选与政治过滤倾向。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 544–547)]]

---

### 命题三　量化知识的深度武器化激化了跨国治理复合体内部的利基争夺

> [!concept-lens] 知识武器化与利基壁垒
> 分析各大国际组织如何将指标与测评工具转化为维系自身排他性权力同盟与资助地盘的武器。

> [!claim] [[Argument_Steiner-Khamsi_2024_CE|Steiner-Khamsi et al. (2024)]]
> **量化知识的排他性武器化与多边利基割据** 在数字治理 2.0 阶段，量化指标与测评工具不仅被用于向受测主权国家施加问责压力，更在跨国机构之间被深度“武器化”（weaponised），演变为各大国际组织维系专属治理领地、防范职能被蚕食的防御性制度武器。尽管跨国组织在修辞上高调宣称多边协同（如发起 BE2 智库联盟），但在实践中极力捍卫自身不可替代的测量品牌与项目资助利基。例如当联合国教科文组织与世界银行共同发起“学习数据协定”（Learning Data Compact）意图统合格局时，经合组织明确拒绝加入，转而独立与德国开发署合作推广 PISA-D，揭示出数字治理表面量化中立性掩盖下的跨国地盘割据与制度权力同盟。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 539, 549–550)]]

---

### 命题总览

> [!contrast-table] 所有命题归纳
> | 命题类型 | 核心指向 | 适用情境 | 代表学者 |
> |---|---|---|---|
> | **跨国软治理机制 (1.0)** | 揭示大型测评数据与排名压力如何引导主权国家改革方向 | 国际大型评估与去情境化排名 | Grek (2009); [[Argument_Li_2025_HSSC\|Li et al. (2025)]] |
> | **政策中介与周期博弈 (2.0)** | 阐明信息过载下国际组织的中介工具打包与国家政策周期的选择性过滤 | 数字化时代证据过剩与跨国政策转移 | [[Gita Steiner-Khamsi\|Steiner-Khamsi]] et al. |
> | **知识武器化与利基割据** | 揭示量化工具在跨国治理复合体内部演变为机构防卫与地盘竞争的制度武器 | 国际组织间竞合与多边治理网络 | Steiner-Khamsi et al. |

---

## 概念演变

> [!dev-timeline] 概念演变
> - **2009 — 数字治理 1.0 确立** Grek 提出以数字治理概念，系统剖析[[OECD|经合组织]]如何依托 [[PISA]] 产生的数据流将欧洲乃至全球教育空间纳入软治理网络。
> - **2018 — 硬事实软权力机制提炼** Niemann & Martens 提出 Soft governance by hard facts 命题，揭示量化事实作为非强制治理工具的有效性。
> - **2024 — 数字治理 2.0 理论跃升** [[Argument_Steiner-Khamsi_2024_CE|Steiner-Khamsi et al. (2024)]] 提出数字治理 2.0 议程，将研究视域拓展至多学科、跨国[[Relational Space|关系性空间]]，聚焦[[Policy Brokerage|政策中介]]在信息过载时代的多阶段证据包装与组织竞合。

---

## 争议与批评

> [!debates] 学术争议
>
> > [!axis] [[Technical Rationality|技术理性]]中立性还是隐蔽的政治规制与意识形态控制
> > 争论数字治理是提供了客观中立的公共政策改进工具，还是推行新自由主义绩效问责与市场化管制的意识形态工具。
> >
> > - **技术理性派** 强调量化指标克服了主观臆断与地方保护主义，为跨国政策优化与资源精准配置提供了不可或缺的实证基底。
> > - **批判政治学派** 揭露数字治理将复杂的教育伦理与育人过程简化为单一分值，借“中立硬事实”行使不透明的跨国软治理霸权。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 538–540)]]
>
> > [!axis] 全球趋同重力还是主权行政的策略性工具化
> > 讨论数字治理最终导致全球政策[[Paradigm|范式]]完全趋同，还是仅成为民族国家政客策略性借力与掩盖内部矛盾的“外在化”工具。
> >
> > - **世界文化[[Convergence Theory in Comparative Education|趋同论]]** 认为跨国指标具有无法抗拒的[[Institutional Isomorphism|制度同形]]力量，推动全球教育体制加速向新自由主义模型靠拢。
> > - **关系性借用论** 实证表明各国政府依据本土政治需要选择性借用甚至曲解数据，并在法定出台阶段实施剧烈的行政过滤。[[Argument_Steiner-Khamsi_2024_CE|(Steiner-Khamsi et al., 2024, pp. 546–548)]]

> [!warning] 适用局限
> 数字治理依赖于受测体系具备起码的数据采集、计算机施测网络与统计官僚能力；在高度前现代、战乱频仍或坚决排斥西方标准化的边缘[[Champ|场域]]，数字治理的传导链条极易发生脱嵌与失效。

---

## 实证数据

> [!ref-table]- 其他实证结果（无[[Effect Size|效应量]]）
> <span class="concept-other-empirical-table-marker" aria-hidden="true"></span>
>
> | 研究 | 样本与情境 | 研究设计 | [[Variable\|变量]]或指标 | 原始统计结果（无效应量） | 不确定性或显著性 | 解释边界 |
> |---|---|---|---|---|---|---|
> | [[Argument_Steiner-Khamsi_2024_CE\|Steiner-Khamsi et al. (2024, p. 547)]] | 挪威能力课程改革（5 个专家委员会[[Document\|文献]] vs 最终颁布的部长令） | 跨国政策咨询文献引用网络追踪分析 | 专家委员会参考文献总数 vs 最终进入部长令的文献数 | 专家委员会总计列入 464 篇文献，最终仅有 22 篇被部长令引用；且被引的 22 篇中有 20 篇全部来自专门审查[[OECD\|经合组织]] DeSeCo 项目的同一专家委员会 | — | 强有力证明国家行政当局在法定决策阶段对数字治理中介知识的极端选择性吸收与政治过滤 |
> | [[Argument_Steiner-Khamsi_2024_CE\|Steiner-Khamsi et al. (2024, pp. 544, 549)]] | 全球 30 个涉教育政府间组织（IOs）；世界银行 SABER 与教育回报率数据库 | 跨国治理机构职能数据库追踪；多边政策工具计量普查 | 经济类国际组织占比；指标与基准工具规模 | 涉教育国际组织中 33.3% 为非专门的经济合作协调组织；世界银行 SABER 涵盖 1,600 余项系统性指标，政策仓库汇聚 1,120 项教育回报率自指性研究 | — | 实证揭示跨界经济组织在数字治理中的主导地位，以及跨国中介依赖自指性量化证据建构组织利基的规模 |

---

## 相关研究

> [!evidence-grid-a] 相关研究索引
> - [[Argument_Steiner-Khamsi_2024_CE|Steiner-Khamsi et al. (2024)]] — 提出数字治理 2.0 理论框架，系统探讨[[OECD|经合组织]]与世界银行的[[Policy Brokerage|政策中介]]机制及其在国家政策周期中的多阶段效应。
> - [[Argument_Li_2025_HSSC|Li et al. (2025)]] — [[Systematic Review|系统综述]]了 [[PISA]] 如何通过数字治理机制影响全球基础教育改革，提出三层机制模型。

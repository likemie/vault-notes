...
>         direction TB
>         A["吸引力火花（丑闻化/颂扬/警示/合法化）"] <-->|双向匹配| B["方案先行、问题后建（国内政治议程）"]
>         B -->|话语投射| C["外化 (Externalization)"]
>         C -->|论证正当性| D["合法化功能 (Legitimation)"]
>         D -->|巩固既定方案| B
>     end
> 
>     subgraph 物质与拓扑装配环 ["数据基础设施与拓扑装配环"]
>         direction TB
>         E["四阶段生命周期（吸引力→决策→实施→内化）"]
>         F["形态变异与本土转译 (Morphing)"]
>         G["跨尺度数据基础设施（题库/计算中心/平台）"]
>         H["拓扑折叠与远距离治理 (Topological Rescaling)"]
>         E -->|落地调适| F
>         F -->|技术固化| G
>         G -->|穿透微观教学| H
>     end

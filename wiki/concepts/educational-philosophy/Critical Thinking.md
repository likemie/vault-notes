---
title: Critical Thinking
aliases:
  - 批判性思维
  - CT
  - critical thinking skills
  - critical thinking dispositions
summary: "以有目的的、自我调节的判断为核心的思维技能与倾向，被广泛认为是大学学习和专业工作的核心学术能力。其教学方式存在显性独立课程与隐性学科融入两种路径的持续争论。"
type: concept
domain: "educational-philosophy"
related_count: 19
related_level: 1
related_stars: "⭐"
related_color: "#bfdbfe"
tags:
  - theme/critical-thinking
  - theme/curriculum
  - theme/higher-order-thinking
related_concepts:
  - "[[Hypothesis]]"
  - "[[Conatus]]"
  - "[[Reliability]]"
  - "[[Variable]]"
  - "[[Effect Size]]"
  - "[[Dependent Variable]]"
  - "[[Confidence Interval]]"
  - "[[Heterogeneity]]"
  - "[[Document]]"
  - "[[Theory of Knowledge]]"
related_theories:
  - "[[Explicit Critical Thinking Instruction]]"
related_methods:
  - "[[Meta-analysis]]"
  - "[[Mixed Methods Research]]"
related_persons: []
related_facts:
  - "[[International Baccalaureate]]"
related_arguments:
  - "[[Argument_Cole_2015_AJE]]"
  - "[[Argument_Zemplen_2007_SciEduc]]"
related_instruments:
  - "[[California Critical Thinking Disposition Inventory]]"
  - "[[Critical Thinking Strategies Scale]]"
  - "[[Motivated Strategies for Learning Questionnaire Critical Thinking Subscale]]"
confidence: medium
status: draft
created: 2026-06-30
updated: 2026-08-03
---

# Critical Thinking

---

## 定义

> [!def] 核心定义
> 批判性思维（critical thinking）是"基于严格的智力概念和原则进行有目的的、自我调节的判断的能力……被广泛认为是一项核心学术技能"（Facione, 1990, p. 2）。它包括分析论证、评估主张、澄清意义、考虑不同视角以及在证据基础上得出结论等一系列认知技能，也包括求真、开放、分析性、系统性等思维倾向（dispositions）。

> [!concept-lens] 概念透镜
> - **含义** 批判性思维指向一种超越记忆和复述的认知活动。个体主动澄清问题、识别论证结构、评估证据质量、考虑替代解释，并在反思自身[[Hypothesis|假设]]的基础上做出判断。
> - **用途** 它帮助研究者和教育者识别学生是否具备独立推理能力，以及课程是否真正培养了超越学科内容的高阶思维。
> - **边界** 批判性思维不等于一般智力或学科知识掌握；它也不等于纯粹的否定或怀疑态度，而是包含建设性的判断和论证能力。

---

## 核心要素

> [!feature] 批判性思维的核心技能（基于 Delphi Report, Facione, 1990）
> - **澄清意义（Clarification）** [[Conatus|努力]]澄清不熟悉术语的含义和定义，确保理解的精确性。
> - **理解新材料的关联（Integration）** 将新材料与已有知识或经验建立联系。
> - **多角度表述问题（Reformulation）** 以多种方式重新表述问题，以澄清其含义和范围。
> - **分解问题（Decomposition）** 将复杂问题分解为子问题逐一处理。
> - **识别论证（Identification）** 主动识别他人正在做出的论证和主张。
> - **分析论证（Analysis）** 拆解论证的结构、前提和推理链条。
> - **评估主张（Evaluation）** 评估主张的[[Reliability|可靠性]]，而非被动接受。
> - **设想替代方案（Alternatives）** 想象解决同一问题的不同方式。
> - **基于证据得出结论（Evidence-based Conclusion）** 依据手头证据而非直觉做出判断。
> - **限制主张范围（Scope Restriction）** 将主张限制在证据所能支持的范围内。
> - **搜索对立信息（Counter-evidence Search）** 同时搜索支持和反对自身立场的信息。
> - **反思[[Hypothesis|假设]]（Assumption Reflection）** 审视自身假设如何塑造观点。
> - **论证策略辩护（Strategy Justification）** 能够辩护解决问题或构建论证所使用的策略。
> - **论证呈现（Presentation）** 以他人能理解的方式呈现论证。

> [!feature] 多套分类框架反复出现的技能与扩展边界
> - **共同核心** Facione、Paul、Halpern、Black 与修订版 Bloom 分类法反复强调辨别、分析、判断或评价、解释和论证，可作为课程设计与评估的最低覆盖标准。
> - **理由权衡** 理性判断要求识别不同理由与判断的相关性，并恰当地衡量其分量；批判性思维因而同时涉及技能和对理由的承诺。
> - **自我调节与倾向** 开放心态、理解他人、独立思考、智识谦逊和自我纠正决定技能能否以公平而反思的方式使用。
> - **主动生成活动** 推论、预测、假设检验、不确定性判断、决策、问题解决和创造揭示了批判性思维中从分析既有材料走向生成判断与方案的一面。
> - **边界分歧** Halpern 将问题解决与创造纳入构念，Black 则把它们置于边缘或构念之外；因此，命令词清单只能诊断课程覆盖，不能单独证明思维质量。[[Argument_Hughes_2014_JRIE|(Hughes, 2014, pp. 31–35)]]

> [!feature] 批判性思维的倾向维度（[[California Critical Thinking Disposition Inventory|CCTDI]] 七维度）
> - **求真（Truth-seeking）** 即使在证据不支持自身立场时也追求真相。
> - **开放心态（Open-mindedness）** 容忍不同观点，对自身偏见保持敏感。
> - **探究性（Inquisitiveness）** 对事物的好奇心和求知欲。
> - **分析性（Analyticity）** 对潜在问题的警觉性和对推理的重视。
> - **系统性（Systematicity）** 有条理、有组织地处理复杂问题。
> - **推理自信（Confidence in Reasoning）** 对自己推理能力的信任。
> - **判断成熟度（Maturity of Judgement）** 在做判断时考虑多种可能性和情境。

---

## 围绕概念形成的命题

---

### 教学路径之争　显性教学与隐性融入

> [!concept-lens] 教学路径之争
> 批判性思维应当作为独立课程显性教授，还是融入各学科隐性培养？这一争论是批判性思维教育研究的核心议题。

> [!claim] Abrami et al. (2008)
> 对 117 项研究、20,698 名参与者的[[Meta-analysis|元分析]]表明，显性教学（explicit instruction）比隐性教学（implicit instruction）更能有效提升学生的批判性思维技能；当批判性思维被作为独立课程教授时，学生获益最大。

> [!claim] Marin & Halpern (2011)
> 在低表现学校的实验表明，接受[[Explicit Critical Thinking Instruction|显性批判性思维教学]]的高中生在思维技能上显著优于接受隐性教学的学生，无论课程在课后还是课内进行。

> [!claim] Willingham (2007) / Bailin (2002) — 领域特异性立场
> 批判性思维依赖于具体领域的知识——不同学科中构成有效证据、论证和标准的内容各不相同。在特定学科内学习批判性思维比在通用意义上学习更容易，因为学生已经掌握了该领域的背景知识。

---

### 命题总览

> [!contrast-table] 教学路径命题归纳
> | 教学路径 | 核心指向 | 证据强度 | 代表研究 |
> |---|---|---|---|
> | **显性独立教学（Stand-alone）** | 批判性思维应作为专门课程教授，学生获得通用思维技能后可迁移至各学科 | 强（元分析支持） | Abrami et al. (2008); Marin & Halpern (2011) |
> | **隐性融入教学（Infusion）** | 批判性思维应在各学科内部培养，作为学科学习的副产品 | 弱（学生难以迁移） | Willingham (2007); Bailin (2002) |

---

## 实证数据

> [!effect-table]- 一级研究结果
> <span class="concept-effect-table-marker" aria-hidden="true"></span>
>
> | 研究 | 比较或干预 | 结果[[Variable\|变量]] | 分析样本 | 组别统计 | [[Effect Size\|效应量]] | 显著性或不确定性 | 设计与解释边界 |
> |---|---|---|---|---|---|---|---|
> | [[Argument_Cole_2015_AJE\|Cole et al. (2015)]] | ToK 学习阶段<br>Y12 与 Y11 | [[Critical Thinking Strategies Scale\|CTSS]] 总分 | Y12 622<br>Y11 620 | Y12 4.85（1.05）<br>Y11 4.62（1.08） | Hedges' $g=0.216$（派生） | $t(1240)=3.04$<br>$p<.001$ | 横断调查；自报测量；年级差异不能证明 ToK 导致能力增长 |
> | Cole et al. | ToK 学习阶段<br>Y12 与 Y11 | [[Motivated Strategies for Learning Questionnaire Critical Thinking Subscale\|MSLQ-CT]] | Y12 622<br>Y11 620 | Y12 4.69（1.15）<br>Y11 4.48（1.22） | Hedges' $g=0.177$（派生） | $t(1240)=3.04$<br>$p<.01$ | 横断调查；自报测量；年级差异不能证明 ToK 导致能力增长 |

> [!ma-table]- [[Meta-analysis|元分析]]总体结果
> <span class="concept-meta-analysis-table-marker" aria-hidden="true"></span>
>
> | 元分析 | [[Dependent Variable\|结果变量]] | k / N | 效应指标与模型 | 汇总效应与 95% [[Confidence Interval\|CI]] | [[Heterogeneity\|异质性]]与预测区间 | 证据确定性 |
> |---|---|---|---|---|---|---|
> | Abrami et al. (2008) | 批判性思维技能 | 117 / 20,698 | — | — | — | — |
>
> 待处理该[[Document|文献]]后回填效应指标、汇总效应量、异质性和证据确定性。

> [!ma-table]- 调节变量与亚组分析
> <span class="concept-meta-moderator-table-marker" aria-hidden="true"></span>
>
> | 元分析 | 调节变量 | 分析方式 | 效应较大的条件或方向 | 条件效应或 β | 交互或回归检验 | 是否预设 | 解释边界 |
> |---|---|---|---|---|---|---|---|
> | Abrami et al. (2008) | 教学路径（显性独立 vs 隐性融入） | 亚组分析 | 显性教学条件效应更大 | — | — | 预设 | — |
> | Abrami et al. | 教师培训（受培训且受评估 vs 未受培训） | 亚组分析 | 教师接受专门培训且教学受评估时效应最大 | — | — | 预设 | — |
>
> 待处理该文献后回填各亚组的条件效应量和交互检验结果。

### 批判性思维的认知发展与课程设计

> [!claim] 批判性思维期待可能超出学生的认知发展水平
> King & Kitchener（1994）的反思判断模型表明，高中生的推理能力通常还无法达到"对比不同来源的证据、阐明决策标准"的水平。Dotger（2005）在讨论科学本质（NOS）与批判性思维的关系时指出：NOS 文献中隐含的理解深度期望，实际上"在大多数高阶研究生中才会发展"。如果批判性思维课程的目标超出学生的认知发展阶段，评估就可能在测量一般认知发展——而以"[[Theory of Knowledge|知识论]]"命名的课程成绩在多大程度上反映了批判性思维能力，值得质疑。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, pp. 173–174)]]

> [!claim] 批判性思维课程可能出现术语表演
> 当评估标准使用特定的分析术语作为评分依据时，学生可能学会的是术语表演（name-dropping）——在论文中系统地罗列课程术语并嵌入显示"批判性反思"的语句——而非真正的批判性思维。[[Argument_Zemplen_2007_SciEduc|Zemplén (2007)]]将这一现象称为"框架问题"（framework-problem）：批判性思维课程为评估目的创造的词汇框架，可能恰恰阻碍了它声称培养的自由探究精神。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, pp. 171–172)]]

---

## 争议与批评

> [!tension] 通用技能 vs 领域依赖
> - **通用技能立场（蓝方）** 批判性思维是一组可跨学科迁移的通用认知技能，独立课程教授效果最佳。ToK 课程正是基于这一[[Hypothesis|假设]]设计。[[Argument_Cole_2015_AJE|(Cole et al., 2015)]]
> - **领域依赖立场（红方）** 批判性思维无法脱离具体学科知识运作。历史中的"证据"与科学中的"证据"标准不同，通用课程难以覆盖所有领域的论证标准。学生可能学了批判性思维的"形式"而无法在具体学科中应用。

> [!tension] 批判性思维议程 vs 科学教育议程
> - **CT 优先立场（蓝方）** 批判性思维要求对所有知识主张——包括科学的主张——进行自由的、不受预先约束的审查。如果将批判性思维课程创建为独立科目，就应允许学生运用批判性思维质疑科学的权威、价值预设和社会影响，而不应预设"科学是最高级知识形式"的结论。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, p. 181)]]
> - **科学教育优先立场（红方）** 学校教育中科学科目的首要目的是传授关于物质世界的最权威知识。将批判性思维从属于科学教育目标具有智识和政治上的合法性——不能为了批判性思维的形式自由而牺牲科学知识的传播。[[Argument_Zemplen_2007_SciEduc|(Zemplén, 2007, pp. 179–180)]]

> [!warning] 课程声称与评估覆盖之间可能形成构念不足呈现
> 课程目标即使明确写入批判性思维，也不代表评估已经覆盖该构念。以 ToK 为例，论文和展示能够考察清晰论证、反主张、观点比较、评价与真实情境联系，却没有系统要求批判性阅读、来源可信度评价、推论、假设检验、概率与不确定性判断、决策、问题解决和创造。课程内部目标与评估的部分对齐，不能替代对批判性思维整体构念的充分表征。[[Argument_Hughes_2014_JRIE|(Hughes, 2014, pp. 38–43)]]

> [!warning] 课程改革面临系统性障碍
> 即使在实证证据支持下，在澳大利亚全国课程中推行显性批判性思维课程仍面临重大挑战，包括职前教师培训、学校人员配置、课程组织与内容以及课时安排等多个层面的改革需求。澳大利亚目前的课程格局已经非常拥挤（[[Argument_Cole_2015_AJE|Cole et al., 2015, pp. 258–259]]）。

---

## 相关研究

> [!evidence-grid-a] 相关研究索引
> - [[Argument_Cole_2015_AJE|Cole et al. (2015)]] — 以 [[International Baccalaureate|IB]] DP 的 ToK 课程为案例，通过[[Mixed Methods Research|混合方法研究]]检验[[Explicit Critical Thinking Instruction|显性批判性思维教学]]在澳大利亚学校的效果。
> - [[Argument_Zemplen_2007_SciEduc|Zemplén (2007)]] — 以 IB [[Theory of Knowledge|TOK]] 课程为案例，分析批判性思维议程与科学教育议程之间的结构性冲突，指出评估框架可能制造术语表演而非真正的批判性思维。
> - [[Argument_Hughes_2014_JRIE|Hughes (2014)]] — 综合多套批判性思维分类框架，审查 ToK 课程目标、评估目标与描述符是否充分覆盖批判性思维构念。

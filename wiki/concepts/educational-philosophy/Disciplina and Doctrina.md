---
title: Disciplina and Doctrina
aliases:
  - 规训与教学
  - 知识的规训与更新
  - disciplina
  - doctrina
summary: "西方教育思想史上关于知识组织和传授的一对经典二元范式：disciplina 侧重既有知识的灌训与规训，doctrina 侧重知识的动态生产与开放更新"
type: concept
domain: "educational-philosophy"
related_count: 0
related_level: 0
related_stars: "☆"
related_color: "#e5e7eb"
tags:
  - educational-philosophy
  - curriculum-theory
  - sociology-of-knowledge
related_concepts: []
related_theories: []
related_methods: []
related_persons: []
related_facts: []
related_arguments: []
confidence: high
status: draft
created: '2026-06-22'
updated: '2026-06-22'
---

# Disciplina and Doctrina

---

## 定义

> [!def] 核心定义
> **Disciplina 与 Doctrina** 是西方教育史和知识社会学中用于界定知识传递与方式的一对经典二元概念。**Disciplina（规训/教规）** 关注对既有、已发现知识的灌输与巩固，侧重维持秩序和正误纠正；**Doctrina（教学/教义/更新）** 则是指发展、质疑和更新知识的动态系统（Stichweh, 2001, cited in [[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 5]]）。在现代教育政策中，这对概念被用于批判以测量和结果为导向的机械问责，并呼吁恢复教学的开放与批判面向（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 10]]）。

> [!concept-lens] 概念透镜
> - **含义**：`disciplina` 指向知识被发现后在学科库中的沉淀与外在规训灌输；`doctrina` 指向知识系统的自我更新、生产与开放研究。
> - **用途**：帮助课程研究者辨析为什么某些改革（如跨学科现象本位学习）容易在政策操作中退化为刻板的指标检测（即被 `disciplina` 侵蚀），并揭示课程背后的权力与知识再生产逻辑。
> - **边界**：它不简单等同于“传统教学 vs. 现代教学”，而是在中世纪大学诞生起便并存的、组织人类 curiosity 的两种互补又对立的态度。

---

## 概念辨析

> [!contrast-table] 概念辨析
> | 维度 | Disciplina (规训/教规) | Doctrina (教学/更新) |
> |------|---|---|
> | **核心指向** | 巩固、存储并复制既有的知识与规范（[[Argument_Schaffar_2024_CogentEdu\|Schaffar & Wolff, 2024, p. 5]]）。 | 生产、质疑、调整并更新人类的知识系统（[[Argument_Schaffar_2024_CogentEdu\|Schaffar & Wolff, 2024, p. 5]]）。 |
> | **教育手段** | 方法灌输、纪律约束、对错误的纠正或惩罚。 | 行动者反思、社会互动协作、批判性对话与未知探索。 |
> | **时间取向** | 面向过去（维护传统的稳定性与继承性）。 | 面向未知和 radical 开放的未来（非目的论过程）。 |
> | **政策表现** | 标准化测试、成果导向课程、细分技能指标考核。 | 跨学科探究、围绕时代关键问题开展非预设讨论。 |

---

## 核心要素

> [!feature] 核心要素
> - **Disciplina 的静态存储性**：扮演“存放已发现知识的场所”角色，其将知识结构化为严密的科目，主要执行代际间的社会化与再生产职能（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 5]]）。
> - **Doctrina 的动态生产性**：视知识为动态演化的体系，承认过去的知识无法完全涵盖未来的可能性，因此教学必须向学生的提问与创新开放（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 5]]）。
> - **绩效时代的失衡**：当代全球教育行业（GEI）过度追求“可测量结果”与“横向绩效评估”，通过技能原子化技术，强行将具备 `doctrina` 特质的探究活动塞入 `disciplina` 的流水线框架，损害了教育作为自我转变和民主反思的过程（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, pp. 9–10]]）。

---

## 围绕概念形成的命题

---

### 命题类型一：课程标准的二元调和（Tension in Curriculum Design）

> [!concept-lens] 学科深度与跨学科的统合
> 这一命题群探讨如何在尊重既有学科深度（disciplina）的前提下，实现促进知识更新与创新的跨学科广度（doctrina）。

> [!claim] <Schaffar, B., & Wolff, L.-A.>
> 优秀的教育设计不能抛弃学科，因为学科分类是人类探索现象的历史沉淀，具备本体论深度（disciplina）；但教学不能止步于此，必须引导学生围绕“时代关键问题”（如可持续性发展）开展跨学科的协同探索，激发知识的创造（doctrina）（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, pp. 4–6, 10]]）。

---

### 命题类型二：新自由主义绩效评估的局限（Accountability & Metric Erosion）

> [!concept-lens] 可测量性对教学批判力的抹杀
> 这一命题群剖析数字资本主义与评估产业如何通过标准化控制将教学异化为单一的规训程序。

> [!claim] <Foran, T.>
> 当学生被还原为度量衡和政府数据时，原本指向反思与合作的生命世界体验在学校中失去了意义，`doctrina` 的批判潜能被指标体系彻底窒息，学校蜕变为纯粹的规训机器（`disciplina`）（Foran, 2020, cited in [[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, pp. 9–10]]）。

---

## 概念演变

> [!dev-timeline] 概念演变
> - **中世纪 — 术语起源**：大学和修道院教育中分化出 `disciplina`（强调对学生行为的惩戒、纠错和规训）与 `doctrina`（强调真理的讲授和知识研究的进展）（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 5]]）。
> - **现代 — 知识社会学分析**：Stichweh (2001) 追溯学科建制与社会系统的演化，将二者界定为知识在“静态沉积”与“动态自我更新”之间的基本张力（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, p. 5]]）。
> - **21世纪 — 课程政策批判应用**：该二元概念被引入针对 OECD/PISA 测量风暴的政策批判，用以解释为何现象本位学习（PhBL）在吸纳进课程政策时会发生技能原子化（[[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff, 2024, pp. 9–10]]）。

---

## 相关研究

> [!evidence-grid-a] 相关研究索引
> - [[Argument_Schaffar_2024_CogentEdu|Schaffar & Wolff (2024)]] — 运用这一范式剖析了芬兰现象本位学习在绩效评估压力下被技能原子化收纳的本质。

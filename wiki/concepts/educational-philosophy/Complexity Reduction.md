...
> [!abstract] 与 Latour 计量学的连接
> 复杂性化简为 Latour 的**计量学（Metrology）**提供了一个具体的教育机制：教育中的"有效干预"之所以能"运作"，不是因为科学知识被"应用"到课堂——而是因为**课堂先被化简为更接近实验室的条件**（时间表、课程、评估制度等）。
>
> > 例：一项 [[Randomised Controlled Trials|RCT]] 证明某阅读干预在每日固定 45 分钟、学生按[[Tracking|能力分组]]的条件下有效。学校若要"应用"该证据，必须先通过时间表（锁定 45 分钟）、分组（拆分班级）和统一教学法（培训所有教师使用相同步骤）进行三重化简——这是 Latour 的"着陆跑道"在教室中的具体建造过程。（编者类比）
...
> - **vs [[Evidence-Based Education]] "应用"模型** — 证据本位教育假设进步来自"应用知识"；复杂性化简揭示的是"改造世界以[[Matching|匹配]]知识"
...
> - 权力分配差异（谁决策化简的参数？教师？政策制定者？企业？）——英格兰的中央化学化简 vs 芬兰的教师[[Professional Judgment|专业判断]]化简提供对比案例

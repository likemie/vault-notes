---
title: Evidence-Informed Decision-Making
aliases:
  - 证据知情决策
  - 证据知情决策制定
  - EIDM
...
> [!def] 核心定义
> 证据知情决策（Evidence-Informed Decision-Making, EIDM）指决策者在规划、实施或调整公共政策、教育项目与专业服务前，系统咨询包括最佳可用研究证据在内的多元信息来源的专业审议过程（Langer et al., 2016; [[Argument_Torres_2022_KMModels|Torres, 2022, pp. 46–47]]）。与传统将证据利用视为单向信息传递或单一技术操作的视角不同，EIDM 框架的核心理论突破在于将证据利用确立为旨在**引发决策者行为改变（changing behaviour）**的复杂社会心理与制度干预过程。
...
> 证据知情决策被定义为在做出规划、实施或调整政策、项目及其他服务的决定之前，咨询多元信息来源（尤其是最佳可用研究证据）的过程。这些干预的终极目标在于通过证据使用来改变行为。[[Argument_Torres_2022_KMModels|Torres, 2022, pp. 46–47; Langer et al., 2016, p. 28]]
>
...
> [!feature] 证据知情决策（EIDM）的核心构件体系（Langer et al., 2016）
> - **四层干预层级（Levels of Intervention）** 涵盖个体决策者（Individual）、紧邻组织（Immediate Organisation）、广义组织系统（Wider Organisation）与国际治理网络（International）四大交互层级。
> - **六大核心因果机制（Six Causal Mechanisms）**
...
> ```mermaid
> flowchart LR
>     subgraph 干预层级["多层级干预环境 (Levels)"]
>         L1["个体 / 组织 / 国际"]
>     end
> 
>     subgraph 机制模块["六大因果机制 (Mechanisms)"]
>         M1["意识建立 Awareness"]
>         M2["认同协商 Agreement"]
>         M3["能力建设 Skills"]
>         M4["沟通可及 Communication"]
>         M5["互动机会 Interaction"]
>         M6["结构规程 Structure"]
>     end
> 
>     subgraph 心理状态["中间心理结果 (COM-B)"]
>         C["能力 Capability"]
>         M["动机 Motivation"]
>         O["机会 Opportunity"]
>     end
> 
>     subgraph 最终结果["终极行为改变"]
>         B["决策者实质性采纳证据<br>(Evidence-Informed Behaviour)"]
>     end
> 
>     L1 --> 机制模块
>     M1 & M2 & M3 & M4 & M5 & M6 --> 心理状态
>     C & M & O --> B
> ```
...
> | 命题类型 | 核心指向 | 适用情境 | 代表学者 |
> |---|---|---|---|
> | **行为本体命题** | 将证据利用界定为由多层机制驱动的行为改变过程，确立终极行动锚点 | 公共政策循证改革、智库影响力评估、政策决策行为诊断 | Langer et al. (2016) |
> | **机制协同命题** | 阐明单一信息推力的失效，论证六大因果机制对能力-动机-机会的立体支撑 | 循证政策能力培训、政府决策规程重塑、跨部门研讨机制设计 | [[Argument_Torres_2022_KMModels\|Torres (2022)]] |
...
> > - **政治经济学学者** 指出宏观教育决策本质上是利益集团博弈与价值仲裁，许多时候官员不采用证据并非缺乏能力或动机，而是受制于执政联盟的政治合法性权衡。

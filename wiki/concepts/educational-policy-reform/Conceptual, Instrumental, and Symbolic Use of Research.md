...
>     A --> C["工具性使用"]
>     A --> D["象征性使用"]
>     E["深层使用（Farley-Ripple 等）"] --> F["如何用 + 用什么"]
>     F --> B
>     F --> C
>     F --> D
> ```
...
> - **1977–1978 — 象征与工具区分** Knorr（1977）区分象征性使用与工具性使用，Pelz（1978）扩展研究使用的视角。
> - **2007 — 三分类系统化** Nutley 等（2007）在公共部门证据使用研究中系统化概念性、工具性与象征性三分类。
> - **2018–2022 — 深层使用扩展** Farley-Ripple 等（2018, 2022）把类型学与"用什么、如何用"的深层使用框架结合。
> - **2026 — 学校情境实证检验** Hagevold 等人（2026）在[[Systematic Review|系统综述]]中记录学校研究使用以象征性与工具性为主，并据以论证从实施转向整合。
...
> [!warning] 适用局限
> 类型学是描述性分类，不能直接用于评价某项[[Research Utilization|研究使用]]"好不好"；象征性使用在社会与政治情境中可能具有正当功能，不应一律视为缺陷。将类型学当作规范等级会低估概念性使用的长期价值，并高估工具性使用的直接效果。

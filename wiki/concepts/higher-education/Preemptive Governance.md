...
先发制人治理的核心特征是治理时间的**前置**（preemption）。传统教育治理在以下时间点运作（Thompson et al., 2022, pp.221–224）：
...
先发制人治理的具体操作依赖于 [[Digital Self|数字自我]]-[[Digital Optimum|数字最优]]差异机制：算法生成一个"完美起点"的理想形象（数字最优），OSA 数据建构个体的统计画像（数字自我），两者之间的差距驱动学生进行弥补性自我优化——上语言班、补修先修课程、调整专业选择（Thompson et al., 2022, p.226）。详见 [[Digital Self#与数字最优的差异]]。

## 理论基础

> [!tip]
> - **[[Governmentality|治理术]]**：先发制人治理是治理术在算法时代的运作形态——"行为的引导"通过数据预测和差异展示来实现，个体"自由地"选择符合治理目标的路径（Thompson et al., 2022, pp.220–221, 226）
> - **[[Societies of Control|控制社会]]**：Deleuze（1992）的调制概念解释了先发制人治理的持续性——在控制社会中"解雇不再存在"，权力通过永不终止的调制运作。[[Online Self-Assessment|OSA]] 在入学前启动调制，[[Learning Analytics|学习分析]]在学习全程中持续追踪，校友网络在毕业后继续维护连接——治理窗口永不完全关闭（Thompson et al., 2022, pp.221–222）
> - **Beer 的算法治理**：Beer（2016）的算法治理概念解释了[[Digital Optimum|数字最优]]如何使"通过数据的质询"（interpellation of data）成为可能——算法不发出命令，但通过不断呈现差距来塑造行为（Thompson et al., 2022, p.226）
...
> - **结构性不平等的遮蔽**：先发制人治理将学业失败的风险归因于个体特征的缺失（"外语经历不足"、"数学基础薄弱"），而非教育资源分配的结构性不平等。入学前的"起点差异"在自我评估框架中被[[Transfer Translation Transformation|转化]]为"个人需要弥补的不足"（Thompson et al., 2022, p.227）

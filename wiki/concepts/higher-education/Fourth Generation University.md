---
title: "Fourth Generation University"
aliases:
  - 第四代大学
  - Fourth Purpose
  - 第四宗旨
  - fourth generation university model
  - 4th generation university
  - fourth mission
summary: "Malkov et al.（2024）提出的大学代际演化框架，第四代大学在教学、研究与经济参与之外强调直接参与复杂社会挑战的解决，哥伦比亚大学将其类似理念称为第四宗旨（Fourth Purpose）"
type: concept
tags:
  - "theme/university-industry-collaboration"
  - "level/higher-education"
  - "theme/innovation"
  - "theme/higher-education-policy"
related_concepts:
  - "[[Third Mission (Universities)]]"
  - "[[Knowledge Exchange]]"
  - "[[Knowledge Production]]"
  - "[[Entrepreneurial University]]"
related_theories: []
related_methods: []
related_persons: []
related_facts: []
related_arguments: []
sources:
  - "[[Ch9_LernerLam_DeNoon_Holgate_Broyles_2025]]"
confidence: low
status: draft
created: 2026-05-30
updated: 2026-05-30
---

# Fourth Generation University

<!-- CONTENT_START: 以上为写作参考，以下为实际条目内容 -->

## 定义

> [!info]
> 第四代大学（Fourth Generation University）是 Malkov et al.（2024）提出的大学代际演化概念，指在教学（第一代）、研究（第二代）和经济参与（第三代）之外，承担起"加速研究转化并参与应对重大挑战所需创新"的社会责任（Lerner-Lam et al., 2025, pp.161-162）。
>
> 哥伦比亚大学将其类似理念称为"第四宗旨"（Fourth Purpose），将其界定为在教学（scholarship, teaching）和学科服务（disciplinary service）三项传统职能之外新增的职责——直接参与复杂社会挑战（如气候变化、人工智能、公共卫生）的解决方案开发（pp.161-162）。

---

## 概念背景

> [!note]
> 第四代大学的提出背景是大学面临的跨学科复杂社会挑战。传统的学科划分无法覆盖气候变化等问题的跨领域性，大学需要建立新的机构形态（如跨学科专业学院、研究中心）来协调不同学科的教研活动。哥伦比亚大学组建气候学院（Columbia Climate School）即为这一逻辑的实践体现（pp.161-162）。

> 这一概念区别于[[Third Mission (Universities)|第三使命]]（[[Third Mission (Universities)]]）之处在于：第三使命侧重**经济发展与技术商业化**（[[Knowledge Exchange|知识交流]]、专利许可、区域经济发展），第四代大学 / 第四宗旨侧重**直接参与复杂社会挑战的解决方案**——其目标不是将知识转化为经济产出，而是将大学的[[Knowledge Production|知识生产]]能力直接对准气候变化、公共卫生等紧迫的全球性问题。

---

## 概念演化位置

> [!abstract]
> 大学的代际演化可概括为：
> - **第一代**：中世纪大学，以教学为核心
> - **第二代**：洪堡大学模式，教学 + 研究
> - **第三代**：[[Entrepreneurial University|创业型大学]]（[[Entrepreneurial University]]），教学 + 研究 + 经济参与（Etzkowitz et al., 2000）
> - **第四代**：第四代大学，教学 + 研究 + 经济参与 + 社会挑战解决方案

> 第四代话语的出现在一定程度上反映了[[Third Mission (Universities)|第三使命]]在制度化数十年后，大学正在寻找超越经济量化指标的社会价值叙事（pp.161-162）。

---

## 来源

- [[Ch9_LernerLam_DeNoon_Holgate_Broyles_2025]]

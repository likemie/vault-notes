---
title: Six Rs of University-Industry Relations
aliases: ["产学关系六R框架", "6R框架", "IBM 6 Rs", "IBM six Rs", "six Rs framework"]
...
> [!info]
> 六 R 框架（Six Rs of University-Industry Relations）是 IBM 全球大学项目（GUP）用于组织和衡量产学投资的分类框架，由 Jim Spohrer（2010）提出。该框架将产学合作活动按功能分为六个领域：Research（研究）、Readiness（技能准备）、Recruiting（人才招聘）、Revenue（收入）、Responsibility（社会责任）、Regions（区域发展）（pp.88–89）。Narayan 与 Spohrer（2025）在此基础上增加了第七个维度 Refresh（更新），用于衡量对市场变化的动态响应（p.97）。
...
> [!abstract]
> 每个 R 领域对应特定的活动类型和量化度量指标（pp.88–89, 97）：

| 领域 | 活动内容 | 推荐度量 |
|------|---------|---------|
| Research | 创造可发表且可保护为 IP 的新知识 | 联合出版物数、学术与新闻发布数、合作协议的启动/进行中/结束数 |
| Readiness | 在业务重点领域发展技能 | 参加 IBM 培训和认证课程的学生与教师数 |
...
> [!example]
> - vs [[Partnership Continuum]]（UIDP 伙伴关系连续体） — UIDP 的连续体侧重于产学合作关系的深度演进（从交易型到战略型），六 R 框架侧重于按功能领域分类管理产学投资组合。两者互补：连续体描述关系的纵向深度，六 R 描述投资的功能广度。
...
- 2000 年代 — 业务重点是全球整合企业（globally integrated enterprise）和服务业务增长，产学合作重点在服务科学和业务流程方面的人才与研究。

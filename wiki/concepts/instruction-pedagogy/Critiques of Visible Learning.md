...
> [!formula] 效应量与显著性的换算（Allerup, 2015, p.46）
> $$t = d\sqrt{n}$$
> - $t$ — 近似 t 统计量（检验两组均值是否相等）
> - $d$ — Cohen's *d* 效应量
> - $n$ — 研究样本量
>
> **示例**：$n=25$ 时，$d=0.40 \Rightarrow t = 0.40 \times 5 = 2.00$，接近双侧临界值 $t_{0.05}(48) \approx 2.060$，故 Hattie 的 0.40 阈值在此样本量下*恰好*接近 $p=0.05$。

> [!critique-method] *d* = 0.40 阈值无法脱离样本量解读
> 由上式可知，同一 *d* 值对应的显著性水平随 $n$ 变化——样本量不同，$d=0.40$ 可能显著，也可能完全不显著。VL 的"期望效应区"因此不是固定的教育判断边界，而是一个与研究规模深度耦合的相对值（Allerup, 2015, pp.45–46）。

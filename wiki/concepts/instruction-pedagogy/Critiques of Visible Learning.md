...
> - **研究质量未被评估** — 任何未排除低质量研究的[[Meta-analysis|元分析]]都是误导性的（"垃圾进垃圾出"，Eysenk）。Hattie 在 charter schools 与 learning styles 案例中选择性排除，暴露了标准不一致（p.94–95）。
>
> - **[[Effect Size|效应量]] d=0.40 截断值武断** — 阈值选择任意：医学中阿司匹林效应量仅 **d=0.07**，却意味着每千人拯救 34 人（p.99）。Hattie 自己对 d=0.17 的户外教育效应感到兴奋，暴露了其阈值应用不一致（p.99–100）。
>
> - **社会背景被系统性排除** — 阶级、贫困、家庭资源等因素解释校间差异的 **70–80%**，却被 VL 方法论性地搁置。政策制定者常混淆"仅比较校内变量"与"比较社会变量与学校变量"的两类研究（p.98–99）。
>
> - **平均化掩盖复杂性** — 家庭作业总体 d=0.29 掩盖小学 **d=0.15** vs 中学 **d=0.64** 的巨大差异；班级规模平均值掩盖了 STAR 随机实验中 **d=0.62–0.66** 的效应（p.97–101）。
>
> - **政策误用的系统风险** — VL 结论可被简化为：班级规模"无效"→ 削减经费；"教师最重要"→ 绩效工资——均与 Hattie 强调的"关爱、信任氛围"自相矛盾（p.103）。引 Dewey：证据不提供行动规则，只为明智的问题解决提供假设（p.104）。

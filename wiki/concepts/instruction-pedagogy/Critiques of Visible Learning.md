...
> - **排名从 \#26 降至 \#98**——超过 50% 的位次移动（p.29-30）
...
> - **需要一级数据**：正确做法是汇总所有一级元分析的主要研究数据进行综合元分析——但这需要原始数据，Hattie 未执行（p.32-33）
...
>
> - **方法层面**：该方法在当前出版条件下"以科学合理的方式是不可行的"（p.1）——但 Hattie 的基本愿景（全面整合实证教育研究）"是有远见的"和"为他赢得了宝贵而持久的优点"（p.34-35）
>
...
> [!critique-logic] 5 项最重元分析：仅 1 项研究的是 Hattie 定义的反馈
> | 元分析 | N | Hattie 报告的 *d* | 实际研究内容 | 是否研究反馈 |
> |--------|---:|---:|-------------|:---:|
> | Lysakowski & Walberg (1980) | 4,842 | 1.17 | 39 种**强化技术** | **否**——Hattie 本人批评"奖励是否应被视为反馈值得怀疑"（Hattie, 2009, p.174） |
> | Lysakowski & Walberg (1982) | 15,689 | 0.97 | **线索、参与和纠正性强化** | **否**——聚焦教学策略而非反馈 |
> | Kluger & DeNisi (1996) | 12,652 | 0.38 | **反馈干预**（[[Feedback]] interventions） | **是**——唯一与 Hattie 定义一致，但 *d* 仅 0.38，且 **38% 效应为负** |
> | Witt et al. (2004) | 24,474 | 1.15 | **教师即时性**（verbal and nonverbal immediacy） | **否**——"文本中未提及反馈"；22/34 项（7,539 人）未发表 |
> | Swanson & Lussier (2001) | 5,104 | 1.12 | **考官辅助／动态评估** | **否**——研究考官在测试中提供的"线索、暗示或提示" |
>
> 5 项中仅 Kluger & DeNisi（1996）与 Hattie 定义一致——其效应量 0.38 不到综合结果（0.73）的一半。Hattie 将强化、线索、即时性、考官辅助统一纳入"反馈"汇总，结论因此无效。

> [!case] Witt et al. (2004)：贡献最大却最不相关（Nielsen & Klitmøller, 2021, pp.10–11）
> Witt et al. 对 Hattie 反馈效应贡献最大（N=24,474，*d*=1.15），但研究的是"教师语言和非语言即时性与学生情感、认知和感知学习结果的关系"（Witt et al., 2004, p.150）——文本中未提及反馈。逐项抽查 3 项基础研究：Chesebro & McCroskey (2001) 研究接收者焦虑，Frymier & Houser (2000) 研究沟通技能，Gorham & Christophel (1990) 研究幽默——均未明确涉及反馈。Eysenck 的"**垃圾进垃圾出**"警告由此得到例证：概念偏差通过元分析层层传递，聚合之后愈发难以识别（Borenstein et al., 2009, p.380）。
...
> *d* 本身不能直接接受常规显著性判断；只有形成 $t = d\sqrt{n}$ 后才接近均值差异的 t 检验。以 $n=25$ 为例，双侧临界值 $t=2.060$ 对应 $d \approx 0.412$，Hattie 的 0.40 阈值在此样本量下接近 $p=0.05$；但样本量一变，这种对应关系随之改变（Allerup, 2015, pp.45–46）。VL 的"期望效应区"因此不是可脱离研究规模直接使用的教育判断边界。

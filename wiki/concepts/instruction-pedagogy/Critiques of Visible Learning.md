...
> [!critique] CLE 计算产生不可能概率
> - **错误内容**：Hattie 在 common language effects（CLE）计算中产生**负概率**和**超过 100% 的概率**
> - **严重程度**："对任何上过至少一门统计课的人来说是一个巨大的失误"（p.239）
> - **发现者**：Topphol（2012）首先注意到此错误

> [!critique] 基线选择的任意性决定符号方向
> - **核心问题**：效应量比较"是任意的，他对此完全不知"（p.241）——符号仅取决于隐含基线的反转方向
> - **SES 示例**：若比较贫穷 vs 富裕学生，$d=-0.59$ 将成为 VL 中最负面的效应
> - **政策逆转**：这一符号反转将使 VL 的政策含义从"SES 无法改变"逆转为"如何缓解社会不平等"（p.241）

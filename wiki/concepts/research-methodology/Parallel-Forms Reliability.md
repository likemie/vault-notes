---
title: Parallel-Forms Reliability
aliases:
  - 复本信度
  - 平行信度
...
> [!def] 复本信度（Parallel-Forms Reliability）
> 复本信度是使用相似测量工具所得测量结果的一致性程度——即使用同一测量目标的不同测量工具（如两套难度、结构和内容相当的平行问卷），对同一组受试者施测，所得结果的一致性程度（齐梅, 2015, Ch.5）。

> [!concept-lens] 概念透镜
> - **含义** 复本信度是[[Reliability|信度]]作为**等值性**（equivalence）的操作化形式——衡量不同但等价工具之间分数的一致性，属于"跨形式之间一致性的等值系数"。
...
> [!warning] 前提条件
> 复本信度的有效性依赖于两套工具在测试目标和结构上的**高度一致性**——如果两套工具实质上测量了不同的构念或覆盖了不同的内容范围，即使相关系数很高也不代表真正的复本信度。开发真正等价的平行版本在实践中有相当难度。

---

## 概念辨析

> [!contrast-table] 复本信度 vs 重测信度 vs 内部一致性
...
> | 施测次数 | 一次或两次 | 至少两次（不同时间） | 一次即可 |
> | 信度类型 | 等值性（跨形式） | 稳定性（跨时间） | 条目间一致性 |
> | 计算方式 | Pearson 积矩相关系数 | Pearson 积矩相关系数 | Cronbach α、Spearman-Brown 等 |
...
> - 如果两套工具施测时间不同，复本信度与[[Test-Retest Reliability|重测信度]]会产生混淆——同时包含了跨形式和跨时间两种变异来源。

...
> [!case]- 实证检验案例：生成式 AI 教育[[Meta-analysis|元分析]]中的发表偏倚诊断
> - **偏倚审计情境** 在针对生成式 AI 赋能[[Higher-Order Thinking Skills|高阶思维]]的 29 项实验与准[[Experimental Research|实验研究]]（共 59 个[[Effect Size|效应量]]）的一阶元分析中，研究展开了多重发表偏倚质控与稳健性诊断。
> - **[[Funnel Plot|漏斗图]]与定量回归双重核验** 首先借助[[Funnel Plot|漏斗图]]目视检验散点空间对称性，确认除 8 个离群点外大多数研究围绕合并均值对称分布；进而采用 Egger 线性回归截距检验进行参数化检验，结果为 $t = 1.871, p = 0.066 > 0.05$。由于未达统计显著水平，排除了严重“文件抽屉效应”对总体合并效应量（$g = 0.609$）的实质性扭曲，证实了实证结论的稳健性。[[Argument_Zhao_2025_JIntell|(Zhao et al., 2025, pp. 9–10)]]

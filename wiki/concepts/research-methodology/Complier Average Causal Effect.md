...
> 4. **对抗者（Defiers）** $D(1)=0$ 且 $D(0)=1$；分配到干预组反而故意不参加、分配到对照组反而设法参加。现代[[Causality|因果推断]]通常引入**单调性[[Hypothesis|假设]]（Monotonicity / No-Defiers Assumption）**排除此类群体。
...
> - **因果缩放机理** 若某课后辅导项目的 ITT [[Effect Size|效应量]]为 $+0.15\text{ SD}$，但实际只有 50% 的学生真正按时出勤（顺从率 $\text{ITT}_D = 0.50$），则干预对真正顺从者的真实成效为：
>   $$\text{CACE} = \frac{+0.15}{0.50} = \mathbf{+0.30\text{ SD}}$$
> - **排除性约束（Exclusion Restriction）** 假定[[Random Assignment|随机分组]] $Z$ 仅通过影响实际参与状态 $D$ 进而影响结果 $Y$，分组本身不产生直接心理影响（如未因知道自己被分入对照组而产生挫败感）。

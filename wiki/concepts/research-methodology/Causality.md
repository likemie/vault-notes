...
> [!example]
> **关联（association）vs 因果（causation）**。流行病学健康科学研究强调适度每日红酒消费（每天 1–2 杯）与心脏病风险降低 20% 之间存在正关联（Szmitko & Verma, 2005），但这不等同于因果。关联是因果的必要条件而非充分条件（[[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022, p.57]]）。
...
> [!feature] 因果性的结构化前提
> - **时间顺序**：原因必须在时间上先于效果。休谟（Hume, 2000）将其列为因果准则之一。[[Quantitative Research|量化研究]]者在[[Purpose Statement|目的陈述]]、[[Research Question|研究问题]]和视觉模型中按从左到右排列变量（[[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022, p.57]]）。
...
休谟本人也指出，这些准则中可察觉的实际上是**相关**而非真正的因果性——因果性是由人类归纳推断的，而非客观事实。这构成了概率因果性的哲学起点：教育研究中的因果推断更多是推测性的和概率性的，而非绝对的和演绎的。
...
> 一个特定效果是多个原因的产物，而其中每一个原因本身都足以单独产生该效果（Morrison, 2009: 51）。例如，两发子弹同时击中一人头部——每发子弹单独都足以致命；学生成绩提升可能同时由家庭作业、家长压力、金钱激励和学校压力导致——任何一项单独都可能产生效果（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, pp. 63–64]]）。因果过度决定意味着单一原因的归因需要更谨慎的论证——效果很少由单一原因产生。

---

### 因果过程与行动叙事

因果性不仅是变量的输入-输出关系，更应当被视为**动态过程**而非静态事件。理解[[Causal Processes|因果过程]]需要高粒度地考察动机、意志、理由、理解、感知、个体性、条件和语境，以及因素之间的动态互动（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, pp. 71–72]]）。

[[Rashomon Effect|罗生门效应]]进一步揭示了因果解释的多重性：同一事件被不同行动者以矛盾且不可兼容的因果叙事加以解释（Roth & Mehta, 2002）。教育研究中的启示是：存在多重因果路径，而非简单的输入-输出模型。[[Action Narratives|行动叙事]]在揭示这些因果过程方面具有核心重要性，质性数据能进入情境中行动者的头脑（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, pp. 72–74]]）。
...
> - **1956** — Reichenbach 提出筛选隔离（screening off）概念，区分相关与因果
> - **1969–1991** — Blalock 倡导将口头理论重构为因果模型，建立视觉因果图的基本符号规则（[[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022, p.58]]）
> - **1986** — Holland 提出"因果推断的基本问题"：个体层面反事实永远不可观察
> - **1993** — Mackie 将反事实推理系统引入因果分析
> - **1998** — Salmon 进一步发展筛选隔离概念，强调因果过程而非因果事件
> - **2009** — Pearl 提出边界条件和限定概念，系统化因果推断中的变量选择问题；Morrison 提出从效果追溯原因的七步推理过程
...
> - [[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04]] — 系统梳理教育研究中因果推断的核心议题：概率因果性、反事实推理、筛选隔离、因果过度决定、行动叙事，以及研究原因之效果与效果之原因的两种策略
> - [[Argument_Creswell_2022_SAGE]] — 从量化研究角度定义因果性的三要件（关联证据、时间顺序、排除替代解释），并介绍 Blalock 因果建模传统
...
> [!tension] 核心争议
> - **决定论 vs 概率论**：教育研究中绝对确定的因果性几乎不可能被发现——概率因果性是更恰切的刻画方式。但接受概率因果性意味着研究结论永远带有不确定性。
> - **量化 vs 质性路径**：量化方法（实验、回归、结构方程）能识别变量关联和规律性，但不能揭示因果如何运作（how）；质性方法能揭示因果过程，但难以建立可推广的因果结论。混合方法被主张为综合路径。
...
> [!case] 冰面摔倒——多重因果的完整展示
> 冰面导致摔倒骨折。但反事实推理表明：冰只是众多促成原因之一，不是充分原因。其他因素（平衡感差、视力不佳、环境黑暗、鞋底滑、骨质脆）合在一起才构成充分条件组合。这一案例同时展示了反事实推理、复合原因、必要条件与充分条件的区分。

> [!case] 阅读时间与阅读能力——因果过程的"望远镜"式展开
> 增加阅读时间导致阅读能力提高看似简单因果，但增加时间如何转化为能力提升涉及至少 18 个维度的因果过程（动机、专注、兴趣、难度、目的、环境、教师支持等）。简单的输入变量可能只是众多原因之一，或是一个伞状术语，或解放了一系列其他原因。确定效果的真实原因极其困难（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, pp. 75–76]]）。

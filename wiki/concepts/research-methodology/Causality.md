...
### 边界条件与限定（Boundary Conditions）

Pearl（2009: 420）提出了**边界条件**（boundary conditions）和**限定**（circumscription）概念：哪些因素被纳入或被排除，会影响对因果性的判断。由于可能存在无限数量的原因和效果（取决于回溯多远和包含多广），研究者很少纳入全部条件，只从条件全集中选择一部分——这可能扭曲因果判断。危险在于循环性：只对感兴趣的效果 Y 寻找可能的原因 X，然后因未考虑替代解释而"发现"X 是 Y 的原因（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, pp. 65–66]]）。应对策略是识别并论证变量的纳入与排除，足够广泛地选择关注领域，考虑因果与效果的替代解释。

### 决定论因果性（Deterministic Causation）

决定论因果性是因果性的一种经典刻画方式——X 必然导致 Y，原因与效果之间存在确定的、必然的联系。休谟（Hume, 2000）提出了决定论因果性的经典框架——四准则：时空邻近性、优先性/先后顺序、恒定联结和必然联系。然而，休谟本人也指出这些准则中可察觉的实际上是相关而非真正的因果性——因果性是由人类归纳推断的，而非客观事实。教育研究中几乎不可能发现绝对确定的因果性，因此[[Probabilistic Causation|概率因果性]]是更恰切的刻画方式。

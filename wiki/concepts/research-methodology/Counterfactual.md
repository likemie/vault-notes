...
> [!case] 实验控制组作为反事实
> 在教育实验中，控制组充当了反事实的代理——控制组的结果被假定为反映了实验组"如果没有接受干预"会发生的情况。这一假定的有效性取决于随机分配是否成功实现了 ceteris paribus（其他条件相同）条件（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|Cohen et al., 2011, p. 56]]）。

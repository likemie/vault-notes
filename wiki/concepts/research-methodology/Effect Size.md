...
> [!example]
> d=0.40 更像体检报告里的参考范围，而不是红绿灯。它能提醒教师“这里值得追问”，但不能替代[[Professional Judgment|专业判断]]。若一个中学阶段干预达到 d=0.30，它可能低于《可见的学习》总平均，却高于某些中学同类干预的常见水平；反之，一个 d=0.45 的项目若只测量短期记忆，也不能自动说明它促进了[[Surface and Deep Learning|深层学习]]。
...
Bergeron & Rivard（2017, p.242）揭示了 Hattie 使用最多的效应量计算技巧之一的深层问题：r→d 转换公式在 r 接近 1 时"迅速爆炸"，且对弱相关也产生相对强的 d 值。r=0.196 即可达 d=0.4（《可见的学习》的期望效应区），但在简单线性回归中这仅对应 r²=0.0385——即仅 3.85% 的变异被解释，96.15% 为未解释的随机噪音。
...
**[[Class Size|班级规模]]的案例**：《可见的学习》中班级规模效应量为正（小班优于大班，d≈0.2）。如果反转比较方向，比较大学校相对小学校，效应量就变为负数。Hattie 的解释（"班级规模没有显著影响"）将完全改变，因为负面效应在《可见的学习》框架中被视为"有害"（Bergeron & Rivard, 2017, p.241）。

...
> - **vs 原始分数差异（Raw Score Difference）** — 原始分数差异保留了原始测量单位，效应量通过除以标准差消除了单位——这使跨研究比较成为可能，但也抹除了效果的实际教育意[[Rightness|义]]。
...
> [!success] 经验发现
> - [[What Works Clearinghouse (WWC)|WWC]] 数据库中全样本平均效应量为 +0.216（755 个效应量，169 项研究）（Wolf et al., 2020, p. 441）
...
> [!warning] 争议结构
> 效应量争议不是一个单点问题，而是一条从“计算口径”到“教育政策使用”的问题链。支持者强调它提供跨研究比较的共同尺度；批评者则指出，公式、测量、对照条件、聚合方式和政策转译都会改变效应量的含[[Rightness|义]]。

### 支持者立场

**Qvortrup（2015）的"相对效果"论证**：[[Lars Qvortrup]]（2015, pp.25–27）从支持者立场认为，教育[[Meta-analysis|元分析]]不需要等待所有研究使用完全相同的学习结果定[[Rightness|义]]才可比较；效应量提供的是干预组与控制组或干预前后之间的相对效果，因此可以跨研究比较"相对学习结果"。他同时承认这种比较的代价是学习结果变得抽象，因此提出[[Dynamic Knowledge and Learning Model]]来区分不同知识形式（Qvortrup, 2015, p.27）。
...
> - **1988** — Cohen 出版 *Statistical Power Analysis* 第 2 版，以身高类比等直观方式解释效应量的实质含[[Rightness|义]]（Terhart, 2011, p.427）。

...
> 共变关系研究是事后回溯研究中的一种基本设计类型，有时也称因果研究（causal research）。它旨在识别当前条件的先行因素（antecedents），通过收集两组数据（其中一组为回溯性数据）来确定自变量（X）与因变量（O）之间的关系（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch15|Cohen et al., 2011, Ch. 15, p. 307]]）。

> [!concept-lens] 概念透镜
> - **含义** 共变关系研究的基本设计可表示为 X → O（Campbell & Stanley, 1963 符号：X = 自变量，O = 因变量）。研究者收集自变量和因变量各一组数据，通过计算关联度量来探索二者之间是否存在事后关系。
...
> [!feature] 共变关系研究的三个关键特征
> - **回溯性数据** 关于自变量 X 的数据具有回溯性特征，容易受到历史证据的弱点、局限和扭曲的影响。
> - **关联度量产出** 研究的核心产出是变量之间的关联度量（measures of association），而非确定性的因果推断。
> - **探索性定位** 价值主要在于探索性或启发性特征，作为因果研究的第一步而非终点。

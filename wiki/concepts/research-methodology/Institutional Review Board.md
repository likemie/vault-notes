---
title: Institutional Review Board (IRB)
aliases: ["IRB", "机构审查委员会", "伦理审查委员会", "研究伦理委员会"]
summary: "设立于高校和研究机构内部的伦理审查委员会，依据美国联邦法规在数据收集前审查涉及人类受试者的研究方案，评估参与者风险并保护弱势群体权益"
type: concept
tags: [method/research-ethics, theme/research-governance]
related_concepts: []
related_theories: []
related_methods: []
related_persons: []
related_facts: []
related_arguments: []
sources:
  - "[[Creswell_2022_SAGE]]"
confidence: medium
status: draft
created: 2026-05-31
updated: 2026-05-31
---

# Institutional Review Board (IRB)

## 定义

> [!info]
> IRB 是设立在大学和研究机构内部的委员会，其法律基础是美国联邦法规中对人权的保护条款。IRB 的基本职能是在研究开始之前审查研究方案，评估参与者面临的风险类别，确保研究在伦理上可接受。任何涉及人类受试者的研究都必须在数据收集前获得 IRB 批准（Creswell & Creswell, 2022, 第4章）。

## 核心要素

> [!abstract]

IRB 审查关注的风险类别包括：

- **身体风险**（Physical risk）：研究程序可能对参与者身体造成伤害
- **心理风险**（Psychological risk）：研究可能引发参与者的心理不适或情绪困扰
- **社会风险**（Social risk）：研究可能影响参与者的社会关系或社会地位
- **经济风险**（Economic risk）：研究可能给参与者带来经济损失
- **法律风险**（Legal risk）：研究可能导致参与者面临法律后果（Sieber, 1998; 引自 Creswell & Creswell, 2022, 第4章）

IRB 特别关注**弱势群体（vulnerable populations）**的保护，包括：未成年人（不满 19 岁）、精神能力受限者、受害者、神经系统损伤者、孕妇或胎儿、囚犯，以及艾滋病患者（Creswell & Creswell, 2022, 第4章）。

知情同意书（informed consent form）是 IRB 审查的核心文件，研究者在提交 IRB 申请时需同时提交。同意书须包含以下标准要素（Sarantakos, 2005）：

1. 研究者身份
2. 资助机构身份
3. 研究目的
4. 参与收益
5. 参与程度和类型
6. 潜在风险声明
7. 对参与者的保密保证
8. 参与者可随时退出的保证
9. 出现疑问时的联系人信息

## 操作流程

研究者在 IRB 框架下的标准操作步骤（Creswell & Creswell, 2022, 第4章）：

1. **提交 IRB 申请**：在数据收集之前，向所在校区的 IRB 委员会提交研究方案，说明研究程序、参与者信息和风险管控措施
2. **获得 IRB 批准**：委员会审查参与者风险程度后决定是否批准
3. **获取参与者知情同意**：在数据收集前，确保每位参与者签署知情同意书
4. **获取现场许可**：同时需要获得研究场所负责人（gatekeepers）的准入许可
5. **持续合规**：研究过程中遵守 IRB 批准的方案，如有变更需重新报批

## 来源

- [[Creswell_2022_SAGE]]

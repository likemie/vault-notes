...
> 信度是效度的必要前提（necessary precondition, sine qua non），而效度可能是信度的充分但非必要条件（Brock-Utne, 1996, p. 614）。这一逻辑关系意味着：一项研究可以可靠（稳定一致）但无效（测量了错误的东西），但一项有效的研究必须以可靠为前提。[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch10|(Cohen et al., 2011, Ch. 10, p. 157)]]
...
齐梅 (2015, Ch.5) 将[[Survey Research|问卷调查研究]]中的信度分为四种类型，与上述三分法互补：
...
> [!warning] 信度系数阈值
> 信度系数多以相关系数表示。一般认为：低于0.65不可接受；0.65~0.70勉强可以接受；0.70~0.80可以接受；高于0.80完全接受。较为宽松的标准则以0.60为下限。如果信度系数低于0.65（严格）或0.60（宽松），就需要重新修订问卷或量表（[[Argument_QiMei_2015_EducationalResearchMethods|齐梅, 2015]], Ch.5）。

#### 信度与效度的关系

信度是效度的[[Necessary and Sufficient Conditions|必要条件]]而非充分条件。信度低则效度不可能高，但信度高时效度却未必高。二者存在一定冲突——提高信度可能降低效度，反之亦然。效度甚至比信度更为重要：测量首先需要有效性，其次才是可信性的问题，没有效度的信度是没有测量意义的（[[Argument_QiMei_2015_EducationalResearchMethods|齐梅, 2015]], Ch.5）。
...
> [!contrast-table] Table 10.2 精选：量化与[[Qualitative Reliability|质性信度]]基础对照
> | 量化信度基础 | 质性信度基础 |
> |---|---|

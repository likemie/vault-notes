...
> [!warning] 信度系数阈值
> 信度系数多以相关系数表示。一般认为：低于0.65不可接受；0.65~0.70勉强可以接受；0.70~0.80可以接受；高于0.80完全接受。较为宽松的标准则以0.60为下限。如果信度系数低于0.65（严格）或0.60（宽松），就需要重新修订问卷或量表（齐梅, 2015, Ch.5）。

#### 信度与效度的关系

信度是效度的必要条件而非充分条件。信度低则效度不可能高，但信度高时效度却未必高。二者存在一定冲突——提高信度可能降低效度，反之亦然。效度甚至比信度更为重要：测量首先需要有效性，其次才是可信性的问题，没有效度的信度是没有测量意义的（齐梅, 2015, Ch.5）。

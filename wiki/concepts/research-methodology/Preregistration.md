...
> [!info] 定义
> 预注册（Preregistration）指在研究开始前公开登记研究设计、结果测量和分析计划的做法。在教育项目评估中，预注册被视为缓解选择性报告、[[Publication Bias|发表偏倚]]和[[Researcher Degrees of Freedom|研究者自由度]]等"可疑研究[[Praxis|实践]]"（questionable research practices）的关键机制。
>
...
> 作为预注册在教育领域的具体制度化形式，IES 的 REES（Registry of Efficacy and Effectiveness Studies）要求评估者预先指定：
> - 干预描述和对照组条件
> - 研究设计和随机化方案
> - 主要和次要结果测量
> - 分析方法和子组分析计划
...
> [!example] 与相关概念的区别
> - **vs [[Publication Bias]]** — 发表偏倚是预注册旨在解决的问题之一；预注册是解决方案
> - **vs [[Researcher Degrees of Freedom]]** — 研究者自由度是预注册旨在约束的行为；预注册通过预先锁定分析计划减少自由度，但不能完全消除
> - **vs 开放数据（Open Data）** — 开放数据是补充措施：预注册限制事前自由，开放数据使事后检验成为可能
...
> - **实施灵活性的需求** 学校环境的干预评估常需要根据实际情况调整——过于僵化的预注册要求可能与[[Praxis|实践]]脱节

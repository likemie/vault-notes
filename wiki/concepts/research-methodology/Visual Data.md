...
> | 类型 | 说明 |
> |---|---|
> | **发现的数据（found data）** | 既有图像，如历史照片、档案影像。 |
> | **研究者创造的数据（researcher-created data）** | 自行拍摄或制作的图像，或提供已拍好的图像作为讨论起点。 |
> | **受访者创造的数据（respondent-created data）** | 参与者应研究者之邀拍摄或制作，可提供相机让参与者决定什么值得留存。 |
> | **再现（representations）** | 对视觉材料的再加工、再呈现。 |

> [!dimension] 视觉数据的三重维度
> - **图像的生产（production）**
>   图像由特定技术（油画、录像、摄影器材、计算机软件）、特定构图特征（视觉形式、色彩、质感、象征等）与特定社会语境共同生产，制作方式本身携带意义。
> - **图像本身（the image）**
>   图像具有自身的形式与效果，这些属性会影响观看者；图像是现实呈现而非现实复制，需经解读。
> - **图像的受众（audiences）**

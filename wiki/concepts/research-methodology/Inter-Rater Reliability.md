...
Harlen (1994: 140–2) 认为教师和学校本位评估中不一致的来源包括：（a）评估目的、任务和内容的解释差异；（b）实际任务设置和情境的差异；（c）在测试过程中给予受试者的帮助量；（d）评分标准的特定性程度；（e）评分标准及评分系统的应用；以及（f）参考了多少关于学生或情境的额外信息。她倡导使用测试前后的**调节策略**——小组调节评分、协议小组、界定评分标准、范例说明和小组调节会议等（p.225）。

---

### 访谈分析中的评分者间信度

Silverman (1993) 建议通过评分者间信度来增强访谈的信度——在编码受访者回应时确保不同编码者之间的一致性。Oppenheim (1992: 96–7) 将"不一致的回应编码"列为访谈偏差的九个来源之一。[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch10|(Cohen et al., 2011, Ch. 10, pp. 208–209)]]

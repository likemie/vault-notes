...
> [!info]
> 构念（construct）是[[Variable|变量]]在理论层面的更抽象、更一般的意义表达([[Argument_Creswell_2022_SAGE|Creswell & Creswell, 2022, p.130]])。它是研究者为了理论建构而创造的概念工具，代表不可直接观察的理论实体，需要通过[[Operationalization|操作化]]（operationalization）转化为可测量的变量，才能在实证研究中进行检验。
...
构念与变量共同构成[[Quantitative Research|量化研究]]从理论到实证的桥接机制。构念存在于理论空间，是研究者认为存在但在现实中无法直接触及的实体；变量存在于测量空间，是研究者通过工具实际获得的数值表达。从构念到变量的转化过程就是操作化过程。
...
> - **不可直接观察性**。构念本身不能被研究者直接看到或测量。研究者的测量工具（[[Questionnaire|问卷]]、测试、观察量表）测量的是构念的可观察指标（indicators），而非构念本身。
...
>
> 典型的理论到构念到变量链条示例：
> - [[Attribution Theory|归因理论]] → 归因风格构念 → 归因风格[[Questionnaire|问卷]]（ASQ）得分（变量）
> - [[Social Learning Theory|社会学习理论]] → [[Self-Efficacy|自我效能]]构念 → 自我效能量表得分（变量）
>
> 在研究方法的日常写作中，越是理论导向的讨论越倾向使用构念一词；越接近数据收集和分析的讨论越倾向使用变量一词。

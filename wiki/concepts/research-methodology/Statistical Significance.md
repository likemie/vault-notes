...
> [!info] 定义
> 统计显著性（Statistical Significance）用于判断观察到的差异是否足以拒绝"两个总体均值相同"之类的[[Null Hypothesis|零假设]]。统计检验的核心不是把差异评为"大"或"小"，而是判断检验统计量是否超过临界值；超过则认为差异具有统计显著性，否则不能排除零假设仍然成立的可能（Allerup, 2015, p.45）。
...
> [!abstract] 核心要素
> - **[[Null Hypothesis|零假设]]**：在两个分布均值比较中，零假设可以写为 `H: µ1=µ2`（Allerup, 2015, p.45）。

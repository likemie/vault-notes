...
> ```mermaid
> flowchart LR
>   A["时间点 T1：施测预测工具<br/>(例如：认知潜能 / 发散思维 / 入学测验)"] --> B["自然发展与干预时期<br/>(个体经历常规教学或社会实践)"]
>   B --> C["时间点 T2：收集外部效标数据<br/>(例如：学业 GPA / 工作绩效 / 创造性成就)"]
>   C --> D["计算跨期相关系数 r 与回归系数 β<br/>(检验预测效度强度与置信区间)"]
>   D --> E["二阶元回归检验调节变量<br/>(识别变量角色不对称性与边界条件)"]
> ```
...
### 命题一　跨时间预测效度高度受制于效标测量的无偏性与效标污染控制
...
> - **1970s–1980s — 效标问题与效标污染反思** Shapiro (1970) 与 Wolf (1994) 系统批判效标缺失与效标污染对预测效度推断的扭曲。
...
> > - **实证审慎派** 强调未经校正的真实观察数据更能反映实际应用环境下的保守预测功效。

> [!warning] 适用局限
> 1. **不可跨情境无条件泛化** 某一文化或学段建立的预测效度不能直接套用于其他群体，需定期进行局部效度复核（Local Validation）；
> 2. **自证预言效应** 高预测分数带来的标签效应可能促使教师投入更多资源，造成预测效度的虚假放大。
...
> |---|---|---|---|---|---|---|
> | [[Argument_Runco_2026_CRJ\|Runco et al. (2026)]] | [[Creativity\|创造力]]作为[[Independent Variable\|预测变量]]对未来成就与表现的预测效度 | 52 项元分析 (164 ES) / 2609 项研究 / 1,248,416 人 | 皮尔逊 $r$ / 多水平 CHE + RVE | **$r = 0.29$** $[0.18, 0.41]$ | 显著异质性 | 高，展现中等偏强的跨领域预测效度 |
...
> [!implication]- 提升预测效度的四大[[Praxis|实践]]原则

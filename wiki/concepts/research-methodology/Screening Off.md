...
> [!claim] 筛选隔离区分相关与因果
> 筛选隔离的核心洞见是：两个变量之间的相关关系不能自动等同于因果关系。气压计读数下降与暴风雨高度相关，但气压计读数下降不导致暴风雨——两者都被第三个因素（气压下降）所引起（pp.59–60）。
...
> [!case] 脚大与手大的虚假相关
> 一个人的手可能因脚大而被预测为大——但手大并不导致脚大。共同原因在于对二者的遗传倾向。这体现了筛选隔离的基本逻辑：表面相关掩盖了一个共同的（遗传）因果来源（p.60）。

> [!case] 吸烟-运动-心脏病
> 吸烟（A）与运动（C）高度相关——吸烟者比不吸烟者运动更多。尽管吸烟导致心脏病（B），但运动实际上是对抗心脏病的更强预防措施。表面上，出现了吸烟"预防"心脏病的悖论——因为运动（C）这一第三变量筛选隔离了吸烟（A）和心脏病（B）之间的真正关系（Morrison, 2009: 45; Hitchcock, 2002: 9）（p.68）。

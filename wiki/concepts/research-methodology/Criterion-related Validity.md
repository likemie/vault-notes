...
> [!def] 核心定义
> 效标关联效度（Criterion-related validity）是一种将某一特定测量工具的结果与另一个**外部标准**（external criterion）关联起来以验证其效度的方式。基本逻辑是：如果一个工具能够与一个已知有效的外部标准高度一致，那么我们对该工具的效度就有更大置信度。[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch10|(Cohen et al., 2011, Ch. 10, p. 179)]]
...
> [!concept-lens] 概念透镜
> - **含义** 效标关联效度回答的是"这个工具的测量结果能否与某个公认有效的外部标准匹配"——它不评判工具的内在质量，而是通过**外部关联**来验证。
> - **用途** 在测试和工具开发中，效标关联效度用于验证新工具是否能替代或补充已有的公认测量。对于高风险的选拔性测试（如大学入学考试），预测效度尤为关键。
> - **边界** 效标关联效度的有效性完全取决于所选的**效标本身是否有效**。如果外部效标本身有偏、不可靠或不相关，那么即使工具与效标高度相关也不能证明效度——这变成了"用一把弯曲的尺子去校验另一把尺子"。

---

## 核心要素

> [!dimension] 效标关联效度的两种形式

---
summary: "通过质性案例研究，探究 IB 知识论课程中认知者通过提问、反思性思考和创造性沟通三种角色行使知识建构自主性的过程，提出自主性依赖批判性思维能力和自我信心的双条件机制"
type: argument
authors:
  - "Raheel Tajuddin Lakhani"
source_language: en
citation: "Lakhani, R. T. (2012). Knower's roles of autonomy for knowledge construction: A case of International Baccalaureate (IB) Curriculum. In Search of Relevance and Sustainability of Educational Change: An International Conference at Aga Khan University Institute for Educational Development, November 1-3, 2012 (pp. 187–194)."
year: 2012
doi: ""
citation_aliases:
  - "Raheel Tajuddin Lakhani, 2012"
  - "Raheel Tajuddin Lakhani (2012)"
isbn: ""
tags:
  - theme/autonomy
  - theme/critical-thinking
  - theme/curriculum
  - region/pakistan
subtype: conference-paper
publication_type: conference-paper
title: "Argument_Lakhani_2012_AKUIED"
argument_key: "Argument_Lakhani_2012_AKUIED"
argument_display_title: "Knower's roles of autonomy for knowledge construction: A case of International Baccalaureate (IB) Curriculum"
argument_kind: "conference-paper"
argument_related_count: 25
argument_related_level: 1
argument_related_stars: "⭐"
argument_related_color: "#e5e7eb"
issuing_organization: Aga Khan University Institute for Educational Development
sources:
  - "[[sources/Lakhani_2012_AKUIED/Lakhani_2012_AKUIED|Lakhani_2012_AKUIED]]"
related_concepts:
  - "[[Critical Thinking]]"
  - "[[IB Diploma Programme]]"
  - "[[Theory of Knowledge]]"
  - "[[Reflexivity]]"
  - "[[Learner Autonomy]]"
  - "[[Epistemology]]"
  - "[[Operationalization]]"
  - "[[Document]]"
  - "[[Metacognition]]"
  - "[[Definition of Terms]]"
  - "[[Homework]]"
  - "[[International Education]]"
  - "[[Heterogeneity]]"
  - "[[Variable]]"
  - "[[Constructivist Paradigm]]"
  - "[[Necessary and Sufficient Conditions]]"
  - "[[Leading Questions]]"
  - "[[Sample Size Determination]]"
related_methods:
  - "[[Coding in Qualitative Research]]"
  - "[[Case Study]]"
  - "[[Triangulation]]"
  - "[[Purposeful Sampling]]"
  - "[[Member Checking]]"
related_facts:
  - "[[National Education Policy 2009 Pakistan]]"
  - "[[International Baccalaureate]]"
status: draft
created: 2026-07-02
updated: 2026-07-02
---
# Argument_Lakhani_2012_AKUIED

---

## 研究问题

> [!question]
> [[National Education Policy 2009 Pakistan|巴基斯坦 2009 年国家教育政策]]将"培养具备分析和原创思维能力的自力更生个体"列为核心目标之一（Ministry of Education, 2009, p. 11）。然而，实际运作中的课程和学校实践并未能实现这一目标。既有的课程实施方式和教学常规无法有效培养学生的独立思考和批判分析能力。政策文本与实践之间的落差引出一个问题：如果国家政策期望课程培养自主思考者，那么已经内置了[[Critical Thinking|批判性思维]]成分的课程，是否以及如何在教学中实现这一目标？

> [!claim] 核心主张
> [[IB Diploma Programme|IB 文凭项目]]的[[Theory of Knowledge|知识论]]（Theory of Knowledge, TOK）课程中，认知者通过扮演**提问者（questioning mind）**、**[[Reflexivity|反思性]]思考者**（reflective thinker）和**创造性沟通者（creative communicator）**三种角色来行使知识建构的自主性。这三种角色构成了一个完整的自主性认知循环。提问启动探究，反思进行深度加工，创造性表达完成知识输出。自主性的实现依赖两个条件的同时满足：批判性思维能力的建设，以及认知者对自身反思能力的自信。

> [!concept-lens] 阅读透镜
> - **对象** 巴基斯坦卡拉奇一所私立学校 The Urban Academy 的 IBDP 项目中，三名第一年学生和一名 TOK 教师的[[Learner Autonomy|自主学习]]实践与感知。
> - **张力** 国家教育政策号召培养自主思考者，但实际的课程运作未能实现；具有[[Epistemology|认识论]]训练成分的 TOK 课程在结构上为自主性预留了空间，但这一空间能否以及如何在实践中被填充，取决于认知者的能力和信心。
> - **贡献** 将[[Learner Autonomy|学习者自主性]]研究从传统的语言学习领域扩展到 [[International Baccalaureate|IB]] 认识论课程情境，识别出认知者行使自主性的三种可观察角色，证明自主性依赖能力与信心的双条件机制，并提出自主的相互依存性（autonomous interdependence）：自主性是在社会互动中实现的有支持的独立。

---

## 理论框架

### 自主性的定义演进

自主性概念经历了从单一维度到多维结构的演进。对这一概念的[[Operationalization|操作化]]是研究的分析起点。

> [!def] Holec (1988) 的经典定义
> 自主性是学习者为自己的学习负责的能力（taking charge of one's own learning, p. 3）。这一定义确立了自主性的核心意涵：学习者从被动接收者转向学习过程的主动管理者。Holec 的定义侧重于能力维度，未区分学习者能否和是否愿意的问题。

> [!def] Dam (1995) 引入准备度概念
> Dam 将自主性从单一能力概念扩展为包含**准备度（readiness）**的双维结构。准备度由两个要素构成：**能力（capacity）**，指独立和在合作中执行学习任务的能力；**意愿（willingness）**，指内在动机或意图强度，即学习者是否真正想要承担学习责任。这一区分意味着自主性不仅是技能训练的结果，还涉及学习者的动机状态和自我信念。

> [!claim] 自主性的操作性特征
> 综合[[Document|文献]]，自主性在操作层面涉及三个核心特征（pp.173–174）：
>
> - **内部反思（Internal Reflection）** 自主性体现在个体的信念和欲望是理性和知情反思的结果（Carter, Kramer, & Steiner, 2007, p. 323）。个体经过有意识的反思后形成判断和选择。
> - **[[Metacognition|元认知]]意识（Metacognitive Awareness）** 自主性要求对自身认知保持意识（consciousness），包括关注和自评自己知道什么、正在做什么、以及认知和情感状态如何（Hacker & Dunlosky, 1998; Wilkins, 1996; Wenden, 1991）。这种意识使学习者能够做出知情的决策。
> - **能力（Competence）** 学习者需要具备[[Critical Thinking|批判性思维]]、分析性写作和决策等具体技能。Little（2007）指出，只有当学习者通过这些学习经验获得相关技能时，自主性的潜力才能实现。

### 自主学习者特征作为观察框架

> [!feature] Chan (2001) 的[[Learner Autonomy|自主学习]]者六项特征（pp. 512–513）
> - **高度动机（Highly Motivated）** 对学习主题本身有内在兴趣，从学习过程中获得满足。
> - **好奇心强（Inquisitive）** 主动追问超出课堂规定内容的问题，自发寻找额外信息和解释。
> - **组织有序（Well-organized）** 能够规划和管理学习任务的时间、资源与进度。
> - **主动发起（Active Initiators）** 不等教师指令，主动识别和利用学习机会。
> - **灵活（Flexible）** 根据任务情境和学习条件调整策略和方法。
> - **利用一切学习机会（Use Every Opportunity to Learn）** 将日常经验、社会互动和课外资源都视为学习的组成部分。

> [!warrant]- 概念框架如何转化为实证观察
> 自主性的[[Definition of Terms|操作性定义]]（能力、意愿、元认知意识）规定了分析的基本维度；Chan 的六项特征提供了在课堂和访谈中识别自主性表现的行为指标。学生在课堂讨论中提出的问题、在论文中采用的论证策略、在[[Homework|作业]]中尝试的表达方式，都可以被这套框架所捕获和分析。这一设计将自主性从一个抽象的教育理想转化为可观察、可[[Coding in Qualitative Research|编码]]的角色行为。

---

## 研究方法

### 研究设计与案例选择

> [!method-panel] 研究设计
> | 模块 | 材料与处理方式 |
> |---|---|
> | **[[Case Study\|质性案例研究]]** | 以 [[IB Diploma Programme\|IBDP]] 的 [[Theory of Knowledge\|TOK]] 课程及其相关的课程空间为"有限定边界"（bounded）的研究情境。质性案例研究（Gerring, 2005）适合深入特定课程情境，理解认知者如何赋予其学习行为以意义，而非测量变量间的因果关系。 |
> | **半结构化访谈** | 对三名学生和一名 TOK 教师分别进行录音半结构化访谈。访谈围绕学习实践、自主性体验、意义建构过程和对 TOK 课程角色的感知展开。学生访谈聚焦于"你如何学习""你如何提出问题""你如何处理[[Homework\|作业]]中的不确定性"等具体实践。 |
> | **课堂观察** | 在 TOK 课堂中观察学生的提问行为、讨论参与模式、与教师的互动方式，以及教师对学生提问和观点的回应方式。观察数据用于交叉检验访谈中学生的自我报告。 |
> | **文件分析** | 审查学习指南（study guide）、教材（textbook）和学生论文，作为[[Triangulation\|三角互证]]的第三数据源。教材分析关注其如何设计反思活动和提问引导；学生论文分析关注论证结构、原创性和表达方式。 |
> | **主题分析** | 对多源数据进行转录、审查和三角互证，持续识别和提炼涌现主题（Merriam, 1998）。分析在数据收集过程中即已开始，使后续访谈和观察能够追踪初步发现。 |

### 案例背景与参与者画像

The Urban Academy 是巴基斯坦卡拉奇的一所私立学校，提供两年制 IBDP 课程（面向 16 至 19 岁学生的高中后阶段）。采用目的抽样，以课堂参与度和性别平衡为标准，选取信息丰富的案例（information-rich cases）。抽样目标是最大化从参与者身上获得的理解深度，而非追求样本的代表性或推广性。

> [!sample-panel]- 研究地点与抽样
> | 样本层面 | 构成 |
> |---|---|
> | **研究地点** | 巴基斯坦卡拉奇私立学校 The Urban Academy；提供两年制 IBDP 课程（16–19 岁）；TOK 为 DP 核心必修课 |
> | **抽样逻辑** | [[Purposeful Sampling\|目的抽样]]，基于课堂参与度（高/中/低不同层次）和性别平衡（两女一男）选择信息丰富案例 |
> | **学生样本** | 3 名 IBDP 第一年学生，教育背景和先前经历高度异质 |
> | **教师样本** | 1 名 TOK 教师，具有[[International Education\|国际教育]]背景和[[Critical Thinking\|批判性思维]]专业训练 |
> | **数据收集时间** | 访谈集中在 2012 年 2–3 月（学生访谈：3 月 8 日、12 日、14 日；教师访谈：2 月 8 日） |

> [!ref-table] 表1：研究参与者信息
> | 认知者 | 年龄 | 先前教育 | 课堂参与特征 |
> |---|---|---|---|
> | **Ariana** | 15 | 从 IB 中学项目（[[IB Diploma Programme\|MYP]]）过渡至 DP；此前在肯尼亚接受 O-level 课程教育 | 课堂讨论的积极参与者：主动发言、回应他人观点、在作业中尝试超出课堂内容的例子 |
> | **Ayaan** | 19 | 完成巴基斯坦 Matriculation 考试后在一家软件公司工作过一段时间，随后加入 IBDP | 以替代方式参与：不常在课堂讨论中发言，但在阅读过程中产生问题，利用课余时间与教师单独讨论 |
> | **Sabaina** | 16 | 完成 O-level（中等教育）后直接加入 IBDP | 参与程度相对低于其他两位学生；对自己的学习能力存在不确定性，需要较多外部确认和指导 |
> | **Simak** | — | 外国文理学院文学与经济学 B.A.；五年教学经验；外国大学批判性与创造性思维方向 M.Ed.；本学年以兼职身份加入 The Urban Academy | TOK 课程教师（pedagogue） |

三位学生的背景差异对理解自主性机制至关重要。Ariana 年龄最小（15 岁），但具有最完整的 [[International Baccalaureate|IB]] 教育轨迹（MYP → DP）和跨国教育经历（肯尼亚 → 巴基斯坦）；Ayaan 年龄最大（19 岁），在中学毕业后先工作再回到学校，这种间隔可能赋予他不同的学习动机和反思深度；Sabaina 从传统英式 O-level 体系转入 IBDP。三人在同一课程情境中呈现出差异化的自主性实践，使能力与信心的双条件机制得以浮现。

> [!warrant]- 参与者[[Heterogeneity|异质性]]的分析价值
> 三位学生不同的参与模式——Ariana 的主动探索、Ayaan 的课后深度追问、Sabaina 的自信不足——是将自主性的条件机制（能力与信心）从背景[[Variable|变量]]中分离出来的关键证据。如果三位学生背景相同且表现一致，就只能描述 TOK 中发生了什么，而无法解释什么条件下自主性能够或不能发展。

---

## 论证结构

> [!logic-map]- 核心论证逻辑链
> ```mermaid
> flowchart LR
>     A["政策目标<br>vs 课程现实"] --> B["自主性概念框架<br>能力 + 意愿 + 元认知"]
>     B --> C["TOK 课程空间<br>认知者中心的组织结构"]
>     C --> D["角色 A<br>提问者"]
>     C --> E["角色 B<br>反思性思考者"]
>     C --> F["角色 C<br>创造性沟通者"]
>     D --> G{"三种角色<br>→ 两个条件"}
>     E --> G
>     F --> G
>     G --> H["条件一<br>批判性思维能力"]
>     G --> I["条件二<br>自我信心"]
>     H --> J["自主的<br>相互依存性"]
>     I --> J
>     G -.-> K["反例：Sabaina<br>有能力但缺信心<br>→ 自主性受阻"]
> ```

---

### 论证步骤一　TOK 作为自主性训练的课程空间

> [!claim] 步骤一主张
> [[Theory of Knowledge|TOK]] 课程以认知者处于中心位置、环绕认知方式和知识领域的结构，为[[Learner Autonomy|学习者自主性]]提供了制度化的课程空间。但课程结构只设置可能性，可能性能否转化为现实取决于认知者的能力和信心。

> [!chain-link] TOK 的结构特征与自主性空间
> TOK 是[[International Baccalaureate|IB]] DP 三项核心要素中唯一直接以[[Epistemology|认识论]]为内容的课程。其核心设计是要求学生反思自身如何知道所知，而非记忆已知什么，这使其根本区别于以内容覆盖为逻辑的传统课程（p.173）。课程围绕**认知方式（ways of knowing）**和**知识领域（areas of knowledge）**两个维度展开，认知者（knower）在结构中占据中心位置（IBO, 2011）。这一结构意味着课程的核心活动是促使每个认知者对自己已经接受的知识和信念进行系统性质疑。
>
> TOK 在结构上是一个已经包含[[Critical Thinking|批判性思维]]成分的课程（p.173）。案例选择的逻辑是追问在这样一个已经为自主性预留了空间的课程中，认知者实际上以什么方式行使了自主性，以及什么条件决定了自主性实践的质量。

> [!example]- 图1：TOK 模型图（Diagram of TOK Model）
> ![](https://img.mylikemie.icu/sources/Lakhani_2012_AKUIED/figures/Lakhani_2012_AKUIED_Fig1_TOK_Model.jpg)

> [!warrant]- 案例选择的推理逻辑
> 以 TOK 作为自主性研究案例，遵循的是最可能案例（most-likely case）的研究设计逻辑。在一个已经在结构上为批判性思维预留空间的课程中，自主性实践若仍高度依赖认知者的个体条件和课程实施方式，那么对于没有这种结构设计的课程（如巴基斯坦国家课程），仅靠政策文本的号召就更不足以实现培养分析和原创思维的目标。这一设计的核心输出是：在最好的条件下，自主性还需要什么额外条件才能发生。

---

### 论证步骤二　三种角色：自主性的实证形态

> [!claim] 步骤二主张
> 认知者在 TOK 学习中扮演了三种角色：**提问者（questioning mind）**、**[[Reflexivity|反思性]]思考者**（reflective thinker）和**创造性沟通者（creative communicator）**。这三种角色是自主性认知循环的三个功能环节：提问启动自主探究，反思对探究所得进行深度加工，创造性表达将加工后的知识输出为可交流的形式。

---

> [!line-a] 角色一：认知者作为提问者（Knower as a Questioning Mind）
**核心表现** 提问是认知者在 TOK 学习过程中最核心的实践之一。学生在课堂讨论中提出问题、在独立阅读中使用探究精神、在课堂中使用电子工具查找信息。这些问题不是教师布置的任务，而是认知者自发生成的——它们引导了意义建构，并转化了认知者对各种概念的原有理解（p.175）。

> [!chain-link] 证据链
> **Ariana 的案例** 在一次[[Homework|作业]]中，Ariana 使用了一个课堂未曾讨论过的例子——关于"认知的谬误"（fallacy of knowing）。当被问及为何以及如何将一个超出课堂学习范围的概念纳入作业时，她这样解释：
>
> > "因为我一直在思考，我开始追问：这类认知中还有什么其他的谬误？好奇心让我提前阅读了书和互联网上的内容，我想把那些学到的东西放进作业里。你的书不能回答你所有的问题，所以你必须去查更多的来源。作业的主要问题很重要，但我自己的问题对我来说同样重要。"（Interview with Ariana, March 14, 2012, p.175）
>
> 这个回答包含三个值得注意的自主性信号：第一，Ariana 的提问不是对课堂内容的反应，而是对自身思考的推进——"我开始追问还有什么其他的谬误"；第二，她主动扩展了信息来源——书、互联网；第三，她在"作业问题"和"我自己的问题"之间建立了平等关系——课程任务和自身求知欲被放在同一层面。
> > **Sabaina 的补充视角** Sabaina 也表达了类似的经验，但侧重点不同——她强调讨论作为提问的触发机制："对我来说，讨论引导提问，然后我自己思考并独立研究"（Interview with Sabaina, March 12, 2012, p.176）。这揭示了提问行为的**社会起源** 即使最终表现为独立研究，提问本身往往始于与他人的对话。

> [!warning] 反面证据：自信缺失时提问能力的失效
> 数据同时显示，提问能力不是无条件的。Sabaina 的情况提供了一个关键的反面证据：她对自己的知识建构能力和建构结果的充分性缺乏自信——"我猜我不够快；我对自己的信念不足"——这导致她即使掌握了内容知识，也无法将其转化为有组织的知识产品（p.176）。具体表现为：
>
> > "我需要监督的原因是我猜我不够快，我对自己的信念不足。你会感到困惑，但当你知道大家都是这样做的时候，你就照着做。很多次，在写论文的时候，我无法完成，因为我不知道该怎么推进。有时候当我达到要求的字数时，我就直接提交，不想再努力了。"（Interview with Sabaina, March 08, 2012, p.176）
>
> 这个案例直接导向了论证步骤三的核心命题：**自由思考若无处理自由的技能和自我信念，就会失去效用（p.176）**。Sabaina 有能力提问和获取信息，但她无法相信自己建构的知识是有效的——这使她的探究停留在表面上，无法进入反思和系统表达的层面。

---

> [!line-a] 角色二：认知者作为反思性思考者（Knower as a Reflective Thinker）
**核心表现** 提问获得的信息需要经过反思才能转化为知识。研究发现认知者通过反思日常生活经验，积极参与课堂讨论和论文写作中的知识建构（p.176）。

> [!chain-link] 证据链
> **Ayaan 的案例** Ayaan 的参与方式与 Ariana 不同。他不常在课堂讨论中发言，但在阅读过程中产生问题后，会利用课余时间与教师单独讨论（p.175）。在一篇论文中，Ayaan 通过对社区服务经历的反思，转变了他对**享乐主义**（hedonism）的理解——将自我愉悦重新放在社区服务的光照下审视。他首先接受了享乐主义作为一种生活哲学，然后在社区服务的实际体验中发现了这种哲学的局限，最后在论文中将两种经验并置并进行理论性反思（p.176）。这一转变展示了一个完整的反思链条：从接受既有观念、到在实践中遭遇挑战、再到通过写作完成理论整合。Ayaan 的课后追问习惯为这种深度反思提供了持续的认知动力——没有那些与教师的单独讨论，对享乐主义的反思可能停留在个人感悟而非理论论证的层面。
>
> **Ariana 的反思桥梁比喻** Ariana 对 TOK 的反思功能做了一个比喻：TOK 给你一座反思的桥梁——比如某个活动诉诸情感还是理性？为什么这很重要？它如何帮助你学习？（Interview with Ariana, March 08, 2012, p.176）。桥梁的比喻精确捕捉了 TOK 反思的结构特征：它连接了具体的学科活动和抽象的认识论问题。一个科学实验不仅是做实验，更是一个关于如何通过感官和理性来认识自然世界的认识论事件。

> **教师视角** Simak 从教学立场确认了反思的核心地位："TOK 帮助学生理解自己与他们正在做的事情的理由之间的关系"（Teacher Interview, February 8, 2012, p.176）。这里的"理由"（rationale）是关键——反思不是让学生描述"我做了什么"，而是追问"我为什么这样做，以及这说明了知识的什么本质"。

**教材的反思支撑结构** 文件分析发现，TOK 教材的结构设计以促进反思为目标：每章以一系列引语和谚语开始，正文中穿插活动框，有时图片本身就是反思的对象（p.176）。但教材只提供了反思的"机会"——是否真正参与这些反思，取决于认知者自身的选择。

> [!warrant]- 教材、教师、学生在反思中的角色分工
> 教材提供了反思的材料和触发点（引语、活动框、图片），教师提供了概念的澄清和方向引导，但反思的执行始终在认知者一端。反思性思考既不由教材自动触发，也不由教师替代执行。Ariana 的桥梁比喻和 Ayaan 的享乐主义反思都展示了认知者如何主动将外部刺激转化为内部思考过程。

---

> [!line-a] 角色三：认知者作为创造性沟通者（Knower as a Creative Communicator）
**核心表现** 知识建构的最终环节是**表达**——将经过提问和反思加工的知识以可交流的形式呈现出来。研究发现认知者在这一环节展现出创造性，但这种创造性的实现受到学校惯例和教师反馈的深刻影响（p.177）。

> [!chain-link] 证据链
> **Ariana 的创意冒险** Ariana 在一篇关于"日常生活中的理性"（reason in daily life）的论文中，采取了完全不同于她以往写作的路径——将自我置于论述的中心，分析自己从早到晚的每一个细微活动中理性如何运作。这是一种从"理性的客观定义"到"理性的主体经验"的转向（p.177）。她对这种转向的动机和风险有清晰的自觉：
>
> > "当我用不同的方法写这篇作业时，我其实不确定它会被怎么接收。我以前一直写的是客观的、有既定概念的东西。这次我想冒险，加入了主观的个人经验。"（Interview with Ariana, March 14, 2012, p.177）
>
> Ariana 的案例展示了创造性表达的三个关键要素：意识到常规做法（"我一直写客观的"）、有意识地偏离常规（"这次我想冒险"）、以及对后果不确定性的接受（"我不确定会被怎么接收"）。
> >**Sabaina 的视觉表达尝试——及其被压缩** Sabaina 的第一篇 TOK 作业充满了图片。她认为视觉有助于传达观点："这是写作者的选择——因为有些人通过图片能更好地表达自己的想法。有时候你知道，不是写一段文字，而是放一张图片，它就说明了一切"（Interview with Sabaina, March 12, 2012, p.177）。她的作业从外部来源选取图片，将自己的想法编织进去，试图建构新的意义。
> >然而，作业并未得到积极评价。在学校的情境中，学生被期望遵循论文写作的传统规范——只能通过文字进行创造性表达，使用图片不被鼓励（p.177）。

> [!tension] 课程设计意图 vs 学校实施惯例
> - **课程设计意图（蓝方）** 文件分析发现，学习指南本身允许视频或海报作为演示的一部分，且没有明确禁止隐喻或视觉表达（p.177）。TOK 的课程框架为多种表达方式预留了空间。
> - **学校实施惯例（红方）** 学生基于教师反馈和评价实践，形成了"只能通过文字创造性表达"的认知。Sabaina 的作业不被接受，不是因为课程规则禁止，而是因为学校层面的评价惯例将"学术写作"等同于"纯文字论文"。

> [!implication]- 惯例对自主性的隐性压缩
> 当教师仅以论文写作规范来评价作业而排斥视觉表达时，造成的不仅是单一学生的挫折，而是一种**结构性的自主性压缩** 认知者学会了自我审查——他们不再尝试不符合"惯例"的表达方式。Sabaina 后续是否会继续使用图像进行知识表达，取决于她是否愿意再次承担"不被接受"的风险。研究数据暗示，大多数学生会选择顺应惯例而非坚持冒险。

---

> [!warrant]- 三种角色的功能关系
> 三种角色是自主性认知循环中三个功能环节的实证表现（p.178）：
>
> **提问**是自主性的**启动机制**。它打破被动接收状态，将认知者从等待教师给出答案的位置推向主动寻找解释的位置。没有提问，自主性缺乏动力来源。
>
> **反思**是自主性的**深度加工**。它将提问获得的碎片化信息与个人经验、已有知识和课程框架进行整合。没有反思，提问可能停留在表面好奇的层面。
>
> **创造性表达**是自主性的**输出形式**。它将经过反思加工的知识以个人化的方式呈现和传播，同时也是对理解的最终检验。没有创造性表达，自主性的产出仍然是隐性的。
>
> 提问、反思、表达的顺序对应了自主性从获取自由到行使自由再到展示自由的发展轨迹。Sabaina 的案例恰好在衔接处出现了断裂：她能提问（获取自由），但缺乏自信进入系统性反思（难以行使自由），因此表达停留在模仿和字数填充的层面（无法展示自由）。

---

### 论证步骤三　能力与信心：自主性的双条件机制

> [!claim] 步骤三主张
> 三种角色的实证分析共同指向一个核心机制：自主性需要通过两个条件的满足来实现。条件一是通过批判性思维知识和技能发展**能力（competence）**，条件二是认知者对自身反思和意义建构能力的**自信（self-confidence）**。能力提供可能性，信心决定可能性是否被实际行使。

> [!chain-link] 条件一：能力建设——批判性思维作为自主性的技能基础
> IB 课程尤其是 TOK 鼓励对信念和知识主张进行质疑与辩护。Little（1991）直接将自主性界定为批判性反思的能力。这意味着自主性和批判性思维在概念层面互嵌：学习者无法在没有批判性思维技能的情况下成为自主学习者（p.177）。认识论知识提供了具体的分析工具，例如学会区分感官认知、理性推理、情感直觉等认知方式。这些工具使认知者能够在面对知识主张时做出有根据的判断。
>
> 从 Benn（1988）的角度看，当学习者的行为、选择和行动的理由来自内在的自我意识时，这些行为就是自主的。但来自内在的前提是认知者具备做出基于理由的选择的能力，这正是批判性思维训练所提供的。

> [!chain-link] 条件二：信心建设——从有能力到敢使用
> 能力本身不足以保证自主性实践。Sabaina 的案例提供了关键的反面证据：她掌握了内容知识，但因为对自己的能力缺乏信任，无法将知识转化为有组织的知识产品（p.176）。她的应对策略是模仿而非建构：当你知道大家都是这样做的时候，你就照着做。
>
> 能力和信心交织在一起，不是先有能力再建信心的线性过程。当认知者具备信心时，他们更愿意承担风险（如 Ariana 尝试主观风格）、更有信心进行独立探究（如 Ayaan 的课后深度追问）、更珍视自己的问题。缺乏信心时，认知者会退缩到最低要求的完成，如 Sabaina 在达到字数后直接提交，而不是审视自己是否真正表达了想说的内容。

> [!implication]- 双条件机制对课程设计的含义
> 课程设计面临两重任务。提供批判性思维的内容和技能（认识论概念、论证分析方法）可以满足能力条件，但如果不创造让学生体验思维成功的环境——在安全的空间中尝试新想法、接受建设性反馈而非仅被评价是否符合规范——信心条件就难以建立。Sabaina 的案例揭示了一个结构性矛盾：TOK 学习指南允许视觉表达，但教师的评价实践却在事实上禁止了它。这种矛盾比明确禁止更有害，因为它让认知者无法确定规则，从而进一步削弱冒险的信心。

---

### 论证步骤四　自主的相互依存性与课程建议

> [!claim] 步骤四主张
> 自主性是在社会互动中实现的。即使在最独立的反思中（如 Ariana 在作业中尝试主观经验路径），经验仍根植于社会互动。认知者高度需要课堂中互动、讨论和对话的空间，这是最重要的实证发现之一（p.178）。

> [!claim] 自主的相互依存性
> Atkins（2006/2009）将自主性的这一社会维度称为自主的相互依存性（autonomous interdependence）。核心判断是：即使认知者独立解读个人经验，那些经验仍来源于社会互动；即使一个人在学习认识论课程以提升批判性思维，她仍需要课堂来建立能力，需要他人的著作来学习概念。**阅读不是自主学习经验的终点（p.178）**。
>
> 认知失衡（disequilibrium）对[[Constructivist Paradigm|建构主义]]学习至关重要，正是认知者在知识建构中的能动性提出了解决方案（Moon, 2008）。但认知失衡本身是由社会互动触发的，包括讨论中的观点碰撞和教师的挑战性问题。教师的首要任务是帮助学生安于知识必然产生悖论这一事实，因为不和谐本身就是建构主义学习的[[Necessary and Sufficient Conditions|必要条件]]。同时，学生需要被告知：重要的不是找到唯一正确答案，而是智识上的严谨性（p.178）。

> [!pathways] 三项课程建议
> - **引入视觉媒介作为意义建构和表征工具** 将图像、视频、海报等视觉媒介纳入书面评估的合法表达方式，而非仅将其视为口头演示的辅助手段。这一建议直接回应了 Sabaina 的案例（p.178）。
> - **通过引导式方法、反思性问题和角色期望的明确沟通来提供支架（scaffolding）** 在独立阅读和探究任务中，教师应以[[Leading Questions|引导性问题]]和明确的角色期望搭建支架。支架的目的是帮助认知者建立从跟随到独立的过渡路径。Sabaina 不知道自己该怎么推进的困惑，正提示了支架的缺失（p.179）。
> - **采用合作学习策略以支持自主性中的关联性** 辩论、戏剧和[[Case Study|案例研究]]等策略可以为学生提供互动空间，让他们在执行独立任务时分享自己的关切和困难。与他人的对话和协作是自主性发展的社会条件，而非其对立面（p.179）。

---

## 主要发现

> [!finding-cards] 核心发现
> 1. **知识建构遵循提问、反思、表达三阶段认知循环** 认知者通过自发性提问启动自主探究，通过对搜索内容和生活经验的反思进行深度加工，通过课堂讨论和论文进行创造性表达。三种角色分别对应循环的三个功能环节（pp.175–177）。
> 2. **自主性依赖能力与信心的双条件机制** [[Critical Thinking|批判性思维]]的知识和技能提供能力基础，但对自身反思和意义建构能力的自信是自主性从可能到实现的[[Necessary and Sufficient Conditions|必要条件]]（pp.177–178）。Sabaina 的案例证明：有能力而无信心时，认知者会退缩到模仿和最低要求的完成。
> 3. **自主性本质上是社会性实践** 阅读不是[[Learner Autonomy|自主学习]]经验的终点。即使在最独立的反思活动中，认知者仍依赖课堂互动、讨论和对话来激发思维、检验观点和建立自信（p.178）。自主性是在社会支持中独立判断，而非独自学习。

---

## 关键引用

> [!citation-card]- 探究者、思考者与创造者的统一
> Briefly, it can be asserted that, the roles of being Inquirers, Thinkers, Risk-takers, Reflective and Creative mentioned in the learner profile and also their enactments highlight the learners' initiative, [[Critical Thinking]], diverse perspectives, creative expression and self-reflection.（pp.178–179）
>
> *简言之，可以断言，学习者培养目标中提到的探究者、思考者、风险承担者、反思者和创造者角色及其实际表现，共同凸显了学习者的主动性、批判性思维、多元视角、创造性表达和自我反思。*

> [!citation-card]- 自由需要技能的支撑
> Furthermore, in such a situation, the freedom to think loses its usefulness when the person is not skilled to deal with that freedom.（p.176）
>
> *此外，在这种情况下，当一个人不具备处理自由的技能时，思考的自由就失去了效用。*

> [!citation-card]- 自主性的核心：从内在自我意识出发
> The reasons for their behaviours, choices and actions of learning come from inside within the consciousness of self.（p.177, citing Benn, 1988）
>
> *他们学习行为、选择和行动的理由，来自内在的自我意识之中。*

> [!citation-card]- 认知失衡与自主性的关系
> Pedagogue first needs to comfort students that knowledge would certainly create paradoxes because dissonance is vital for constructivist learning and it is also the agency of knower in knowledge construction which proposes the solution.（p.178, citing Moon, 2008）
>
> *教师首先需要让学生安心：知识必然会产生悖论，因为不和谐对[[Constructivist Paradigm|建构主义]]学习至关重要，而正是认知者在知识建构中的能动性提出了解决方案。*

---

## 自述局限

> [!warning]
> **单案例设计的推广边界** 研究涉及一所学校的三名学生和一名教师，[[Sample Size Determination|样本量]]极小且高度情境特定。The Urban Academy 作为一所提供 [[IB Diploma Programme|IBDP]] 的私立学校，其资源、教师质量和学生群体与巴基斯坦公立学校系统存在系统性差异，研究结论不能直接推广到更广泛的教育情境（pp.173–174）。
>
> **横断设计的时序局限** 数据收集限于 IBDP 第一年。学生在两年项目结束时，经过更长时间的 [[Theory of Knowledge|TOK]] 训练和更多的论文写作经验后，可能出现不同的自主性实践模式。由于没有进行纵向追踪，无法判断观察到的三种角色是稳定特征还是发展阶段的产物。
>
> **未讨论的[[Reflexivity|反身性]]问题** 研究未报告研究者自身在田野中的位置、与被研究学校的关系以及这些因素对数据收集和解读的潜在影响。在巴基斯坦的多语环境中，访谈语言（英语还是乌尔都语）可能影响学生表达的丰富性和准确性，但论文未对此做出说明。研究也没有报告是否对参与者进行了成员检查（[[Member Checking]]）或同伴审阅。

---

## 来源

- [[sources/Lakhani_2012_AKUIED/Lakhani_2012_AKUIED|Lakhani_2012_AKUIED]]

...
概率（随机）样本的大小可通过两种方式确定：（1）研究者谨慎估计以最少案例数代表总体特征；（2）使用数学公式生成的表格（p.146）。Krejcie & Morgan（1970）的表格基于有限总体比例样本量公式：

> [!formula] 有限总体比例样本量
> $$n = \frac{N \cdot Z^2 \cdot p(1-p)}{(N-1) \cdot e^2 + Z^2 \cdot p(1-p)}$$
>
> 其中 $N$ 为总体大小，$Z$ 为置信水平对应的 $Z$ 值（95% 时 $Z = 1.96$），$p$ 为总体比例估计（未知时取 $p = 0.5$ 使分子最大化），$e$ 为边际误差。当 $N$ 很大时，$(N-1) \cdot e^2$ 主导分母，$n$ 逼近 $\frac{Z^2 \cdot p(1-p)}{e^2}$，代入 $Z = 1.96$、$p = 0.5$、$e = 0.05$ 即得 $n \approx 384$。这就是 384 作为天花板的数学来源。
...
> [!exegesis]- "趋于恒定"意味着什么
> 这是 Krejcie & Morgan 最反直觉的发现：样本量并不会随总体规模线性增长。总体从 100 增到 1,000，所需样本从 80 增到 278；但从 10,000 增到 100,000，所需样本几乎不变。数学上，当总体足够大时，样本量公式的分母趋近于一个常数，使得所需样本量稳定在约 384 例（95% 置信水平、5% 边际误差下）。这意味着一个 10 万人的城市和一个 1 亿人的国家，做随机调查所需的样本量差不多——政治民调通常取 1,000–1,500 而非 10,000，正是基于这个原理。
...
>
> 样本量既是数学精度问题，也是判断问题，即使公式驱动的方法也清楚表明，样本量确定中包含预测、[[Sampling Error|标准误]]和人类判断的元素（p.149）。
...
这里呈现清晰的反比关系，样本量越大，能被判定为显著的估计相关系数可以越小（p.149）。

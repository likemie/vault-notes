...
>
> Hopkins 等（1996: 159–62）通过[[Computer Simulation|计算机模拟]] 10,000 次抽样验证了这一规律。Rose & Sullivan（1993: 144）提醒，95% 的所有样本均值落在总体均值 $\pm 1.96$ 个[[Standard Error|标准误]]的范围内，即我们有 95% 的把握使单次抽样均值落在这些界限内（p.150）。
...
> 少于 30 的样本危险地小，允许相当程度的标准误存在。但超过约 80 例后，进一步增加样本量对标准误的影响很小（p.152）。Gorard（2003: 62）强调：功效（power）是对你所使用的检验将[[Effect Size|效应量]]与随机变异区分开来之能力的估计，大样本帮助研究者实现统计功效。

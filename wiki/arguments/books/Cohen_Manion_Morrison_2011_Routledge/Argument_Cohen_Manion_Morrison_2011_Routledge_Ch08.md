...
>
> 即便样本量看起来足够大（80 例），实际数据分布仍可能导致常用的卡方检验无法可靠使用。Gorard（2003, p. 63）建议从每个单元格最少案例数出发（如 6 例），乘以单元格总数再翻倍，即 $6 \times 10 \times 2 = 120$ 例。为更稳妥（每格 10 例），可取 $10 \times 10 \times 2 = 200$ 例。但即便 200 例也不保证满足所有统计要求（pp.144–145）。
>
...
> [!case] 样本量按研究类型（Borg & Gall, 1979, pp. 194–5）
>
...
Bartlett et al.（2001, pp. 48–9）还指出：
- **因子分析** 样本量 ≥ 100 例为一般规则。
...
> [!warning] 关键建议
> 应当高估（oversample）而非低估所需样本量，建立冗余（Gorard, 2003, p. 60）。若无准入和回应的保证（例如研究者无法亲自在场监督问卷填写），可能需要将所需样本量翻倍以应对干净完整回应的损失（p.148）。
...
>
> 她需要从 1,000 名学生中抽取多少人才算"准确"？取决于她愿意接受的精度和概率水平（Blalock, 1979, pp. 215–18）：
>
...
Borg & Gall（1979, p. 201）还提出了一种逆向使用相关系数显著性表的方法。原理基于 Pearson 相关系数的 $t$ 检验：
...
> - **经验阈值** Hopkins 等（1996, p. 159）指出，除非存在极不寻常的分布，≥ 25 例的样本通常即能产生正态的均值抽样分布（p.151）。
...
> 每个总体成员被选中的概率相等，且每次选择完全独立于其他选择。操作方法：从总体名单（抽样框）中随机抽取所需数量的受试者，可通过抽签或使用随机数表（如 Hopkins et al., 1996, pp. 148–9）。由于概率与随机性，样本应包含与总体特征相似的受试者，有些年长、有些年轻、有些高、有些矮等。
>
...
> > - **守门人保护偏差** 守门人可能通过不转介朋友来保护他们（Heckathorn, 1997, p. 175），偏差既可以纳入也可以排除某些群体成员，可以创造其他隐藏人群（Browne, 2005, p. 53）。
> > - **多波次缓解** Noy（2008, p. 333）指出，随着序数波次的推进，后期样本成员可能与早期样本成员具有不同特征，多波次减少了初始接触偏差的影响（Heckathorn, 1997, p. 197）。
...
Onwuegbuzie & Leech（2007, p. 242）建议质性研究的样本量应：
...
Teddlie & Tashakkori（2009, pp. 180–91）指出，混合方法研究通常在同一研究中使用多种样本类型（概率和非概率）以及不同规模、范围和类型的样本，不仅包括人，还包括书面材料、口头/观察材料、社会情境元素（场所、时间、事件等）。
...
Teddlie & Tashakkori（2009, pp. 186–7）还推荐了两种混合变体：

...
> | 数据类型 | 多种类型混合（观察、访谈、文件、实物，Robson, 2002, p. 178） | 以[[Document\|文献]]和档案为主 |
> | 描述方式 | 描绘在特定情境中是什么感觉，追求[[Rich and Thick Description\|厚描述]]（Geertz, 1973） | 基于史料重建过去的情境，受限于材料的完整度 |
> | 研究者立场 | 让事件和情境自己说话，减少研究者解释干预，类似电视纪录片 | 研究者必须主动阐释和填补空白，类似历史叙事 |

个案研究是描述性的、详细的，焦点狭窄，结合主观和客观数据（Dyer, 1995, pp. 48–49）（p. 283）。
...
Simons（1996）认为个案研究需要同时拥抱六组看似矛盾的立场，每组都是一对需要平衡的张力（p. 283）：
...
> - **从传统分析中解放心智** 不预设分析框架和理论透镜，让材料本身引导理解的方向，避免过早将复杂现实塞进既有概念格子
...
五位学者从不同维度对个案研究进行了分类，以下按分类标准整理（pp. 283–285）：
...
> | | | 行动研究个案研究 | 以[[Action Research|行动研究]]为方法论 |
> | | | 评价性个案研究 | 以评价为目的 |
> | | | 教育个案研究 | 教育领域的独立类别 |
...
> | | | 社会群体研究 | 以群体为单位 |
> | | | 组织和机构研究 | 以组织为单位 |
> | | | 事件/角色/关系研究 | 以事件或关系为单位 |

Adelman et al.（1980）提醒：不要仅将个案研究用作其他研究的前奏（如作为[[Pilot Testing|预实验]]或预调查），个案研究作为一种重要且合法的方法有其独立存在的价值。Stake（1994）补充：由于个案研究提供精细颗粒度的细节，它们也可用于补充大规模研究，为宏观政治决策提供有力的人类尺度数据——例如Ball（1990）、Bowe et al.（1992）和Ball（1994a）关于政府政策对具体学校影响的研究（pp. 283–285）。

> [!contrast-table] Robson（2002, pp. 181–182）的两种特殊个案
> | 类型 | 定义 | 示例 |
> |---|---|---|
> | 关键个案（critical case） | 理论预测明确，找到符合的案例即可强力推动知识进展 | 选择极度破坏性的班级研究学生破坏行为，而非破坏不明显的班级 |
> | 极端或独特个案（extreme / unique case） | 试验台功能：如果在这里能行，在任何地方都能行；或选理想条件充分了解运作方式后再推广 | 在最佳条件下试验新教学法，为大规模推广做准备 |

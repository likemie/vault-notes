...
> [!contrast-table] 个案研究者与实验者、调查者的区别
> | | 实验者 | 调查者 | 个案研究者 |
> |---|---|---|---|
> | 做什么 | 操控变量以确定因果显著性 | 对代表性样本询问标准化问题 | 观察个体单位（孩子、班级、学校、社区）的特征 |
...
对统计-实验范式的反感催生了大量个案研究经典：帮派（Patrick, 1973）、辍学生（Parker, 1974）、药物使用者（Young, 1971）和学校（King, 1979）等。
...
有些个案研究是单一情境（单个孩子、单个社会群体、单个班级、单个学校），上述六种方法都可能适用。有些个案研究需要事件的展开，有些按快照方法操作（如在特定时间点对多所学校或多个班级进行）。在前者中保持时间顺序可能很重要；在后者中这种时间顺序可能无关。有些个案研究分为两个主要部分（如Willis, 1977）：数据报告，然后是分析和解读（p. 297）。

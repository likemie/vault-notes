...
> [!evidence-grid] Yin（2009, p. 27）的五项设计要素
> - **个案研究的问题** 回答如何和为什么类问题时特别有力。问题越具体，个案研究越可能保持在轨道上且在限定范围内（Yin, 2009, p. 29）
> - **个案研究的命题**（如有）待检验的假设
> - **个案研究的分析单位** 界定什么构成案例。分析单位应是具体的（真实生活现象）而非抽象的（Yin, 2009, p. 32）。Verschuren（2003, p. 126）示例：对单个企业抽500名员工属调查而非个案研究，因为分析单位是个体而非组织
> - **连接数据与命题的逻辑** 数据如何被分析：寻找模式、解释、随事件展开的分析、跨地点和跨案例分析（Yin, 2009, p. 34）
> - **解释发现的标准** 所给解释为何优于竞争性解释

...
调查在教育中通常使用测试结果、自填问卷和[[Rating Scale|态度量表]]。采用调查模式的研究者通常从尽可能具有代表性的样本中收集大规模数据，以便在统计上以一定置[[Reliability|信度]]说明：某些观察到的特征以一定规律性出现；某些因素聚集在一起（见第37章）；某些因素彼此相关（相关和协方差）；或某些因素随时间和地点变化（如使用测试分数结果来确定教育的"增值"维度，可能使用回归分析和残差分析来确定预测分数和观察分数之间的差异），或使用回归分析利用一个[[Variable|变量]]的数据预测另一个变量的结果（p.259）。

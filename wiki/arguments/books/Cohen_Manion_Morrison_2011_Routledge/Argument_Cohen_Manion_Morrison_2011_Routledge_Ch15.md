...
事后回溯研究对新手研究者可能较为陌生，其关键特征及实施方法如下（p. 304）。作为[[Experimental Research|实验研究]]的引言，本章展示如何利用已有数据构建实验形式并探索[[Causality|因果]]与效应关系，阅读时可结合因果关系（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch04|第4章]]）、抽样（[[Argument_Cohen_Manion_Morrison_2011_Routledge_Ch08|第8章]]）和实验研究（第16章）。
...
> | 分析逻辑 | 比较 IV 高低两组在 DV 上的差异 | 比较 DV 高低两组在各 IV 上的差异，即在自变量已发生后，通过比较结果高的学生与结果低的学生来探索原因 |

Ary et al.（2009, p. 335）提出了前瞻性（proactive）和回溯性（retroactive）两种事后回溯研究设计的区分（p. 306）：
...
事后回溯研究中可识别两种基本设计：**共变关系研究（co-relational study，有时称因果研究 causal research）**和**标准组研究（criterion group study，有时称因果-比较研究 causal-comparative research）（p. 307）**。
...
> | 对 O 的观察 | 观察 O 是否出现伴随变异（concomitant variation），即与 X 的变化相应的预期变异 | O 已被观察 → 回溯寻找合理的 X |
...
> - **实验情境**
>   至少拥有操纵控制，至少有一个主动变量。如果是真正实验，还可通过[[Random Assignment|随机化]]施加控制：可以随机分配被试到各组，或至少随机分配处理到各组。
> - **事后回溯情境**
>   对自变量的控制不可能，也许更重要的是，随机化同样不可能。必须接受事物的既有状态并尝试理清它们。
> - **缓解可能**
...
事后回溯研究因其性质可以为任意数量、甚至相互矛盾的假设提供支持，它们如此完全灵活，以至于在很大程度上是根据个人偏好来假定假设的问题。从某些数据出发，寻找与数据一致的解释，然而通常有多种解释可供选择。再次考虑城镇交通事故增加这一假设情境：一次回溯性原因搜索会揭示半打看似合理的原因（p. 309）。

...
> | [英国循证政策中心](https://www.ukri.org/councils/esrc/)（UK Centre for Evidence-based Policy） | 原属英国经济与社会研究理事会（Economic and Social Research Council，ESRC），ESRC 后并入英国国家科研与创新署（UK Research and Innovation，UKRI） | 推动循证政策研究与实践。ESRC 原有专门页面已下线，职能可能已整合至 UKRI 体系 |
...
> | [研究知情实践网站](https://www.gov.uk/government/organisations/department-for-education)（Research Informed Practice Site） | 原属英国儿童、学校与家庭部（Department for Children, Schools and Families，DCSF），DCSF 于 2010 年改组为教育部（Department for Education，DfE） | 提供研究主题与证据资源。原网址已废弃，相关职能并入英国教育部 GOV.UK 体系
...
Anderson & Biddle（1991）和 Fitz-Gibbon 指出：政策制定者在制定政策时回避证据，实践者在日常忙碌中依赖默会知识而非来自随机对照试验（Randomised Controlled Trial，RCT）的知识（pp. 338–339）。
...
McGaw（1997: 371）认为，定量元分析用透明和明确的程序替代了叙事综述中常见的主观判断（Wood, 1995: 389），在研究间差异很大时尤其必要。Jackson（1980）、Cook et al.（1992: 13）和 Wood（1995: 390）指出叙事综述容易犯以下错误（pp. 342–343）：
...
> Glass et al.（1981: 226–229）的回应是：元分析恰好适合纳入未发表的学位论文，这些论文报告的相关系数往往比已发表研究更弱，可以用来对冲已发表研究中那些更引人注目的结论。但反过来看，如果元分析没能纳入足够的未发表研究，偏倚仍然无法避免（pp. 363–364）。

> [!success] Rosenthal, R. (1991) — 文件抽屉问题的量化
...
> 同一研究的多个结果被同时使用，使总体元分析看起来比实际更可靠，因为结果不独立。Glass et al.（1981: 153–216）指出可以用复杂数据分析技术来处理非独立性问题，但代价是操作更复杂、过程更不透明（pp. 358, 375–376）。
...
> [!tension] 样本量增加但复杂性不增加——Tripp (1985) 与元分析支持者的对立
> - **Tripp 的批评：油和水不能混为一谈** 量化研究模式只要求每个案例测量少量共同变量，因此研究数量的累积主要增加样本量，并不增加数据复杂性。把本来不够相似的研究强行合并在一起（Glass et al., 1981: 22; McGaw, 1997: 372），就像把油和水放在一起说它们都是"液体"，结论可能只停留在毫无帮助的概括水平上（pp. 367–368）。
> - **元分析支持者的回应：差异可以编码** 研究之间的差异及其与发现的关系可以通过编码类别和分析来处理，不需要因为研究不完全相同就放弃综合。
...
以上八项批判揭示了元分析在方法论前提、操作程序和统计推断层面的系统性问题。Wolf（1986: 55–56）在归纳六项批评之后，紧接着给出了十条程序层面的改进建议，试图从操作规范上回应这些批评。此外 Thomas & Pring（2004: 54–55）补充了研究设计和报告层面的要求。这些方案不试图解决前提性问题（如研究是否可比），而是聚焦于"如果要做元分析，怎样才能做得更严谨"（pp. 359–362）。
...
Thomas & Pring（2004: 54–55）从研究设计层面补充了元分析应明确说明的内容：[[Research Question|研究问题]]、概念框架、综述协议、搜索和检索策略，以及多项研究发现被整合的具体方式（pp. 362）。
...
> [!timeline] EPPI-Centre 的五阶段流程（pp. 389–392）
> - **阶段一　综述方法** 包括用户的参与、不同类型综述的选择以及方法论事项
> - **阶段二　启动** 设定综述的范围和方法，确保综述的质量
> - **阶段三　收集与描述研究** 定位和筛选纳入综述的研究，描述研究并阐明其范围
> - **阶段四　评估与综合数据** 评估研究质量和相关性，综合发现，得出结论和建议，撰写报告
> - **阶段五　使用综述** 传播研究发现，提供如何使用和应用报告结论的建议

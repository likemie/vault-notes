...
> Tripp 观察到，量化研究模式只要求每个案例测量少量共同变量，因此研究数量的累积主要增加样本量，并不增加数据复杂性（以变量数量衡量）。元分析有这样一种风险：把本来不够相似的研究强行合并在一起（Glass et al., 1981: 22; McGaw, 1997: 372），得出的结论可能停留在毫无帮助的概括水平上。打个比方，这就像把油和水放在一起，说它们都是"液体"；元分析的支持者则会回应，研究之间的差异及其与发现的关系可以在编码和分析中得到处理（pp. 367–368）。

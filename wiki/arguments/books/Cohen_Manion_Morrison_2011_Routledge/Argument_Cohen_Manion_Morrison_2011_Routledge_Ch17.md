...
"证据"在这里通常来自各种形式的[[Randomised Controlled Trials|随机对照试验]]（Tymms, 1999; Coe et al., 2000; Thomas & Pring, 2004, p. 95），强调谨慎抽样、[[Variable|变量]]控制（包括外部变量和纳入变量）和[[Effect Size|效应量]]测量。循证研究也来自质性研究的系统综述。这些证据积累起来，意在为政策和实践提供一个可靠的知识基础（Coe et al., 2000）。不过 Morrison（2001）指出，"什么有效"这一宣称本身就值得怀疑（pp. 335–336）。
...
> Fitz-Gibbon（1997, pp. 35–36）提出了循证方法的三个必要性论证（pp. 338–339）：
>
...
McGaw（1997, p. 371）认为，定量元分析用透明和明确的程序替代了叙事综述中常见的主观判断（Wood, 1995, p. 389），在研究间差异很大时尤其必要。Jackson（1980）、Cook et al.（1992, p. 13）和 Wood（1995, p. 390）指出叙事综述容易犯以下错误（pp. 342–343）：
...
##### Cook et al. (1992, pp. 7–12) 的四阶段模型
...
> [!formula-step] Glass et al. (1981, p. 29, 102) 的效应量公式
...
第17章以较大篇幅（pp. 356–383）展开了对[[Meta-analysis|元分析]]的多维度讨论。Wolf（1986, pp. 14–17）将元分析面临的主要批评系统化为六个方面（亦见 Elliott, 2001; Thomas & Pring, 2004），随后用十条建议给出了程序层面的改进方案。以下先梳理八项核心批判及支持者的当场回应，再单独列出 Wolf 的改进方案和 Thomas & Pring 的补充。详细批判见 [[Critique of Meta-analysis]]。
...
> Eysenck 在元分析早期即提出著名批评：早期元分析混合了苹果和橙子，将不可比的研究混为一谈。Glass et al.（1981, p. 22）和 McGaw（1997, p. 372）也承认，元分析存在试图综合相似性不足以支持合法合并的研究的风险，其结果可能停留在一个无帮助的概括水平（pp. 366–368）。
...
> - **不论质量均可使用** Smith & Glass（1977）、Levačić & Glatter（2000）主张不论方法论质量如何都可以使用研究发现。Evans et al.（2000, p. 221, 226）进一步声称弱研究加起来可以产生强结论，且高效度与低效度研究之间的效应差异"小得令人惊讶"。Glass et al.（1981, pp. 220–226）也支持这一立场（pp. 373–375）。
...
> Wolf（1986, p. 16）的第六项批评警告：元分析表面上的客观性和精确性可能掩盖纳入研究中的程序缺陷，导致"**有害后果**"（mischievous consequences）。看似中立的数字，可能被用来为有缺陷的研究结论背书。
...
> | 错误类型 | 支持者论断 | 批评者证据 |
> |---|---|---|
> | **Type II 错误**（未能发现真实效应） | Cooper & Rosenthal（1980）声称元分析有助于避免 Type II 错误 | Hedges & Olkin（1980）和 Cook et al.（1992, p. 297）表明随着纳入研究数量增加，Type II 错误反而更可能发生 |
...
> RCT 经常使用聚合和平均数据而非个体数据（Clarke & Dawson, 1999, p. 130），可能忽视数据的分布或传播。在医学 RCT 中，为确保因果关系清晰，患有多种疾病的患者可能被故意排除，导致样本非常小。在教育中同样如此，为了建立清晰的因果关系而将样本缩减至子集，可能产生统计上真实但微不足道的结果，无法推广到更广泛的人群或情境（pp. 376–377）。
>
...
> - **Tripp 的批评：油和水不能混为一谈** 量化研究模式只要求每个案例测量少量共同变量，因此研究数量的累积主要增加样本量，并不增加数据复杂性。把本来不够相似的研究强行合并在一起（Glass et al., 1981, p. 22; McGaw, 1997, p. 372），就像把油和水放在一起说它们都是"液体"，结论可能只停留在毫无帮助的概括水平上（pp. 367–368）。
...
以上八项批判揭示了元分析在方法论前提、操作程序和统计推断层面的系统性问题。Wolf（1986, pp. 55–56）在归纳六项批评之后，紧接着给出了十条程序层面的改进建议，试图从操作规范上回应这些批评。此外 Thomas & Pring（2004, pp. 54–55）补充了研究设计和报告层面的要求。这些方案不试图解决前提性问题（如研究是否可比），而是聚焦于"如果要做元分析，怎样才能做得更严谨"（pp. 359–362）。
...
Thomas & Pring（2004, pp. 54–55）从研究设计层面补充了元分析应明确说明的内容：[[Research Question|研究问题]]、概念框架、综述协议、搜索和检索策略，以及多项研究发现被整合的具体方式（pp. 362）。
...
> | **元研究** | 对研究的研究进行综合 | — |
> | **[[Realist Evaluation\|实在论综合]]** | 关注干预"为什么有效、对谁有效、在什么条件下有效" | — |
...
##### Evans & Benefield (2001, pp. 533–537) 六项原则

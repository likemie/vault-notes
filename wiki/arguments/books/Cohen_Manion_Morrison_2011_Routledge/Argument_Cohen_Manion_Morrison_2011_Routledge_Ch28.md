...
> [!def] 质性数据分析
> 质性数据分析包括组织、说明与解释数据，即以参与者对情境的定义来理解数据，注意其中的模式、主题、类别与规律。质性数据分析没有单一或正确的分析与呈现方法，一切应服从目的适配性原则。（28.1）

> [!info] 分析与诠释、数据收集与分析的融合
> 质性数据分析区别于其他方法的特征在于，它把分析与诠释融合起来，也常常把数据收集与数据分析融合在一起（Gibbs, 2007, p. 3），处于迭代往复的过程之中（Teddlie & Tashakkori, 2009, p. 251）。分析的结果本身也构成进一步分析的数据。研究者在田野中或访谈、观察期间写下的笔记、备忘录、想法与反思，同样成为数据。（28.1）
...
> 可以分析线程式（threaded）电子对话或交流，即同一主题被串联起来，或同一方之间的持续交流进入同一邮箱收件箱。图28-1展示了一组刻意准备的 Gmail 邮件序列，说明邮件如何标示线程式对话。单一线程中有八封邮件，Gmail 记录线程中先前每封邮件的开头词（位于当前打开邮件上方），并附先前邮件发送的时间（即年代顺序）。
>
...
> | 概括 | 诠释 |
> | 发现模式 | 生成主题 |
> | 理解个体（[[Nomothetic and Idiographic\|个例式]]） | 理解群体（通则式） |
> | 提出问题 | 证明或演示 |
> | 解释并寻求[[Causality\|因果]] | 探索 |
> | 检验 | 发现共性、差异与相似性 |
> | 考察同一议题在不同情境中的应用与运作 | |
...
> [!info] 尽早开始分析
> 理论层面，质性研究的一大特征是分析往往在数据收集早期就开始，以便生成理论（LeCompte & Preissle, 1993, p. 238）。实践层面，质性研究迅速累积海量数据，早期分析能通过选出显著特征供未来聚焦来减少数据过载问题。Miles & Huberman（1984）建议研究者在收集到第一批数据后就尽早并频繁地开始书写与分析，而非等数据收集结束，这能促成[[Progressive Focussing|渐进聚焦]]（progressive focusing）并选出关键议题供进一步研究。Gibbs（2007, p. 25）提出写作即思考。这类分析本身应注明日期时间，并可纳入[[Field Notes|田野笔记]]日记，记录研究者当时在做什么、身处何地、发生了什么事、在场者是谁、数据是什么、事件的显著特征、反思与观察（Miles & Huberman, 1994, pp. 50–4）。（28.2）
...
> 站稳脚跟是一桩令人不安的事业，它从未比远距离地成功更多，这正是民族志研究作为一种个人经验所包含的。（Geertz, 1973, p. 13）（28.2）
...
>
> Whyte 进一步报告了对研究者的评论。研究者放弃建立科学结论的任何希望，转而谈论通过呈现你的为人来使你的记述可信。民族志今天越来越公开地采取一种内省转向，要成为习惯性的 I 见证者（I-witness），看来必须先成为一个令人信服的 I。民族学写作因而依赖于对读者的说服。他认识到客观与主观的区分并不像曾经以为的那样清晰，我们寻求观察对[[Research Purpose|研究目的]]有意义的行为，选择因此依赖于某种内隐或外显的理论，这一过程在很大程度上是主观的，但选择并非随机。如果说明理论假设与研究采用的方法，他人就能运用同样的假设与方法去验证或挑战结论。（pp. 366–7）
>
...
> 论及撰写，反身性原则蕴含若干含义。研究者记述的建构原则上与其他种类的记述并无不同，正如没有中立的描述语言，也没有中立的报告方式。因此，反身的研究者必须作为作者保持自我意识，所选用的写作方式不应被视为理所当然，不存在把写作视为纯技术事务的问题，质性分析或记述更非正式、更印象式，因此以第一人称书写。（Hammersley & Atkinson, 1983, pp. 207–8）质性数据分析本身成为文本，即被建构起来的诠释，其组织、排序、年代顺序、主题选择与叙事风格都必须经受反身性的检验（pp. 212–17）。（28.2）

> [!warning] 受访者验证的局限
> 质性数据分析的[[Qualitative Validity|效度]]与信度问题在于，除受访者验证外可诉诸的外部检验点有限；研究者纳入哪些数据与事件必然涉及个人选择，但这一选择必须对所研究现象公平。（28.2）
...
> [!warning] 可识别性、保密与隐私
> 质性数据分析常涉及个体案例与独特实例，可能涉及个人与敏感事项，因而提出个体可识别性（identifiability）、保密与隐私问题。数值数据可以聚合而使人无法追溯个体，质性数据分析则未必，即使不点名或使用化名。研究者有伦理义务反思第5章的不伤害与善行原则、忠诚（及对谁忠诚），并确保遵循 primum non nocere，即不伤害参与者。这可能不仅需要受访者验证，还需要受访者对纳入内容的许可（respondent clearance），由此使研究者陷入两难：是否纳入未经许可、参与者不愿纳入或持不同意见的材料。（28.3）
...
> 计算机不消除人的参与，人仍需决定并生成编码与类别、验证并诠释数据。文本的算法式诠释存在严格限度（Kelle, 2004, p. 277）。CAQDAS 不执行分析，只支持研究者通过组织数据、记录编码与节点来分析；它更像文字处理器与整理器而非分析工具（Flick, 2009, p. 359）。García-Horta & Guerra-Ramos（2009, pp. 152–3）认为软件不能替代研究者赋予意义、识别异同、建立关系的能力，迄今没有软件能处理意义的生成与诠释。（28.4）

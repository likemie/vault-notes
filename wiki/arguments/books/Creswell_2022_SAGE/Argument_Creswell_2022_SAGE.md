...
第5章的逻辑线是：**理解[[Introduction (Research Writing)|引言]]的重要性 → 撰写[[Abstract|研究摘要]] → 区分三种路径的引言差异 → 拆解[[Deficiencies Model for an Introduction|缺陷模型]]的四个部分。**

---

**第一部分：引言的重要性与研究摘要。** [[Introduction (Research Writing)|引言]]是期刊论文或学位论文的第一部分文字，需要同时履行创造读者兴趣、确立[[Research Problem|研究问题]]、置于学术文献语境和触及受众四项功能，且须浓缩在几页之内。Wilkinson（1991）将其定义为"为读者提供研究报告背景信息的部分，其目的是为研究建立框架，使读者能够理解该研究如何与其他研究相关联"。

在介绍引言模板之前，Creswell 先讨论了置于引言之前的[[Abstract|摘要]]。APA 出版手册称摘要可能是研究中最重要的单个段落（约 250 词），必须准确、非评价性、连贯、可读和简洁。实证研究方案的摘要包含五个核心组成部分：研究问题 → 数据来源与特征 → 方法核心特征 → 基本发现 → 结论与启示，并按此顺序呈现。
...
**（四）说明研究对受众的重要性。** 这一部分为研究创建清晰的理据，通常涵盖三个层面：研究如何增加学术文献（三到四个理由）、如何改善实践（几个理由）、如何改进政策或决策（多个理由）。引言以一句话总结总体贡献收尾。

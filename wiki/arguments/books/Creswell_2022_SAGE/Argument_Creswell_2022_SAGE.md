...
| 第4章 写作策略与伦理考量 | 提供量化、质性和混合方法研究计划的结构模型，讨论学术写作的过程策略、连贯性技巧和语言精炼方法，按研究前—启动—数据收集—分析—报告五个阶段系统梳理伦理问题与应对策略。 | [[Research Ethics]], [[Institutional Review Board (IRB)]], [[Informed Consent]], [[Hook and Eye Technique]] |
...
第4章的逻辑线是：**确定研究计划的结构 → 掌握写作策略 → 预见并应对伦理问题。** 三个板块从宏观结构到微观写作技巧，再到贯穿始终的伦理框架，构成研究设计在"执行层面"的三根支柱。

---

**第一部分：研究计划的结构设计。** 所有研究计划都需要回答 Maxwell (2013) 提出的九项核心论证问题（pp.74–75），它们是：读者需要理解什么、读者已知什么、计划研究什么、研究场所和参与者是谁、如何收集数据、如何分析数据、如何验证发现、研究涉及哪些伦理问题、初步结果显示了什么。这九个问题构成了好研究的基石，其中验证、伦理考量和初步证据是经常被忽视但至关重要的组成部分。

有了这九个问题的共同基础，不同研究路径的计划结构开始分化。

**质性研究计划：两种格式。** 质性研究没有统一的结构模板，但有两种替代模型可供参考。建构主义/解释主义格式（Example 4.1）按"引言—程序—初步发现—预期影响"组织，引言下设问题陈述、目的、研究问题和哲学假设，程序部分覆盖质性设计、研究者角色、数据收集与分析、叙事结构、验证策略和伦理问题。附录中建议纳入访谈协议、观察表格、时间表和预算——这些对委员会评估方案的可行性极有价值。

参与式/社会正义格式（Example 4.2）在前者基础上增加了社会正义理论框架（阐述其核心要素、过往使用和对本研究的适用性），并强调与合作者的数据收集方式，在结尾讨论预期产生的变革性变化。两种格式的核心差异在于研究是否明确服务于社会变革议程。

**量化研究计划：标准 IMRD 格式。** 量化研究计划遵循期刊论文的经典结构——引言与文献综述、方法、结果、讨论（Example 4.3）。引言部分以问题陈述（含理论、已有文献、文献缺口、受众关联性）为核心，后续衔接目的和研究问题或假设。方法部分涵盖总体与样本、研究设计类型、操作程序（含变量、工具和伦理考量）和数据分析计划。各部分的排列顺序可能因研究而异，尤其是理论和文献的使用位置（Rudestam & Newton, 2014）。

**混合方法研究计划：最全面的格式。** 混合方法格式（Example 4.4）将量化和质性要素整合在一个框架中，其结构最长。独有要素包括：混合方法的定义和使用理由、混合方法设计类型及其效度挑战、设计流程图、整合声明（integration statement）、联合展示模板（joint display template）和预期整合推论（metainferences）。研究问题分三层——量化问题或假设、质性问题和混合方法整合问题。此外，研究者的混合方法研究资源和技能也需要专门说明。

> [!note]- 四种格式对照
> | 特征 | 质性建构主义 | 质性社会正义 | 量化 | 混合方法 |
> |---|---|---|---|---|
> | 哲学基础 | 建构主义 | 变革主义 | 后实证主义 | 实用主义 |
...
**设计计划各部分的实操建议：**
- 尽早确定各部分标题，先写大纲再填充细节，各部分之间互相启发
- 找导师认可的往届学生计划作为范本，分析其主题、顺序和详细程度
- 了解所在院系是否提供计划撰写课程，利用同伴支持网络
- 与导师明确期望的格式和长度——建议不超过 30 页，但不同导师和委员会差异很大

---

**第二部分：写作策略。** 这部分从最宏观的写作过程，逐层收束到写作习惯、段落组织、连贯性，最后抵达句子和词语层面的打磨。

**写作过程：从脑到纸。** 把想法写下来本身就在促进澄清思路。Zinsser (1983) 区分了两类写作者：一是"砖匠"（bricklayer），每段写完美才进入下一段；二是"先全倒出来"型（let-it-all-hang-out），不介意第一稿的混乱。Elbow (1973) 提出折中方案：如果有一小时写作时间，写四稿（每 15 分钟一稿）而非一小时只写一稿。Franklin (1986) 的三阶段模型将写作分为先做大纲 → 写出草稿并移动和排序段落 → 最后编辑和打磨每句话。这三个阶段分别对应不同的认知任务——构思结构、组织内容、精炼语言——不应混为一谈。

**写作习惯：日常纪律。** Boice (1990, pp.77–78) 的策略体系围绕一个核心原则——"每天都写"——展开：

> [!abstract] Boice 的写作习惯策略
> 1. 将写作设为优先事项，每天进行，不受情绪或"准备好与否"影响
> 2. 如果感觉没有时间，先以半小时为单位记录一两周日常活动，通常会找到可用时间
> 3. 在一天中精力最充沛的时段写作
> 4. 避免暴饮暴食式写作（binges）
> 5. 小量、定期写作
> 6. 将写作任务拆分为每次可完成的、可管理的单元
> 7. 每天绘制三张图表：写作时间、完成的页数等效量、计划任务完成百分比
> 8. 设定超出每日目标的中期计划
> 9. 在感到准备好公开之前，先与支持性的、有建设性的朋友分享写作

写作的物理环境和工具也同样重要：在线词典、同义词库、平板电脑、一杯咖啡、几块饼干（Wolcott, 2009），甚至一支喜欢用的钢笔。Annie Dillard (1989, pp.26–27) 刻意选择没有风景的房间："人需要一个没有风景的房间，让想象在黑暗中与记忆相遇。"这种对物理环境的关注本质上是对注意力管理的关注——减少视觉干扰，将认知资源集中在写作本身。

**清晰简洁的写作：术语一致性与叙事思想。** 术语一致性是读者理解的基础。量化研究中为变量使用一致名称，质性研究中为研究现象使用一致名称，避免同义词替换——术语的细微变动会导致读者误解语义的微妙偏移。

Tarshis (1982) 将叙事文本中的思想按功能分为四种类型：

| 思想类型 | 功能 | 常见问题 |
|---|---|---|
| 伞式思想（Umbrella thoughts） | 文本试图传达的总体或核心观念 | 初学者常有太多伞式思想而内容不足以支撑——表现为引言中的短小、断裂段落 |
| 大思想（Big thoughts） | 落在伞式思想范围内的具体观念或意象，用于强化、澄清或阐述 | 需要足够详细以支撑伞式思想 |
| 小思想（Little thoughts） | 用于强化大思想的观念或意象 | 通常较容易处理 |
| 注意力思想（Attention/interest thoughts） | 保持读者在轨道上，组织内容，维持注意力 | 初学者往往最缺乏——表现为缺乏引导读者从一个主题到下一个主题的"路标" |

注意力思想具体体现为组织性段落和过渡性陈述，类似于研究中的"路标"——如目的陈述、研究问题和假设。好的基金申请书会在叙事中不时暂停，提供注意力思想来组织读者对即将呈现内容的预期。

**连贯性（Coherence）：从句子到章节。** [[Coherence in Writing|连贯性]]意味着观念相互联结、句子之间逻辑流动、段落之间自然衔接。在量化研究中，标题、目的陈述、研究问题和文献综述标题中重复同一变量名称，本身就是连贯性的体现。[[Hook and Eye Technique|钩眼技术]]将连贯性检查操作化为一个可以逐句执行的过程：在每句话中圈出关键词（眼），用线（钩）连接相邻句子的关键词，连接顺畅表明连贯良好，连接困难则需添加过渡词（详见 [[Hook and Eye Technique]] 条目）。

> [!example] 连贯性与章节连接
> McPherson (1988) 的 The Battle Cry of Freedom 是章节间连贯性的典范——每章结尾的观念自然地将读者引入下一章。这一观察暗示连贯性不仅在句子和段落层面运作，也应延伸至章节层面。

**语态、时态和"脂肪"：句子与词语层面的打磨。** 这是 Franklin 三阶段模型中的第三阶段——"打磨"。以下三个维度各自指向不同的语言选择：

- **主动语态（Active voice）**：如果主语执行动作，语态即为主动（Ross-Larson, 1982, p.29）。被动语态的标记是助动词变体（was, will be, have been, is being）。当行为者的身份从逻辑上可以省略，或受动者是段落剩余部分的主语时，被动语态是合适的。
- **动词时态（Tense）**：APA (2020) 的指导原则是——文献综述和基于过去事件的程序使用过去时（"Jones reported"）或现在完成时（"Researchers have reported"）；结果描述使用过去时（"stress lowered self-esteem"）；讨论结果和呈现结论使用现在时（"the qualitative findings show"）。研究计划中使用将来时表示研究将在未来进行。混合方法研究中的时态通常反映其主要取向（量化或质性）。
- **"脂肪"（Fat）**：多余的词语，需要修剪。常见问题包括堆积修饰语、过度使用介词和"the-of"结构（如"the study of"）（Ross-Larson, 1982）。一个好的写作标准是：读者在好的文字中眼睛不会停顿、思维不会磕绊。

关于期刊阅读的建议：好的量化研究可阅读 Administrative Science Quarterly、American Educational Research Journal、Journal of Applied Psychology 等；好的质性研究可关注 Journal of Contemporary Ethnography、Qualitative Health Research、Qualitative Inquiry 等；混合方法研究可查阅 Journal of Mixed Methods Research、Field Methods、Quality and Quantity 等。

---

**第三部分：伦理问题。** [[Research Ethics|研究伦理]]问题贯穿研究全过程，需要一个系统框架来预见和应对。

> [!abstract] 表 4.1 研究伦理框架（五阶段 × 三列）
> Creswell & Creswell (2022, 第4章) 将伦理问题按研究阶段组织为以下框架：

**研究开始之前：**
- 查阅所在专业协会的伦理守则——APA、ASA、AAA、AERA、ANA、AMA 等均有官网可查的现行版本
- 通过 [[Institutional Review Board (IRB)|IRB]] 审批——提交研究方案和知情同意书，由委员会评估参与者风险
- 获得现场许可——向守门人（gatekeepers）提交书面申请，说明研究时长、影响和预期成果
- 选择无利益关系的研究场所——研究者对结果有个人利害关系的场所无法保证量化客观性或质性多元视角
- 协商发表署名——在项目早期就作者顺序达成初步理解，避免赠予署名和幽灵署名（Israel & Hay, 2006）

**研究启动阶段：**
- 选择对参与者有益的研究问题——通过需求评估或非正式对话确保研究对参与者有意义（Punch, 2014）
- 向参与者披露研究目的——如果研究者与参与者理解的目的不一致，即构成欺骗
- 不强求签署 [[Informed Consent|知情同意书]]——参与是自愿的
- 尊重原住民规范——联系原住民首领并在所有研究阶段让他们参与（LaFrance & Crazy Bull, 2009）；采用文化对话方法（如 talking circles、storytelling）而非主流文化语言（Chilisa & Tsheko, 2014）
- 控制研究对参与者的负担——使用简短工具、预测试、在方便时间收集数据、提供补偿

**数据收集阶段：**
- 尊重研究场所并减少干扰——将访问时间安排在不会打断参与者活动的时段
- 确保所有参与者受益——通过等候名单设计使对照组也获得处理
- 与合作者建立互惠——将参与者纳入研究设计、数据收集分析和报告写作全过程
- 避免欺骗——在数据收集开始时再次提醒研究目的
- 识别权力失衡——访谈者和观察者与被研究者之间存在权力不对等，需从这一前提出发设计互动
- 避免剥削——通过小额奖励、分享报告、纳入合作者来回报参与者；收集完数据后迅速离场构成剥削
- 避免收集有害信息——事前规划访谈中可能出现的有害信息（如学生谈家庭虐待）并依据所在机构的伦理守则处理

**数据分析阶段：**
- 避免偏袒——质性研究中不要只报告对参与者有利的结果，量化研究中不要忽视证伪个人假设的数据
- 避免选择性报告——质性研究需展现参与者视角的多样性，包括与主题相反的发现；量化研究中数据分析应完整反映统计检验结果
- 保护参与者隐私——量化研究中在编码阶段将姓名与数据分离，质性研究中使用化名

**报告、分享和存储阶段：**
- 提供准确信息——压制、伪造或编造发现构成科学不端（Neuman, 2009）
- 避免抄袭——直接引用需加引号和页码，改写也需注明出处（APA, 2020）
- 避免披露有害信息——参与者可能因被识别而受到伤害
- 使用无偏见语言——年龄用具体数值、残障用具体状况名称、种族用具体来源地区（如 Japanese Americans 而非 Asian Americans）、性别用带修饰词的具体类别、社会经济地位用具体指标（APA, 2020）
- 分享数据——向参与者和利益相关者提供报告副本，考虑网站分发和多语言发表（Stadnick et al., 2021）
- 保存原始数据——期限为 5 年（Sieber, 1998），到期后销毁以防误用
- 不重复或拆分发表——同一研究不可产生多篇内容重复的论文（Israel & Hay, 2006）
- 提交合规证明和利益冲突声明——部分学术机构要求作者声明无利益冲突
- 明确数据所有权——在研究计划中预先讨论并通过个人协议明确数据归属（Berg, 2001; Punch, 2014）

> [!warning] 伦理问题贯穿全程
> 伦理不是一个在研究开始前一次性处理的事项，而是每个研究阶段持续存在、需要反复审视的核心议题。表 4.1 的五阶段框架本质上是一个"伦理检查清单"——从选择研究问题的那一刻起，伦理决策就已经开始，一直到数据销毁后才结束。

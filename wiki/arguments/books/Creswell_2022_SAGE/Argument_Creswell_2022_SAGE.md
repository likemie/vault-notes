...
%% ![178](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/chapter-1-figure-1.svg) %%
...
Table 8.3 提供了常用统计检验选择标准的完整对照矩阵，按研究问题性质（组间比较、组间关联、变量关联）、变量数量和类型（类别／连续）、分数分布和协变量数量，引导研究者选择最合适的检验——[[t-test]]（两组比较）、[[Analysis of Variance (ANOVA)|方差分析]]（多组比较）、ANCOVA（含协变量的多组比较）、chi-squared（类别变量关联）、Pearson 相关（两连续变量关联）或多元回归（多预测变量关联一个结果变量）。所有检验统计量在报告时应以斜体呈现（如 t、F、p、r、R²）。

...
| 第6章 目的陈述 | 系统讲解研究计划中目的陈述（或研究目标）的核心地位。区分研究问题、目的陈述和研究问题三个环节的递进逻辑，逐一剖析质性目的陈述七要素、量化目的陈述八要素和混合方法目的陈述四要素，为每种路径提供填空式写作脚本，并辅以已发表研究的多个范例。 | [[Purpose Statement]], [[Central Phenomenon]], [[Nondirectional Language]], [[Script (Research Writing)]], [[Qualitative Research]] |
...
全书为研究者提供设计研究计划或研究方案的框架、流程和写作方法，覆盖量化、质性和[[Mixed Method and Multi-method Research|混合方法]]三种研究路径。这一比较从三个路径共有的初步要素（哲学假设、[[Literature Review|文献综述]]、理论使用、写作与伦理）出发，再逐一拆解研究设计的各个关键组成部分，包括引言撰写、[[Purpose Statement|目的陈述]]、[[Research Problem|研究问题]]与假设，以及数据收集、分析和解释的方法程序。
...
**第一部分：引言的重要性与研究摘要。** 引言是期刊论文或学位论文的第一部分文字，需要同时履行创造读者兴趣、确立[[Research Problem|研究问题]]、置于学术文献语境和触及受众四项功能，且须浓缩在几页之内。Wilkinson（1991）将其定义为"为读者提供研究报告背景信息的部分，其目的是为研究建立框架，使读者能够理解该研究如何与其他研究相关联"。
...
**（一）陈述研究问题。** 开篇使用[[Narrative Hook|叙事钩子]]激发读者兴趣——这是从英语写作借用的概念，指吸引或"钩住"读者的开篇语句。Creswell 用"木桶下井"的隐喻说明：有经验的作者缓缓将读者放入研究深度，让读者逐步适应；初学者则直接将读者抛入陌生细节。叙事钩子之后，必须清晰识别[[Research Problem|研究问题]]。
...
这一递进关系是整个研究设计的基础逻辑：目的陈述建立在研究问题的需求之上，又进一步细化为具体的研究问题。

> [!warning] 区分三个概念
> 目的陈述不是研究问题，也不是研究问题。Proposal 中三者必须明确区分。
...
| 4 | 非方向性语言 | 避免"成功的""有益的""积极的"等暗示结果方向的词 |
| 5 | 核心现象的初步定义 | 尝试性和演变的，非固定不变 |
| 6 | 质性路径名称 | 如民族志、扎根理论、案例研究、现象学、叙事研究 |
| 7 | 参与者与研究场所 | 说明谁在何处参与研究 |

质性目的陈述的写作脚本（script）将所有要素串联为一段可填空的模板，四个范例分别展示了现象学（Lauterbach, 1993）、案例研究（Frelin, 2015）、叙事研究（Chan, 2010）和扎根理论（Harley et al., 2007）中的实际写作方式。

> [!note] 非方向性法则
> McCracken (1988) 提出质性研究中的"非方向性法则"（law of nondirection, p.21）：访谈者和目的陈述写作者很容易通过使用暗示方向性结果的词语而违反这一法则。应探索"个体的自我表达经验"而非"个体的成功自我表达"。
...
三个范例展示了调查研究（Shawyer et al., 2017; DeGraw, 1984）和实验研究（Esposito et al., 2017）中的实际目的陈述写法。

> [!note] 构念与变量
> 构念（construct）是变量的更抽象、更概括的表达形式。在研究实践中二者常互换使用，但对操作化要求严格的量化研究更强调变量的可测量性。
...
三种设计各自有填空式写作脚本。三个已发表研究范例分别展示了聚敛式（Classen et al., 2007）、解释性序列（Ivankova & Stick, 2007）和探索性序列（Enosh et al., 2015）目的陈述的实际写作。
...
**全章总结。** 目的陈述是研究中最关键的陈述。质性目的陈述需要识别单一核心现象并提出初步定义，使用强烈的行动动词和非方向性语言。量化目的陈述需要指明被检验的理论、变量及其关系或比较，自变量在前、因变量在后。混合方法目的陈述则包含总体意图声明、设计类型、量化与质性两个分支，以及整合后产生的洞见。
...
> [!quote]
> "This passage is called the purpose statement because it conveys the overall intent of a proposed study in a sentence or several sentences. In proposals, researchers need to distinguish clearly among the purpose statement, the research problem, and the research questions."（第6章）
...
> [!quote]
> "McCracken (1988) referred to the need in qualitative interviews to let the respondents describe their experiences. Interviewers (or purpose statement writers) can violate easily the 'law of nondirection' (McCracken, 1988, p. 21) in qualitative research by using words that suggest a directional orientation."（第6章, p.126）

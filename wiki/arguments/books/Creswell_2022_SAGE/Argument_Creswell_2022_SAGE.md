...
- [[Quantitative Research|量化研究]] 通过检验变量间关系或群体间比较来验证客观理论。变量可通过标准化工具测量，产生可用统计程序分析的数值数据。报告结构固定（引言—方法—结果—讨论），研究者以演绎方式检验理论，控制偏差，寻求推广和复制。
...
- [[Constructivist Paradigm|建构主义]] 认为个体通过与世界的互动和社会历史背景建构主观意义。研究目标是理解参与者的多元和复杂视角，问题设计宽泛而开放。不预设理论，而是从数据中归纳生成意义模式。思想来源包括 Berger 和 Luckmann (1967)、Lincoln 和 Guba (1985)、Crotty (1998) 等。它是质性研究的主导哲学。
...
> [!abstract] 世界观与方法论的对应
> 四种世界观各自倾向于某种研究路径：后实证主义 → 量化，建构主义 → 质性，变革主义 → 质性或混合方法，实用主义 → 混合方法。这一对应并非强制锁定，而是反映了各路径的典型哲学倾向。
...
第2章的逻辑线是：**先确定你要研究什么 → 再判断这个主题值不值得做 → 然后理解文献综述在不同路径中扮演什么角色 → 最后才去实际检索、加工和呈现文献。** Creswell 坚持一个核心信念：文献检索不能盲目开始，必须先有明确的方向。

---

**第一步：确定研究主题——为什么从"一句话标题"开始？**

Creswell 观察到一个常见失误：研究者往往跳过草拟标题这一步，直接进入文献检索。在他看来，工作标题是研究中的"路标"（road sign）——一个让研究者可以在项目推进过程中不断重新聚焦的有形锚点。他的建议很具体：完成句子"My study is about . . ."，用最简单直白的语言作答，控制在 10 到 12 个词以内，避免"An Approach to . . .""A Study of . . ."这类浪费词汇的表达（Wilkinson, 1991）。也可以将主题表述为一个简短问题，如"What treatment is best for depression?"。

> [!note] 简单 vs 复杂的张力
> Creswell 指出初学者的一个典型错误是用复杂晦涩的语言表述主题。"Good, sound research projects begin with straightforward, uncomplicated thoughts." 这与他看到的现实形成对比：发表的期刊文章之所以读起来干净清晰，是因为经过了大量修改，而非一开始就如此。

---

**第二步：主题的可行性审查——"可以做"和"应该做"是两回事。**

Creswell 在此设立了一道门槛。"可以做"（can）是实操层面：有没有参与者愿意参与？有没有资源持续收集数据和分析数据？"应该做"（should）则是一组更复杂的判断。

最核心的标准是对文献的贡献。Creswell 特别强调了一个场景：一个研究在各方面都很完备——研究问题清晰、数据收集全面、统计分析精良——但因为没有为已有研究增加任何新东西，得不到导师和委员会的支持。"How does this project contribute to the literature?" 这个问题必须在研究设计之初就回答清楚。

他列举了提供新角度的六种方式：研究不寻常的地点（如美国农村）、不寻常的参与者群体（如难民）、反向预期（如研究婚姻为什么成功而非为什么失败）、新颖的数据收集方式（如收集声音数据）、不寻常的呈现方式（如地理定位图表）、以及时效性强的议题（如移民问题）（Creswell & Bàez, 2020）。

另外两条标准：研究是否具有超越本地机构的广泛兴趣（"我们倾向于选择具有全国性吸引力的主题，而非仅有地区性兴趣的主题"），以及是否与研究者的个人目标一致——考虑到完成、修改和传播研究成果所需的大量时间投入。

---

**第三步：文献综述的目的——不只是"列举已有研究"。**

[[Literature Review|文献综述]]的定义、多重目的，以及 Cooper (2010) 提出的四种文献综述类型（整合式、批判式、桥梁式、核心议题识别式），详见该条目。这里重点关注 Creswell 论证的关键转折：目的确定后，下一步是"怎么组织"——而组织方式因研究路径而异。这是第2章最具设计意义的判断。

---

**第四步：三种路径的组织逻辑——为什么量化、质性、混合方法的文献综述不能写成一样的？**

Creswell 的核心论断是：文献综述的组织方式不是中性的技术选择，而是由研究路径的认识论立场决定的。

在量化研究中，文献以**演绎**方式使用——文献提供框架，引出研究问题或假设。结构围绕变量组织，详见 [[Literature Review]] 条目中"量化文献综述的五部分结构"。Creswell 指出这一结构的关键在于第三部分（将自变量与因变量联系起来的文献）：它应相对简短，包含与计划研究极其接近的研究；理想情况下，因为尚无研究直接处理该确切主题，这一部分可能需要退而求其次，综述更一般层面的相关研究。

> [!warning] 五部分结构的隐藏风险
> Creswell 没有明说但隐含的逻辑是：如果第三部分已经有很多高度相关的研究，那么计划研究的"贡献"就需要重新论证——它是否只是在重复已有工作？这个结构的精妙之处在于，第三部分的空白或稀疏恰恰为计划研究提供了合法化空间。

在质性研究中，情况完全不同。Creswell 将文献的角色从"提供方向"转为"不限制参与者的视野"。他提出三种位置选择：

> [!example]- 图片占位
> 表2.1：质性研究中文献的使用方式  
> ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/Table_2-1_Using_Literature_in_Qualitative_Study.png)

这三种位置对应着不同的认识论立场：把文献放在开头框定问题，更接近后实证主义传统；放在独立章节，适合习惯于传统文献综述的读者（通常是量化取向强的人）；放在结尾与发现进行比较，最符合质性研究的归纳逻辑——文献在模式或类别被识别后才介入，作为辅助工具而非指导框架。选择哪种位置，本质上取决于受众（第2章）。

> [!note] 一个容易被忽略的判断
> Creswell 补充说，在以理论为导向的质性设计中（如民族志或批判民族志），有关文化概念或批判理论的文献会在报告开头作为导向框架引入；而在扎根理论、案例研究和现象学中，文献在开头使用的频率要低得多。这意味着"质性研究"不是铁板一块——设计类型本身决定了文献的摆放策略。

在混合方法研究中，文献使用取决于设计类型和质性与量化成分的相对权重（详见 [[Literature Review]] 条目）。Creswell 的实用主义立场在此最明显：不是"应该怎样"，而是"取决于你用的是哪种设计以及受众偏好"。

---

**第五步：实际检索——七步流程和背后的取舍。**

Creswell 将文献检索分解为七个步骤（详见 [[Literature Review]] 条目），但有几个设计层面的判断值得在此展开。

首先是**数据库的选择逻辑**。Creswell 在逐一介绍 ERIC、Google Scholar、PubMed、ProQuest、EBSCO、Scopus、DOAJ 等数据库时，隐藏了一条选择原则：同时使用免费和付费资源，跨学科搜索而非局限于本领域，优先使用提供全文访问的数据库以减少检索时间。他对 ERIC 的介绍尤其详细——不仅因为本书面向教育研究者，更因为 ERIC 提供了在线词表（Thesaurus of ERIC Descriptors），可以用描述词精确搜索。

> [!tip] 迭代搜索的价值
> Creswell 提出一个被许多方法论教材忽略的技巧：先用词表中的描述词做初步搜索，找到一篇相关文献，再查看这篇文章使用了哪些描述词，然后用这些词进行第二轮搜索。这样做的逻辑是：索引者使用的描述词往往比研究者自己想到的关键词更精确，从而"最大化获取相关文章列表的可能性"。

其次是**文献优先级的设计意图**。Creswell 推荐的搜索顺序——百科全书综述 → 期刊文章 → 书籍 → 会议论文 → 联系作者——不是随意的。这个顺序从概括到具体、从易获取到难获取、从已沉淀到最新。会议论文排在期刊文章和书籍之后，因为它们虽然报告最新进展，但尚未经过完整的同行评审。

第三是**质量评估的门槛功能**。Creswell 列出的标准（国家级评审期刊、有编辑审查委员会、知名出版社、最近 10 年内发表）实际上构成了一套过滤机制，目的是在进入文献综述之前排除低质量来源。对网络文献的警告——"评估这些文章是否代表严谨、深思熟虑和系统的研究"——反映了 21 世纪学术写作面临的新挑战。

---

**第六步：加工文献——从摘要到地图。**

检索到文献之后，如何加工和组织它们？Creswell 提供了两个工具。

**文献摘要**（详见 [[Literature Review]] 条目）的关键在于区分两类文献：实证研究摘要需要提取研究问题、目的、样本、关键结果和方法缺陷；理论性或方法论文章的摘要则需要提取问题、核心主题、主要结论和论证缺陷。Creswell 以 Hirose & Creswell (2022) 的混合方法质量标准文章为例，展示了 120 词的简洁摘要如何同时传达研究问题、目的、主题和贡献。

**[[Literature Map|文献地图]]** 是 Creswell 自己开发的工具。它的逻辑不是简单分类，而是一种论证装置：通过可视化展示"已有研究的版图是什么样的，我的研究将延伸版图的哪一部分"。
...
图 2.1 以 Janovec (2001) 的组织公平感研究为例展示了一个层级式地图：顶层是"组织中的程序公平"，其下分出三个分支——公平感形成、公平效应和组织变革中的公平，每个分支下面又有子分支和代表性引文，底部用"Need to Study"框标识计划研究的位置，并用连线指明它将延伸已有文献的哪些分支。

Creswell 在分析中揭示了几个设计原则：不同分支的展开程度不必均匀——这取决于每个分支下已有文献的多少和研究者的探索深度；地图中应同时包含量化、质性和混合方法研究；地图需要配合一段叙述性说明，解释顶层的广泛主题、搜索过的数据库、文献的划分逻辑以及计划研究的定位。具体绘制程序和[[Recommendations for Practice|实践建议]]详见 [[Literature Map]] 条目。

---

**第七步：呈现规范——格式手册与术语定义。**

文献综述的呈现涉及两套规范。**格式手册**方面，APA 出版手册（第7版）是社会科学的通行标准。Creswell 特意标出第7版的几项关键更新（详见 [[Literature Review]] 条目），其中出版社所在地的省略和包容性语言（使用单数 "they"）是最显著的变化。他的立场明确：格式统一不是装饰，而是让读者"专注于内容而非格式"。

**术语定义**方面，Creswell 综合了多位学者的建议。核心原则是操作化（operational）而非抽象——定义要具体到研究者如何使用该术语，而非照搬词典。Locke et al. (2014) 提供了最严格的规则：不自行发明词汇、在首次出现时即定义、不使用日常语言形式、使用已在文献中确立的约定语言。

> [!note] 三种路径的术语策略差异
> Creswell 指出了一个与文献综述平行的不对称性：量化研究要求在开头全面、精确地定义术语（通常放在独立章节）；质性研究因归纳式设计，通常不设独立章节，术语在过程中涌现和定义；混合方法研究则视起始阶段而定。这意味着"术语定义"不仅是写作规范问题，也是认识论选择——你是在研究之前就"锁定"术语的意义，还是允许意义在研究过程中浮现？详见 [[Literature Review]] 条目。

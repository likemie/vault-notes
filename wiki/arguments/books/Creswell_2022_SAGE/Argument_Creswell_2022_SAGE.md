...
**量化研究问题与假设。** 量化研究同时使用研究问题和假设。研究问题就[[Variable|变量]]间关系提问，可以是描述性的（描述单一变量）或关系型的（考察变量间关联）；假设则是对变量关系的方向性预测，通过统计程序检验。假设的两种基本形式是[[Null Hypothesis|零假设]]（预测无关系）和[[Alternative Hypothesis|备择假设]]（预测存在显著关系），后者最好写为方向性形式。撰写时需遵循五条指南：陈述变量、包含理论、区分描述性与关系型、创建方向性备择假设、使用方向性量化词汇。Moore (2000) 的性别认同研究提供了研究问题与假设并存的范例。（相关条目：[[Research Question]]、[[Hypothesis]]、[[Null Hypothesis]]、[[Alternative Hypothesis]]）。

---

**质性研究问题。** 质性研究只使用研究问题，不使用假设。[[Central Question|核心研究问题]]（一至两个）以 what 或 how 开头，聚焦单一[[Central Phenomenon|核心现象]]，辅以五至七个子问题收窄焦点。撰写需遵循九条指南，包括：关联特定质性路径（[[Ethnography|民族志]]、[[Phenomenology|现象学]]、[[Grounded Theory|扎根理论]]、案例研究的问题形式各有不同）、使用探索性动词、预计问题会演变、不引用文献或理论、指定参与者和研究场所。不同质性路径的研究问题形式存在显著差异（相关条目：[[Central Question]]）。

---

**[[Mixed Methods Question|混合方法研究问题]]。** 混合方法研究需要三类问题：量化问题或假设、质性研究问题以及[[Mixed Methods Question|混合方法研究问题]]。混合方法研究问题是一个关于方法的、相对较新的概念（Tashakkori & Creswell, 2007），询问研究者希望通过整合量化与质性数据获得什么洞见。三类问题的顺序反映设计类型：聚敛式可先可后、解释性序列量化在前、探索性序列质性在前。Moseholm et al. (2017) 提供了量—质—混合方法三部分目标陈述的范例。（相关条目：[[Mixed Methods Question]]）。

...
%% ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/preface-concept-map.svg) %%
...
量化设计中，[[Survey Research|调查研究]]通过问卷或结构化访谈从样本收集数据以推广至总体（Fowler, 2008）。[[Experimental Research|实验研究]]通过随机分配处理条件并比较组间差异来检验因果效应（Keppel, 1991），真实验使用随机分配，准实验使用非随机分配，[[Single-Subject Design|单受试者设计]]在时间维度上对少数个体施加和撤销处理（Cooper et al., 2007）。
...
%% ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/chapter-4-concept-map.svg) %%
...
%% ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/chapter-5-concept-map.svg) %%

#### 图片占位

> [!example]- 图片占位
> 表5.1：Illustration of an Introduction to a Study With Annotated Markers of Components  
> ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/Table_5-1_Introduction_Annotated_Markers.png)
...
%% ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/chapter-6-concept-map.svg) %%

#### 图片占位

> [!example]- 图片占位
> 图6.1：Relationship Among the Problem, the Purpose (Aim), the Research Questions, and the Data
> ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/Figure_6-1_Problem_Purpose_Questions_Data.png)
...
> [!note] 非方向性法则
> McCracken (1988) 提出质性研究中的[[Nondirectional Language|非方向性法则]]（law of nondirection, p.21）：访谈者和目的陈述写作者很容易通过使用暗示方向性结果的词语而违反这一法则。应探索"个体的自我表达经验"而非"个体的成功自我表达"。详细讨论见 [[Nondirectional Language]] 条目。

> [!tip] 写作脚本
> Script 是一种填空式模板工具，将质性目的陈述的所有要素串联为一段可直接操作的研究写作脚本。四种质性设计（现象学、案例研究、叙事研究、扎根理论）的范例和要素拆解见 [[Purpose Statement]] 条目。
...
三种核心设计各自有填空式 Script，三个已发表范例（Classen et al., 2007; Ivankova & Stick, 2007; Enosh et al., 2015）和 Ivankova & Stick 的要素拆解见 [[Purpose Statement]] 条目。

> [!note] 复杂设计的整合洞见
> 除三种核心设计外，混合方法还可以将质性或量化数据嵌入实验或评估等框架中，以丰富该框架或过程。这类复杂设计详见第 10 章。
...
%% ![](https://img.mylikemie.icu/books/Creswell_2022_SAGE/figures/chapter-7-concept-map.svg) %%

#### 概览

第7章的逻辑线是：**理解研究问题和假设的总体角色 → 拆解量化研究问题和假设的撰写 → 拆解质性研究问题的撰写 → 拆解[[Mixed Methods Question|混合方法研究问题]]的撰写 → 大量范例辅助理解。**

---

**第一部分：研究问题与假设的总体角色。** 研究问题是继[[Purpose Statement|目的陈述]]之后的第二组"路标"（signposts），将研究目的进一步收窄为可回答的问题或可检验的预测。研究问题的形式因研究路径而异：量化研究同时使用研究问题和假设；质性研究只使用研究问题，不使用假设——因为质性研究者不想预测研究结果的方向（Creswell & Creswell, 2022, Ch7）。

> [!info] 三种路径的研究问题对比
> | 路径 | 使用工具 | 核心特征 |
> |---|---|---|
> | 量化 | 研究问题 + 假设 | 围绕[[Variable\|变量]]间关系提问或预测 |
> | 质性 | 只使用研究问题 | 以开放方式探索[[Central Phenomenon\|核心现象]] |
> | 混合方法 | 量化问题/假设 + 质性问题 + 混合方法整合问题 | 三类问题覆盖三类研究组件 |

---

**第二部分：量化研究问题与假设。**

**量化研究问题与假设的区分。** 量化研究问题就变量间关系提问；量化假设是对预期关系的预测，通过统计程序检验并基于样本对总体做出推论。实验或[[Intervention Research|干预研究]]（含组间比较的）由正式假设指导（Creswell & Creswell, 2022, Ch7）。

量化研究问题可以是描述性的（descriptive，描述单一[[Variable|变量]]的频率和变异，使用均值、标准差、方差和范围），也可以是关系型的（relational，考察变量间关系，使用推论性统计）。

**撰写量化研究问题和假设的指南。** 五条核心指南依次展开。第一，在研究问题或假设中陈述变量——量化研究通常使用三类变量：比较组间自变量以观察其对因变量的影响（实验或组间比较）；将一个或多个预测变量与一个或多个结果变量相关联（调查研究）；描述自变量、中介变量或因变量的得分。第二，包含一个理论——最严格的量化研究以检验理论为起点，研究问题或假设应逻辑上跟随或挑战该理论。第三，研究问题可以是描述性或关系型的。第四，创建一个[[Alternative Hypothesis|方向性备择假设]]——这是假设检验中的两种基本形式之一，另一种是[[Null Hypothesis|零假设]]。零假设预测总体中无关系或无显著差异，通过否定性预期来反驳理论，典型表述为"There is no difference (or relationship)" between the groups。[[Alternative Hypothesis|备择假设]]预测存在显著关系，且应写为方向性形式，如"It is predicted that there is a positive relationship between height and weight, such that taller people are more likely to weigh more"。第五，使用量化研究的方向性词汇——affect, influence, predict, impact, determine, cause, relate（Creswell & Creswell, 2022, Ch7）。

> [!tip] 写作脚本
> Creswell & Creswell (2022, Ch7) 提供了两套量化研究问题的[[Research Writing Script|写作脚本]]：
>
...
> [!example]
> Moore (2000) 在以色列社会中宗教与世俗犹太和阿拉伯女性性别认同意义的研究中，以全国概率样本检验了两条方向性假设（Creswell & Creswell, 2022, Ch7）：
>
...
质性研究只陈述研究问题，不陈述目标（objectives）或假设。研究问题有两种形式：核心问题（[[Central Question]]）和子问题（sub-questions）。

**核心问题**是询问核心现象的主要开放性问题，通常为一至两个，通过访谈参与者、观察环境或审阅文件来回答。子问题将核心现象进一步细分收窄，通常五至七个，成为数据收集（如访谈）中的具体问题（Creswell & Creswell, 2022, Ch7）。

**撰写质性研究问题的指南。** 九条核心指南依次展开：

1. **提出一个或两个核心研究问题**：质性研究者应从最宽泛的层面出发——"在我的研究中，我能提出的最宽泛问题是什么？"习惯量化思维的学生往往难以适应，因为他们惯于将问题收窄到几个具体变量。质性研究的意图恰恰是探索围绕核心现象的一般性复杂因素组合（Creswell & Creswell, 2022, Ch7）。

2. **辅以五至七个子问题**：子问题将核心问题收窄但保持提问的开放性。Miles & Huberman (1994) 建议总共不超过 12 个质性研究问题。在访谈协议中，研究者通常以破冰问题开始，随后是五至七个子问题，以总结性问题或"Who should I turn to, to learn more about this topic?"结束（Asmussen & Creswell, 1995）（Creswell & Creswell, 2022, Ch7）。

3. **将核心问题与特定的质性路径关联**：[[Ethnography|民族志]]中的问题具有特定的分类——Spradley (1980) 提出包括文化共享群体的微型导览、经历、本土语言使用、与其他群体的对比和数据准确性验证的分类法。在批判民族志中，研究问题建立在已有文献体系之上，成为工作指南而非被证实的真理（Thomas, 1993）。[[Phenomenology|现象学]]中的问题通常宽泛陈述，不特别引用已有文献——Moustakas (1994) 建议询问参与者经历了什么以及在什么情境下经历。现象学示例："What is it like for a mother to live with a teenage child who is dying of cancer?"（Nieswiadomy, 1993, p.151）。[[Grounded Theory|扎根理论]]中的问题指向生成某一过程的理论，如"在医院环境中，照顾者和病人如何互动？"。案例研究中的问题可包括对案例的描述和研究涌现的主题（Creswell & Creswell, 2022, Ch7）。

4–9 条分别为：以 what 或 how 开头传达开放设计（why 暗示因果解释思维，属于量化范式）；聚焦单一核心现象或概念；使用探索性动词传达涌现设计的语言——report/reflect（叙事研究）、describe the essence（现象学）、discover/generate（扎根理论）、seek to understand（民族志）、explore（案例研究）、describe the themes（描述性分析）；预计研究问题会演变和改变（在扎根理论研究中尤其如此）；使用开放性问题，不引用文献或理论（除非质性路径另有要求）；指定参与者和研究场所（Creswell & Creswell, 2022, Ch7）。

> [!tip] 写作脚本
> 质性核心问题的[[Research Writing Script|写作脚本]]（Creswell & Creswell, 2022, Ch7）：
>
> ____ (How or what?) is the ____ (central phenomenon) for ____ (participants) at ____ (research site).

> [!example] 民族志核心问题
> Mac an Ghaill & Haywood (2015) 研究了英国出生的工薪阶层巴基斯坦和孟加拉裔年轻男性在三年间的文化状况变化。虽未明确构建研究问题，但建议的核心问题为："What are the core beliefs related to ethnicity, religion, and cultural belonging of the group of British-born, working-class Pakistani and Bangladeshi young men over a 3-year time period, and how do the young men construct and understand their geographically specific experiences of family, schooling, and social life as well as growing up and interacting within their local community in a rapidly changing Britain?"（Creswell & Creswell, 2022, Ch7）

> [!example] 案例研究核心问题
> Hernandez et al. (2020) 评估了一家养老院中的代际游戏小组，核心问题为："How do participants engage in an intergenerational playgroup (IGP) within the context of a residential aged care facility (RACF)?"分析中涌现了相互学习（learning from each other）、欣赏当下体验（appreciating experience in the moment）和通过游戏建立连接（connecting through play）三个主题（Creswell & Creswell, 2022, Ch7）。

> [!example] 子问题范例
> Creswell & Bàez (2020) 以校园多样性氛围为例（Creswell & Creswell, 2022, Ch7）：核心问题为"What is the campus climate toward diversity?"。子问题包括：学生对校园中形成多样化社交群体的态度是什么？中央行政部门如何鼓励多样性？本科课堂如何鼓励多样性？校园警察如何鼓励多样性？

---

**第四部分：混合方法研究问题与假设。**

理解混合方法研究问题的关键在于：在混合方法研究中，研究者需要同时陈述量化研究问题或假设、质性研究问题以及一个**[[Mixed Methods Question|混合方法研究问题]]**。这一要求之所以必要，是因为混合方法既不单独依赖质性也不单独依赖量化，而是依赖两种探究形式（Creswell & Creswell, 2022, Ch7）。

混合方法研究问题是研究方法论中的一个新概念，其正式讨论始于 Tashakkori & Creswell (2007) 在 Journal of Mixed Methods Research 上发表的社论。它询问的是研究者希望通过组合（整合）量化与质性数据学到什么。

**撰写混合方法研究问题的考量：**

1. **三类问题缺一不可**：在两类数据库被整合之前，它们需要分别依据量化问题或假设和质性研究问题进行分析。这些问题可以在研究开始时提出，也可以在研究后期阶段出现。对于两阶段设计，如果以量化阶段开始，研究者可能先引入假设，随后在质性阶段引入质性研究问题（Creswell & Creswell, 2022, Ch7）。

2. **混合方法研究问题的特殊性**：这是一个关于方法的问题，而非关于研究内容的问题——它询问的是整合带来的学习，而整合正是混合方法研究的核心。Creswell & Plano Clark (2018) 为每种设计类型提供了大量混合方法研究问题的范例（Creswell & Creswell, 2022, Ch7）。

3. **放置位置**：在期刊论文中，三类问题通常出现在引言的结论部分；在开题报告中，可能出现在一个独立章节，紧随目的陈述或研究目标之后。

4. **顺序反映设计步骤**：在单阶段[[Convergent Design|聚敛式设计]]中，量化或质性问题可先可后；在[[Explanatory Sequential Design|解释性序列设计]]中，量化在前、质性在后；在[[Exploratory Sequential Design|探索性序列设计]]中，质性在前、量化在后（Creswell & Creswell, 2022, Ch7）。

> [!tip] 写作脚本
> 混合方法研究问题的[[Research Writing Script|写作脚本]]包含整合意图与整合程序两个组件（Creswell & Creswell, 2022, Ch7）：
>
...
> [!example] 混合方法目标陈述范例
> Moseholm et al. (2017) 在健康科学领域研究非特异性症状患者的诊断评估过程（Creswell & Creswell, 2022, Ch7）。虽然作者将三项陈述称为目的（purpose）而非研究问题，但其内容对应了我们推荐的量化—质性—混合方法三部分结构：
>
> - 量化陈述：测量诊断评估过程中健康相关生活质量（HRQoL）的变化
> - 质性陈述：描述患者对 HRQoL 的体验
> - 混合方法陈述：合并这些发现以获得对压力性生活事件中 HRQoL 体验的更全面理解（p.994）

> [!example] 混合方法研究问题范例
> DeCoito & Estaiteyeh (2022) 研究加拿大一个省份科学／STEM 教师的在线教学经历（Creswell & Creswell, 2022, Ch7）。四项研究问题分别是：
>
> 1. 教师在在线环境中使用了哪些数字工具和资源？（描述性量化问题）
> 2. 教师的在线课程开发和实施采用了哪些策略？（描述性量化问题）
> 3. 教师在线上实施了哪些学生评估模式？（由质性体验支撑的量化问题——混合方法问题）
> 4. 教师观察到的在线教学对学生成果的影响是什么？（量化问题）
>
> Creswell & Creswell 指出，这项研究未能明确标记质性研究问题和混合方法研究问题，也没有突出在混合方法设计中整合两类数据的意图。如果作者能提供更清晰的质性研究问题和混合方法研究问题，研究会更加清晰。

---

> [!success] 全章总结
> 研究问题和假设将目的陈述收窄，为读者提供关键路标。量化研究者使用研究问题和正式假设，这些形式包含被描述、关联或比较的[[Variable|变量]]。假设是对结果的预测，通常写为方向性备择假设。质性研究者提出至少一个核心问题和若干子问题，以 how 或 what 开头，使用探索性动词，提出宽泛问题以让参与者表达自己的观点，最初聚焦单一核心现象。混合方法研究者应在其研究中写入量化研究问题或假设、质性研究问题和混合方法研究问题——通过加入混合方法研究问题，研究者传达整合或组合量化与质性元素的重要性。理想格式是将三类问题分别写入不同章节。

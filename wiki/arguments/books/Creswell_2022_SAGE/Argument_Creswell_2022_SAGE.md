...
**写作过程**的三个关键框架（详见 [[Writing Habit]]）：Franklin (1986) 的三阶段模型——大纲 → 草稿并排序段落 → 编辑打磨每句话——将写作分解为不同认知任务，不应混为一谈。Elbow (1973) 建议用一小时写四稿（每 15 分钟一稿）而非一小时只写一稿。Zinsser (1983) 区分了"砖匠"和"先全倒出来"两类写作者，两极之间有迭代改进的中间路径。

**写作习惯**的核心是"每天都写"（Boice, 1990, pp.77–78），包括九项策略——从时间审计、定时写作、避免暴饮暴食式写作到每日绘制三张进度图表和同伴分享（详见 [[Writing Habit]] 条目）。物理环境同样参与习惯建立：Dillard (1989, pp.26–27) 刻意选择没有风景的房间以集中注意力，这与 Boice 的时间管理策略形成互补。

**清晰简洁的写作**涉及三个层面。术语一致性是[[Coherence in Writing|连贯性]]的前提——量化研究中为变量使用一致名称，质性研究中为现象使用一致名称，同义词替换会打断阅读流（详见 [[Coherence in Writing]]）。[[Narrative Thought Types|叙事思想类型]]（Tarshis, 1982）将文本思想按功能分为伞式思想、大思想、小思想和注意力思想四种，初学者最常见的两个问题是伞式思想过多（缺乏支撑细节）和注意力思想缺失（缺乏路标）。[[Coherence in Writing|连贯性]]要求每句话是前一句的逻辑延续（Zinsser, 1983），[[Hook and Eye Technique|钩眼技术]]（Wilkinson, 1991）将这一检查操作化为逐句连接关键词的具象过程。语态、时态和简洁是打磨层面的三项规范：主动语态优于被动（Ross-Larson, 1982）、动词时态遵循 APA (2020) 分位置指南、多余词语（fat）需通过多轮编辑修剪——好的写作标准是读者的眼睛不会停顿、思维不会磕绊。

---

**第三部分：伦理问题。** [[Research Ethics|研究伦理]]是一个贯穿研究全过程的系统框架，按五阶段组织：研究开始之前、研究启动、数据收集、数据分析、报告分享和存储。每个阶段有各自的核心议题和应对策略（详见 [[Research Ethics]] 条目）。核心制度包括查阅专业协会伦理守则、通过 [[Institutional Review Board|IRB]] 审批、获得 [[Informed Consent|知情同意]]。关键原则包括避免赠予署名和幽灵署名（Israel & Hay, 2006）、识别权力失衡、避免剥削参与者、完整报告发现（包括相反结果）、不得重复或拆分发表，以及通过个人协议明确数据所有权归属（Berg, 2001; Punch, 2014）。

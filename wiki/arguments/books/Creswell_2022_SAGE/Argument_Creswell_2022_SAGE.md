...
**第一步：确定研究主题。** Creswell 观察到研究者常跳过草拟标题直接检索。工作标题是"路标"（road sign）。
...
**第二步：可行性审查。** "可以做"（can）是实操层面，"应该做"（should）的核心是对文献的贡献。Creswell 描绘了一个典型失败场景：研究技术层面无可挑剔，但因没有新贡献得不到委员会支持。[[Research Topic]] 条目提供了六种新角度和辅助标准。

---

**第三步：文献综述的角色。** [[Literature Review|文献综述]]是对已有研究的系统检索、评估和总结。Creswell 论证的关键转折：目的之后是"怎么组织"——而组织方式因研究路径而异。
...
**第七步：呈现规范。** APA 格式提供统一的学术写作规范，Creswell 强调格式让读者"专注于内容而非格式"。[[Definition of Terms|术语定义]]的核心原则是操作化。量化研究倾向于在开头"锁定"术语意义，质性研究则允许意义在研究过程中"涌现"。
...
本章区分了七种 [[Variable|变量]] 类型：自变量（independent）、因变量（dependent）、预测变量（predictor/antecedent）、结果变量（outcome/criterion/response）、中介变量（intervening/mediating）、调节变量（moderating）和混淆变量（confounding）。
...
关于**因果性**（causality），核心问题是"我们预期变量 X 导致变量 Y"。评价因果主张时，关键考量是是否存在未测量的第三变量 Z。时间顺序（temporal order）指变量在时间上一个先于另一个，量化研究者从左到右排列变量（Punch, 2014）。检验因果主张的最佳设计是真实验（第 8 章）；调查方法检验的是关联主张。
...
[[Herbert Blalock]] 提出了构建视觉因果图的基本符号规则：
- 因变量置于图中右侧，自变量置于左侧

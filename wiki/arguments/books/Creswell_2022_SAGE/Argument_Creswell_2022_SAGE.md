...
| 第5章 引言 | 提供撰写学术引言的系统方法，首先讨论撰写[[Abstract|研究摘要]]的五个核心组成部分，随后比较量化、质性和混合方法研究引言写法的差异，最后重点介绍引言缺陷模型的四个部分——陈述研究问题（含[[Narrative Hook|叙事钩子]]）、综述文献证据、指出[[Deficiencies in Past Literature|文献缺陷]]和说明研究对受众的重要性。 | [[Research Problem]], [[Deficiencies Model for an Introduction]], [[Narrative Hook]], [[Deficiencies in Past Literature]], [[Abstract]] |
...
| 第9章 质性方法 | 介绍质性数据收集、分析、解释和报告写作的程序，涵盖叙事研究、[[Phenomenology\|现象学]]、扎根理论、民族志、案例研究和描述性研究等具体路径。提供程序检查清单和大量示例。 | [[Narrative Research]], [[Grounded Theory]], [[Ethnography]], [[Case Study]], [[Qualitative Descriptive Method]] |
| 第10章 混合方法程序 | 界定混合方法研究的核心特征，介绍三种核心设计（聚敛式、解释性序列、探索性序列）和四种复杂设计，讨论量化与质性数据的整合分析，包括联合展示的使用和设计选择的流程图。 | [[Mixed Method and Multi-method Research]], [[Joint Display]], [[Complex Mixed Methods Design]] |
...
全书为研究者提供设计研究计划或研究方案的框架、流程和写作方法，覆盖量化、质性和[[Mixed Method and Multi-method Research|混合方法]]三种研究路径。这一比较从三个路径共有的初步要素（哲学[[Hypothesis|假设]]、[[Literature Review|文献综述]]、理论使用、写作与伦理）出发，再逐一拆解研究设计的各个关键组成部分，包括引言撰写、目的陈述、研究问题与假设，以及数据收集、分析和解释的方法程序。
...
| [[Qualitative Descriptive Method\|描述性方法]] | 案例研究 | |
...
混合方法设计中，聚敛式设计同时收集两类数据并合并分析；解释性序列先量化后质性，用质性深入解释量化结果；探索性序列先质性后量化，用量化检验或推广质性发现；复杂设计将核心设计嵌入更大的框架或流程之中。
...
关于写作模型，实用的写作脚本依次涵盖理论名称与来源、核心假设或命题、过去理论使用和应用文献、与当前研究的关联（附带"如果—那么"逻辑）。示例 3.1 完整展示了 Crutchfield (1986) 博士论文中以 [[Social Learning Theory|社会学习理论]] 为框架的理论视角撰写实例。
...
**学科理论**从社会、行为或健康科学文献中提取，识别变量或构念并将它们相互关联，形成并解释对个人和组织的预期结果。常见例子包括组织理论、领导力理论、采纳理论、归因理论和健康促进理论。Kennett et al. (2008) 在慢性疼痛管理混合方法研究中，使用 Rosenbaum (1990) 的 [[Self-control|自我控制]]理论指导量化测量（SCS 量表）和质性访谈，在开头段落即宣告研究目的，在末尾以图表回顾最显著的因素。Clark & Plano Clark (2019) 则使用积极心理学理论来框架化研究设计、个案选择以及结果和发现的解释（p.104）。
...
- **量化研究计划**：遵循 IMRD 经典结构（引言与文献综述—方法—结果—讨论），以问题陈述（含理论、文献缺口、受众关联性）为核心。各部分顺序可能因研究而异（Rudestam & Newton, 2014）。
...
**清晰简洁的写作**涉及三个层面。术语一致性是[[Coherence in Writing|连贯性]]的前提，量化研究中为变量使用一致名称，质性研究中为现象使用一致名称，同义词替换会打断阅读流。[[Narrative Thought Types|叙事思想类型]]（Tarshis, 1982）将文本思想按功能分为伞式思想、大思想、小思想和注意力思想四种，初学者最常见的两个问题是伞式思想过多（缺乏支撑细节）和注意力思想缺失（缺乏路标）。[[Coherence in Writing|连贯性]]要求每句话是前一句的逻辑延续（Zinsser, 1983），[[Hook and Eye Technique|钩眼技术]]（Wilkinson, 1991）将这一检查操作化为逐句连接关键词的具象过程。[[Voice, Tense, and Concision in Academic Writing|语态、时态和简洁]]是打磨层面的三项规范：主动语态优于被动（Ross-Larson, 1982）、动词时态遵循 APA (2020) 分位置指南、多余词语需通过多轮编辑修剪。好的写作标准是读者的眼睛不会停顿、思维不会磕绊。
...
第5章的逻辑线是：**理解引言的重要性 → 撰写研究摘要 → 区分三种路径的引言差异 → 学习引言缺陷模型 → 逐一拆解模型的四个组成部分。**

---

**第一部分：引言的重要性。** 引言是期刊文章、学位论文或学术研究中的第一部分文字，为整个项目搭建舞台。Wilkinson（1991）将引言定义为"为读者提供研究报告背景信息的部分，其目的是为研究建立框架，使读者能够理解该研究如何与其他研究相关联"（引自第5章）。引言必须创造读者对主题的兴趣、确立引导研究的问题、将研究置于学术文献的更大语境中、并触及受众。这些内容需要浓缩在几页的简洁段落中。

---

**第二部分：研究摘要的撰写。** 在介绍引言缺陷模型之前，需要先讨论置于引言之前的摘要。[[Abstract|摘要]]概括研究内容，让读者快速识别项目的核心要素，出现在研究方案、学位论文和期刊文章中。APA 出版手册指出摘要可能是研究中最重要的单个段落（American Psychological Association, 2020），必须准确、非评价性、连贯、可读和简洁。大多数摘要约 250 词。

对于实证研究方案，摘要包含五个核心组成部分，按顺序呈现：

1. **研究问题或议题**：开篇陈述正在调查的问题，可引用一两条参考文献
2. **数据来源与特征**：说明数据来源及其相关特征
3. **研究方法的核心特征**：陈述设计、分析、数据收集、样本量、材料等
4. **基本发现**：量化研究包括效应量和显著性；质性研究说明主要发现及情境
5. **结论与启示/应用**：以研究发现的应用或启示收尾

---

**第三部分：量化和质性和混合方法引言的差异。** 在介绍统一的引言模板之前，Creswell 先区分了三种研究路径在引言写法上的微妙差异。

量化引言通常结构固定。问题来源于理解哪些因素或变量影响某一结果的需要。量化引言还常用于改进和检验理论——先描述已有理论，再描述挑战或扩展该理论的文献。量化引言可能以非个人视角、过去时态撰写，以传达客观性。

质性引言结构更加多样。作者描述一个最好通过探索概念或现象来理解的研究问题。质性研究问题是探索性的，当变量、问题和理论尚不明确时使用。Morse（1991）概括了质性研究问题的四个特征：(a) 概念因缺乏理论和先前研究而"不成熟"；(b) 现有理论可能不准确、不适当、不正确或有偏见；(c) 需要探索和描述现象并发展理论；(d) 现象的本质可能不适合量化测量。质性引言可能以作者的个人经验陈述开头（如现象学研究），也可能使用变革性透镜审视问题（如性别薪酬不平等或种族态度研究），并以第一人称主观视角撰写。

混合方法引言可以采用量化或质性路径（或两者结合）。当两类研究权重相等时，问题可能同时需要在量化层面理解变量关系和在质性层面深入探索主题。混合方法问题也可能指出：现有研究主要是量化或质性的，需要扩展路径以包含更多样化的方法论。

---

**第四部分：引言缺陷模型。** [[Deficiencies Model for an Introduction|引言缺陷模型]]是一种基于文献缺口构建引言的方法，包含四个部分：陈述研究问题、综述已有研究、指出已有研究中的缺陷、说明研究对受众的重要性。该模型在社会科学领域广泛应用，通常每部分写一段，整个引言约两页。以下逐一拆解。

---

**（一）陈述研究问题。** 开篇段落引入研究主题，使用[[Narrative Hook|叙事钩子]]激发读者兴趣。叙事钩子是从英语写作借用的概念，指吸引或"钩住"读者的开篇语句，应让广泛受众能够理解。Creswell 使用"木桶下井"的隐喻来说明：有经验的作者缓缓地将读者（木桶）放入研究的深度（井），让读者逐步适应；初学者则直接将读者抛入陌生的专业细节之中。

撰写叙事钩子的建议：研究顶级期刊文章的第一句话；避免在开篇使用引文（尤其是长引文）；远离习语或陈词滥调；考虑使用数字信息增强冲击力；可用问题开篇创造读者兴趣。

在叙事钩子之后，必须清晰识别引导研究需求的[[Research Problem|研究问题]]。研究问题与具体的研究问题句不同——前者是需要解决的难题或议题，后者是通过数据收集和分析来回答的具体问题。陈述研究问题时的常见误区：

- 只识别文献中存在什么，而非识别需要解决的问题。例如引用青少年怀孕的普遍性是陈述事实，真正的问题是学校在接纳怀孕青少年方面做得很差。
- 问题不清晰，让读者自行判断研究的重要性。
- 混淆研究问题与研究问题句。
- 引言承载着鼓励读者继续阅读的重任。

> [!tip] 撰写建议
> 开篇句应激发兴趣并传达广泛受众能理解的议题；确保有明确的句子传达研究问题；通过引用大量参考文献说明问题的重要性（建议第一页至少引用十几条参考文献）；确保问题框架与研究路径一致；考虑并列出一个或多个引导研究需求的问题。

---

**（二）综述文献中的证据。** 在确立研究问题之后，讨论已有文献中涉及该问题的先前研究。这不是完整的文献综述——完整的综述出现在研究方案后面的文献综述部分。引言中的文献综述应总结大的研究群组，而非逐一详述个别研究。建议反思[[Literature Map|文献地图]]，查看地图顶部的宽泛类别作为需要总结的大研究群组。

当研究者说"我的题目没有研究做过"时，可用"倒三角"思维应对：倒三角的底部尖端代表计划的窄而集中的研究。从底部向上拓宽文献回顾的范围，就能找到文献，尽管它们可能与当前研究有一定距离。例如"小学阶段的风险儿童"研究很少，但"小学风险学生"或"任何教育阶段的风险学生"可能已有研究。

引言文献综述的撰写要点：总结研究群组而非个别研究；将文内引用放在段落末尾或总结点末尾以弱化单个研究；回顾使用量化、质性或混合方法的研究；优先回顾近 10 年的文献，仅在被广泛引用的前提下引用较早的研究。

---

**（三）指出[[Deficiencies in Past Literature|既往文献缺陷]]。** 在陈述问题和综述文献之后，研究者识别已有文献中的缺陷。缺陷的性质因研究而异，这是该模型被称为"缺陷模型"的核心原因——其主要组成部分是确立已有研究中的不足。

常见缺陷类型包括：主题、群体、样本或总体未被探索；研究需要复制以确认混合结果；未处理特殊群体或新场所；未为未被充分代表的群体发声；缺乏质性数据；理论发展不充分；测量工具未经验证；未关注中介变量；文献未同时包含量化和质性数据等。

陈述缺陷的实用建议：引用多个缺陷来构建强有力的案例；具体识别其他研究的缺陷（如方法缺陷、变量忽略）；写出过去研究忽略的领域；讨论计划研究将如何弥补这些缺陷；查看已发表研究中"未来研究"部分寻找可引用的缺陷线索；可用一系列短段落识别三到四个不足，或聚焦于一个主要不足。
...
**（四）说明研究对受众的重要性。** 最后，作者阐述研究对特定受众的重要意义，为研究创建清晰的理据，帮助读者理解研究的广泛适用性。可涵盖三个层面（Creswell & Creswell, 2022, 第5章）：
...
> [!quote]
> "If you feel you do not have time for regular writing, begin with reflection on daily activities for a week or two in half-hour blocks. This reflection will likely lead to time to write."（Boice, 1990, pp.77–78; 引自第4章）
...
> [!quote]
> "The ability to frame the study in this way separates novices from more experienced researchers. The experienced researcher has reviewed and understands topics or problems in the field. This knowledge comes from years of experience following the development of problems and their accompanying literature."（第5章）

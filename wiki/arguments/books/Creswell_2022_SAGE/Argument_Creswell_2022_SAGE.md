...
Creswell 观察到一个常见失误：研究者往往跳过草拟标题，直接进入文献检索。工作标题是研究中的"路标"（road sign），一个让研究者在项目推进中不断重新聚焦的锚点。具体操作方法详见 [[Literature Review]] 条目。
...
[[Literature Review|文献综述]]的定义、多重目的及 Cooper (2010) 的四种类型，详见该条目。Creswell 论证的关键转折在于：目的之后是"怎么组织"——而组织方式因研究路径而异。
...
量化研究以演绎方式使用文献，结构围绕变量组织（详见 [[Literature Review]]）。关键在于五部分结构中的第三部分（连接自变量与因变量的文献）——它应相对简短且极其接近计划研究。
...
混合方法研究则取决于设计类型和两类成分的相对权重，详见 [[Literature Review]]。Creswell 的实用主义在此最明显：不是"应该怎样"，而是"取决于你用什么设计以及受众偏好"。

---

**第五步：实际检索——七步流程和背后的取舍。**

文献检索的七步流程、数据库详情、优先级顺序和质量评估标准详见 [[Literature Review]] 条目。这里关注三个设计层面的判断：
...
[[Literature Review|文献综述]]条目详述了摘要的撰写要点（区分实证与非实证文章、提取要素）。[[Literature Map|文献地图]]是 Creswell 自己开发的工具，其逻辑不是简单分类，而是论证装置：通过可视化展示"已有研究的版图是什么样的，我的研究将延伸哪一部分"。
...
格式手册（详见 [[Literature Review]]）方面，Creswell 的立场明确：格式统一不是装饰，而是让读者"专注于内容而非格式"。术语定义（详见 [[Literature Review]]）的核心原则是操作化而非抽象。

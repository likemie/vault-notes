...
> [!note] 非方向性法则
> McCracken (1988) 提出质性研究中的[[Nondirectional Language|非方向性法则]]（law of nondirection, p.21）：访谈者和目的陈述写作者很容易通过使用暗示方向性结果的词语而违反这一法则。应探索"个体的自我表达经验"而非"个体的成功自我表达"。[[Nondirectional Language]] 提供了进一步讨论。

> [!tip] 写作脚本
> Script 是一种填空式模板工具，将质性目的陈述的所有要素串联为一段可直接操作的研究写作脚本。四种质性设计（现象学、案例研究、叙事研究、扎根理论）的范例和要素拆解由 [[Purpose Statement]] 提供。
...
> [!note] 复杂设计的整合洞见
> 除三种核心设计外，混合方法还可以将质性或量化数据嵌入实验或评估等框架中，以丰富该框架或过程。这类复杂设计是混合方法程序的重要组成部分。
...
第7章紧随第6章的[[Purpose Statement|目的陈述]]，系统讲解研究设计中第二组"路标"——[[Research Question|研究问题]]与假设——的撰写方法。全章按量化 → 质性 → 混合方法的顺序展开，每一类均提供填空式[[Research Writing Script|写作脚本]]和已发表研究范例。

---

**量化研究问题与假设。** 量化研究同时使用研究问题和假设。研究问题就[[Variable|变量]]间关系提问，可以是描述性的（描述单一变量）或关系型的（考察变量间关联）；假设则是对变量关系的方向性预测，通过统计程序检验。假设的两种基本形式是[[Null Hypothesis|零假设]]（预测无关系）和[[Alternative Hypothesis|备择假设]]（预测存在显著关系），后者最好写为方向性形式。撰写时需遵循五条指南：陈述变量、包含理论、区分描述性与关系型、创建方向性备择假设、使用方向性量化词汇。Moore (2000) 的性别认同研究提供了研究问题与假设并存的范例。这些元素共同构成了量化研究的问题架构（见 [[Research Question]]、[[Hypothesis]]、[[Null Hypothesis]]、[[Alternative Hypothesis]]）。

---

**质性研究问题。** 质性研究只使用研究问题，不使用假设。[[Central Question|核心研究问题]]（一至两个）以 what 或 how 开头，聚焦单一[[Central Phenomenon|核心现象]]，辅以五至七个子问题收窄焦点。撰写需遵循九条指南，包括：关联特定质性路径（[[Ethnography|民族志]]、[[Phenomenology|现象学]]、[[Grounded Theory|扎根理论]]、案例研究的问题形式各有不同）、使用探索性动词、预计问题会演变、不引用文献或理论、指定参与者和研究场所。不同质性路径通过 [[Central Question]] 展示其研究问题形式的差异。

---

**[[Mixed Methods Question|混合方法研究问题]]。** 混合方法研究需要三类问题：量化问题或假设、质性研究问题以及[[Mixed Methods Question|混合方法研究问题]]。混合方法研究问题是一个关于方法的、相对较新的概念（Tashakkori & Creswell, 2007），询问研究者希望通过整合量化与质性数据获得什么洞见。三类问题的顺序反映设计类型：聚敛式可先可后、解释性序列量化在前、探索性序列质性在前。Moseholm et al. (2017) 提供了量—质—混合方法三部分目标陈述的范例。[[Mixed Methods Question]] 定义了这一整合逻辑的核心。

...
- [[Qualitative Research|质性研究]] 探索和理解个体或群体赋予社会或人类问题之意义。过程包含[[Emergence|涌现]]式问题、在参与者自然情境中收集数据、从具体到主题的归纳式分析，以及研究者对数据意义的解释。报告结构灵活，研究者注重个体意义和情境复杂性。
...
- [[Postpositivism|后实证主义]] 持[[Determinism|决定论]]和还原论立场。知识是推测性的，绝对真理无法被找到；研究者从理论出发，通过经验观察与测量收集数据，检验、验证并完善理论。它来自 19 世纪的 Comte、Mill、Durkheim、Newton、Locke，以及 Phillips & Burbules(2000)，是量化研究的主导哲学。
...
三种位置对应不同[[Epistemic Stances|认识论立场]]，选择本质上取决于受众。此外，质性研究内部也不统一——理论导向的设计（民族志、[[Critical Ethnography|批判民族志]]）在开头引入文献框架，而扎根理论、案例研究和现象学则较少在开头使用文献。
...
**第七步：呈现规范。** [[APA Style|APA]] 格式提供统一的学术写作规范，其目的是让读者"专注于内容而非格式"。[[Definition of Terms|术语定义]]的核心原则是[[Operationalization|操作化]]。量化研究倾向于在开头"锁定"术语意义，质性研究则允许意义在研究过程中"涌现"。
...
**学科理论**从社会、行为或健康科学文献中提取，识别变量或[[Construct|构念]]并将它们相互关联，形成并解释对个人和组织的预期结果。常见例子包括组织理论、领导力理论、采纳理论、归因理论和健康促进理论。Kennett et al. (2008) 在慢性疼痛管理混合方法研究中，使用 Rosenbaum (1990) 的 [[Self-control|自我控制]]理论指导量化测量（SCS 量表）和[[Qualitative Interview|质性访谈]]，在开头段落即宣告[[Research Purpose|研究目的]]，在末尾以图表回顾最显著的因素。Clark & Plano Clark (2019) 则使用积极心理学理论来框架化研究设计、个案选择以及结果和发现的解释(p.104)。
...
**全章总结。** 理论使用在三种研究路径中有本质差异。量化研究中，理论是解释或预测变量间关系的框架，回答"为何"和"如何"。理论可宽可窄，通过假设链、"如果—那么"逻辑或视觉模型陈述，通常置于研究前端。质性研究中，理论可以是广泛的演绎解释（如民族志）、立场透镜（如女性主义、批判理论）或研究的归纳终点（如扎根理论）。混合方法[[Research Utilization|研究使用]]学科理论或社会正义理论来指导设计的各个层面。理论可以在研究设计的几乎所有阶段提供指导，关键在于将理论与具体的研究设计类型明确地关联起来。
...
第7章系统讲解研究设计中[[Research Question|研究问题]]与假设的撰写方法。全章按以下顺序展开：
...
Moseholm et al. (2017) 在健康科学领域提供了量—质—混合方法三部分目标陈述的范例(Ch7)。DeCoito & Estaiteyeh (2022) 的 [[STEM Education|STEM]] 教师在线教学研究也是典型范例。
...
进入具体设计之前，两种设计随后被放在一起做系统对比，以揭示它们在[[Epistemic Aims|认识论目标]]、操作方式和推论强度上的本质差异。
...
> [!example] 表8.1–8.2 — 完整内容见 [[Survey Research]] 条目。表8.1 为调查方法计划检查清单（六组件自查表），表8.2 为变量-定义-题项对照示例（教师研究产出调查，三变量：预测变量→结果变量→控制变量）。
...
> | 4. 信度检查 | 反向计分、量表总分计算、[[Internal Consistency\|内部一致性]] | [[Cronbach's Alpha\|Cronbach's α]] | [[Cronbach's Alpha\|内部一致性（Cronbach's α）]] |
...
> [!example] 表8.4 — 设计实验方法计划的检查清单（Checklist of Questions for Designing an Experimental Study Plan），完整内容见 [[Experimental Research]] 条目。
...
| [[Audiovisual, Social Media, and Digital Materials\|视听和数字材料]] | 照片、录像、网站、社交媒体文本、声音等 | [[Creativity\|创造性]]形式、吸引注意力、反映日常生活 | 解读困难、版权与隐私问题、技术设备门槛 |
...
Creswell & Creswell 提出混合方法研究的六个定义性特征，从核心到外层依次嵌套：**(1) 收集两种类型的数据**——质性（开放式）和量化（封闭式）；**(2) 连接两组数据**——即整合；**(3) 使用特定程序来整合**——即混合方法设计；**(4) 在联合展示中分析整合**——以表格或图表并排呈现；**(5) 得出元推论**——从组合数据库中提取超越单一数据库的额外推论；**(6) 以世界观和理论框架定位研究**——构成最外层的整体框架。这六个特征的嵌套结构见图 10.1（见 [[Mixed Methods Research]] 条目）。与[[Multimethod Research|多方法研究]]的关键区分在于：多方法研究仅收集多种量化来源或多种质性来源，不同时跨越两类数据；混合方法研究的标志是同时收集两类数据并加以整合。使用混合方法的理由涵盖从"获得更完整的理解"到"为边缘群体变革提供证据"等多个层面，包括：结合两类结果形成更完整理解、用质性数据详细解释量化结果、通过参与者反馈改进量表、用个体视角丰富实验、发展案例用于比较、结合两类数据评价项目、理解边缘群体所需的变革。

...
> [!question]
> 围绕 [[Global Universities Rankings|全球大学排名]] 与高等教育使命的关系，这里[[Probes|追问]]一个看似矛盾的问题：既然各方都承认排名有严重缺陷，为什么它们的影响力仍然与日俱增？具体拆为三个子问题：（1）排名所推崇的"好大学"标准，与高等教育实际上承担的多元使命之间，到底有没有冲突？（2）如果冲突确实存在，它具体表现在哪些方面？（3）既然冲突这么深，大学能不能干脆不理排名、回归自己的使命？（Boyadjieva, 2022, pp.126, 137）
...
有了对大学作为制度的理解，下一步就是系统地回答：大学到底为社会做什么？以 Drèze & Sen 的 [[Capability Approach|可行能力路径]]（Boyadjieva, 2022, p.128）为概念零件，搭建一个 2×3 的矩阵模型。
...
需要强调的是，这个模型是一个"[[Max Weber|韦伯]]意义上的理想类型"——现实中没有一所大学能在这六个格子上都做到满分。它的作用是作为分析工具：审视排名时，可以[[Probes|追问]]——排名覆盖了这六个格子中的哪几个？漏掉了哪几个？（Boyadjieva, 2022, p.130）

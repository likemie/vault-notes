...
> [!note]- 团队与[[Theoretical Standpoint\|理论立场]]
> 三位研究者分别来自挪威、芬兰和土耳其，前两位有从教背景，第三位是心理学研究者。整体分析持[[Critical Realism\|批判实在论]]（critical realism）立场（Bhaskar, 2013）：要找出生成可观察模式的深层机制，而不只是数障碍与促进因素的频数。这一取向让配置式分析更关注系统与关系性模式，也可能使个体层面因素相对不突出。（pp.6, 8）
>
...
> [!claim] 步骤一主张
> 纳入的研究大多把研究参与说成一种"缺失"或"挑战"，并把问题和部分解决办法归到个体、组织或系统三个层面之一。但聚合统计显示，个体层面的因素出现在 83% 的研究里，组织与系统层面的因素则很少被提及，说明[[Research Utilization\|研究使用]]研究明显偏向个体视角。（p.9）

> [!dimension] 研究使用在[[Document\|文献]]中被描述的四个分析层面
> - **系统层面（k = 46）**
>   文献里同时存在两种对立的模型：
>   **自上而下** 政策驱动的工具性预期：教学实践应建立在证据上，但问责体系只考核学生成绩，并不奖励学校用研究。
>   **自下而上** 理想化模型：研究使用是集体的意义建构和"集体协商"，系统应创造支持"长时间、聚焦讨论"的结构。
>   前者最常见，后者被当作理想。（pp.9–10）
> - **组织层面（k = 56）**
>   三个子成分紧密交织、彼此强化：
...
> - **核心构件（k = 94）**
>   **恰当研究（k = 59）** 可归纳为四个主题：与本校情境匹配、实用（可及且可操作）、能与其他[[Source of Knowledge\|知识来源]]整合、可信。
>   **箭头[[Coding in Qualitative Research\|编码]]（k = 70）** 涉及教师为何、在何时、在哪里参与研究：来源上，同事和网络最常见，正式学术渠道最少被使用。
>   **深思熟虑参与（k = 57）** 在文献中比工具性、象征性使用都更罕见。（pp.11–12）
...
### 论证步骤三　塑造研究使用的四类跨层机制：信任、技能发展、可持续结构、关系

> [!claim] 步骤三主张
> 一些主题既反复跨越多个层级，又同时出现在"框定/描述"和"障碍/促进"两类数据里，它们构成了四类治理机制：信任、技能发展、可持续结构和关系。这四类机制不属于任何单一层级，而是横跨并贯通整个系统，共同塑造学校里的研究使用。修订框架因而用四类机制取代了原先的层级嵌套逻辑。（pp.15–17）
...
> 教师与所提供研究文章的深度讨论中，"支持既非全盘拒绝，也非不加批判地接受基于研究的建议，而是涉及[[Opportunist Mode\|机会主义]]的决策"；当研究被整合进其它[[Source of Knowledge\|知识来源]]，并在长时间、聚焦的讨论中参与时，即使没有总是伴随实施，也已经构成深思熟虑参与。（pp.11–12）
>

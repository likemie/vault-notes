...
> [!line-a] 第一层：PCA 与服务业分类验证
> 研究使用六个创新变量进行主成分分析，根据 Kaiser 准则保留两个主成分，累计解释 $71.36\%$ 的总方差（$\lambda_1 = 3.06$，$\lambda_2 = 1.22$）。
>
...
> > 第二主成分（$\lambda_2 = 1.22$）与内部研发占比（载荷 $0.78$）和创新产品占营业额比重（载荷 $0.44$）高度相关，捕捉通过研发和新产品开拓市场的策略。两个成分精确对应了 Schumpeter 关于产品创新和流程创新的经典区分（pp.163-165）。
>
> > [!success] PCA 二维图的四类空间分布

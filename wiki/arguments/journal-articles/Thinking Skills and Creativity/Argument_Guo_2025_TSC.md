...
> | **词汇创造力** | 16 | $\mathbf{0.641}\ [0.339, 0.943]$ | $SE = 0.154,\ Z = 4.16\ (p < .001)$ | **强增益**，字词远距离联想（RAT）与语义概念快速重组 |
> | **言语创造力** | 23 | $\mathbf{0.606}\ [0.407, 0.805]$ | $SE = 0.102,\ Z = 5.97\ (p < .001)$ | **中高增益**，非常规用途（AUT）生成与口头/书面表达外化 |
> | **问题解决** | 10 | $\mathbf{0.539}\ [0.316, 0.762]$ | $SE = 0.114,\ Z = 4.73\ (p < .001)$ | **中等稳健增益**，不良结构情境识别与目标障碍突破 |
> | **情绪创造力** | 9 | $\mathbf{0.397}\ [0.208, 0.587]$ | $SE = 0.097,\ Z = 4.11\ (p < .001)$ | **低增益响应滞后**，深层情绪体验对常规认知干预敏感度有限 |
...
> | **学段层次** · **研究生** | 2 | $\mathbf{0.991}\ [0.468, 1.515]$ | $SE = 0.267,\ \mathbf{Z = 3.71\ (p < .001)}$ | **强增益**，高阶批判思维与深厚专业知识底座赋能（样本量偏小） |
> | **组织形式** · 小组协作 | 61 | $0.567\ [0.462, 0.673]$ | $SE = 0.054,\ Z = 10.54\ (p < .001)$ | 易受评价顾虑、生产力阻塞与从众心理抑制 |
> | **组织形式** · **独立练习** | 48 | $\mathbf{0.682}\ [0.501, 0.863]$ | $SE = 0.092,\ \mathbf{Z = 7.38\ (p < .001)}$ | **个体深思优势**，无外界干扰，深度沉浸于自由发散构思 |
> | **训练方式** · **言语训练** | 15 | $\mathbf{0.998}\ [0.437, 1.560]$ | $SE = 0.286,\ \mathbf{Z = 3.49\ (p < .001)}$ | **效应量居首**，语言符号即时外化澄清思维，增强创造自我效能 |
> | **训练方式** · **思维训练** | 13 | $\mathbf{0.762}\ [0.469, 1.055]$ | $SE = 0.149,\ \mathbf{Z = 5.10\ (p < .001)}$ | **显著优势**，侧向思维、头脑风暴与元认知监控驱动 |
...
> **效应量统计** $N = 15$ 项效应量，**$g = 0.998$**（95% CI $[0.437, 1.560]$），$SE = 0.286$，$Z = 3.485\ (p < .001)$，在所有训练方式中效应量居首。
>
...
>
> **典型实证循证支持** 任务型代表如阿联酋高校的 STEAM 挑战（Vally et al., 2019）；技术型代表涵盖德国（Malycha & Maier, 2017）、加拿大（Morin et al., 2018; Puccio et al., 2020）、荷兰（Ritter & Mostert, 2017）与法国（Fleury et al., 2020）等工程与设计课程（详见 Table 4 清单）。
>
> **核心操作规程与教学设计**
...
> - **认知迁移阻力（Transfer Limitations）** 技术型过于依赖机械化规则套用，易导致学生产生“工具依从性”，在脱离特定技术提示的标准创造力测验中，其远迁移（Far Transfer）效果略低于纯粹的思维发散训练。（pp.7–9）
...
>
> **典型实证循证支持** 代表性实证研究如约旦高校开展的情绪创造力干预（Mahasneh & Gazo, 2019，详见 Table 4 清单）。
>
> **核心操作规程与教学设计**

---
summary: "基于 2000–2024 年 29 项实证研究与 109 个效应量的多层元分析，系统拆解创造力干预对大学生的中等促进效能、多维构念响应差异与倒U型时间调节机制。"
type: argument
authors:
  - "Guo, H."
  - "Zhou, Z."
...
> | 纳入研究（APA 格式） | 国别与样本构成 | 测量工具与设计 | 干预方案（周期 / 组织形式 / 训练方式） |
> |---|---|---|---|
> | Baruah & Paulus (2008) | 美国（$EG = 11 / CG = 11$） | OSVM；实验组/对照组（a: 无训练） | < 1 周；小组协作；综合训练 |
> | Campion & Levita (2014) | 英国（$EG = 56 / CG = 56$） | SEES, [[Torrance Tests of Creative Thinking\|TTCT]]；实验组/对照组（b: 替代训练） | < 2 个月；独立练习；言语训练 |
> | Chiu (2015) | 中国台湾（$EG = 39 / CG = 39$） | OTT；实验组/对照组（b: 替代训练） | < 1 周；独立练习；综合训练 |
> | Fink et al. (2015) | 奥地利（$EG = 24 / CG = 29$） | [[Alternate Uses Test\|AUT]]；实验组/对照组（b: 替代训练） | < 2 个月；小组协作；言语训练 |
> | Hargrove & Nietfeld (2015) | 美国（$EG = 118 / CG = 88$） | RAT, 元认知意识量表（Metacognitive Awareness Inventory, MAI）；[[Pre-test and Post-test\|前后测设计]]（b: 替代训练） | < 4 个月；独立练习；思维训练 |
> | Karakelle (2009) | 土耳其（$EG = 15 / CG = 15$） | OSVM；前后测设计（c: 等待名单） | < 4 个月；独立练习；综合训练 |
> | Kienitz et al. (2014) | 美国（$EG = 15 / CG = 13$） | TTCT-F, 大五人格量表简版（NEO Five-Factor Inventory, NEO-FFI）；实验组/对照组（b: 替代训练） | < 2 个月；独立练习；言语训练 |
> | Lewis & Lovatt (2013) | 英国（$EG = 21 / CG = 20$） | AUT；实验组/对照组（b: 替代训练） | < 1 周；小组协作；言语训练 |
> | Malycha & Maier (2017) | 德国（$EG = 80 / CG = 80$） | SEES, TTCT；实验组/对照组（c: 等待名单） | < 1 个月；小组协作；技术型训练 |
> | Meinel et al. (2019) | 德国（$EG = 59 / CG = 59$） | ATTA；实验组/对照组（b: 替代训练） | < 2 个月；小组协作；思维训练 |
> | Morin et al. (2018) | 加拿大（$EG = 59 / CG = 59$） | CEDA；前后测设计（c: 等待名单） | > 1 年；独立练习；技术型训练 |
> | O'Connor et al. (2016) | 澳大利亚（$EG = 62 / CG = 62$） | WRT；前后测设计（d: 情商训练） | < 1 周；独立练习；综合训练 |
> | Onarheim & Friis-Olivarius (2013) | 丹麦/加拿大（$EG = 50 / CG = 50$） | AUT；实验组/对照组（b: 替代训练） | < 2 个月；小组协作；思维训练 |
> | Oppezzo & Schwartz (2014) | 美国（$E = 48 / C = 48$） | AUT；实验组/对照组（b: 替代训练） | < 1 周；独立练习；综合训练 |
> | Ritter & Mostert (2017) | 荷兰（$EG = 31 / CG = 31$） | AUT, [[Remote Associates Test\|RAT]]；前后测设计（b: 替代训练） | < 1 周；小组协作；技术型训练 |
> | Wang & Horng (2002) | 中国台湾（$EG = 21 / CG = 18$） | TTCT；实验组/对照组（b: 替代训练） | < 1 年；小组协作；思维训练 |
> | West et al. (2012) | 美国（$EG = 93 / CG = 93$） | TTCT；实验组/对照组（b: 替代训练） | < 1 年；小组协作；综合训练 |
> | Zitek & Vincent (2015) | 美国（$EG = 97 / CG = 97$） | RAT；实验组/对照组（b: 替代训练） | < 1 个月；小组协作；综合训练 |
> | Zampetakis et al. (2008) | 希腊（$EG = 52 / CG = 42$） | OSVM；实验组/对照组（b: 替代训练） | < 4 个月；独立练习；综合训练 |
> | Lubart (2014) | 法国（$EG = 63 / CG = 63$） | 创造潜能评估测验（Evaluation of Potential for Creativity, EPOC）；实验组/对照组（b: 替代训练） | < 1 周；小组协作；综合训练 |
> | Gu et al. (2023) | 中国（$EG = 41 / CG = 39$） | AUT；实验组/对照组（b: 替代训练） | < 4 个月；小组协作；综合训练 |
> | Mahasneh & Gazo (2019) | 约旦（$EG = 35 / CG = 32$） | ECI；实验组/对照组（b: 替代训练） | < 1 周；独立练习；情绪训练 |
> | Puccio et al. (2020) | 加拿大（$EG = 11 / CG = 10$） | TTCT；实验组/对照组（b: 替代训练） | < 3 年；小组协作；技术型训练 |
> | Fleury et al. (2020) | 法国（$EG = 16 / CG = 16$） | GAU, RAT；实验组/对照组（c: 等待名单） | < 1 周；独立练习；技术型训练 |
> | Dow & Mayer (2004) | 美国（$EG = 11 / CG = 11$） | OSVM；实验组/对照组（b: 替代训练） | < 1 个月；小组协作；综合训练 |
> | Karwowski & Soszynski (2008) | 波兰（$N = 15$） | 创造性意象测验（Test of Creative Imagery, TCI）, TCT-DP；实验组/对照组（b: 替代训练） | < 2 个月；独立练习；综合训练 |
> | Robbins & Kegley (2010) | 美国（$N = 51$） | TTCT；前后测设计（b: 替代训练） | < 4 个月；独立练习；思维训练 |
> | Stolaki & Economides (2018) | 希腊（$N = 81$） | TTCT；前后测设计（b: 替代训练） | < 1 个月；小组协作；综合训练 |
> | Vally et al. (2019) | 阿联酋 | TTCT；前后测设计（e: 神经创造力） | < 4 个月 | 小组协作；任务型训练 |
...
> ```mermaid
> flowchart TD
>     A["研究起点：高等教育创造力干预有效性与调节机制缺乏系统量化"] --> B["多层元分析建模：纳入 29 项实证研究与 109 个效应量"]
...
>
> | 模型设定 | 研究数 $k$ | Hedges' $g$ [95% CI] | 统计检验与异质性说明 |
> |---|---|---|---|
> | **固定效应模型** | $k = 29$ | $0.572\ [0.504, 0.639]$ | $SE = 0.034,\ Z = 16.65\ (p < .001)$；基准同质性比较模型 |
> | **随机效应模型** | $k = 29$ | **$0.628\ [0.446, 0.809]$** | $SE = 0.092,\ \mathbf{Z = 6.79\ (p < .001)}$；**$Q(28) = 178.51^{***},\ I^2 = 84.31\%,\ \tau^2 = 0.189$**（真实异质性高，确立中等促进基准） |
...
> <span class="concept-meta-analysis-table-marker" aria-hidden="true"></span>
>
> | 创造力构念子维度 | 效应数 $N$ | Hedges' $g$ [95% CI] | 统计检验与效应特征 |
> |---|---|---|---|
> | **创造性思维** | 24 | **$0.671\ [0.439, 0.903]$** | $SE = 0.118,\ Z = 5.68\ (p < .001)$；**增益居首**，发散认知与观念流畅性极易受启发式策略激活 |
> | **图形创造力** | 27 | **$0.644\ [0.439, 0.849]$** | $SE = 0.105,\ Z = 6.15\ (p < .001)$；**强增益**，具象视觉表征与空间拓扑重组展现高度可塑性 |
> | **词汇创造力** | 16 | **$0.641\ [0.339, 0.943]$** | $SE = 0.154,\ Z = 4.16\ (p < .001)$；**强增益**，字词远距离联想（RAT）与语义概念快速重组 |
> | **言语创造力** | 23 | **$0.606\ [0.407, 0.805]$** | $SE = 0.102,\ Z = 5.97\ (p < .001)$；**中高增益**，非常规用途（AUT）生成与口头/书面表达外化 |
> | **问题解决** | 10 | **$0.539\ [0.316, 0.762]$** | $SE = 0.114,\ Z = 4.73\ (p < .001)$；**中等稳健增益**，不良结构情境识别与目标障碍突破 |
> | **情绪创造力** | 9 | **$0.397\ [0.208, 0.587]$** | $SE = 0.097,\ Z = 4.11\ (p < .001)$；**低增益响应滞后**，深层情绪体验对常规认知干预敏感度有限 |
> | **全域合并效应** | 109 | **$0.569\ [0.480, 0.658]$** | $SE = 0.045,\ Z = 12.54\ (p < .001)$；**综合中等显著促进效能**（多维构念加权合并基准） |
...
> [!warrant]- 推理桥梁
> 构念响应的非均质性证明了创造力并非单一无差别的心理特质，而是包含发散认知、语言重组、视觉表征与情感情绪的多维系统；教学干预应针对不同构念的认知特性设计适配的教学支架，优先从[[Divergent Thinking|发散思维]]与图形表征切入以实现效能最大化。（pp.7–9）

---

### 论证步骤三：干预特征调节机制——时间周期的倒 U 型演变与认知言语训练优势
...
>
> | 调节变量与亚组分类 | 效应数 $N$ | Hedges' $g$ [95% CI] | 统计检验与机制解读 |
> |---|---|---|---|
> | **训练周期** · < 1 周 | 37 | $0.576\ [0.424, 0.728]$ | $SE = 0.078,\ Z = 7.42\ (p < .001)$；快速策略启动，但受限于认知内化时间不足 |
> | **训练周期** · **< 2 个月** | 30 | **$0.844\ [0.647, 1.042]$** | $SE = 0.101,\ \mathbf{Z = 8.38\ (p < .001)}$；**黄金干预窗口（解释 13.4% 组间方差）**，策略精熟与动机最优平衡 |
> | **训练周期** · < 4 个月 | 24 | $0.642\ [0.444, 0.840]$ | $SE = 0.101,\ Z = 6.36\ (p < .001)$；维持中高效应，技能巩固与初步迁移 |
> | **训练周期** · < 1 年 | 9 | $0.233\ [0.085, 0.381]$ | $SE = 0.076,\ Z = 3.08\ (p = .002)$；边际效用递减，常规化教学引发动机倦怠 |
> | **训练周期** · < 2 年 | 4 | $0.418\ [0.237, 0.599]$ | $SE = 0.092,\ Z = 4.52\ (p < .001)$；低水平平台期，长期干预缺乏持续挑战刺激 |
> | **训练周期** · < 3 年 | 5 | $-0.122\ [-0.493, 0.250]$ | $SE = 0.190,\ Z = -0.64\ (p = .522)$；[[Habituation\|习惯化]]定势与测验脱敏，干预效应归零（统计不显著） |
> | **学段层次** · 本科生 | 107 | $0.613\ [0.517, 0.709]$ | $SE = 0.049,\ Z = 12.50\ (p < .001)$；绝大多数实证样本基准，呈现稳健的中等促进效能 |
> | **学段层次** · **研究生** | 2 | **$0.991\ [0.468, 1.515]$** | $SE = 0.267,\ \mathbf{Z = 3.71\ (p < .001)}$；**强增益**，高阶批判思维与深厚专业知识底座赋能（样本量偏小） |
> | **组织形式** · 小组协作 | 61 | $0.567\ [0.462, 0.673]$ | $SE = 0.054,\ Z = 10.54\ (p < .001)$；易受评价顾虑、生产力阻塞与从众心理抑制 |
> | **组织形式** · **独立练习** | 48 | **$0.682\ [0.501, 0.863]$** | $SE = 0.092,\ \mathbf{Z = 7.38\ (p < .001)}$；**个体深思优势**，无外界干扰，深度沉浸于自由发散构思 |
> | **训练方式** · **言语训练** | 15 | **$0.998\ [0.437, 1.560]$** | $SE = 0.286,\ \mathbf{Z = 3.49\ (p < .001)}$；**效应量居首**，语言符号即时外化澄清思维，增强创造自我效能 |
> | **训练方式** · **思维训练** | 13 | **$0.762\ [0.469, 1.055]$** | $SE = 0.149,\ \mathbf{Z = 5.10\ (p < .001)}$；**显著优势**，侧向思维、头脑风暴与元认知监控驱动 |
> | **训练方式** · 任务型训练 | 5 | $0.685\ [0.302, 1.068]$ | $SE = 0.195,\ Z = 3.50\ (p < .001)$；真实工程挑战与 STEAM 跨学科项目驱动 |
> | **训练方式** · 技术型训练 | 20 | $0.552\ [0.314, 0.789]$ | $SE = 0.121,\ Z = 4.55\ (p < .001)$；创新技法工具箱（TRIZ / SCAMPER / 形态分析法） |
> | **训练方式** · 综合训练 | 54 | $0.513\ [0.410, 0.617]$ | $SE = 0.053,\ Z = 9.73\ (p < .001)$；多模态融合覆盖，多要素叠加略微增加认知负荷 |
> | **训练方式** · 情绪训练 | 2 | $-0.066\ [-0.401, 0.269]$ | $SE = 0.171,\ Z = -0.38\ (p = .701)$；非认知调适脱离任务支架，难以自发转化为外显创新产出 |
...
> 2. **教学方式的分化格局** 言语训练（$g = 0.998$）与思维训练（$g = 0.762$）展现出压倒性优势，任务型（$g = 0.685$）、技术型（$g = 0.552$）与综合型（$g = 0.513$）保持中等效果，单纯情绪训练无显著促进（$g = -0.066$）。
> 3. **个体沉思与高阶学段优势** 独立训练（$g = 0.682$）显著高于小组协作（$g = 0.567$）；研究生群体（$g = 0.991$）展现出比本科生（$g = 0.613$）更高的干预收益。（pp.6–7）

> [!spectrum] 训练周期与效应量的倒 U 型时间演变谱系
> **← 短期启动**　·　**长效衰减 →**
>
> - **< 1 周（g = 0.576）** 快速策略启动，但受限于认知内化时间不足
> - **< 2 个月（g = 0.844）** 黄金干预窗口（解释 13.4% 组间方差），认知策略精熟与练习强化达到最优平衡
> - **< 4 个月（g = 0.642）** 维持中高效应，技能巩固与初步迁移
> - **< 1 年（g = 0.233）** 边际效用递减，常规化教学引发动机倦怠
> - **< 2 年（g = 0.418）** 效应有所回升但维持在中低水平
> - **< 3 年（g = -0.122）** 认知定势重构与注意力耗竭，干预效应归零（统计不显著）

#### 实证干预案例与认知机制深度拆解

> [!case] 案例一　言语训练（Verbal Training，$g = 0.998$）
> **效应量统计** $N = 15$ 项效应量，**$g = 0.998$**（95% CI $[0.437, 1.560]$），$SE = 0.286$，$Z = 3.485\ (p < .001)$，居所有训练方式之首。
>
> **代表性实证研究与纳入[[Document|文献]]**
> - **Fink et al. (2015)**（奥地利） 采用[[Alternate Uses Test|另类用途测验]]（AUT），在 $< 2$ 个月周期内通过小组同伴言语观念碰撞、非常规用途即时口头生成与指导语提示开展训练（$EG = 24 / CG = 29$）。
> - **Lewis & Lovatt (2013)**（英国） 采用 AUT，通过 $< 1$ 周的即兴口头叙述、戏剧式言语发散与情境即兴对话展开干预（$EG = 21 / CG = 20$）。
> - **Campion & Levita (2014)**（英国） 结合 [[Torrance Tests of Creative Thinking|TTCT]] 与主观运动体验量表（SEES），在 $< 2$ 个月内通过独立言语表达与反思重构训练，显著提升大学生的创造性言语产出（$EG = 56 / CG = 56$）。
> - **Kienitz et al. (2014)**（美国） 运用 TTCT 图形版与大五人格量表简版（NEO-FFI），在 $< 2$ 个月内通过结构化言语反馈与叙事重组训练，有效提高思维独创性（$EG = 15 / CG = 13$）。
>
> **具体操作规程与干预设计**
> 1. **言语符号即时外化** 要求学生针对日常物品或开放命题进行快速、多向度的口头与书面表述，将头脑中模糊的发散意象即时[[Coding in Qualitative Research|编码]]为明确的语言符号。
> 2. **积极言语反馈与支架提供** 教师与导师在干预过程中给予显式指导语鼓励（Explicit Instructions to be Creative）与积极正向反馈，明确肯定非常规联想。
>
> **深层认知机制拆解**
> - **远距离语义网络激活** 语言符号具有高度的表征灵活性与检索效率，显性言语训练能够迅速解除[[Working Memory|工作记忆]]中的语义抑制，加速远距离概念联想与新颖组合的生成。
> - **[[Creative Self-Efficacy|创造自我效能]]与心理安全感跃升** 积极的言语鼓励和安全平等的言语互动环境直接打破了大学生的评价顾虑（Evaluation Apprehension），显著提升了创造[[Self-Efficacy|自我效能感]]，因而在[[Pre-test and Post-test|后测]]中展现出极其强劲的干预收益。（pp.7–9, 10–11）

> [!case] 案例二　思维训练（Thinking Training，$g = 0.762$）
> **效应量统计** $N = 13$ 项效应量，**$g = 0.762$**（95% CI $[0.469, 1.055]$），$SE = 0.149$，$Z = 5.102\ (p < .001)$，在纯认知策略中展现显著优势。
>
> **代表性实证研究与纳入文献**
> - **Meinel et al. (2019)**（德国） 采用成人托兰斯创造性思维简表（ATTA），开展 $< 2$ 个月的系统设计思维（Design Thinking）与侧向思维（Lateral Thinking）训练（$EG = 59 / CG = 59$）。
> - **Hargrove & Nietfeld (2015)**（美国） 结合[[Remote Associates Test|远距离联想测验]]（RAT）与[[Metacognition|元认知]]意识量表（MAI），在 $< 4$ 个月课程中系统嵌入元认知监控、反思性日志与思维发散演练（$EG = 118 / CG = 88$）。
> - **Onarheim & Friis-Olivarius (2013)**（丹麦/加拿大） 采用 AUT，基于应用神经创造力（Applied NeuroCreativity）理论，开展 $< 2$ 个月的大脑远距离联想抑制解除与发散思维神经认知训练（$EG = 50 / CG = 50$）。
> - **Robbins & Kegley (2010)**（美国） 采用 TTCT，在 $< 4$ 个月内通过[[Mind Mapping|思维导图]]与发散性问题解决训练打破学生的认知固着（$N = 51$）。
> - **Wang & Horng (2002)**（中国台湾） 采用 TTCT，实施长达 $< 1$ 年的系统创造性思维课程教学（$EG = 21 / CG = 18$）。
>
> **具体操作规程与干预设计**
> 1. **思维导图与视觉化拓扑重组** 运用思维导图与[[Concept Mapping|概念图]]将复杂的发散脉络显性可视化，训练多路径搜索策略。
> 2. **侧向思维难题与逆向推理演练** 引入经典侧向思维难题（Lateral Thinking Puzzles），强迫学生跳出常规因果定势，从非常规视角切入问题。
>
> **深层认知机制拆解**
> - **元认知监控与固着解除** 通过[[Metacognition|元认知监控]]，学生能够实时觉察并抑制大脑自动化的陈规偏见，有效克服功能固着（Functional Fixedness）。
> - **注意力广度与远距离概念联结** 系统拓展认知搜索广度，增强对微弱、边缘语义线索的敏锐度，从而持续激发高质量的发散思维产出。（pp.7–9, 10–11）

> [!case] 案例三　任务型与技术型训练（Task-based $g = 0.685$ & Technique-based $g = 0.552$）
> **效应量统计** 任务型训练 $N = 5$ 项效应量，$g = 0.685$（95% CI $[0.302, 1.068]$），$SE = 0.195$，$Z = 3.503\ (p < .001)$；技术型训练 $N = 20$ 项效应量，$g = 0.552$（95% CI $[0.314, 0.789]$），$SE = 0.121$，$Z = 4.550\ (p < .001)$。
>
> **代表性实证研究与纳入文献**
> - **Vally et al. (2019)**（阿联酋） 任务型训练代表；采用 TTCT 测验，在 $< 4$ 个月内依托 STEAM 跨学科项目与真实工程挑战开展任务驱动训练（$EG/CG$）。
> - **Malycha & Maier (2017)**（德国） 技术型训练代表；结合 SEES 与 TTCT，在 $< 1$ 个月内开展小组创新技法工具箱（TRIZ / 形态分析法）训练（$EG = 80 / CG = 80$）。
> - **Morin et al. (2018)**（加拿大） 技术型训练代表；采用创意工程设计评估测验（CEDA），在 $> 1$ 年的工科课程中开展独立工程技法训练（$EG = 59 / CG = 59$）。
> - **Ritter & Mostert (2017)**（荷兰） 技术型训练代表；采用 AUT 与 RAT，开展 $< 1$ 周的 SCAMPER 启发式算子（替换、合并、重组等）密集工作坊（$EG = 31 / CG = 31$）。
> - **Fleury et al. (2020)**（法国） 技术型训练代表；采用生成另类用途测验（GAU）与 RAT，开展 $< 1$ 周的视觉联想暗示技术训练（$EG = 16 / CG = 16$）。
> - **Puccio et al. (2020)**（加拿大） 技术型训练代表；采用 TTCT，开展长达 $< 3$ 年的创造性问题解决（CPS）规范流程训练（$EG = 11 / CG = 10$）。
>
> **具体操作规程与干预设计**
> 1. **任务型——在真实造物中发散** 将学生置于真实的工程原型设计、跨学科设计工作坊或复杂社会问题情境中，以可交付的实体成果驱动创造性解决方案。
...
> - **认知迁移阻力（Transfer Limitations）** 技术型过于依赖机械化规则套用，易导致学生产生“工具依从性”，在脱离特定技术提示的标准创造力测验中，其远迁移（Far Transfer）效果略低于纯粹的思维发散训练。（pp.7–9, 10–11）

> [!case] 案例四　多模态综合训练（Combined Training，$g = 0.513$）
> **效应量统计** $N = 54$ 项效应量（占纳入样本半数），**$g = 0.513$**（95% CI $[0.410, 0.617]$），$SE = 0.053$，$Z = 9.728\ (p < .001)$，呈现极其稳健的中等促进效果。
>
> **代表性实证研究与纳入文献**
> - **Gu et al. (2023)**（中国） 采用 AUT 测验，在大学开展 $< 4$ 个月的综合创新创业课程（整合课堂讲授、头脑风暴、角色扮演与跨学科小组研讨）（$EG = 41 / CG = 39$）。
> - **Baruah & Paulus (2008)**（美国） 采用视觉隐喻原创性量表（OSVM），在 $< 1$ 周内融合个体构思与合作式小组头脑风暴（$EG = 11 / CG = 11$）。
> - **Karwowski & Soszynski (2008)**（波兰） 采用创造性意象测验（TCI）与发散性图形绘画测验（TCT-DP），在 $< 2$ 个月内综合运用意象演练、启发式教学与视觉拓扑重构（$N = 15$）。
> - **Zampetakis et al. (2008)**（希腊） 采用 OSVM，开展 $< 4$ 个月的大学创业教育综合创造力培养（$EG = 52 / CG = 42$）。
> - **Lubart (2014)**（法国） 采用创造潜能评估测验（EPOC），在 $< 1$ 周内整合多种发散认知与艺术创作任务（$EG = 63 / CG = 63$）。
> - **West et al. (2012)**（美国） 采用 TTCT，开展 $< 1$ 年的多模态综合课程干预（$EG = 93 / CG = 93$）。
> - **Zitek & Vincent (2015)**（美国） 采用 RAT，开展 $< 1$ 个月的综合创新训练（$EG = 97 / CG = 97$）。
> - **Oppezzo & Schwartz (2014)**（美国） 采用 AUT，结合身体漫步（Walking）与认知发散综合干预（$E = 48 / C = 48$）。
> - **Dow & Mayer (2004)**（美国） 采用 OSVM，开展 $< 1$ 个月的空间、言语与逻辑综合训练（$EG = 11 / CG = 11$）。
> - **Stolaki & Economides (2018)**（希腊） 采用 TTCT，开展 $< 1$ 个月的数字化协作综合训练（$N = 81$）。
>
> **具体操作规程与干预设计**
...
> - **认知负荷分散与稀释效应** 由于干预要素过于庞杂，学生的注意力焦点容易被频繁切换的活动分散，产生额外的认知负荷干扰，导致其在特定认知子维度上的增益峰值（$g = 0.513$）低于高度聚焦的单项言语（$g = 0.998$）或思维训练（$g = 0.762$）。（pp.7–9, 10–11）

> [!case] 案例五　情绪训练的局限（Emotional Training，$g = -0.066$）
> **效应量统计** $N = 2$ 项效应量，**$g = -0.066$**（95% CI $[-0.401, 0.269]$），$SE = 0.171$，$Z = -0.384\ (p = .701)$，在统计上不显著且效应量接近于零。
>
> **代表性实证研究与纳入文献**
> - **Mahasneh & Gazo (2019)**（约旦） 采用情绪创造力量表（ECI），开展 $< 1$ 周的纯情绪觉察、情感表达与正向心理调适训练（$EG = 35 / CG = 32$）。结果表明，相较于对照组，干预未对大学生的显性创造力指标带来显著提升。
>
> **具体操作规程与干预设计**
> 1. **非认知情绪调适** 主要依托正念冥想、情绪唤醒、正向心理暗示及情绪感知体验分享，试图通过营造积极情绪氛围激发创造灵感。
> 2. **脱离认知与任务支架** 教学过程中未提供具体的发散思维策略、逻辑重组算子或明确的问题解决任务载体。
>
> **深层认知机制拆解**
> - **情绪改善与创造产出的割裂** 心理学证据表明，虽然积极情绪有助于拓展注意力范围，但情绪调节本身并不等同于认知加工能力。缺乏显性的发散策略和思维脚手架时，良好的情绪体验无法自发转化为结构化的创新观念或问题解决方案。
> - **测验敏锐度不足** 标准化的创造力测验（如发散思维与观念独创性指标）高度依赖认知重组与言语/图形表征，对纯粹的情绪状态变化不敏感，因而在实证评估中呈现出无显著效应。（pp.7–9, 10–11）

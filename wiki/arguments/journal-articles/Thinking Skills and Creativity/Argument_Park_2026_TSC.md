---
summary: "基于29项研究51个效应量的三水平元分析，估计学生样本中创造力与批判性思维的整体相关为中等正相关（r=0.386），并发现测量类型显著调节相关大小，双主观测量组合相关最高"
type: argument
authors:
  - "Park, J. H."
  - "Myszkowski, N."
  - "Niu, W."
source_language: en
citation: "Park, J. H., Myszkowski, N., & Niu, W. (2026). Exploring the intersection of creativity and critical thinking among students: A meta-analysis. Thinking Skills and Creativity, 60, 102119. https://doi.org/10.1016/j.tsc.2025.102119"
year: 2026
...
sources:
  - "[[sources/Park_2026_TSC/Park_2026_TSC|Park_2026_TSC]]"
part_of: ""
status: draft
created: 2026-08-25
updated: 2026-08-25
...
> [!concept-lens] 阅读透镜
> - **对象** 29 项同时测量学生创造力与批判性思维并报告两者相关的研究，共 51 个相关系数，总样本 $N = 12{,}548$，覆盖北美、东亚、中东与其他国家，学段以大学生为主。
> - **张力** 创造力与批判性思维被概念化为非预期与非传统对理性、有目的、基于标准的思维风格，表面上对立；理论上又共享评价、判断与自我调节等认知成分，且测量方式（主观自陈 vs 客观任务表现）可能改变观察到的相关强度。
> - **贡献** 以[[Three-Level Meta-Analysis|三水平元分析]]将两者关系从定性的概念讨论推进到可检验的总体[[Effect Size|效应量]]，首次系统检验测量类型、学段、年龄、地理区域与发表年份等调节[[Variable|变量]]，并区分自陈测量污染与表现型测量的真实认知重叠，为联合培养与联合评估提供量化基础。

> [!assumptions] [[Hypothesis|研究假设]]
> 依据概念重叠理论与先前综述（Park et al., 2023），提出四项可检验假设（p. 3）：
> 1. **总体正相关** 元分析将显示创造力与批判性思维呈正相关。
> 2. **显著[[Heterogeneity|异质性]]** 报告效应在跨研究间存在显著异质性。
> 3. **学段调节** 学段显著调节相关大小，关系随学生发展水平（中学至大学）增强。
> 4. **测量类型调节** 测量类型解释部分异质性：两构念均用主观（自陈）测量时相关更高，推测源于社会期望与自尊等共同污染因素。
> 此外在方法部分检验两测量类型的交互项，预期同型组合相关更高。
...
> - **证据单位** 51 个相关估计（$k = 51$）嵌套于 29 项研究（同一研究可贡献多个估计），总样本 $N = 12{,}548$。

> [!search-protocol] 检索与纳入方案
> - **数据库** PsycINFO、Academic Search Ultimate、ProQuest、ScienceDirect。
> - **关键词** "creativity"、"creative"、"creativ*" 与 "critical thinking"，使用高级检索排除书籍章节、综述与非同行评审[[Document|文献]]。
> - **纳入** 同行评审期刊论文与公开学位论文；须同时包含创造力与批判性思维两种测量、报告两者定量相关、且样本为学生。
> - **文献量** 在 Park et al. (2023) 综述数据基础上新增 16 项发表、剔除 4 项不合标准的研究，最终纳入 29 项发表。（p. 4）
...
> - **权重** 抽样方差倒数（大样本贡献更精确的估计）（p. 4）。
> - **模型** 三水平随机效应[[Meta-analysis|元分析]]：Level 1 [[Sampling Error|抽样误差]]、Level 2 研究内方差、Level 3 [[Between-Study Variance|研究间方差]]（p. 4）。
> - **异质性** $I^2$ 与 $Q$ 检验（p. 4）。
...
> | 样本层面 | 构成 |
> |----------|------|
> | **效应量样本** | 51 个相关估计（$k = 51$），嵌套于 29 项研究，总样本 $N = 12{,}548$。（p. 4） |
> | **研究嵌套来源** | 同一参与者施测不同测量（Gilbert, 1961; Guan et al., 2024; Komsu, 2021; Long & Long, 2023; Lu et al., 2024; Misset, 2012; Qiang et al., 2020; Runco & Smith, 1992 等）或同一测量用于不同参与者（Brubaker, 1971; Li et al., 2022; Shaber et al., 2025; Silva et al., 2023; Yurt, 2025 等）。（p. 4） |
> | **地理与学段构成** | 样本覆盖北美、东亚、中东与其他国家；学段以大学生为主，含部分中学/高中与研究生。（pp. 5–6） |
...
> ```mermaid
> flowchart LR
>     A["两种21世纪关键技能关系研究稀缺"] --> B["两构念概念上表面对立但共享认知成分"]
>     B --> C["先前系统综述存在五类方法学缺口"]
>     C --> D["三水平元分析 + RVE + 元回归 + 偏倚检验"]
>     D --> E["总体中等正相关 r = 0.386，高异质性，无显著发表偏倚"]
>     D --> F["测量类型显著调节：双主观 (0.509) > 双客观 (0.317) > 混合 (0.195–0.260)"]
>     E --> G["支持构念重叠与互补高阶认知的理论主张"]
>     F --> H["共同方法变异 + 表现型测量的真实认知重叠"]
>     G --> I["教育协同培养、创造力对批判性思维的预测效度"]
>     H --> I
>     I --> J["未来需要个体内/纵向研究验证方向性"]
> ```

---

### 论证步骤一　研究背景：两种技能被广泛强调，关系研究却稀缺

> [!claim] [[Creativity|创造力]]与[[Critical Thinking|批判性思维]]被机构、经济界与学界共同视为关键技能，但两者关系的直接实证研究长期稀缺
> 教育机构、心理学会与经济论坛均强调两种技能的发展价值，批判性思维研究在 2000–2021 年间持续增长；人工智能对人工创造与人工思考的争论进一步抬高研究兴趣；然而直接考察两者关系的实证研究很少，构成明确的知识缺口。（pp. 1–2）

> [!chain-link] 证据到判断
> - 全美教育协会把创造力与批判性思维列为教育者应帮助学生发展的 21 世纪核心技能（National Education Association, 2012）。（p. 1）
> - 美国心理学会与世界经济论坛强调两者的发展价值与经济意义（American Psychological Association, 2016; Di Battista et al., 2023）。（p. 1）
> - 批判性思维研究的[[Document|文献]]计量综述显示 2000–2021 年间相关发表稳步增长（Dong et al., 2023）；近期综述把两者定位为 21 世纪技能的核心成分（Thornhill-Miller et al., 2023）。（pp. 1–2）
> - 人工智能与人工创造、人工思考的争论强化了研究这两种人类独特能力的需求（Partnership for 21st Century Skills, 2009; Runco, 2023; Spector & Ma, 2019）。（pp. 1–2）
> - 尽管兴趣高涨，两者关系的直接实证研究依然稀缺（Runco, 2003; Runco & Chand, 1994）。（p. 2）

> [!warrant]- 推理桥梁
> 机构、经济与学术三股力量共同抬升两者的重要性，而关系证据的稀缺构成明显的知识缺口，这为以[[Meta-analysis|元分析]]系统综合两者相关提供了研究动机。（pp. 1–2）

---

### 论证步骤二　概念界定：创造力与批判性思维表面对立

> [!claim] 两个[[Construct|构念]]的定义传统上呈现发散生成对收敛评价的表面对立
> 创造力共识定义强调新颖且有用，批判性思维强调以理由为基础的反思判断与自我调节；两者被描述为非预期与非传统对理性、有目的、基于标准的思维风格，这种对立构成理解它们如何共存的概念障碍。（pp. 1–2）

> [!chain-link] 证据到判断
> - 创造力定义在 Guilford 1950 年 APA 主席演讲后被持续研究，学界共识为特定领域中既新颖又有用的观念或产品（Amabile, 2019; Kaufman & Beghetto, 2009; Stein, 1953; Sternberg & Lubart, 1998）。（p. 2）
> - 批判性思维从哲学、教育扩展到心理学的科学构念，涵盖态度、知识、技能与倾向，以及质疑、分析、解释、评价、判断与辨识信息的能力（Glaser, 1941）。（p. 2）
> - 自我调节是批判性思维的本质特征：需要控制、维持与管理自己的思想、情感与行为以达成目标（Dwyer, 2017）。（p. 2）
> - 两个构念均具[[Domain Specificity|领域特殊性]]：心理批判性思维强调按心理科学基本原则评价信息（Lawson, 1999, 2015）；护理批判性思维强调为改善病人照护而进行的批判分析与条件识别（Alfaro-LeFevre, 1999; Bandman & Bandman, 1988; Papathanasiou et al., 2014）。（p. 2）
> - 表面对立构成理解两者在问题解决与创新中如何共存的挑战（Ennis, 1987; Sternberg & Halpern, 2020）。（p. 2）
...
> [!warrant]- 推理桥梁
> 表面对立恰恰是研究两者关系的动机：若它们共享认知基础，则对立只是概念风格的表象；实证相关是检验对立是否可调和的途径。（pp. 1–2）
...
> [!chain-link] 证据到判断
> - 共享的认知与元认知成分：评价、判断、自我调节要求灵活而目标导向的推理（Halpern, 2003; Sternberg & Halpern, 2020）。（p. 2）
> - 共同的执行功能基础：抑制、认知灵活性、工作记忆（Benedek et al., 2012, 2014）。（pp. 2–3）
> - 复杂问题解决中的互补阶段：创造力生成可能性，批判性思维负责精炼与选择（Cropley, 2006; Sternberg & Lubart, 1998）。（p. 3）
> - 这种概念重叠为观察到的实证相关提供理论依据，并预示教育项目可联合培养两者。（p. 3）

> [!warrant]- 推理桥梁
> 共享成分说明两者应呈正相关，互补关系又说明两者不应完全相同，最优预期是显著且大小适中的正相关，且测量方式（自陈 vs 任务表现）可能改变观察到的相关强度。（pp. 2–3）

---

### 论证步骤四　文献缺口：Park et al. (2023) 综述的五项方法学不足

> [!claim] Park et al. (2023) 的[[Systematic Review|系统综述]]存在五项方法学缺口，导致总体相关大小仍不确定
> 该综述仅用单一数据库（PsycINFO）、关键词覆盖有限、未分析[[Publication Bias|发表偏倚]]、总体相关基于未按[[Sample Size Determination|样本量]]加权的简单平均、未处理嵌套[[Effect Size|效应量]]、未进行调节[[Variable|变量]]分析。（pp. 2–3）

> [!chain-link] 证据到判断
> - **检索局限** 仅依赖 PsycINFO 单一数据库，关键词限于 critical thinking、creative thinking 与 creativity，可能限制综述的包容性。（pp. 2–3）
> - **无偏倚分析** 未分析发表偏倚及其对报告相关的影响。（p. 3）
> - **简单平均** 未考虑纳入研究的样本量，总体相关大小不确定。（p. 3）
> - **嵌套忽略** 部分研究包含嵌套效应量，需要[[Three-Level Meta-Analysis|三水平元分析]]。（p. 3）
> - **调节缺失** 综述提示调节变量存在，却未提供[[Meta-regression|元回归]]分析。（p. 3）

> [!warrant]- 推理桥梁
> 五项缺口逐一对应成熟的元分析方法（元分析、三水平模型、[[Robust Variance Estimation|RVE]]、元回归、偏倚检验），为采用更严格方法提供正当性；这为以元分析与元回归补足先前综述无法回答的问题提供依据。（pp. 2–3）

---

### 论证步骤五　方法设计：三水平元分析、RVE、元回归与偏倚检验系统补足缺口

> [!claim] 采用三水平元分析处理嵌套效应量、RVE 稳健推断、元回归检验调节变量、Egger 回归检验偏倚，可系统补足先前综述的缺口
> 51 个相关估计嵌套于 29 项研究，不满足独立性[[Hypothesis|假设]]，故用三水平随机效应元分析；以 Fisher's $z$ 变换稳定方差，以[[Inverse-Variance Weighting|逆方差加权]]，以 $I^2$/$Q$ 量化[[Heterogeneity|异质性]]；用 RVE 提供小样本稳健[[Standard Error|标准误]]；用元回归检验调节变量及交互项；用 Egger 回归与等高线[[Funnel Plot|漏斗图]]检验偏倚。（pp. 4–5）

> [!chain-link] 证据到判断
> - 检索覆盖 PsycINFO、Academic Search Ultimate、ProQuest 与 ScienceDirect，关键词 "creativity"、"creative"、"creativ*"、"critical thinking"，排除书章节/综述/非同行评审文献，纳入同行评审期刊论文与公开学位论文；纳入标准为同时含两种测量、报告定量相关、学生样本。（p. 4）
> - 在 Park et al. (2023) 基础上新增 16 项、剔除 4 项，最终纳入 29 项发表、51 个相关估计、总样本 $N = 12{,}548$。（p. 4）
> - 测量按主观（自陈/自评）与客观（观念/产品产出或任务表现）[[Coding in Qualitative Research|编码]]；提取年龄、学段、年份、地理区域四个调节变量。（pp. 4–5）
...
### 论证步骤六　总体效应：中等正相关、高异质性、无显著发表偏倚

> [!claim] 学生样本中创造力与批判性思维的总体相关为中等正相关，且对研究内依赖稳健
> 总体相关 $r = 0.386$，95% CI $[0.296, 0.470]$，$z = 7.81$，$p < 0.001$；RVE 敏感性分析得一致估计；未检出显著发表偏倚。（pp. 6–7）

> [!chain-link] 证据到判断
> - 总体相关 $r = 0.386$，95% CI $[0.296, 0.470]$，$z = 7.81$，$p < 0.001$（图 2 森林图）。（p. 6）
> - 异质性极高且显著：$I^2 = 95.6\%$，95% CI $[94.8\%, 96.2\%]$，$Q(50) = 1133.19$，$p < 0.001$，支持后续调节分析。（p. 6）
> - RVE 敏感性分析：$r = 0.386$，95% CI $[0.292, 0.473]$，$t(27.81) = 7.81$，$p < .001$，结果对研究内依赖稳健。（p. 6）
> - Egger 回归：$t(149) = 0.29$，$p = 0.771$，无显著漏斗图不对称；等高线增强漏斗图对称（图 3）。（pp. 6–7）
...
> ![](https://img.mylikemie.icu/sources/Park_2026_TSC/figures/Park_2026_TSC_Fig2_Forest_Plot.jpg)

> [!effect-summary] 总体效应与异质性摘要
> | 证据规模 | 效应指标与模型 | 汇总效应与 95% CI | 异质性 | 稳健性 |
> |---|---|---|---|---|
> | $k = 51$（29 项研究）<br>$N = 12{,}548$ | 皮尔逊 $r$（Fisher's $z$）/ 三水平随机效应 | $r = 0.386$ $[0.296, 0.470]$，$z = 7.81$，$p < .001$ | $I^2 = 95.6\%$ $[94.8\%, 96.2\%]$；$Q(50) = 1133.19$，$p < .001$ | RVE：$r = 0.386$ $[0.292, 0.473]$，$t(27.81) = 7.81$ |

> [!example]- 图3：等高线增强漏斗图（Contour-Enhanced Funnel Plot）
> ![](https://img.mylikemie.icu/sources/Park_2026_TSC/figures/Park_2026_TSC_Fig3_Contour_Enhanced_Funnel_Plot.jpg)

> [!bias-audit] 发表偏倚审计
> - **Egger 回归** $t(149) = 0.29$，$p = 0.771$，无显著漏斗图不对称（图 3）。
> - **等高线增强漏斗图** 对称，未显示缺失非显著研究所致的偏倚模式。
> - **边界** 非显著的漏斗图不对称检验不证明发表偏倚不存在。

> [!warrant]- 推理桥梁
> 中等正相关把先前定性的概念讨论转化为可检验的总体证据，支持两个构念是互补的高阶认知而非独立领域；高异质性提示测量与样本特征会改变相关，为调节分析提供正当性；无显著发表偏倚则增强结论可[[Reliability|信度]]。（pp. 6–9）
...
> [!chain-link] 证据到判断
> - 双主观组合合并相关 $r = 0.509$，双客观组合 $r = 0.317$，创造力客观×批判性思维主观 $r = 0.260$，创造力主观×批判性思维客观 $r = 0.195$（图 4）。（p. 9）
> - 年龄整体不显著（$Q_M(1) = 1.203$，$p = .273$），仅在控制学段或地理区域时显著（相关随年龄下降）。（p. 7）
> - 学段整体不显著（$Q_M(1) = 0.875$，$p = .350$），仅在控制年龄时显著（大学生高于中学/高中生）。（pp. 7–8）
> - 发表年份边际显著（$Q_M(1) = 3.707$，$p = .054$，相关随年份上升），各控制下不再稳健。（p. 8）
> - 地理区域不显著（$Q_M(3) = 2.270$，$p = .518$），仅在控制年龄时东亚低于其他区域。（p. 8）
...
> | 学段 | $Q_M(1) = 0.875$，$p = .350$ | — | 整体不显著 |
> | 发表年份 | $Q_M(1) = 3.707$，$p = .054$ | 相关随年份上升 | 边际显著 |
> | 地理区域 | $Q_M(3) = 2.270$，$p = .518$ | — | 不显著 |

> [!warrant]- 推理桥梁
> 同型测量组合相关更高，提示方法相似性贡献观察相关；双主观最高符合自陈测量共享作答倾向（社会期望、默许、自尊）所致的[[Common Method Variance|共同方法变异]]；双客观仍高于混合组合，则说明表现型测量之间存在真实认知重叠。（pp. 8–9）

---

### 论证步骤八　解释：共同方法变异与表现型测量的真实认知重叠

> [!claim] 测量类型效应有两种互补解释：自陈测量的共同方法变异使相关虚高，表现型测量之间则存在真实共享认知过程
> 双主观相关虚高可由社会期望、默许、自尊等共享作答倾向解释；双客观相关仍显著高于混合组合，说明推理、问题解决与联想灵活性等共享认知过程构成两构念的真实连接。（pp. 8–10）

> [!line-a] 解释一：自陈测量的共同方法变异
> 双主观（双自陈）组合相关虚高（$r = 0.509$），可由社会期望、默许、自尊等共享作答倾向同时抬高两构念自评来解释（Podsakoff et al., 2003）。（p. 9）

> [!line-b] 解释二：表现型测量的真实认知重叠
> 客观测量（如非常规用途测验与沃森-格拉泽批判性思维评价）都依赖推理、问题解决与联想灵活性，故双客观组合相关（$r = 0.317$）仍显著高于全部混合组合；混合组合相关最低，因同时包含主观自评与外部评定任务两个不同方差来源；思维型创造力测量（远距离联想、[[Divergent Thinking|发散思维]]任务）与分析推理、联想流畅性高度一致（Mednick, 1962; Silvia et al., 2013），产品型评定（如[[Consensual Assessment Technique|同感评估技术]]）则使用不同评价标准。（pp. 9–10）

> [!warrant]- 推理桥梁
> 双重解释共同说明：两构念既非完全独立也非完全相同，自陈层面有方法污染，表现层面有真实认知共享；这为联合培养提供机制基础，也为测量设计提出方法学警告：未来研究应以表现型或行为指标对自陈测量进行[[Triangulation|三角验证]]。（pp. 8–10）

---

### 论证步骤九　含义与边界：协同培养、预测效度与个体内验证

> [!claim] 两构念的共享基础支持协同培养与创造力[[Predictive Validity|预测效度]]，但方向性仍需个体内纵向证据
> 学校不必在创造性与批判性之间二选一；创造力可能预测批判性思维，反之批判性思维训练也可能带动创造力；但当前相关基于个体间数据，不能直接推及个体内共同变化。（pp. 9–10）

> [!chain-link] 证据到判断
> - 创造力对批判性思维具有某种[[Predictive Validity|预测效度]]，创造性成就测量（直接或代理）可能有助于学术选拔情境。（p. 10）
> - [[Creative Problem Solving|创造性问题解决]]模型把批判性评价视为创造力的必要阶段（Isaksen & Treffinger, 2004），批判性思维模型则强调创造性[[Hypothesis|假设]]生成是反思判断的一部分（Kuhn, 1999），两构念边界比传统假设更可渗透。（p. 10）
> - 同时培养创造性构想与评价性推理的教学可能比单技能训练产生更广认知收益；可采用开放式任务先发散生成、再结构化反思、批评与修正的教学序列（[[Argument_Abrami_2015_RER|Abrami et al., 2015]]）；设计思维、探究式项目与[[Socrates|苏格拉底]]式提问是兼顾两者的课堂实践。（p. 10）
> - 创造性训练（发散任务、头脑风暴）可能附带提升批判性思维；反之以提升批判性思维为目标的[[Effective Teaching|有效教学]]（[[Argument_Abrami_2015_RER|Abrami et al., 2015]]）也可能附带发展创造力。（p. 10）
> - 相关基于个体间变异（Molenaar, 2004），需纵向与交叉滞后面板分析（Kenny, 1975）验证方向性。（p. 10）
...
> [!finding-cards] 核心发现
> 1. **[[Creativity|创造力]]与[[Critical Thinking|批判性思维]]呈显著中等正相关** [[Three-Level Meta-Analysis|三水平元分析]]综合 29 项学生研究 51 个相关系数（$N = 12{,}548$），总体相关 $r = 0.386$（95% CI $[0.296, 0.470]$，$p < .001$），RVE 敏感性分析结果一致。（pp. 6–7）
> 2. **测量类型显著调节相关，双主观组合最高** 双主观测量组合 $r = 0.509$，双客观 $r = 0.317$，混合组合 $r = 0.195$–$0.260$；测量类型与交互项均显著，同型测量放大观察相关。（pp. 8–9）
> 3. **年龄、学段、地理区域与发表年份缺乏稳健调节作用** 这些[[Variable|变量]]仅在特定控制模型下偶达显著，不能可靠解释相关变异。（pp. 7–8）
> 4. **未检出显著[[Publication Bias|发表偏倚]]** 艾格回归（$p = 0.771$）与等高线增强[[Funnel Plot|漏斗图]]均未显示偏倚，但非显著检验不证明不存在偏倚。（pp. 6–7）

> [!stat-cards]- 核心数据
> - **$r = 0.386$** 总体相关（95% CI $[0.296, 0.470]$，$z = 7.81$，$p < .001$，$k = 51$，$N = 12{,}548$）。（p. 6）
> - **$I^2 = 95.6\%$** 总[[Heterogeneity|异质性]]（95% CI $[94.8\%, 96.2\%]$，$Q(50) = 1133.19$，$p < .001$）。（p. 6）
> - **$r = 0.509$** 双主观测量组合相关（双客观 $r = 0.317$；创造力客观×批判性思维主观 $r = 0.260$；创造力主观×批判性思维客观 $r = 0.195$）。（p. 9）
> - **$Q_M(1) = 6.524$** 测量类型交互项检验（$p = .011$）。（p. 8）

---

## 关键引用

> [!citation-card]- 总体相关的核心结论
> 尽管显著[[Heterogeneity|异质性]]存在，但正如所[[Hypothesis|假设]]的，我们发现的总体相关估计明确落在中等范围且显著为正，表明两个[[Construct|构念]]的测量之间存在某种重叠。（p. 9）
>
...
> [!citation-card]- 协同培养与个体内边界
> 学校层面，教师与课程设计者可采用开放式任务先发散生成、再结构化反思、批评与修正，同时培育创造性构想与评价性推理；但当前关系在个体间层面检验，其强度不一定迁移到个体内层面，因此训练其一带动其二仍需纵向证据。（pp. 9–10）
>
> *Instruction that simultaneously cultivates creative ideation and evaluative reasoning may yield broader cognitive benefits than programs targeting either skill in isolation... the relationship between the two constructs is investigated at the between-individual level, and this relationship (or its strength) may not translate to the within-individual level.*

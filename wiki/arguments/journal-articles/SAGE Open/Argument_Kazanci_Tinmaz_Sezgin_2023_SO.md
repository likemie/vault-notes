...
> [!claim] 步骤二主张
> 在全新独立样本中，竞争模型选拔证实单因子模型严重失效，而二阶高阶因子模型拟合优异，证实四维度归属于一个统摄性的全局研究素养构念；量表收敛效度、区分效度与复合信度体系完备；跨性别严格测量等值性的确立为后续无偏跨群体比较奠定了坚实基础。（pp.7–12）
...
> - **信度指标卓越** 总量表 [[Cronbach's Alpha|Cronbach's $\alpha = .94$]]、[[McDonald's Omega|McDonald's $\omega = .95$]]、[[Split-Half Reliability|折半信度]] $= .91$；各分量表 $\alpha$ 介于 $.83 \sim .90$，$\omega$ 介于 $.75 \sim .90$，[[Composite Reliability|组合信度（CR）]]介于 $.75 \sim .90$。（p. 11）
...
> [!ref-table]- 表5：跨性别多组验证性因子分析测量等值性检验（N=258）
>
...
> [!finding-cards] 核心发现
> 1. **成功开发出 20 题 4 维度的[[Research Literacy Scale for Teachers|教师研究素养量表]]（RLS）** 确立了包含研究意识（4 题）、态度（3 题）、技能（6 题）与使用（7 题）的二阶模型，具备优良的信效度（[[Cronbach's Alpha|$\alpha=.94$]], [[McDonald's Omega|$\omega=.95$]]）与跨性别严格[[Measurement Invariance|测量等值性]]。（p.9, p.11）
...
> - **$\alpha = .94, \omega = .95$** 20 题 RLS 总量表的 [[Cronbach's Alpha|Cronbach's $\alpha$]] 与 [[McDonald's Omega|McDonald's $\omega$]] 复合[[Reliability|信度]]。（p.11）

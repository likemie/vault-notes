...
> **离群值缩尾处理** Grubbs' (1950) 检验证实 Kuhn et al. (2017) 的效应量（$d = 3.2$）属于极端离群值，按惯例缩尾（Winsorization）至数据集中次大效应量 $d = 2.6$，以避免单项极大值扭曲整体分布，同时保留真实研究。
>
> **漏斗图诊断**
>
...
> [!result-reading] 效应量的教育意义
> 随机效应模型下的平均效应量为 $d = 0.509$（$95\% \text{ CI } [0.326, 0.692], p < .001$）。根据 Hattie (2015) 对大量教育干预研究的综述，常规教育教学措施的平均效果约为 $d = 0.40$。认识论认知干预不仅超越了这一基准线，而且在 Hattie 汇总的 195 类教学措施中位列前 50 位。这意味着接受认识论指导的学生，平均学业表现超过了对照组约 69% 的学生。（p. 485）

#### 2. 考虑多重测量相关性后因果促进效应依然高度稳健

> [!evidence-grid] 多重测量相关性敏感性检验
> - **合并计算（保守假设）** 假定同一研究内各测验高度相关，先平均为单一指标（$k = 28$），得出随机效应 $d = 0.509$。
> - **完全独立计算（宽松假设）** 假定所有测验相互独立，直接合并 59 个效应量，得出随机效应 $d = 0.495$（$p < .001$）。

两种算法结果仅相差 $0.014$，显著性完全一致，表明数据间的相关性并未干扰总体因果结论的有效性。（p. 484）
...
> - **信源评估** Mason et al. (2014) 在中学生阅读冲突性网络健康材料时，直接讲授鉴别网络信息的操作准则——查看作者专业资质、出版机构商业利益倾向、结论是否有数据支撑。学生学会这套规则后迅速摆脱盲信盲从，跨文本理解能力显著提升；Barzilai & Ka'adan (2017) 也发现教授信息整合策略能有效提高学生权衡不同观点的能力。
...
> [!result-reading] 理论框架的实证分野
> 教学干预如果精准瞄准知识如何确证和证据如何检验等核心环节（例如 Kammerer et al., 2015; Mason et al., 2014），效果最为理想；如果仅泛泛强调学习需要坚持、知识由个人自主建构等宽泛信念，却缺乏可操作的证据处理方法，学生很难在具体学科考试中获得实质提升。
...
> [!case] 代表研究：测验敏感度的分野
> - **论证与概念转变** Kuhn et al. (2017) 的论证评估中，接受认识论干预的学生在陈述论点、搜集反驳理由以及协调多元观点方面表现出跨越式进步；Barger et al. (2018) 的大学化学测验中，认识论的转变帮助学生主动推翻朴素日常直觉，完成高难度的化学概念重塑。
> - **机械程序与记忆任务** 一般常规小测验（如 Osborne et al., 2013 中考查知识点回忆的普通测验）以及程序性操作测验（如 Schiefer et al., 2017 中单纯按操作规程执行的测试，$d = 0.140$）并未因认识论提升而改善。（pp. 488, 492）
...
> [!result-reading] 反常数据的方法学解读
> 未对齐测验表现反常偏高并非教学悖论，而是测量工具性质所致（以 Lam & Chan, 2008 以及 Shekoyan, 2009 为代表）：未对齐研究通常采用学科大领域的认知干预，但测验却选用了针对特定主题、设计精巧的自编概念测验，这类测验专为检验深度理解而设计，敏锐捕捉到了论证逻辑和概念的转变。相反，学校常规期末考和外部标准化统考大多考查现成公式与[[Rote Learning|死记硬背]]，无法测出深层思维变化。（pp. 488, 492）
...
> [!result-reading] 延时后测效应翻倍的内化机制
> 在追踪了后续表现的 5 项研究中（如 Barzilai & Ka'adan, 2017; Hefter et al., 2015; Huang et al., 2017），学生在干预结束一段时间后的表现比刚学完时更为优异。认识论不是机械背诵的知识点，而是一种思维习惯；干预结束后，学生在后续自主学习中继续运用审视证据与推敲可信度的方法，并在即时测试反馈的辅助下完成内化吸收，从而在延时测验中展现出更强的思维后劲。（pp. 491, 493）
...
> [!result-reading] 中小学生更易获益的认知发展解释
> 中小学生获益更明显与 [[Argument_Greene_2018_JEP|Greene et al. (2018)]] 的元分析发现高度一致：大学生经过十余年应试训练与学科专门化，往往已形成较为固定的思维套路和认知定势，改变阻力较大；而中小学生正处于认知信念剧烈塑形的萌芽期与敏感期，思维开放度高，一旦获得合理的证据确证指导，就能迅速完成[[Ways of Knowing|认知方式]]的升级。（pp. 492–493）
...
> [!result-reading] 推广可行性的方法学确认
> 方法学检验确立了认识论干预在中小学推广的可行性：学校不必依赖校外专家的长期驻场，一线在职科任教师经过常规培训就能独立胜任教学；而借助智能学习平台中的反思提示与自适应支架，同样能获得显著收益。自然教学班中开展的准实验与严密的随机实验取得了近乎相同的提升效果，排除了选择性偏差导致效果虚高的担忧。（pp. 489–492）

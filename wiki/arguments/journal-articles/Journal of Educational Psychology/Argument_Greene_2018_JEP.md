...
> [!claim] 测量工具的情境“对齐（Alignment）”能显著放大预测效应
> 认识论认知并非放之四海而皆准的恒定特质。当问卷测量的“知识颗粒度”或“学科领域”与最终考核成绩的标尺完全对齐匹配时，这种信念对成绩的预测力才会最强。
>
...
>
> [!chain-link] 证据到判断
> - **颗粒度下沉的优势** 针对具体领域的（Domain-specific）测量的整体效应量（$r = .184$）本就高于宽泛的领域通用（Domain-general, $r = .140$）测量。
> - **双重对齐的红利**
>   - 在**颗粒度完全对齐**的研究中，效应量（$r = .176$）显著高于颗粒度错配的混合使用（$r = .130$）。
>   - 在**学科领域完全对齐**的研究中，效应量（$r = .179$）同样压倒了未对齐的研究（$r = .111$）。
> - **科学学科的特殊性** 在跨学科对比中，科学（Science, $r = .199$）和数学（Mathematics, $r = .157$）领域的效应最为稳健，这可能因为这些学科对知识的确证规则有着更为明确的界定。

...
- **政策借用视角**：论文虽未明确使用该术语，但其核心问题是典型的政策借用分析——西方教育创新（澳大利亚、新西兰、美国、丹麦已采用）是否能且应被转移至俄罗斯？论文的结论（"inapplicable"）构成对不加批判的政策借用的警告（p.240-241）
...
- **方法 2：变差系数统计检验**（variation coefficient analysis）— 对 Hattie 元分析中使用的 133 个（正值）效应量计算变差系数 Vσ，检验其数据集的同质性（Golovchin, 2019, p.238-239）。作者声称这是前人未做过的原创分析（"which has not been done before", p.229）
...
Hattie 的方法本质上是**元-元分析**（mega-analysis）：不是综合原始实证研究，而是综合已有的元分析。涵盖 6 个影响源（家庭、学校、教师、课程、教学策略与方法、学生）、138 个因素，效应量 d 范围 -2 到 2（Golovchin, 2019, p.232-233）。

以 d=0.40 为"铰链点"构建"影响气压计"：
- **负面效应区**（d<0）：如暑假、看电视
- **发展效应区**（d=0-0.15）：如教师学科知识（d=0.09）、教师教育（d=0.11）
- **低中效应区**（d=0.15-0.4）：如班级规模（d=0.21）、家庭结构（d=0.17）
- **期望效应区**（d>0.4）：如微格教学（d=0.88）、教学清晰度（d=0.75）、师生关系（d=0.72）（Golovchin, 2019, p.234）
...
**方法论批评**：元分析数据过时（Lupton, Terhart）；效应量平均化不当（Brown, Kamenetz）；异构研究"苹果与橘子"式比较（Snook et al., Jones）；效应量非因果性（Snook et al.）；d=0.40 阈值任意（Snook et al.）；定性研究被忽略（Terhart, Snook et al.）；未控制社会环境因素（Snook et al., Terhart）；缺乏效度评估（Snook et al.）；低于 0.40 的效应量不应被忽略（Slavin）；数据来自英语国家不可全球推广（Snook et al.）

Myburgh 等识别了元-元分析的六项"慢性问题"：垃圾进垃圾出、发表偏倚、苹果与橘子比较、效应量误用、经验偏见（只使用经验可得的指标）、应用局限（忽视社会互动因素）（Golovchin, 2019, p.237）
...
4. **澳大利亚前车之鉴**：Eacott (2017) 描述澳大利亚实施 VL 后形成的"持续数据生产"政策为"澳大利亚教育领导力的悲剧"（Golovchin, 2019, p.240）
5. **教育中很少有简单的"有效"或"无效"**：正确的问题是"在什么条件下这在学校中有效？"——而这与 Hattie 的方法论逻辑相悖（Golovchin, 2019, p.241, citing Slavin）

## 主要发现

1. Hattie 的元-元分析使用的效应量数据集极端不均匀（Vσ=56.87%），动摇了其结论的统计基础（Golovchin, 2019, p.239）
2. 西方对 Hattie 的批评可分为方法论批评（元分析技术问题）和文化批评（新自由主义、性别、能力政治）两个阵营，但这些批评此前"主要是理论性的"（p.238）
...
- **论文自认局限**：批评文献限于英文和俄文资料；变差系数计算仅基于效应量均值而非原始分布数据（p.238-239）
- **数据来源限制**：Hattie 的元分析数据主要来自美国、澳大利亚和新西兰（1980s-2000s 英语国家），限制了向俄罗斯情境外推的有效性（p.232）

...
> - **Eysenck (1984) 的"苹果和橙子"批评**：[[Meta-analysis|元分析]]合并概念上不可比较的结果测量(p.4)
...
>
> Cohen (1962) 引入标准化均值差（Cohen's d）是为了帮助心理学研究者评估研究的统计功效——是否有足够大的机会拒绝（错误的）[[Null Hypothesis|零假设]]。效应量被设计为研究设计工具，而非教育政策的决策工具。Meta-analysts 和 meta-meta-analysts 将 d 重新解释为教育有效性的测量，这一跳跃缺乏论证基础(pp.3–4)。
>
...
> ### 3. 违反假设的三个系统性偏差来源
>
> Simpson 识别了三个研究设计领域，它们导致效应量计算系统性地违反元分析的假设。这些偏差不是随机噪声——它们不会因为合并大量研究而被"平均掉"——而是随着研究领域系统性地变化(p.5)。
>
> #### 3a. 比较组的不对等性（Comparison Groups）
>
> **思想实验**：两位农民测试肥料对豆子长度的影响。第一位比较肥料 vs 无肥料（d=1.0），第二位比较肥料 vs 粪肥（d=0.5）。结论不可能是第一位农民的肥料效果更大——他们使用了不同的比较基线。两人的效应量也不能被有意义地合并(pp.5–6)。
>
> **教育中的表现**(pp.6–7)：
...
> - 某些元分析的实验条件在另一些元分析中是对照条件——"一个分析的实验组是另一个分析的控制组"(p.7)
> - 调节分析（[[Moderator Analysis]]）揭示：比较"无干预"的效应量是"替代处理"的 3 倍以上（Camilli et al., 2010），但元-元分析未对此进行调整
>
> **系统性含义**：反馈研究可以合乎伦理地使用"无反馈"比较（实验室环境），但行为[[Intervention Research|干预研究]]只能比较"更密集 vs 现有"方案（学校环境）。结果是，在其他条件相同的情况下，我们可以预期反馈研究的效应量系统性地高于行为干预研究——不是因为反馈更有效，而是因为可用的研究设计选择不同(p.7)。
>
...
>
> **系统性含义**：反馈和元认知研究中容易选择限制范围的样本（如前测失败者），但延长学校日或校服研究中没有明显的方法限制样本范围。结果是反馈研究的效应量系统性地更高——不是因为反馈更有效，而是因为更容易在限制范围上进行研究(p.11)。
>
> #### 3c. 测量设计（Measure Design）
>
> 测量设计通过两个渠道影响效应量：**聚焦度**（focus）和**精确度**（precision）(pp.11–14)。
>
> **聚焦度——思想实验**：假设肥料仅对暴露在直射阳光下的豆子有效。第一位农民选择容易够到的豆子（多在阳光下）测量，第二位从整株植物（包括叶子遮蔽的）中随机选择。第一位报告更大的效应量——不是因为肥料更有效，而是因为测量更聚焦于受影响的豆子(pp.11–12)。
>
> **教育中的表现**(p.12)：
...
>
> **精确度——思想实验**：两位农民进行完全相同的肥料试验。第一位每株随机选择 5 颗豆子测量，第二位选 10 颗。第二位报告的效应量可高达第一位的 140%——因为更多测量减少了株内方差，缩小了标准差(pp.12–13)。
>
> **模拟证据**(p.13, Figure 3)：
...
>
> **系统性含义**：反馈研究中容易设计高度聚焦的测试（针对该学术学科），但早期干预研究几乎必须使用标准化测量。反馈元分析中仅 9/68 个效应量使用标准化测试（Bangert-Drowns et al., 1991; Kingston & Nash, 2011），而早期干预元分析中 37/39 个使用标准化测试（Nelson, Westhues, & MacLeod, 2003; Anderson et al., 2003）。结果是反馈效应量系统性地高于早期干预——不是因为反馈更有效(pp.13–14)。
>
> ### 4. 核心结论：联盟表不是有效性排名
>
> > “在 Marzano (1998)、Hattie (2009) 和 Higgins et al. (2013) 中排名较高的领域，是那些研究者能够设计出更敏感的实验的领域。”
> ("Areas which rank highly in Marzano (1998), Hattie (2009) and Higgins et al. (2013) are those in which researchers can design more sensitive experiments.") (p.2)
>
> 效应量排名较高的领域（反馈、元认知）仅说明研究者更容易在这些领域设计低噪声试验，而非这些干预更具教育影响力。以这些排名驱动教育政策是"misdirection"——误导(p.14)。
...
> - **仅在同一比较组、同一样本范围、同一测量工具和同一分析的极端条件下，合并效应量才具有意义**——"在现实中，这种直接复制和比较并不会发生，也不是'证据本位政策'的基础"(p.15, citing Simpson 2019)

---
summary: "探讨中学生认识论信念在不同任务结构（良构与劣构）下的预测效应，发现简单知识信念显著负向预测劣构任务表现与反思性判断，而在良构任务中不具预测力。"
type: argument
authors:
  - "Lodewyk, K. R."
source_language: en
citation: "Lodewyk, K. R. (2007). Relations among epistemological beliefs, academic achievement, and task performance in secondary school students. Educational Psychology, 27(3), 307–327. https://doi.org/10.1080/01443410601104080"
year: 2007
doi: "10.1080/01443410601104080"
citation_aliases:
  - "Lodewyk, 2007"
  - "Lodewyk (2007)"
isbn: ""
tags:
  - "theme/epistemological-beliefs"
  - "level/secondary"
  - "subject/science"
  - "method/quantitative"
related_concepts:
  - "[[Epistemological Beliefs]]"
  - "[[Academic Achievement]]"
  - "[[Task Structure]]"
  - "[[Reflexivity]]"
  - "[[Simplicity of Knowledge]]"
  - "[[Epistemology]]"
  - "[[Growth]]"
  - "[[Self-Regulated Learning]]"
  - "[[Hypothesis]]"
  - "[[Order Effects]]"
  - "[[Rote Learning]]"
  - "[[Going Native]]"
  - "[[Variable]]"
  - "[[Critical Thinking]]"
  - "[[Dialogue in Education]]"
  - "[[Self-Efficacy]]"
  - "[[Learned Helplessness]]"
  - "[[Independent Variable]]"
  - "[[Dependent Variable]]"
  - "[[Scaffolding]]"
  - "[[Metacognition]]"
  - "[[Ways of Knowing]]"
related_theories:
  - "[[Reflective Judgment Model]]"
related_methods:
  - "[[Questionnaire]]"
  - "[[Analytic Framework]]"
  - "[[Survey Research]]"
  - "[[Exploratory Factor Analysis]]"
  - "[[Multivariate Analysis of Variance]]"
  - "[[t-test]]"
  - "[[Multiple Regression]]"
  - "[[Analysis of Variance]]"
  - "[[Effect Size]]"
  - "[[Meta-regression]]"
  - "[[Statistical Significance]]"
related_instruments:
  - "[[Schommer's Modified Epistemological Questionnaire]]"
related_persons: []
related_facts: []
related_arguments: []
sources:
  - "[[sources/Lodewyk_2007_EP/Lodewyk_2007_EP|Lodewyk_2007_EP]]"
part_of:
status: draft
created: 2026-08-19
updated: 2026-08-20
subtype: journal-article
publication_type: journal-article
title: "Argument_Lodewyk_2007_EP"
argument_key: "Argument_Lodewyk_2007_EP"
argument_display_title: "Relations among epistemological beliefs, academic achievement, and task performance in secondary school students"
argument_kind: "journal-article"
argument_related_count: 35
argument_related_level: 2
argument_related_stars: "⭐⭐"
argument_related_color: "#dbeafe"
journal: "Educational Psychology"
book_title: ""
publication_place: ""
publisher: ""
issuing_organization: ""
---
# Argument_Lodewyk_2007_EP

---

## 研究问题

> [!question]
> 尽管既有研究指出学生的[[Epistemological Beliefs|认识论信念]]与其思维推理、认知策略及[[Academic Achievement|学业成绩]]存在关联，但在常规课堂教学情境中，认识论信念如何随办学导向（公立学校与独立教会学校）、[[Academic Achievement|学业成就]]等级、性别以及微观教学[[Task Structure|任务结构]]（良构任务 vs 劣构任务）发生分化？特别是，认识论信念是否对不同结构化特征的任务表现和[[Reflexivity|反思性]]判断具有差异化的预测效力？（pp.307–308, 312）

> [!claim] 核心主张
> 中学生的认识论信念（尤其是固定与快速学习能力、[[Simplicity of Knowledge|简单知识]]信念）显著预测其总体学业成就；更关键的是，认识论信念对学术任务表现的预测力严格受任务结构的调节：简单知识信念显著负向预测劣构任务总分与反思性判断得分，但在良构任务中完全失去预测效力。（pp.320–324）

> [!concept-lens] 阅读透镜
> - **对象** 加拿大大温哥华地区 6 所中学（5 所公立学校，1 所独立基督教学校）447 名十年级科学课学生（358 名公立学生，89 名独立学校学生）。（pp.313–314）
> - **张力** 认识论信念传统上被视作领域一般性或稳定的认知特质，但其对具体学术表现的解释力究竟是普遍成立的，还是依存于具体任务结构所激活的认识动机与反思要求。（pp.310–312）
> - **贡献** 首次在真实课堂教学实验中实证确立了任务结构对认识论信念预测效应的调节机制，揭示了简单知识信念在开放辩护与劣构推理中的瓶颈效应，并系统提供了学校宗教导向与性别在[[Epistemology|认识论]]维度上的精细分化证据。（pp.322–325）

> [!dimension] 本文围绕认识论信念展开的七项具体探究维度（pp.312–313）
> - **维度一：量表因子结构的实证探索** 在中学生群体中检验中学生修订版认识论[[Questionnaire|问卷]]（[[Schommer's Modified Epistemological Questionnaire]], SMEQ）的底层因子维度，提取并确立核心信念维度。
> - **维度二：办学导向（宗教 vs 公立）的信念差异** 比较独立新教基督教学校与非宗教公立学校学生在认识论信念上的分布差异。
> - **维度三：认识论信念对总体学业成就的预测力** 检验各认识论信念因子对学[[Growth|生长]]期总体学业成就预估值（estimates of overall academic achievement, EOAA）的回归预测作用。
> - **维度四：不同学业成就水平组的信念分化** 对比高、中、低三组学业成就学生在各项认识论维度上的朴素程度差异。
> - **维度五：任务结构对认识论预测效应的调节作用** 比较认识论信念对良构任务（well-structured task, WST）与劣构任务（ill-structured task, IST）表现的差异化预测效力。
> - **维度六：劣构任务中反思性判断的预测机制** 检验认识论信念是否能独立预测学生在劣构辩护任务中的反思性判断（[[Reflective Judgment Model|Reflective Judgment]], RJ）深度。
> - **维度七：性别在认识论、任务表现与学业成就上的交互差异** 考察男女生在认识论成熟度、反思能力及两类任务表现上的系统性差异。

---

## 理论框架

> [!framework-table] 理论工具箱
> | 理论工具 | 解释功能 |
> |---|---|
> | **[[Epistemological Beliefs\|认识论信念多维系统模型]]**<br>Schommer (1990, 1993) | 主张个体的[[Epistemology\|认识论]]信念是由学习速度、能力本质、知识结构与知识确定性等相对独立的维度构成的系统，打破了单一线性发展阶段论，为量化测量各维度对[[Academic Achievement\|学业表现]]的独立贡献提供基础[[Analytic Framework\|分析框架]]。（pp.309–310） |
> | **[[Reflective Judgment Model\|反思性判断模型]]**<br>King & Kitchener (1994, 2002) | 描述个体面对没有唯一标准答案的劣构问题时，从前反思阶段（阶段1–3：知识绝对）、准反思阶段（阶段4–5：知识主观相对）到反思阶段（阶段6–7：情境化概率辩护）的发展路径，为评定学生的认知反思水平提供评价准则。（pp.309, 316–317） |
> | **[[Task Structure\|任务结构理论与连续谱假说]]**<br>Frederiksen (1984); Spiro et al. (1988); Doyle (1983) | 区分良构任务（清晰目标、现成算法、透明标准）与劣构任务（开放目标、无现成算法、评价开放），解释不同任务环境对认知负荷、线索依赖与[[Self-Regulated Learning\|自我调节学习]]的差异化要求。（pp.310–312） |
> | **常人认识论与认识动机理论（Lay Epistemic Theory / Epistemic Motivation）**<br>Kruglanski (1990) | 阐明面对模糊与复杂任务情境时，认识动机如何驱动个体展开信息检索、[[Hypothesis\|假设]]检验、矛盾权衡以及寻求适度认知闭合（seeking appropriate closure）的自我调节心理过程。（pp.310–311, 323–324） |

> [!warrant]- 理论如何支撑论证
> 认识论信念被视作学习者在自我调节学习中校准理解与评估反馈的标准。当面对良构任务时，外部环境提供了充分的算法与明确的评分标准，朴素认识论信念的负面影响被外在支架所补偿；然而当置身于劣构任务时，缺乏现成程序与唯一答案的情境激活了认识动机，要求学生动用高阶的[[Reflexivity|反思性]]判断，此时持有知识由简单孤立碎片构成等朴素信念的学生因回避整合与多元辩护而遭遇系统性绩效受损。（pp.310–312, 323–324）

---

## 研究方法

> [!method-panel] 研究设计
> | 模块 | 材料与处理方式 |
> |---|---|
> | **问卷调查与因子分析**<br>[[Survey Research]] & [[Exploratory Factor Analysis\|EFA]] | 采用加拿大十年级科学课修订版 52 题 [[Schommer's Modified Epistemological Questionnaire\|SMEQ]]（5点 Likert 计分），结合探索性因子分析（exploratory factor analysis, EFA）的主轴提取与 Varimax 正交旋转，识别中学生认识论信念的底层因子维度。（pp.314–315, 317–318） |
> | **学业成就与群体差异检验**<br>[[Multivariate Analysis of Variance\|MANOVA]] & [[t-test]] | 自编 14 题人口学问卷（demographic questionnaire, DQ）采集总体学业成就预估值（EOAA，与实际总评成绩相关 $r = .91$）。运用[[Multivariate Analysis of Variance\|多变量方差分析]]（multivariate analysis of variance, MANOVA）与事后 $t$ 检验，对比公立与教会学校、不同学业水平组（高/中/低）及性别间的认识论信念差异。（pp.314, 318–320） |
> | **组内平衡任务实验**<br>Counterbalanced Within-Subject Design | 在独立学校 89 名学生中实施平衡顺序设计的两类癌症学术任务（各4节80分钟课时完成）：[[Task Structure\|良构任务]]（个人生活方式计划）与劣构任务（专案组癌症资金分配辩护）。（pp.315–316） |
> | **任务表现与反思性判断评定**<br>Rubric Scoring & [[Reflective Judgment Model\|RJ]] Rating | 任务总分采用评分量规（权重 .90）与 10 题单选题（权重 .10）合成（评分者信度 $r = .88 \sim .93$）；劣构任务中基于 King & Kitchener (1994) 模型独立评定 1–5 分的反思性判断（RJ）得分（评分者信度 $r = .89$）。（pp.316–317） |
> | **分层与多元回归分析**<br>[[Multiple Regression]] | 采用多元回归与分层回归，在控制学校导向因子（确定知识）前后，分别检验[[Epistemological Beliefs\|认识论信念]]对[[Academic Achievement\|学业成就]]、两类任务总成绩及[[Reflexivity\|反思性]]判断得分的预测力。（pp.318–321） |

> [!sample-panel]- 样本与材料快照
> | 样本层面 | 构成 |
> |---|---|
> | **总样本（Combined Sample）** | 447名十年级科学课学生（男生219人，女生228人；平均年龄15.3岁；85%白人、9%亚裔、6%其他；中产阶级为主；来自大温哥华地区5所公立学校与1所独立学校）。（pp.313–314） |
> | **公立学校子样本（Public School Sub-sample）** | 358名学生（男生174人，女生184人；平均年龄15.2岁；86%白人、7.5%亚裔、6.4%其他）。（p.313） |
> | **独立学校子样本（Independent School Sub-sample）** | 89名学生（男生45人，女生44人；平均年龄15.3岁；全员具备新教基督信仰；完成两项学术任务与全部[[Questionnaire\|问卷]]）。 |
> | **材料情境** | 问卷与任务均在常规科学课课时由第一作者与任课教师共同施测；任务按班级轮换平衡先后顺序以排除[[Order Effects\|顺序效应]]与时间效应。（p.314） |

### 学术任务设计案例对比

> [!contrast-table] 良构任务（WST）与劣构任务（IST）实验案例对比（pp.315–317）
> | 任务设计维度 | 良构任务（WST）：个人生活方式计划<br>Personal Lifestyle Plan | 劣构任务（IST）：专案组议题辩护<br>Task-Force Issue |
> |---|---|---|
> | **任务主题与目标** | 选择一种特定癌症，收集其病因、危险因素、症状、治疗手段及降低患病风险的生活方式预防行为，撰写结构化报告。 | 扮演专案组成员，就政府癌症财政拨款应优先投向预防宣传还是治疗科研撰写议论文，进行立场论证与合理性辩护。 |
> | **先行组织者与导入** | 任务前播放 20 分钟癌症预防生活方式录像，并要求完成对应导学工作表以激活先验认知。 | 无先行组织者或引导录像，学生直接面对开放性争议问题。 |
> | **资源可及性** | 教师直接提供预先打包整理好的癌症特定资料文件袋。 | 教师不提供现成资料包，学生必须在教室书架和既有资源中自行检索、筛选与甄别材料。 |
> | **教学线索与子目标** | 任务说明书中提供详尽的格式、文体、小节标题及内容目标指引。 | 仅提供核心问题框架，线索极少，无预设标准大纲。 |
> | **评价标准透明度** | 详细评分量规在布置任务时直接发放给学生，评分标准完全公开透明。 | 采用不列颠哥伦比亚省教育部历史12考试论文评分标准，但**不向学生展示量规**，考核开放论辩质量。 |
> | **反思性判断要求** | 低。主要考察信息提取、整合罗列与事实性总结能力。 | 高。基于 King & Kitchener (1994) 模型独立评定 1–5 分的反思性判断得分（考察证据对立权衡与偏见审视）。 |

### 反思性判断（RJ）评分等级映射

> [!ref-table]- 劣构任务反思性判断评分细则（基于 King & Kitchener, 1994, pp. 316–317）
> | RJ 评分 | 对应发展阶段 | 阶段认知特征与在癌症辩护任务中的具体表现 | 评分标准依据 |
> |---|---|---|---|
> | **0.5–2.0 分** | **阶段 1：前反思** | 未能意识到议题的复杂性；认为存在唯一绝对正解；未能提供合理论据，将个人直觉或武断主张视作绝对真理；极度简化癌症防治争议。 | 过度简化议题，无有效证据辩护 |
> | **2.0–3.0 分** | **阶段 2：前反思过渡** | 开始意识到议题存在不同侧面，但辩护仍具二元论倾向；过度依赖权威结论（如权威医生论断），缺乏自主论证。 | 诉诸权威，二元对立辩护 |
> | **3.0–4.0 分** | **阶段 3：前反思后期** | 表现出对确定性的怀疑；承认可以存在多元判断，但主要依据个人偏好或未经推敲的个人观点做决策。 | 允许不同立场，但仅基于个人主观意见 |
> | **4.0–5.0 分** | **阶段 4–5：准反思阶段** | **阶段4** 认识到知识具有不确定性，但试图调和所有人的意见导致判断模糊；<br>**阶段5** 明确认识到对复杂议题的判断必须建立在具体情境与相对立场之上，能审视自身与文献局限，并对预防与治疗两方证据进行对照权衡。 | 展现证据对立权衡、情境化相对意识与偏见审视 |

---

## 论证结构

> [!logic-map]- 核心论证逻辑链
> ```mermaid
> flowchart TD
>     A["探索性因子分析提取认识论三因子<br>(FQAL, SK, CK)"] --> B["群体特征与学业成就检验<br>(教会学校CK更成熟；高学业组FQAL/SK更成熟)"]
>     A --> C["学术任务实验实施<br>(良构任务 WST vs 劣构任务 IST)"]
>     B --> D["多元回归预测分析"]
>     C --> D
>     D --> E["任务结构调节效应确立<br>(EB仅显著预测IST表现与反思性判断；对WST无预测力)"]
>     D --> F["简单知识(SK)成为劣构问题核心制约机制"]
>     E --> G["教学设计与认识论干预启示"]
>     F --> G
> ```

---

### 论证步骤一　中学生认识论信念的因子结构及其在办学导向上的分化

> [!claim] 步骤一主张
> 在中学生群体中，[[Epistemological Beliefs|认识论信念]]稳定汇聚为固定与快速学习能力（fixed and quick ability to learn, FQAL）、[[Simplicity of Knowledge|简单知识]]（simple knowledge, SK）与确定知识（certain knowledge, CK）三个相对独立的维度；新教独立学校学生的确定知识信念显著比公立学校学生更为成熟。（pp.317–318）

> [!concept-lens] 通俗解读：中学生的认识论信念究竟在想什么？
> 认识论信念听起来抽象，但在中学生日常学习中表现得非常具体：
> 1. **关于能力的信念（FQAL）** 学生是否认为聪明是天生的？如果一道难题几分钟没做出来，是觉得多钻研就能学会，还是认为自己天生不是这块料、花时间也是白费？
> 2. **关于知识结构的信念（SK）** 学生是把课本知识看成一张相互关联的大网，还是看成一条条互不相干的[[Rote Learning|死记硬背]]清单？遇到新观点时，是主动与已有知识融会贯通，还是觉得混在一起只会徒增困惑？
> 3. **关于真理确定性的信念（CK）** 学生是否坚信世界上所有事情都有唯一的标准答案？是否盲目认为科学家只要努力就一定能找到绝对永恒的真理？

> [!chain-link] 证据到判断：[[Questionnaire|问卷]]编制与因子分析结果
> 早期 Schommer 问卷主要面向大学生，共包含 63 个题项。本研究在加拿大十年级学生中进行了[[Going Native|本土化]]适配：
> - 经 24 名中学生预试，简化了用词表达，剔除了理解困难的题项；
> - 删除了在青少年群体中未稳定成型的全知权威题项（10 题），最终保留 52 题；
> - 采用主轴[[Exploratory Factor Analysis|探索性因子分析]]（Varimax 正交旋转，特征值 $\ge .98$），提取出 18 个因子，其中前 3 个最具理论解释力的核心因子共解释 16.81% 方差（见下表）。
> - **三个因子的相关性极低（$r = -.002 \sim .08$）** 这证实了认识论信念是相互独立的多维系统。一个学生完全可以在学习能力上很开明（相信勤能补拙），但在真理观上依然很保守（迷信绝对标准答案）。（pp.317–318）

> [!ref-table]- 表 1：全样本题项与因子载荷矩阵（Table 1, p. 318）
> | 因子维度与题项内容 | 因子 1：固定与快速学习能力（FQAL, $\alpha = .65$） | 因子 2：简单知识（SK, $\alpha = .51$） | 因子 3：确定知识（CK, $\alpha = .73$） |
> |---|:---:|:---:|:---:|
> | 真正聪明的学生不需要努力学习就能在学校取得好成绩。*(The really smart students don’t have to work hard to do well in school.)* | **.58** | — | — |
> | 成功的学生能够迅速理解事物。*(Successful students understand things quickly.)* | **.55** | — | — |
> | 有些人天生是优秀的学习者，另一些人则受限于有限的能力。*(Some people are born good learners, others are just stuck with limited ability.)* | **.53** | — | — |
> | 花较长时间努力钻研难题只对真正聪明的学生有回报。*(Working hard on a difficult problem for a longer period of time only pays off for really smart students.)* | **.48** | — | — |
> | 人一旦出生，其学习能力就不会改变。*(Once a person is born, their ability to learn will not change.)* | **.39** | — | — |
> | 教我们如何做事的书没有多大帮助。*(Books that teach us how to do something are not much help.)* | — | **.46** | — |
> | 如果你试图把教科书中某个主题的新观点与你已有的知识融合在一起，你只会感到困惑。*(You will just get confused if you try to take ideas that are new to you from a topic in a textbook and try to mix them with what you already know about that topic.)* | — | **.43** | — |
> | 除了我们终有一天会死去之外，我们对任何事都无法确信。*(We can’t be sure of anything, except that we will die someday.)* | — | **.41** | — |
> | 把时间花在不可能得出明确直接答案的问题上是一种浪费。*(It’s a waste of time to work on problems which have no possibility of coming out with a clear-cut and straight forward answer.)* | — | **.33** | — |
> | 科学家最终能够获得真理。*(Scientists can eventually get to the truth.)* | — | — | **.76** |
> | 如果科学家足够努力，他们几乎可以找到任何事物的真理。*(If scientists try hard enough, they can find the truth to almost anything.)* | — | — | **.74** |
> | **方差贡献率（% variance）** | **7.49%** (特征值 3.89) | **6.51%** (特征值 3.38) | **2.82%** (特征值 1.47) |

> [!chain-link] 办学导向组间差异：公立学校 vs 独立基督教学校
> 多[[Variable|变量]][[Analysis of Variance|方差分析]]显示办学导向主效应显著（$F[3,437] = 4.68, p = .003, \eta^2 = .03$）：
> - **确定知识（CK）上的显著差异** 独立基督教学校学生在确定知识维度上的得分显著低于公立学校学生（$t[439] = 3.44, p = .001, \eta^2 = .03$；$M = -.28$ vs $.07$）。因为本量表得分越低代表观念越成熟，这表明教会学校学生面对科学知识时，比公立学校学生更少持有绝对主义真理观，更懂得科学结论存在相对性与发展性。
> - **能力（FQAL）与简单知识（SK）无显著差异** 两类学校学生在固定快速学习能力（$p = .71$）与简单知识（$p = .12$）上表现一致。
> - **排除了民族与族裔混杂** 全样本涵盖 85% 白人、9% 亚裔与 6% 其他族裔，方差分析表明不同族裔之间的认识论信念不存在统计显著差异（$p > .05$）。（p.318）

> [!warrant]- 推理桥梁与机制解析：为什么教会学校学生的真理观反而更成熟？
> 传统直觉和部分既有研究常认为，宗教背景容易让学生迷信绝对权威或教条真理。但本研究打破了这一刻板[[Hypothesis|假设]]，作者提出了三点合理的解释机制（pp.322–323）：
> 1. **思辨课程与情境化协调** 独立基督教学校的课程常常涉及伦理道德、社会正义等复杂议题的讨论，促使学生从小学会从不同情境与多重视角去协调矛盾观点；
> 2. **科学与信仰的长期思辨磨炼** 教会学校学生在科学课上经常需要面对科学实证与信仰文本之间的张力，这种长期的思辨训练让他们更早认识到：科学本身是一种不断修正、演进的人类探索过程，而不是终极僵化的绝对真理；
> 3. **家庭学习环境的讨论氛围** 选择入读独立学校的家庭往往更为关注孩子的[[Critical Thinking|批判性思维]]与价值观培育，提供了更多深度[[Dialogue in Education|对话]]的机会。

---

### 论证步骤二　认识论信念对总体学业成就的预测与能力层级分化

> [!claim] 步骤二主张
> 学生的认识论信念能够强有力地预测其长期总体[[Academic Achievement|学业成绩]]；成绩优异的学生在学习能力观和知识结构观上显著更为成熟，而成绩较差的学生则普遍深信能力天生固定且知识只是孤立碎片。（pp.318–320）

> [!concept-lens] 通俗解读：自评平时成绩真的靠谱吗？
> 研究中采用的总体[[Academic Achievement|学业成就]]预估值（EOAA）是由学生自报平时各科成绩平均百分比（全样本均分为 74.11%）。为了检验学生自报成绩是否真实可信，作者调取了子样本中学生的学校官方实际总评学业成绩（overall academic average, OAA）进行效标检验，发现两者的相关系数高达 $r = .91$（$p < .001$）。这证明中学生对自身学业水平的评估极为客观准确，完全可以作为长期总体学业成就的有效代表。（p.314）

> [!chain-link] 证据到判断：分层回归模型与预测效应
> 因为公立学校与教会学校在确定知识（CK）上存在显著差异，为了排除学校办学导向的干扰，研究者采用了**分层[[Multiple Regression|多元回归]]**
> 1. **第一步（控制变量）** 先放入确定知识（CK），结果显示 CK 对总体学业成就毫无预测力（$R^2 = .00, p = 1.00$）；
> 2. **第二步（检验能力观与知识观）** 同时放入固定快速学习能力（FQAL）与简单知识（SK），模型解释力直接提升了 8%（$\Delta R^2 = .08, F[3,436] = 12.09, p < .001$）。
> 3. **单因子预测强度** FQAL 独立解释 4% 方差（$F[1,438] = 17.38, p < .001$），SK 独立解释 5% 方差（$F[1,438] = 20.48, p < .001$）。
> 
> 这表明：无论学生就读于哪类学校，只要他们坚信聪明是天生的、学不会就是笨（FQAL 高），或者把知识看作互不相干的死记硬背清单（SK 高），他们在学校的整体学业表现就会系统性落后。（pp.318–319）

> [!chain-link] 证据到判断：高、中、低三档学业水平组的信念断层
> 研究者将 447 名学生按平时成绩划分为三组：**高成就组（$>80\%$，124人）**、**中成就组（$70\%–80\%$，198人）**、**低成就组（$<70\%$，123人）**。[[Multivariate Analysis of Variance|多变量方差分析]]（MANOVA）显示出极其显著的组间主效应（$F[6,874] = 5.52, p < .001, \eta^2 = .04$）：
> - **在固定快速学习能力（FQAL）上** 高成就组（$M = -.22$）显著比低成就组（$M = .21, t[242] = 3.89, p < .001, \eta^2 = .06$）和中成就组（$M = .005, p = .01$）成熟得多。学霸普遍坚信勤能补拙，而学困生则更倾向于认命、认为学习必须靠天生开窍。
> - **在简单知识（SK）上** 高成就组（$M = -.21$）同样显著成熟于低成就组（$M = .18, t[242] = 3.97, p < .001, \eta^2 = .06$）和中成就组（$M = .02, p = .007$）。高分学生习惯于构建概念网络，而低分学生则严重依赖机械记忆。
> - **在确定知识（CK）上** 高中低三组得分几乎完全重合（$M = -.06, .07, -.05, p = .29$），在常规总成绩上没有区分度。（pp.319–320）

> [!ref-table]- 表 2：各群体在[[Epistemology|认识论]]因子上的均值与标准差（Table 2, p. 319）
> | 样本与分析组别 | 样本量 $n$ | 固定与快速学习能力（FQAL） | 简单知识（SK） | 确定知识（CK） |
> |---|:---:|:---:|:---:|:---:|
> | **全样本（Combined Sample）** | | | | |
> | 总体（Total） | 447 | .000 (.84) | .000 (.79) | .000 (.86) |
> | 男生（Boys） | 218 | .20 (.85)*** | -.06 (.85) | .11 (.88)* |
> | 女生（Girls） | 227 | -.19 (.78)*** | .06 (.72) | -.10 (.84)* |
> | 低学业成就组（Low EOAA, $<70\%$） | 123 | .21 (.89) | .18 (.85) | -.05 (.89) |
> | 中学业成就组（Moderate EOAA, $70\%-80\%$） | 198 | .005 (.76) | .02 (.77) | .07 (.86) |
> | 高学业成就组（High EOAA, $>80\%$） | 124 | -.22 (.85) | -.21 (.70) | -.06 (.84) |
> | **独立学校子样本（Independent Sub-sample）** | | | | |
> | 总体（Total） | 89 | -.03 (.78) | -.12 (.84) | -.28 (.82)+ |
> | 女生（Girls） | 44 | -.09 (.81) | -.006 (.75) | -.37 (.70) |
> | 男生（Boys） | 45 | .03 (.75) | -.23 (.91) | -.19 (.93) |
> | **公立学校子样本（Public Sub-sample）** | | | | |
> | 总体（Total） | 358 | .008 (.85) | .03 (.77) | .07 (.86)+ |
> | 女生（Girls） | 184 | -.22 (.77) | .08 (.72) | -.04 (.86) |
> | 男生（Boys） | 174 | .24 (.87) | -.02 (.83) | .19 (.85) |
> 
> *注：得分越高代表认识论信念越朴素（naïve），括号内为标准差。+ 表示公立与独立学校差异显著（$p < .001$）；* 表示全样本性别差异显著（$p < .05$）；*** 表示全样本性别差异极显著（$p < .001$）。*

> [!example]- 图1：高、中、低学业成就水平学生的认识论信念因子均分
> ![](https://img.mylikemie.icu/sources/Lodewyk_2007_EP/figures/Lodewyk_2007_EP_Fig1_EOAA_Factor_Scores.jpg)

> [!warrant]- 推理桥梁与机制解析：为什么朴素信念会毁掉平时成绩？
> 针对为什么 FQAL 和 SK 能精准预测中学生的各科学业表现，作者结合教育心理学理论给出了两条核心机制（pp.319–320, 323）：
> 1. **固定能力信念（FQAL）摧毁了学生的抗挫力与[[Self-Efficacy|自我效能感]]** 如果学生深信能力是天生的且必须瞬间开窍，当他们在十年级遇到复杂学科难点时，他们会把挫折直接归结为我不够聪明。这种负向归因会瞬间摧毁其解题自我效能感（Bandura, 1993），让他们产生[[Learned Helplessness|习得性无助]]，进而过早放弃深入钻研；
> 2. **简单知识信念（SK）导致了低效的浅层死记硬背** 如果学生认为知识只是一堆孤立的碎片事实，他们就不会主动寻找概念间的内在逻辑与应用场景。在初中死记硬背或许勉强过关，但到了高中，学科对综合推理与迁移应用要求大幅提升，浅层死背必然导致平时成绩出现断崖式落后；
> 3. **为什么确定真理观（CK）在平时总成绩上没有区分度？** 因为常规中学的大多数日常考试与总评考核，考查的仍然是既定的科学事实、定理公式与解题套路，较少直接考核关于科学结论暂定性与相对性的哲学反思。因此，CK 信念在平时总评中暂时不显山露水（但在后续需要论辩的[[Task Structure|劣构任务]]中才会成为关键决定力量）。

---

### 论证步骤三　任务结构对认识论信念预测力的调节效应与反思性判断机制

> [!claim] 步骤三主张
> 认识论信念对具体学术任务表现的预测作用高度取决于任务结构：在有标准套路与现成资料的良构任务中，认识论信念毫无影响；但在缺乏现成算法、充满矛盾争议的劣构任务中，简单知识信念成为制约任务总分与[[Reflexivity|反思性]]判断深度的核心瓶颈。（pp.320–321）

> [!concept-lens] 通俗解读：两类任务表面均分相当，深层机制却大相径庭
> 89 名学生分别完成了良构任务（个人生活方式计划）与劣构任务（专案组癌症经费分配辩护），每项任务各用时 4 节 80 分钟课时。为了排除先做哪个任务产生的疲劳或熟练效应，各班级轮流调换了任务顺序（$t$ 检验显示顺序无显著影响，$p \ge .05$）。
> 
> - **表面均分看似毫无差别** 良构任务均分为 70.30 分（$SD = 17.61$），劣构任务均分为 67.72 分（$SD = 17.93$），两者的平均得分差异在统计上不显著（$t[88] = -1.38, p = .17$，[[Effect Size|效应量]]（effect size, ES）$ES = 0.15$），且两者得分呈中等程度正相关（$r = .51$）。
> - **深层决定因素的惊人反差** 虽然两个任务的平均分差不多，但背后的决定机制完全不同。多[[Meta-regression|元回归]]模型揭示出剧烈的任务结构差异（见下表）：在良构任务里，认识论成熟与否根本无所谓；但在劣构任务里，认识论水平直接决定了学生的表现与反思深度！（pp.320–321）

> [!chain-link] 证据到判断：回归模型中鲜明的任务结构对比
> 1. **良构任务（WST）：认识论完全失去预测力**
>    认识论三因子整体回归模型不显著（$R^2 = .04, F[3,85] = 1.07, p = .37$），单因子单独检验也全军覆没（FQAL: $p = .52$; SK: $p = .69$; CK: $p = .11$）。这说明在目标明确、资料现成的任务中，无论学生的认识论观念多么幼稚，都不会拉低他的任务得分。
> 2. **劣构任务（IST）：认识论成为决定胜负的关键**
>    认识论三因子整体模型高度显著（$R^2 = .12, F[3,85] = 3.80, p = .01$），直接解释了劣构任务表现 12% 的方差。其中**简单知识（SK）呈现出强烈的负向预测效应（$\beta = -.24, R^2 = .06, F[1,87] = 5.84, p = .02$；在控制学校类型差异变量 CK 后，SK 的增量解释力进一步提升至 $\Delta R^2 = .08, F[2,86] = 5.36, p = .006$）**。
> 3. **反思性判断（RJ）：简单知识信念是唯一的反思瓶颈**
>    认识论三因子整体显著预测劣构任务的反思性判断得分（$R^2 = .10, F[3,85] = 2.97, p = .04$）。其中**简单知识（SK）是唯一的显著负向预测源（$\beta = -.24, R^2 = .06, F[1,87] = 5.56, p = .02$；控制 CK 后增量 $\Delta R^2 = .07, F[2,86] = 4.20, p = .02$）**。（pp.320–321）

> [!ref-table]- 表 3：显著多元回归模型汇总（Table 3, p. 320）
> | [[Independent Variable\|预测变量]]（Predictor） | [[Dependent Variable\|结果变量]]（Outcome） | $R^2$ | $F$ 统计量 | $p$ 值 |
> |---|---|:---:|:---:|:---:|
> | **全样本（Combined Sample, $N = 447$）** | | | | |
> | 认识论信念三因子（EB factors） | 学业成就预估（EOAA） | .08 | $12.09***$ | $<.001$ |
> | 固定与快速学习能力（FQAL） | 学业成就预估（EOAA） | .04 | $17.38***$ | $<.001$ |
> | 简单知识（Simple Knowledge, SK） | 学业成就预估（EOAA） | .05 | $20.48***$ | $<.001$ |
> | **独立学校子样本（Independent Sub-sample, $n = 89$）** | | | | |
> | 认识论信念三因子（EB factors） | 劣构任务表现（IST Performance） | .12 | $3.80*$ | $.01$ |
> | 简单知识（Simple Knowledge, SK） | 劣构任务表现（IST Performance） | .06 | $5.84*$ | $.02$ |
> | 认识论信念三因子（EB factors） | 反思性判断得分（[[Reflective Judgment Model\|Reflective Judgment]], RJ） | .10 | $2.97*$ | $.04$ |
> | 简单知识（Simple Knowledge, SK） | 反思性判断得分（Reflective Judgment, RJ） | .06 | $5.56*$ | $.02$ |
> 
> *注：EOAA = 总体学业成就预估值；IST = 劣构任务；EB factors = FQAL、SK 与 CK 三因子；* $p < .05$; *** $p < .001$。*

> [!warrant]- 推理桥梁与机制解析：为什么任务结构会造成如此剧烈的分化？
> 作者结合认知负荷与认识动机理论，深入揭示了这两种任务环境下的不同认知心理机制（pp.321, 323–324）：
> 1. **为什么良构任务成了朴素观念的避风港？**
>    在良构任务（个人生活方式计划）中，外部环境提供了近乎完备的支架：教师提供了 20 分钟引导录像和导学案、直接发放了打包好的资料袋、规定了清晰的内容大纲，甚至提前公开了评分细则。学生即使把知识看作孤立的死记硬背清单，只要按部就班地检索整理，也能拿到高分。外在[[Scaffolding|脚手架]]完全掩盖并补偿了认识论幼稚带来的短板；
> 2. **为什么劣构任务会击中简单知识（SK）的软肋？**
>    在劣构任务（专案组癌症经费分配辩护）中，所有外在拐杖被全部撤除：没有预设大纲，没有打包资料，评分标准对学生保密，更重要的是，政府该把钱投给预防还是治疗根本没有唯一标准答案。这种高度模糊的情境激活了学生的认识动机（Kruglanski, 1990）：
>    - **成熟学生** 能够容忍不确定性，积极检索相互矛盾的证据，反复权衡利弊，并能在深思熟虑后达成合理适度的认知闭合（seeking appropriate closure），写出高水平的反思性论辩；
>    - **朴素学生（SK 高）** 习惯了寻找标准答案。面对矛盾复杂的证据时，他们会感到无所适从和极度焦虑，要么过早下武断结论（完全无视另一方证据），要么盲目搬用权威医生的只言片语进行二元对立辩护，最终在反思性判断和论文质量上双双落败；
> 3. **课堂教学设计的双重启示**
>    - **良构任务过载的危害（Spiro et al., 1988）** 如果教师长期只布置良构任务（如做标准题、背工作表），会向学生传递该学科领域其实很简单、全是现成死答案的虚假错觉，严重阻碍高阶认知迁移；
>    - **劣构任务过载的危害（Doyle, 1983）** 如果一开始就给学生布置过于开放、过于模糊的高难度劣构任务而缺乏必要引导，又会引发学生的剧烈焦虑甚至学业退缩。因此，教学设计必须在良构与劣构任务之间维持动态平衡与循序渐进的脚手架支持。

---

### 论证步骤四　认识论信念、学术表现与性别的关联

> [!claim] 步骤四主张
> 女生在长期总体学业成绩以及两类学术任务得分上均全面显著优于男生；在认识论信念上，女生对学习能力的可塑性认知与对科学真理的相对性认知也显著比男生更加成熟。（pp.321–322）

> [!concept-lens] 通俗解读：女生在学业与思维成熟度上的全面领先
> 在全样本 447 名中学生（男生 219 人，女生 228 人）中，作者对比了男女生在平时成绩、两类课堂任务表现以及认识论观念上的差异。
> 
> 结果展现出一个极其清晰的模式：**女生不仅在平时的各科成绩和具体的学术任务中得分更高，而且在思维方式上也比男生更具[[Growth|成长]]型与反思性**。男生则表现出更强的天生能力论（认为学不会是脑子笨）和绝对真理观（认为科学有唯一终极答案）。（pp.321–322）

> [!chain-link] 证据到判断：性别在学业、任务与信念上的三重对比
> 1. **平时各科总成绩对比（EOAA）**
>    全样本中女生的总体学业成就预估值显著高于男生（女生 $M = 76.90\%, SD = 9.94$ vs 男生 $M = 72.05\%, SD = 11.23$；$t[444] = 3.99, p < .001$），平均高出近 5 个百分点。
> 2. **两类课堂学术任务表现对比（89人实验子样本）**
>    - **良构任务（WST）** 女生均分 75.60 分（$SD = 17.65$），男生均分 65.12 分（$SD = 16.32$），女生显著领先（$t[87] = 2.92, p = .004$，中到大效应量 $ES = .58$）；
>    - **劣构任务（IST）** 女生均分 71.64 分（$SD = 17.70$），男生均分 63.88 分（$SD = 17.47$），女生同样显著领先（$t[87] = 2.08, p = .04$，中等效应量 $ES = .44$）。
> 3. **认识论信念三因子的[[Multivariate Analysis of Variance|多变量方差分析]]（MANOVA）**
>    多变量方差分析显示性别主效应高度显著（$F[3,437] = 12.82, p < .001, \eta^2 = .08$）。单因子检验显示：
>    - **固定与快速学习能力（FQAL）** 女生显著比男生更成熟（女生 $M = -.19$ vs 男生 $.20, t[439] = -5.07, p < .001, \eta^2 = .06$）。男生严重倾向于认为聪明靠天生，女生则普遍坚信勤奋与策略能够提升能力；
>    - **确定知识（CK）** 女生显著比男生更成熟（女生 $M = -.10$ vs 男生 $.11, t[439] = -2.59, p = .01, \eta^2 = .02$）。男生更容易盲信科学有绝对真理，女生更能理解科学知识的暂定性与发展性；
>    - **简单知识（SK）** 男女之间无统计显著差异（女生 $M = .06$ vs 男生 $-.06, t[439] = 1.63, p = .10$）。（pp.321–322）

> [!warrant]- 推理桥梁与机制解析：为什么男生学业相对落后？超越生理性别的深层反思
> 结合教育心理学与社会学研究，作者对性别分化提出了两点核心解释与反思（pp.322, 324–325）：
> 1. **为什么认识论信念能部分解释男生的学业劣势？**
>    男生普遍持有更朴素的固定能力信念（FQAL 高）。当男生在十年级遇到复杂学科难点时，更容易出现自尊防御或逃避心理，产生我不擅长这个、学不会就算了的消极心态，在长周期学习和复杂任务中的[[Metacognition|元认知]]坚持性较弱。相反，女生更倾向于[[Growth Mindset|成长型思维]]，面对复杂任务时愿意投入更多细致的时间与策略调节，这直接支撑了她们在良构与劣构任务中的双重优势；
> 2. **超越二元生物性别的社会化角色与[[Ways of Knowing|认知方式]]反思**
>    作者特别指出，不能将这种差异简单归因于男女生生理结构的不同，其背后实质上是性别社会化角色与认识方式的差异（pp.324–325）：
>    - **关系性认知与反思思维（Connected Knowing）** 社会文化往往鼓励女性发展出更多倾听、反思与关系性思维（Baxter Magolda, 1992; Belenky et al., 1986），女生在面对复杂情境时更愿意接纳模糊性与多元视角；
>    - **确定性期待与自尊防御** 社会文化往往期望男性表现出果断、主导与确定，这可能促使男生更难容忍不确定性，更倾向于迅速寻求确定结论以保护学术自尊，从而在测验中表现出更强的确定真理倾向（CK 偏高）；
>    - **对未来研究的警示** 仅按生物学生理性别（男/女）粗放分类存在局限，未来需要引入心理性别角色（男性化/女性化特质）、情绪体验与认识方式等微观指标，才能更本质地解开认识论分化的心理机制。

---

## 主要发现

> [!finding-cards] 核心发现
> 1. **[[Task Structure|任务结构]]调节[[Epistemology|认识论]]预测效力** [[Epistemological Beliefs|认识论信念]]对良构任务表现无显著预测力（$R^2 = .04, p = .37$），但显著预测劣构任务总分（$R^2 = .12, p = .01$）及[[Reflexivity|反思性]]判断得分（$R^2 = .10, p = .04$），确立了任务情境的边界调节作用。（pp.320–321）
> 2. **[[Simplicity of Knowledge|简单知识]]信念是劣构推理的核心制约因素** 在控制学校宗教导向后，简单知识（SK）信念仍能独立解释劣构任务表现 8% 的增量方差及反思性判断 7% 的增量方差，视知识为简单孤立事实的学生在开放辩护中表现最差。（p.321）
> 3. **认识论信念深度分化[[Academic Achievement|学业成就]]与性别群体** 固定快速学习（FQAL）与简单知识（SK）显著预测总体学业成就（$R^2 = .08$）；女生在学业表现、任务得分以及 FQAL 和 CK 信念成熟度上全面显著优于男生。（pp.318–322）

> [!stat-cards]- 核心数据
> - **$R^2 = .12$** 认识论信念三因子对劣构任务表现的总体方差解释率（$F[3,85] = 3.80, p = .01$）。（p.320）
> - **$R^2 = .04$** 认识论信念三因子对良构任务表现的方差解释率（$p = .37$，无[[Statistical Significance|统计显著性]]）。（p.321）
> - **$r = .91$** 学生自报预估学业成就（EOAA）与学校实际加权总评学业成绩（overall academic achievement, OAA）的高效标关联度。（p.314）
> - **$\Delta R^2 = .08$** 在控制确定知识后，FQAL 与 SK 对学生总体学业成就的增量解释率（$p < .001$）。（p.319）

---

## 关键引用

> [!citation-card]- [[Simplicity of Knowledge|简单知识]]与[[Task Structure|劣构任务]]困难
> 视知识为简单的学生，即寻求单一标准答案、回避歧义且过度依赖权威的学生，在应对需要[[Reflexivity|反思性]]判断、坚持性以及恰当[[Self-Regulated Learning|自我调节学习]]的模糊任务特征时，往往会体验到更大的困难。（p.324）
>
> *Students who see knowledge as simple—that is, they seek single answers, avoid ambiguity, and depend too much on authority—tend to experience more difficulty with the ambiguous features of tasks that call for reflective judgments, perseverance, and appropriate self-regulated learning.*

> [!citation-card]- 课堂任务结构平衡的教学告诫
> 如果布置过多良构任务，可能会促进一种该学科领域在根本上是简单的虚假理解并刺激表面化投入；然而，教学平衡至关重要，布置过于模糊的任务可能会引发焦虑与退缩。（p.324）
>
> *The type of structure of assigned tasks also merits attention if, as Spiro et al. (1988) conjecture, assigning too many well-structured tasks can promote a false understanding that the domain is fundamentally simple and can stimulate superficial engagement. However, balance is important—assigning especially ambiguous tasks may induce anxiety and withdrawal (Doyle, 1983).*

---

## 自述局限

> [!warning]
> 1. **一般性与领域及任务特殊性测量的张力** 研究采用的 [[Schommer's Modified Epistemological Questionnaire|SMEQ]] [[Questionnaire|问卷]]属于领域一般性（general）[[Epistemological Beliefs|认识论信念]]测量，而在具体科学学科任务中，一般性信念可能未能完全捕捉特定学科或特定任务微观情境下的信念动态，未来需开发针对具体任务与学科领域的精细测量工具。（pp.324–325）
> 2. **宗教办学导向与家庭背景的潜在混杂** 独立基督教学校学生在确定知识上的成熟度虽高于公立学校，但由于公立学校样本中亦可能存在信教学生，且独立学校学生的家庭社会经济地位与家庭教育环境可能存在未被完全控制的混淆[[Variable|变量]]，因果机制尚需深入检验。（p.323）
> 3. **性别二元分类的局限性** 仅以生理性别（男/女）进行分组可能掩盖了更具解释力的性别角色认同（男性化与女性化维度）、认识方式及情绪感受对[[Epistemology|认识论]]思维的深层影响。

---

## 来源

- [[sources/Lodewyk_2007_EP/Lodewyk_2007_EP|Lodewyk_2007_EP]]

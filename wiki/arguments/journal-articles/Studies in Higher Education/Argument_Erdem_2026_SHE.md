...
>     D --> E["总体高效应：调整后 d = 0.60；高异质性 I²=93.7%，轻微发表偏倚"]
...
> [!stat-cards]- 核心综合数据
> - **$d = 0.60$** PBL 对高等教育学生结果的调整后总体效应量（$CI = [0.49, 0.71]$）。（pp. 950, 961）
> - **$d = 0.68$** 观测平均效应量（$CI = [0.57, 0.79], p < .001$）。（p. 960）
...
> [!chain-link] 证据到判断
> - **结果类型** 临床技能（$d = 1.06$）与学生满意度（$d = 1.10$）效应很高，理论性知识（$d = 0.59$）、批判性思维（$d = 0.56$）与态度（$d = 0.57$）效应高，组间差异显著。（pp. 961–962）
> - **地域** 覆盖中国的元分析效应最高（$d = 1.10$），伊朗次之（$d = 0.74$），非特定地区较低（$d = 0.55$）。（pp. 961–962）
> - **抽样方法与质量** 仅含随机对照组的元分析（$d = 0.84$）高于混合抽样（$d = 0.61$）；高质量元分析（$d = 0.82$）高于中等质量（$d = 0.46$）。（p. 962）
> - **报告类型与年份** 文章形式（$d = 0.77$）远高于未发表博士论文（$d = 0.19$）；2010 年前研究（$d = 0.29$）低于 2011–2020（$d = 0.77$）与 2021 后（$d = 0.70$）。（p. 962）
...
> - **$d = 1.10$ / 1.06** 学生满意度与临床技能效应量（$CI = [0.62, 1.59]$ 与 $[0.75, 1.37]$）。（p. 962）

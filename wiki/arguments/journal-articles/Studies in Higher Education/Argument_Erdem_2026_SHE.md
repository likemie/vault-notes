...
> [!warrant]- 理论如何支撑论证
> PBL 的建构主义机制（以问题为出发点、小组协作、自我导向学习）既解释了其对深层理解、临床推理与批判性思维的正向作用，也提示效应可能因实施情境、学科与结果类型而异。学习结果分类框架则为跨 FOM 合并[[Heterogeneity|异质性]][[Effect Size|效应量]]提供操作化基础，使按结果类型分列效应成为核心分析策略。（pp. 952–956）
...
> |---|---|
> | **研究** | 编码为研究者（发表年份）。 |
> | **结果** | 按 FOM 报告的学生原始分数原样编码、不合并；编码的结果类型包括理论性知识、临床技能、批判性思维与学生满意度；FOM 将不同学生结果合并报告时，该结果编码为综合；某结果类型效应量不足以独立成组（k < 3）时编码为其他；技能与知识以外的结果（如倾向、兴趣）编码为学生态度。 |
> | **[[Blue Skies Research\|基础研究]]地点** | FOM 覆盖特定国家或地区时按覆盖地点编码；FOM 不限定于任何特定地点的调查时编码为非特定。 |
> | **高等教育专业** | 编码为医学、牙科、护理、理科与文科。 |
> | **基础研究抽样方法** | FOM 仅含随机对照组（RCG）时编码为 RCG；同时含随机与非随机对照组时编码为 RCG 与非 RCG。 |
> | **FOM 质量** | 采用 Kung 等（2010）修订的 R-AMSTAR 量表评分，编码为不充分、低、中等与高。 |
> | **年份范围** | 编码为 2010 前、2011–2020 与 2021 后。 |
> | **报告类型** | 编码为文章与博士论文。 |
...
> - 基于 FOM 的量化[[Meta-meta-analysis|二阶元分析]]尚未出现；以二阶元分析整合 FOM 结果，可使不同情境的数据获得整体视角。（pp. 951–952）

> [!warrant]- 推理桥梁
> 量化 FOM 数据代表不同情境下的实证效应，将其综合能提供文献的整体图景并加深理解；质性综合无法回答总体效应量有多大的问题，因此二阶元分析是填补该空缺的合适方法。（pp. 951–952）
...
> [!warrant]- 推理桥梁
> 即使考虑发表偏倚、将缺失的负效应研究补入，PBL 的总体效应仍保持在 0.60 的高水平，说明正效应并非由选择性发表单独制造。但 93.70% 的异质性提示平均效应掩盖了实质变异，因此必须通过调节分析解释变异来源。（pp. 960–962）
...
> [!chain-link] 证据到判断
> - **结果类型** 临床技能（$ES = 1.06$）与学生满意度（$ES = 1.10$）效应很高，理论性知识（$ES = 0.59$）、批判性思维（$ES = 0.56$）与态度（$ES = 0.57$）效应高，组间差异显著。（pp. 961–962）
...
> - **理论性知识** $ES = 0.59$ 需结合短期获取较低而长期保持更好的既有发现解读：Dochy et al. (2003) 报告 PBL 学生知识获取较低但长期保持更优，Strobel & Van Barneveld (2009) 报告短期保持欠佳而长期保持与表现提升更优；PBL 促进[[Surface and Deep Learning|深层学习]]而非表层学习（Dolmans et al., 2016）。（p. 963）
...
> [!warrant]- 最终推理桥梁
> 把总体高效应、各结果机制解释与调节模式结合，最终判断是 PBL 应在高等教育中采用，并同时强调发表偏倚与高异质性要求谨慎推广。该判断既来自统计证据，也来自 PBL 机制与既有研究的一致支持。（pp. 963–966）
...
> 结论是，[[Problem-Based Learning|PBL]] 对学生结果具有显著且高的影响。平均[[Effect Size|效应量]]为 ES = .60 [CI = 0.49; 0.71; Q(t) = 757.75]。此外，分析显示，影响 PBL 效应变异的调节变量包括学生结果类型、地理位置、[[Blue Skies Research|基础研究]]所采用的抽样方法、研究质量、发表年份与发表类型。（p. 950）
>
...
> 中国情境中观察到的效应量显著高于其他地点。这可能归因于 PBL 自 1970 年代以来在中国高等教育机构得到实施……此外，中国在 PBL 上的长期经验以及中国学生的自导向学习、成就动机与集体主义倾向可能共同解释了这一结果。（pp. 964–965）
>
...
> - **仅限于英文发表的 FOM** 未纳入其他语言研究。
> - **非健康科学专业 FOM 数量有限** 工程、教师教育、法学、社会科学与人文学科的 FOM 覆盖不足。
> - **仅基于量化数据** 未综合质性系统评价的发现。
> - **存在轻微[[Publication Bias|发表偏倚]]** 纳入 FOM 普遍报告正且大的效应，可能是偏倚来源，解读需结合偏倚报告。

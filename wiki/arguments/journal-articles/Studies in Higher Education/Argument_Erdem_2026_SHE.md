...
> [!warrant]- 推理桥梁
> 每个元分析只是单一结果或单一情境上的局部估计，其相互冲突的结论不能靠逐篇阅读裁决；把元分析作为分析单位进行量化综合，才能把多个结果与情境整合为总体估计，并检验哪些研究特征系统性地改变效应。争论存在本身即为开展二阶综合的理由。（pp. 951–952）
...
> - 20 项元分析覆盖 469 项独立研究，每项元分析纳入 5–82 项研究，均值 23.45、标准差 18.49。（p. 957）
...
> 效应量随结果类型（$Q(6) = 14.65, p = .02$）、地域（$Q(2) = 19.03, p < .01$）、一阶研究抽样方法（$Q(1) = 4.20, p = .04$）、元分析质量（$Q(1) = 15.25, p < .01$）、报告类型（$Q(1) = 23.36, p < .01$）与发表年份区间（$Q(2) = 11.42, p < .01$）显著变化；高等教育专业类型无显著差异（$Q(5) = 1.17, p = .95$）。（pp. 961–962）
...
> | | RCG | 13 | .84 | .65 | 1.04 | | | |
> | **元分析质量** | 高 | 26 | .82 | .70 | .93 | 15.25 | 1 | < .01 |
> | | 中等 | 21 | .46 | .32 | .59 | | | |
...
> - **地域** 覆盖中国的元分析效应最高（$ES = 1.10$），伊朗次之（$ES = 0.74$），非特定地区较低（$ES = 0.55$）。（pp. 961–962）
> - **抽样方法与质量** 仅含随机对照组的元分析（$ES = 0.84$）高于混合抽样（$ES = 0.61$）；高质量元分析（$ES = 0.82$）高于中等质量（$ES = 0.46$）。（p. 962）
> - **报告类型与年份** 文章形式（$ES = 0.77$）远高于未发表博士论文（$ES = 0.19$）；2010 年前研究（$ES = 0.29$）低于 2011–2020（$ES = 0.77$）与 2021 后（$ES = 0.70$）。（p. 962）

> [!warrant]- 推理桥梁
> 高异质性在很大程度上可由研究特征解释：不同结果类型对应不同效应水平，中国情境与较近年份、仅含随机对照组与高质量元分析均伴随更高效应。地点偏倚提示国家课程与实施经验的差异；报告类型差异提示发表偏倚；专业类型不显著则表明 PBL 的总体效果跨健康科学与人文学科具有一致性。（pp. 962–965）
...
> 中国效应最高可归因于长期实施经验与文化因素；未发表博士论文效应更低提示发表偏倚；仅含随机对照组的元分析效应更高源于对实施变量的更强控制；高质量元分析效应更高反映方法学严谨性。（pp. 964–965）
...
> [!implication]- 推论后果
> 二阶元分析的结论同时指向研究议程：未来元分析应纳入不同地域与数据库以降低地点偏倚，纳入未发表博硕士论文以缓解发表偏倚，开展高等教育与 K-12 的比较，并覆盖工程、教师教育、法学、社会科学与人文学科。（pp. 965–966）
...
> - **$k = 47$** 纳入分析的独立效应量（20 项元分析、469 项独立研究）。（pp. 957, 960）

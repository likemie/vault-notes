...
>
> WPR方法在此处的运用聚焦于《强劲开端》报告优先改革1（Priority Reform 1），即"将大脑与学习作为核心内容"，以及附录D（Appendix D）中指定的核心内容规范和参考文献（p.5）。WPR关注问题如何被框架和建构（Bacchi, 2009），审视政策改革如何表征和建构特定假设——"问题"在此过程中获得了权威性，并主动参与了"问题"的建构（O'Connor & Rudolph, 2023, p.9）。
...
> [!warning] 证据薄弱与问题滑动
> 在传统量化标准下，32%的比例构成较弱的论证基础。更重要的是，"问题"在此处发生了悄然滑动：从"教师因工作量过大而离职"滑向了"教师因课堂准备度不足而需要更好的培训"。这一置换将结构性劳动问题转化为了个体能力问题，为标准化核心内容的引入打开了合法性空间。Barnes（2021）指出，持续的改革强化了"对教师的缺陷看法"（a deficient view of teachers, p.8）；Alexander和Bourke（2021）则指出ITE改革历史上导致了"教师教育者日益边缘化"（p.471）。
...
神经推理（neurological reasoning）在历史上曾被用于为压迫性政策提供"科学"依据。在澳大利亚，"被偷走的一代"（Stolen Generations）政策曾借助神经科学话语为强制同化原住民儿童辩护；在纳粹德国，神经科学被用于支持强制绝育和种族卫生政策（Zeidman, 2020, p.7）。

...
本文追问当"证据"被简化为 RCT 和统计平均时，在**三个方法论层级**分别产生了哪些技术缺陷、哲学困境和意识形态后果？每一级聚合如何进一步放大前一级的失真？
...
- **批判实在论（Bhaskar, 1978; Sayer, 2000）** — 区分 real / actual / empirical 三层本体论；教育作为开放系统，规律性需人为封闭条件才出现
- **Pawson (2006) 的现实主义评估理论** — 社会项目通过人的推理（而非物理因果）产生效果；呼吁"实在论综合"取代统计平均
- **新自由主义批判框架（Ball, 2013; Sears, 2003）** — EBE 定位于审计文化（Power, 1997）和市场化的效率要求中

方法：概念分析 + 方法论解构 + 实证案例拆解（Fresh Start RCT、EEF Toolkit、Hattie Visible Learning）
...
**技术问题**：
- **控制组模糊性**：教育中没有"安慰剂"——对照组的活动是"一切照旧"还是特意排除干预元素？两者产生截然不同的效应量
...
- **"三个月的额外进步"是随机化失败的幻象**
- 此外：参与学校领导在随机化前已对项目抱有热情；对控制组情况缺乏描述
...
### Level 3: 元-元分析的失真（Hattie & EEF Toolkit）
...
- 效应量的"铰链点"（0.4）忽视干预持续时间、学生年龄、结果类型
...
Wrigley 将 EBE 方法论精简定位于**新自由主义政策框架**：
...
Wrigley 推崇 **Pawson (2006) 的"实在论综合"**而非统计元分析：
- 深入理解每次干预的"受试者"、研究者提出的因果理论、结果质量、测量充分性和过程障碍
- 研究综述应以因果理论构建为核心，而非以平均效应量计算为核心
...
1. **统计聚合逐级失真**：RCT 的简化在元分析中复合，在元-元分析（Hattie/Toolkit）中进一步被隐藏——"越聚合越空洞化"

2. **Fresh Start 案例显示"政治压力下的方法论妥协"**：EEF 作为政府资助方可能为迎合教育部长的偏好而突显统计上不存在的结果

3. **Toolkit 的效应量排名可能产生反效果**：将教学助理（因结构性使用不当而非方法无效）标签为"低影响"可在预算削减中被用于错误的人事决策

4. **教育中的"人类意志既是必要又是污染"悖论**使 RCT 方法论从根本上不适合——教学中的人类意图、信念和互动恰是教育效果产生的途径，不应被视为需要消除的噪声

5. **批判实在论为教育研究提供了超越 Hume 经验论的因果模型**：区分 real/actual/empirical 三层，追问 causal mechanisms 而非 regularity patterns
...
**批评内容**：Wrigley 推崇 Pawson (2006) 的"实在论综合"（realist synthesis）作为统计元分析的替代方案——但这比"计算平均效应量并放入排名表"复杂得多。实在论综合要求逐项理解每次干预的"受试者"特征、因果理论、结果质量、测量充分性和过程障碍——这对研究综述者是可操作的要求，但对于时间有限的一线教师和学校领导，如何将这种复杂性转化为可用的决策支持工具？
...
**影响**：此全面不信任的基调使 Wrigley 的论文在 EBE 方法论辩论中容易被对手定位为"反量化"而非"方法多元"——削弱了其作为建设性替代方案的可信度。Slavin (2019) 可以将 Wrigley 的立场所概括为"怀疑论者认为我们根本不应该进行教育实验"——尽管 Wrigley 的立场更为精细。

**目前状态**：Wrigley 的批判以高精度的 Fresh Start 解构为亮点（具体到单个 RCT 的数据分析），但其方法多元论的建设性阐述相对薄弱。在方法论辩论中，"批判方"与"建构方"的篇幅不对称是常见现象，但确实限制了论文在 EBE 支持者中的说服力。

### EEF Toolkit 受到过度简化对待

**批评内容**：Wrigley 对 EEF Toolkit 的批评（将教学助理标记为"低影响/高成本"、Fresh Start 案例的行政摘要与全报告差异）具有方法论价值。但读者可能注意：Toolkit 的官方文档明确声明其效应量排名"应补充而非替代专业判断"，且全报告（而非行政摘要）通常包含 Wrigley 要求的更细致的条件分析。

...
> [!line-a] 行为表征与实验案例
> - **A非B任务 (A-not-B Task)** 在 7–12 个月之间，婴儿在完成经典的 A非B任务（物体被隐藏在A处，婴儿数次找到；随后当面将其隐藏在B处，婴儿依然去A处寻找）和客体检索任务 (Object Retrieval Task) 时表现出逐步改善。多通道[[Electroencephalography\|脑电图]] (Electroencephalography, EEG) 研究显示，在成功执行此类任务时 PFC 被显著激活（p.9）。若 PFC 区域受损，则无法完成这两个任务。
> - **注意力与 EF 预测** 注意力 (Attention) 在约 4–6 个月开始显现，被认为是后期 EF 表现（如[[Inhibitory Control\|抑制控制]]和[[Working Memory\|工作记忆]]广度）的底层预测指标；12个月至4岁儿童的[[Cognitive Shifting\|认知转换]] (Cognitive Shifting) 表现高度依赖注意力控制（p.10）。
...
> - **转换速度的滞后性** 梅兰 (Meiran, 1996) 视觉转换任务显示，80% 的 11 岁儿童在 Post-switch 的准确率上达到成人水平，但其反应时间 (Response Time) 仍显著落后于 15 岁和成年人（p.14）。
> - **更新功能的逐步落后** 数字广度任务与视觉空间更新（如 n-back任务、视觉空间模式识别 (Visuospatial Pattern Recognition)）在 7–8 岁表现出阶段性提升，但工作记忆更新 (Updating) 整体发展较慢，不能在 11 岁时成熟。

> [!line-b] 脑发育生理机制
> - **PFC 灰质体积达峰 (倒U形 轨迹)**Giedd et al. (1999) 纵向 MRI 研究发现，PFC 灰质体积在女性 11 岁、男性 12 岁时达到一生中的顶峰，随后因青春期突触修剪而下降（p.13）。dlPFC 的修剪进程显著晚于大脑其他区域。
> - **白质的持续髓鞘化** 4–13 岁间，PFC 白质体积随着髓鞘增厚呈线性增长，使得轴突传导电位效率最大化。
> - **额纹状体通路 (Frontostriatal Pathway) 环路化** 利斯顿等 (Liston et al., 2006) 证明，PFC 与纹状体 (Striatum) 之间的结构性连接在此阶段显著增强，这一 frontostriatal 环路化直接预测了 Go/No-Go 任务的行为表现。fMRI 扫描显示，随着年龄增长，腹侧前额叶区域在抑制期间的激活强度不断提高。
> - **网络分化与化学调节** 8–9 岁时，执行控制网络内部的抑制环路与更新环路开始发生功能分离 (Segregation)。在多巴胺放电 (Firing) 增强的作用下，PFC 内部的抑制性中间神经元 (Inhibitory Interneurons) 对锥体细胞的负反馈调节作用显著增强（p.14）。

---

### 步骤四：形式运算阶段（11-16岁及以后）——皮层修剪与 EF 完全成熟

从 11–16 岁，儿童发展出抽象、[[Hypothesis|假设]]演绎和反射性智能 (Reflective Intelligence)，能够系统预判所有组合和结果。这直接标志着 EF 的最终成熟，在生理上对应着 dlPFC 第 III 层锥体细胞构筑的最终锁定，以及皮层选择性变薄（pp.15–17）。

> [!line-a] 行为表征与实验案例
> - **更新功能 (Updating) 的全面登顶** WCST（威斯康星卡片分类任务）是检验该阶段高级更新能力的典型任务。11岁以下儿童做该任务主要依赖抑制和转换得分预测；11岁之后，工作记忆更新成为唯一预测因子（p.14）。尽管 11 岁儿童的 WCST 错误率与成人无异，但完成分类的分类数要到 15–16 岁才达到成人水平。
> - **转换速度的最终登顶** 15 岁时，认知转换的反应时间 (Response Time) 与准确率同时达到成人水平，彻底克服了注意力惯性。
> - **抑制机制的微调** 大部分抑制任务在 11–12 岁已经稳定，但极其复杂的 Stroop任务 在 21 岁仍有极缓慢的改善。

> [!line-b] 脑发育生理机制
> - **皮层变薄 (Cortical Thinning) 与更新成绩** PFC 灰质体积在 11–12 岁达峰后剧烈减少（尤其是背外侧前额叶和顶叶皮层）（p.15）。塔姆内斯等 (Tamnes et al., 2013) 发现，双侧 PFC 的皮层厚度减少（突触修剪导致）直接关联着 Keep-track任务（更新功能）的成绩提升（p.16）；右侧腹外侧前额叶 (vlPFC) 和前扣带回 (Anterior Cingulate Cortex, ACC) 的皮层变薄预测了抑制反应时间与准确率的提高。
> - **Layer III 锥体细胞网络最终锁定** dlPFC 第 III 层锥体细胞的树突突触构筑在 16 岁时达到成人的稳定网络结构，与形式运算阶段 the end 完美契合。
> - **白质髓鞘化锁定与成熟网络招募** 白质体积在 11–16 岁及 20 多岁间持续线性增加（p.13）。fMRI 扫描显示，青少年在执行 EF 任务时，相较于儿童会招募更多的右侧 dlPFC、下前额叶 (inferior frontal lobes) 以及前扣带回 (ACC)（p.15, p.17），大脑的全局神经网络变得更具整合性和协同效能。

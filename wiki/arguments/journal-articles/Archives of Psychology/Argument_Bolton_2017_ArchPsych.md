...
> [!line-a] 行为表征与实验案例
> - **抑制与转换的线性飞跃** 抑制和认知转换在 7–11 岁表现出平稳的线性提升，并在 11 岁时基本成熟（p.13）。例如 Go/No-Go任务 (Go/No-Go Task) 中，8岁儿童极易因无法克制而按下 No-Go 键，而 10–12 岁儿童的抑制能力已接近成人（p.13）。
> - **转换速度的滞后性** 梅兰 (Meiran, 1996) 视觉转换任务显示，80% 的 11 岁儿童在 Post-switch 的准确率上达到成人水平，但其反应时间 (Response Time) 仍显著落后于 15 岁和成年人（p.14）。
> - **更新功能的逐步落后** 数字广度任务与视觉空间更新（如 n-back任务、视觉空间模式识别 (Visuospatial Pattern Recognition)）在 7–8 岁表现出阶段性提升，但工作记忆更新 (Updating) 整体发展较慢，不能在 11 岁时成熟（p.14）。

> [!line-b] 脑发育生理机制
> - **PFC 灰质体积达峰 (倒U形 轨迹)**Giedd et al. (1999) 纵向 MRI 研究发现，PFC 灰质体积在女性 11 岁、男性 12 岁时达到一生中的顶峰，随后因青春期突触修剪而下降（p.13）。dlPFC 的修剪进程显著晚于大脑其他区域（p.13）。
> - **白质的持续髓鞘化** 4–13 岁间，PFC 白质体积随着髓鞘增厚呈线性增长，使得轴突传导电位效率最大化（p.13）。
> - **额纹状体通路 (Frontostriatal Pathway) 环路化** 利斯顿等 (Liston et al., 2006) 证明，PFC 与纹状体 (Striatum) 之间的结构性连接在此阶段显著增强，这一 frontostriatal 环路化直接预测了 Go/No-Go 任务的行为表现（p.13）。fMRI 扫描显示，随着年龄增长，腹侧前额叶区域在抑制期间的激活强度不断提高（p.13）。
...
> [!line-a] 行为表征与实验案例
> - **更新功能 (Updating) 的全面登顶** WCST（威斯康星卡片分类任务）是检验该阶段高级更新能力的典型任务。11岁以下儿童做该任务主要依赖抑制和转换得分预测；11岁之后，工作记忆更新成为唯一预测因子（p.14）。尽管 11 岁儿童的 WCST 错误率与成人无异，但完成分类的分类数要到 15–16 岁才达到成人水平（p.14）。
> - **转换速度的最终登顶** 15 岁时，认知转换的反应时间 (Response Time) 与准确率同时达到成人水平，彻底克服了注意力惯性（p.14）。
> - **抑制机制的微调** 大部分抑制任务在 11–12 岁已经稳定，但极其复杂的 Stroop任务 在 21 岁仍有极缓慢的改善（p.14）。

> [!line-b] 脑发育生理机制
> - **皮层变薄 (Cortical Thinning) 与更新成绩** PFC 灰质体积在 11–12 岁达峰后剧烈减少（尤其是背外侧前额叶和顶叶皮层）（p.15）。塔姆内斯等 (Tamnes et al., 2013) 发现，双侧 PFC 的皮层厚度减少（突触修剪导致）直接关联着 Keep-track任务（更新功能）的成绩提升（p.16）；右侧腹外侧前额叶 (vlPFC) 和前扣带回 (Anterior Cingulate Cortex, ACC) 的皮层变薄预测了抑制反应时间与准确率的提高（p.16）。
> - **Layer III 锥体细胞网络最终锁定** dlPFC 第 III 层锥体细胞的树突突触构筑在 16 岁时达到成人的稳定网络结构，与形式运算阶段 the end 完美契合（p.15）。

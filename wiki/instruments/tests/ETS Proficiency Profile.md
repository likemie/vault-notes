---
title: ETS Proficiency Profile
aliases:
  - EPP
  - MAPP
  - 学术水平概况测验
  - 基础学业能力测验
  - Measure of Academic Proficiency and Progress
summary: "由美国教育考试服务中心（ETS）开发的通识教育与核心素养标准化测评工具，综合测量大学生在批判性思维、阅读、写作与数学领域的学业增益。"
type: instrument
instrument_type: test
part_of: ""
developers:
  - "Educational Testing Service"
original_year: "2010"
languages:
  - en
item_count: "108 items (Standard Form) / 36 items (Abbreviated Form)"
administration_mode: computer
response_format: "Multiple choice (4 options)"
tags:
  - instrument/test
  - theme/critical-thinking
  - theme/higher-order-thinking
  - standardized-testing
related_concepts:
  - "[[21st Century Skills and Competencies Discourse]]"
  - "[[General Education]]"
  - "[[Learning Gain]]"
  - "[[Critical Thinking]]"
  - "[[Hypothesis]]"
  - "[[Document]]"
  - "[[Reliability]]"
  - "[[Construct]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons: []
related_facts:
  - "[[Voluntary System of Accountability]]"
  - "[[OECD]]"
  - "[[Educational Testing Service]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# ETS Proficiency Profile

---

## 工具定位

> [!instrument-profile] ETS Proficiency Profile（EPP / 原 MAPP）
> - **工具类型** 标准化通识[[21st Century Skills and Competencies Discourse|核心素养]]测评（Standardized [[General Education]] Assessment）。
> - **开发者与年份** 美国教育考试服务中心（Educational Testing Service, ETS, 2010，前身为 MAPP）。
> - **测量目的** 评估高校通识教育课程质量与学生核心认知能力，作为[[Voluntary System of Accountability|美国自愿问责体系]]（VSA）核心推荐工具，测量大一入学到毕业的纵向[[Learning Gain|学习增益]]。
> - **实施方式** 计算机机考或纸笔测试（标准版 2 小时，简缩版 40 分钟）。

---

## 测量构念与维度

> [!construct-table] 通识[[21st Century Skills and Competencies Discourse|核心素养]]与[[Critical Thinking|批判性思维]]
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数（标准版） | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **批判性思维（Critical Thinking）** | 27 题 | 评估论点有效性、识别前提[[Hypothesis|假设]]、推导逻辑结论并处理复杂情境冲突。 | 机器自动计分（0–1 分） |
> | **阅读理解（Reading）** | 27 题 | 分析复杂学术与公共文本，提取修辞目的与核心观点。 | 机器自动计分（0–1 分） |
> | **书面表达（Writing）** | 27 题 | 识别语法谬误、句子结构问题与有效修辞策略。 | 机器自动计分（0–1 分） |
> | **数学推理（Mathematics）** | 27 题 | 解决代数、几何与数据统计中的定量推理问题。 | 机器自动计分（0–1 分） |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 基于人文、社会科学与自然科学真实[[Document|文献]]节选的情境选择题。
> - **作答格式** 4 选 1 客观单项选择题。
> - **计分与常模** 量表分范围为 400–500 分，提供 3 个熟练度水平等级（Proficiency Levels 1, 2, 3）与全美大学增值常模基准。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD|Bouckaert (2023)]] | EPP / MAPP 标准版 | [[OECD]] 报告综述全美大规模标准化考试在高等教育中的演化与局限 | 通识核心技能测试 | 证实 EPP 虽具备优异的心理测量学[[Reliability|信度]]与自动化评分优势，但在低利害问责中受制于学生动机不足与[[Construct|构念]]代表性局限 |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | MAPP 原始版 | 英语（美国） | 本科生各年级 | 108 题 / 36 题 | 确立通识四领域与[[Critical Thinking|批判性思维]]分量表结构 | ETS (2006) |
> | EPP 修订版 | 英语（全球） | 大学生与成人 | 108 题 / 36 题 | 更名为 [[Educational Testing Service|ETS]] Proficiency Profile，更新题库与在线自适应测评平台 | ETS (2010) |

---
title: Collegiate Assessment of Academic Proficiency
aliases:
  - CAAP
  - 大学学业能力测评
  - 大学生学业能力测评
  - Collegiate Assessment of Academic Proficiency (CAAP)
summary: "由美国大学入学考试机构（ACT）研发的标准化通识能力测评工具，其批判性思维模块通过33道情境单选题测量大学生分析要素、评估论证与拓展论点的能力。"
type: instrument
instrument_type: test
part_of: ""
developers:
  - "American College Testing"
original_year: "2006"
languages:
  - en
item_count: "33 items (Critical Thinking Module)"
administration_mode: computer
response_format: "Multiple choice (4 options)"
tags:
  - instrument/test
  - theme/critical-thinking
  - theme/higher-order-thinking
  - standardized-testing
related_concepts:
  - "[[General Education]]"
  - "[[Critical Thinking]]"
  - "[[Hypothesis]]"
  - "[[Causality]]"
  - "[[Reliability]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons: []
related_facts:
  - "[[Voluntary System of Accountability]]"
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# Collegiate Assessment of Academic Proficiency

---

## 工具定位

> [!instrument-profile] Collegiate Assessment of Academic Proficiency（CAAP）
> - **工具类型** 标准化通识能力与增值测验（Standardized [[General Education]] Assessment）。
> - **开发者与年份** 美国大学入学考试机构（American College Testing, ACT, 2006）。
> - **测量目的** 测量高等教育阶段本科生的通识核心学业技能与认知发展水平，作为[[Voluntary System of Accountability|美国自愿问责体系]]（VSA）官方认可的三大通用增值测验之一，评估大一至大四的学业增益。
> - **实施方式** 纸笔测试或计算机机考（[[Critical Thinking|批判性思维]]模块限时 40 分钟），题项基于真实情境阅读材料。

---

## 测量构念与维度

> [!construct-table] [[Critical Thinking|批判性思维]]（Critical Thinking Skills）
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **分析论证要素（Analysis of Elements）** | 10–12 题 | 识别论述中的基本要素，区分事实断言与价值判断，辨析核心[[Hypothesis|假设]]与[[Causality|因果关系]]。 | 机器阅卷（0–1 分） |
> | **评估论据与推理（Evaluation of Arguments）** | 10–12 题 | 评判论据的有效性、相关性与充分性，识别逻辑谬误与修辞偏差。 | 机器阅卷（0–1 分） |
> | **拓展论点与推导结论（Extension of Arguments）** | 9–11 题 | 将论点推导至新情境，预测可能后果，辨析竞争性假设与替代解释。 | 机器阅卷（0–1 分） |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 33 道基于科学研究、社会争议、哲学辩论等短文材料的情境选择题。
> - **作答格式** 4 选 1 客观单项选择题。
> - **计分与常模** 采用 50–70 分的量表分体系，提供基于全美数百所高校常模的百分位等级与院校增值评估报告。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD|Bouckaert (2023)]] | CAAP [[Critical Thinking|批判性思维]]模块 | [[OECD]] 报告综述分析[[Voluntary System of Accountability|美国自愿问责体系]]（VSA）中标准化增值测验的兴衰历程 | 标准化通识能力测试 | 指出 CAAP 作为高[[Reliability|信度]]客观测验虽便于横向对比，但因缺乏与具体学科课程的有机联系，难以直接指导基层教学改进 |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | CAAP 标准版 | 英语（美国） | 大学生各年级 | 33 题（[[Critical Thinking|批判性思维]]模块） | 确立分析、评估与拓展三大子维度的标准化施测常模 | ACT (2006) |

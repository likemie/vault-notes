---
title: California Critical Thinking Skills Test
aliases:
  - CCTST
  - 加州批判性思维技能测验
  - 加州批判性思维测验
  - California Critical Thinking Skills Test (CCTST)
summary: "基于美国哲学学会APA德尔菲共识报告开发的经典标准化批判性思维认知能力测验，包含分析、推论、评价、演绎与归纳五个核心分测验。"
type: instrument
instrument_type: test
part_of: ""
developers:
  - "Facione, P. A."
  - "Facione, N. C."
original_year: "1990"
languages:
  - en
  - zh
  - es
item_count: "34 items"
administration_mode: computer
response_format: "Multiple choice (4–5 options)"
tags:
  - instrument/test
  - theme/critical-thinking
  - theme/higher-order-thinking
  - cognitive-assessment
related_concepts:
  - "[[Critical Thinking]]"
  - "[[Hypothesis]]"
  - "[[Reliability]]"
  - "[[Higher-Order Thinking Skills]]"
  - "[[Internal Consistency]]"
  - "[[Pre-test and Post-test]]"
related_theories:
  - "[[APA Delphi Consensus on Critical Thinking]]"
related_methods:
  - "[[Intervention Research]]"
related_instruments: []
related_persons:
  - "[[Peter Facione]]"
related_facts:
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# California Critical Thinking Skills Test

---

## 工具定位

> [!instrument-profile] California [[Critical Thinking|critical thinking skills]] Test（CCTST）
> - **工具类型** 标准化能力测验（Standardized Cognitive Ability Test）。
> - **开发者与年份** [[Peter Facione|Peter A. Facione]] & Noreen C. Facione (1990, 2000 修订版)。
> - **测量目的** 依据美国哲学学会（APA）[[APA Delphi Consensus on Critical Thinking|德尔菲专家共识]]报告界定的批判性思维核心认知技能，测量个体在日常与专业情境中的分析、推论、评价与逻辑推导能力，广泛应用于高等教育学业增益评估与课程[[Intervention Research|干预研究]]。
> - **实施方式** 纸笔测试或计算机机考（限时 45–50 分钟），题项以文本、图表或短情境呈现。

---

## 测量构念与维度

> [!construct-table] [[Critical Thinking|批判性思维]]认知能力（Critical Thinking Cognitive Skills）
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **分析（Analysis）** | 6–9 题 | 识别文本与论证中各命题之间的意图与逻辑关系，提取核心论点、理由与前提[[Hypothesis|假设]]。 | 正确得 1 分，错误得 0 分 |
> | **推论（Inference）** | 10–14 题 | 识别并获取推导合逻辑结论所需的必要元素，形成推测并权衡相关信息。 | 正确得 1 分，错误得 0 分 |
> | **评价（Evaluation）** | 8–12 题 | 评判陈述或陈述间逻辑推论的置[[Reliability|信度]]、可信性与论据充足性。 | 正确得 1 分，错误得 0 分 |
> | **演绎推理（Deductive Reasoning）** | 10–14 题 | 依据既定规则与逻辑结构，判断从前提出发结论是否必然成立（从一般到特殊）。 | 正确得 1 分，错误得 0 分 |
> | **归纳推理（Inductive Reasoning）** | 10–14 题 | 依据观察事实与经验概率，评估结论在不确定情境下的合理性程度（从特殊到一般）。 | 正确得 1 分，错误得 0 分 |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 34 道基于短文材料、图表数据与日常争论情境的客观单项选择题。
> - **作答格式** 4 选 1 或 5 选 1 单项选择题。
> - **计分与常模** 每题 1 分，总分满分 34 分。测验提供基于全美及国际大学生群体的标准百分位常模（Percentiles）与能力等级切分点。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD|Bouckaert (2023)]] | CCTST 英文标准版 | [[OECD]] 成员国高等教育[[Higher-Order Thinking Skills|高阶思维]]评价政策与实践综述，考察心理测量学客观测试在大学学业评价中的应用边界 | 标准化心理测量测试 | 指出 CCTST 作为 APA 德尔菲传统的经典工具具备优良的[[Internal Consistency|内部一致性]][[Reliability|信度]]，但因受限于客观选择题格式，主要测度“选项再认”而无法直接捕获真实专业实践中的“主动论点生成” |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | Form A / Form B | 英语（美国） | 大学生与成人 | 34 题 | 原始经典版本，建立等值平行卷以支持前[[Pre-test and Post-test|后测]]实验设计 | Facione (1990, 1992) |
> | Form 2000 | 英语（全球） | 本科生及研究生 | 34 题 | 更新情境题干材料，强化归纳/演绎子量表与定量推理（Numeracy）跨学科适配 | Facione & Facione (2000) |
> | 中文修订版 | 中文（中国） | 中国高校大学生 | 34 题 | 跨文化语言调试与本土高校常模标定 | 罗清旭等 (2001) |

---
title: Halpern Critical Thinking Assessment
aliases:
  - HCTA
  - 霍尔珀恩批判性思维测评
  - 霍尔珀恩批判性思维测验
  - Halpern Critical Thinking Assessment (HCTA)
summary: "采用生成式开放情境题与客观选择题双重作答格式的批判性思维测评工具，测量言语推理、论证分析、假设检验、概率不确定性分析以及问题解决与决策制定五大认知领域。"
type: instrument
instrument_type: test
part_of: ""
developers:
  - "Halpern, D. F."
original_year: "2010"
languages:
  - en
  - zh
  - es
  - de
item_count: "25 scenarios (50 items: 25 constructed-response + 25 multiple-choice)"
administration_mode: computer
response_format: "Constructed response (open-ended short answer) and multiple-choice / ranking"
tags:
  - instrument/test
  - theme/critical-thinking
  - theme/higher-order-thinking
  - cognitive-assessment
related_concepts:
  - "[[Critical Thinking Assessment]]"
  - "[[Critical Thinking]]"
  - "[[Hypothesis]]"
  - "[[Variable]]"
  - "[[Causality]]"
  - "[[Ecological Validity]]"
  - "[[Reliability]]"
  - "[[Higher-Order Thinking Skills]]"
related_theories: []
related_methods: []
related_instruments:
  - "[[Cognitive Ability Test KFT]]"
related_persons: []
related_facts:
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# Halpern Critical Thinking Assessment

---

## 工具定位

> [!instrument-profile] Halpern [[Critical Thinking Assessment]]（HCTA）
> - **工具类型** 混合型情境化[[Cognitive Ability Test KFT|认知能力测验]]（Mixed-Format Cognitive Assessment）。
> - **开发者与年份** Diane F. Halpern (2010)。
> - **测量目的** 评估个体在日常、社会与工作复杂情境中综合运用[[Critical Thinking|批判性思维]]技能解决劣构问题的能力，有效克服传统选择题与纯论文测试的方法学局限。
> - **实施方式** 计算机在线施测（限时约 60–90 分钟），题项基于生活、医疗决策、公共政策等生动情境。

---

## 测量构念与维度

> [!construct-table] [[Critical Thinking|批判性思维]]（Critical Thinking Across Five Cognitive Domains）
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **言语推理（Verbal Reasoning）** | 5 个情境（10 题） | 识别修辞陷阱、歧义、情绪化用词以及语言误导的能力。 | 开放题量规打分（0–5 分）+ 选择题客观计分 |
> | **论证分析（Argument Analysis）** | 5 个情境（10 题） | 识别前提与结论、评估论据对结论的支持力度、辨析逻辑谬误的能力。 | 开放题量规打分（0–5 分）+ 选择题客观计分 |
> | **[[Hypothesis\|假设]]检验（Hypothesis Testing）** | 5 个情境（10 题） | 理解科学研究设计逻辑、识别控制[[Variable\|变量]]、区分相关与[[Causality\|因果关系]]的能力。 | 开放题量规打分（0–5 分）+ 选择题客观计分 |
> | **概率与不确定性分析（Likelihood and Uncertainty）** | 5 个情境（10 题） | 运用基准概率、样本代表性与贝叶斯思维评估事件发生可能性的能力。 | 开放题量规打分（0–5 分）+ 选择题客观计分 |
> | **问题解决与决策制定（Problem Solving and Decision Making）** | 5 个情境（10 题） | 生成替代备选方案、权衡多目标利弊并做出合理决策的能力。 | 开放题量规打分（0–5 分）+ 选择题客观计分 |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **双阶段作答机制（Dual-Response Format）** 包含 25 个日常真实情境。每个情境首先要求受测者进行**开放式主动生成（Constructed-Response）**（如提出可能原因或批评漏洞）；作答提交后系统显示第 2 问，要求受测者在预设选项中进行**选择再认与排序（Forced-Choice Recognition）**。
> - **计分体系** 开放生成题依据精细化量规打分（可使用自动化自然语言处理评分或人工评分），选择题自动判分；输出生成分、再认分与总[[Critical Thinking|批判性思维]]分数。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD\|Bouckaert (2023)]] | HCTA 计算机版 | [[OECD\|经合组织]]高等教育评价综述，分析混合格式测评工具在[[Ecological Validity\|生态效度]]与心理测量[[Reliability\|信度]]之间的平衡优势 | 混合格式[[Higher-Order Thinking Skills\|高阶思维]]测评 | 证明 HCTA 成功弥合了“主动生成”与“被动再认”之间的认知鸿沟，是兼具生态效度与测量信度的高教测评范例 |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | 原始标准版 | 英语（美国/全球） | 高中生、大学生及成人 | 25 情境 / 50 题 | 确立双阶段作答格式与五大认知维度 | Halpern (2010) |
> | 多语言机考版 | 西班牙语、德语、中文等 | 国际高校学生 | 25 情境 / 50 题 | 本地化情境语义调试，支持智能辅助评分系统 | Halpern (2016) |

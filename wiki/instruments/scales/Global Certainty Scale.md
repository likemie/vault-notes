---
title: Global Certainty Scale
aliases:
  - 全局确定性量表
  - 全局知识确定性量表
summary: "用于测量个体对科学知识确定性与绝对真理假设的七题自陈量表，涵盖可证伪性与绝对真理两大取向，广泛应用于中学到大学过渡期的认识论信念追踪"
type: instrument
instrument_type: scale
part_of: ""
developers:
  - "Schiefele, U."
  - "Moschner, B."
  - "Husstegge, R."
  - "Trautwein, U."
  - "Lüdtke, O."
original_year: "2002"
languages:
  - German
  - English
item_count: "7"
administration_mode: self-report
response_format: "4点李克特量表（1=完全不同意，4=完全同意）"
tags:
  - theme/epistemological-beliefs
  - instrument/scale
  - field/educational-psychology
related_concepts:
  - "[[Likert Scale]]"
  - "[[Epistemological Beliefs]]"
  - "[[Falsification]]"
  - "[[Questionnaire]]"
  - "[[Certainty of Knowledge]]"
  - "[[Variable]]"
  - "[[Epistemology]]"
related_theories: []
related_methods:
  - "[[Coding in Qualitative Research]]"
related_instruments: []
related_persons: []
related_facts: []
related_arguments:
  - "[[Argument_Trautwein_2007_CEP]]"
confidence: high
status: active
created: 2026-08-19
updated: 2026-08-19
---

# Global Certainty Scale

---

## 工具定位

> [!instrument-profile] 全局确定性量表
> - **工具类型** 7 题自陈式[[Likert Scale|李克特量表]]。
> - **开发者与年份** Schiefele et al. (2002), [[Argument_Trautwein_2007_CEP|Trautwein & Lüdtke (2007)]]。
> - **测量目的** 测量个体对科学知识确定性、稳定性和绝对性的底层[[Epistemological Beliefs|认识论信念]]，评估其对科学理论[[Falsification|可证伪性]]与人类认知边界的理解水平。
> - **实施方式** 纸笔或自陈[[Questionnaire|问卷]]。

---

## 测量构念与维度

> [!construct-table] [[Certainty of Knowledge|知识的确定性]]
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | 全局科学知识确定性 | 7 | 对科学理论的绝对性、不变性、终极真理性以及[[Falsification\|可证伪性]]的总体认知预设 | 4 点计分均值（反向题反向[[Coding in Qualitative Research\|编码]]后求均值或构建潜[[Variable\|变量]]；高分代表高确定性/低[[Epistemology\|认识论]]成熟度） |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 针对科学知识与科学研究属性的陈述句。
> - **作答格式** 4 点[[Likert Scale|李克特量表]]（1 = 完全不同意/totally disagree，2 = 比较不同意，3 = 比较同意，4 = 完全同意/totally agree）。
> - **反向题与跳题** 第 1、2、3 题为反向计分题（强调科学理论的[[Falsification|可证伪性]]与修正性）；建模时建议自由估计反向题之间的残差相关以控制方法效应。

### 全局科学知识确定性

> [!ref-table]- 题项（引自 Trautwein & Lüdtke, 2007, p. 355）
> <span class="instrument-item-table-marker" aria-hidden="true"></span>
>
> | 编号 | 题项 | 作答选项 | 计分 | 来源 |
> |---|---|---|---|---|
> | 1 | 科学理论随时可能被证明是错误的（Scientific theories can be proven false at any time） | 1–4 点 | 反向计分 | [[Argument_Trautwein_2007_CEP\|Trautwein & Lüdtke (2007, p. 355)]] |
> | 2 | 我们目前认为正确的科学理论在未来可能被证明是错误的（Scientific theories that we presently consider to be correct can be proven false in the future） | 1–4 点 | 反向计分 | — |
> | 3 | 即使是科学知识也必须一次又一次地被修正（Even scientific knowledge must be revised time and again） | 1–4 点 | 反向计分 | — |
> | 4 | 在某个阶段，科学家将能够解释整个世界（At some stage, scientists will be able to explain the whole world） | 1–4 点 | 正向计分 | — |
> | 5 | 科学研究表明对于大多数问题都有一个明确的答案（Scientific research shows that for most problems there is one clear-cut answer） | 1–4 点 | 正向计分 | — |
> | 6 | 科学定律是普遍真理（Scientific laws are universal truths） | 1–4 点 | 正向计分 | — |
> | 7 | 科学知识是无可置疑的（Scientific knowledge is unimpeachable） | 1–4 点 | 正向计分 | — |

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Trautwein_2007_CEP\|Trautwein & Lüdtke (2007)]] | 德文改编 7 题版 | 德国巴登-符腾堡州 90 所高级文理中学（Gymnasium）高三学生（T1 $N = 2854$）及两年后大学追踪样本（T2 $N = 1094$）。用于测量科学知识确定性信念，检验其对学业成绩的预测力及大学专业选择中的选择与社会化效应 | 内部一致性：T1 $\alpha = .75$；T2 $\alpha = .73$。结构方程模型中自由估计 3 道反向题残差相关后模型拟合优度良好（$\text{CFI}/\text{TLI} > .90, \text{RMSEA} < .05$） | T1 总体均值 $M = 1.86, SD = 0.48$；跨期稳定性路径系数 $b = .54$；显著负向预测毕业成绩（$\beta = -.15$） |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | SMILE 项目德文原版 | 德语（德国） | 中学生与大学生 | — | 基于 Hofer (2000) 与 Schommer (1990) 的德文翻译与改编 | Schiefele et al. (2002) |
> | TOSCA 项目 7 题修订版 | 德语/英语对照（德国） | 高中毕业生与大学低年级学生 | 7 | 增加强调科学理论[[Falsification\|可证伪性]]的具体题项，适用于泛科学领域（包含自然科学与人文社会科学） | [[Argument_Trautwein_2007_CEP\|Trautwein & Lüdtke (2007)]] |

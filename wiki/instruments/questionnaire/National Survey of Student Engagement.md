---
title: National Survey of Student Engagement
aliases:
  - NSSE
  - 全美学生投入调查
  - 美国大学生学习投入调查
  - National Survey of Student Engagement (NSSE)
summary: "由乔治·库（George Kuh）领衔、印第安纳大学高等教育研究中心常态化运营的全美标志性大学生体验调查，涵盖高级学习、反思性与整合性学习等核心分量表，是全球数十个国家学生投入调查的直接蓝本。"
type: instrument
instrument_type: questionnaire
part_of: ""
developers:
  - "Kuh, G."
  - "Center for Postsecondary Research"
original_year: "2000"
languages:
  - en
  - zh
item_count: "80–100 items (depending on updated modules)"
administration_mode: self-report
response_format: "4-point and 5-point Likert scale"
tags:
  - instrument/questionnaire
  - theme/student-engagement
  - theme/higher-order-thinking
  - higher-education
related_concepts:
  - "[[Student Engagement]]"
  - "[[Surface and Deep Learning]]"
  - "[[Questionnaire]]"
  - "[[Reflexivity]]"
  - "[[Homework]]"
  - "[[Paradigm]]"
  - "[[Higher-Order Thinking Skills]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons: []
related_facts:
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# National Survey of Student Engagement

---

## 工具定位

> [!instrument-profile] National Survey of [[Student Engagement]]（NSSE）
> - **工具类型** 全国性大学生有效学习投入与学业挑战自陈调查系统（National Student Engagement Survey Instrument）。
> - **开发者与年份** George Kuh 团队 / 印第安纳大学高等教育研究中心（Center for Postsecondary Research, CPR, 2000）。
> - **测量目的** 测量大一新生与大四毕业生在促进[[Surface and Deep Learning|深层学习]]、高阶思辨与全面发展的有效教育活动中的投入时间与精力。
> - **实施方式** 年度在线标准化[[Questionnaire|问卷]]施测，覆盖全美与加拿大逾 1,600 所四年制高等院校数百万本科生。

---

## 测量构念与维度

> [!construct-table] 有效教育实践与高阶认知投入（Engagement Indicators）
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 核心维度 | 测量内容 | 题项样例 |
> |---|---|---|
> | **高级学习（Higher-Order Learning）** | 考核课程要求学生应用理论、分析数值/文本证据、评估对立观点以及形成新见解的频率与深度。 | “课程要求您将概念应用于实际问题解决的程度”；“课程要求您评估论据有效性的程度”。 |
> | **[[Reflexivity\|反思性]]与整合性学习（Reflective & Integrative Learning）** | 跨学科联系知识、结合多重视角审视问题、反思自身先入之见的实践频率。 | “将不同学科的观念整合进[[Homework\|作业]]中”；“依据新证据改变个人原有理解”。 |
> | **定量推理（Quantitative Reasoning）** | 运用数据图表解决复杂社会或学术现实问题的能力。 | “使用统计数据得出合乎逻辑的结论”。 |
> | **学术挑战与生师互动（Academic Challenge & Faculty Interaction）** | 课程阅读量、学术写作负荷以及与教师就课程反馈与学术研究开展深度互动的频次。 | “与教授讨论职业规划与学术课题”。 |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **量表格式** 采用 4 点 Likert 频率评定（1-从不、2-偶尔、3-经常、4-非常频繁）与学业挑战等级。
> - **诊断报告体系** 生成 10 项有效投入指标（Engagement Indicators）与 6 项高影响教育活动（High-Impact Practices, HIPs）对标常模报告。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD\|Bouckaert (2023)]] | NSSE 指标框架 | [[OECD]] 报告分析国家级学生调查作为高教质量监测信息工具的全球演进[[Paradigm\|范式]] | 全球[[Student Engagement\|学生投入]]基准调查 | 证实 NSSE 高级学习分量表为各国监测[[Higher-Order Thinking Skills\|高阶思维]]教学提供了最具代表性的实证工具母体 |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | NSSE 经典版 | 英语（北美） | 四年制本科生 | 85 题 | 确立 5 大传统基准指标（五大投入台阶） | Kuh (2000) |
> | NSSE 更新版 | 英语/中文等多语种 | 全球高校本科生 | 100 题 | 升级为 10 大投入指标体系，强化高级学习与反思整合维度 | CPR (2013) |

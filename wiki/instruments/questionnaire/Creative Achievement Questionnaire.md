---
title: Creative Achievement Questionnaire
aliases:
  - 创造性成就问卷
  - 自陈创造性成就问卷
  - CAQ
summary: "由卡森等人于 2003 年开发的自陈式客观创造性成果评估工具，涵盖视觉艺术、音乐、文学、发明等 10 大领域的终身创造性成就与社会公认标志。"
type: instrument
instrument_type: questionnaire
part_of: ""
developers:
  - Carson, S. H.
  - Peterson, J. B.
  - Higgins, D. M.
original_year: "2003"
languages:
  - 英语
  - 中文
item_count: "96"
administration_mode: self-report
response_format: 多级等级勾选与社会公认成就加权计分
tags:
  - field/educational-psychology
  - theme/creativity
  - instrument/questionnaire
  - construct/creative-achievement
related_concepts:
  - "[[Creativity]]"
  - "[[Questionnaire]]"
  - "[[Divergent Thinking]]"
  - "[[Document]]"
  - "[[Construct Validity]]"
  - "[[Variable]]"
  - "[[Critical Thinking]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons: []
related_arguments:
  - "[[Argument_Runco_2026_CRJ]]"
  - "[[Argument_Park_2026_TSC]]"
confidence: high
status: draft
created: 2026-08-24
updated: 2026-08-25
---

# Creative Achievement Questionnaire

---

## 工具定位

> [!instrument-profile] 自陈[[Creativity|创造性]]成就[[Questionnaire|问卷]]（CAQ）
> - **工具类型** 终身创造性成就自陈问卷（Creative Achievement Inventory）。
> - **开发者与年份** Carson, Peterson, & Higgins (2003)。
> - **测量目的** 评估个体在 10 大特定艺术与科学领域的真实客观创造性成就，作为创造力研究中衡量“产品/成果（Product/Outcome）”效标的行业基准工具。
> - **实施方式** 纸笔或计算机化自陈作答。

> [!citation-card]- 关键表述
> 自陈创造性成就问卷作为创造力心理测量学中最具代表性的客观产出测量工具，有效解决了传统纸笔[[Divergent Thinking|发散思维]]测验难以直接反映现实世界高阶创造表现的效标难题。[[Argument_Runco_2026_CRJ|(Runco et al., 2026, pp. 3–4)]]
>
> *The Creative Achievement Questionnaire provides an objective, domain-specific measure of real-world creative accomplishments across ten distinct domains of human endeavor.*

---

## 测量构念与维度

> [!construct-table] [[Creativity|创造性]]成就与成果表现
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **视觉艺术（Visual Arts）** | 8 题 | 绘画、雕塑等艺术品展览、售出与获奖经历 | 等级加权求和（0–7 分阶梯加权） |
> | **音乐创作（Music）** | 8 题 | 乐曲创作、乐器演奏录音发行及专业奖项 | 等级加权求和 |
> | **舞蹈（Dance）** | 8 题 | 编舞、专业舞蹈演出及公开编导成就 | 等级加权求和 |
> | **建筑设计（Architectural Design）** | 8 题 | 建筑物/景观设计、施工落地及专业奖项 | 等级加权求和 |
> | **创意写作（Creative Writing）** | 8 题 | 诗歌、小说、剧本出版及国家级/省级文学奖 | 等级加权求和 |
> | **幽默（Humor）** | 8 题 | 脱口秀表演、喜剧剧本创作及商业演艺 | 等级加权求和 |
> | **发明制作（Inventions）** | 8 题 | 实用新型与发明专利申请、授权及商业转化 | 等级加权求和 |
> | **科学发现（Scientific Discovery）** | 8 题 | 学术论文发表、同行评审期刊引用及科研奖励 | 等级加权求和 |
> | **戏剧与影视（Theater and Film）** | 8 题 | 话剧影视编剧、导演、主演及公开发行 | 等级加权求和 |
> | **烹饪艺术（Culinary Arts）** | 8 题 | 原创菜谱开发、专业烹饪比赛获奖与出版 | 等级加权求和 |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 每个领域包含 8 个按社会公认成就层级由低到高排列的陈述句（从“0 = 我在该领域没有受过任何训练”到“7 = 我的成就获得了全国性/国际性专业权威奖项认可”）。
> - **作答格式** 被试针对自己在该领域曾经达成过的所有成就进行多项勾选。
> - **计分规则** 采用非线性加权求和法，权重随成就公认度呈几何级数增加；可计算单个领域的分数，亦可计算跨 10 大领域的总成就指数。

### 科学发现领域示例

> [!ref-table]- 题项
> <span class="instrument-item-table-marker" aria-hidden="true"></span>
>
> | 编号 | 题项陈述（科学发现维度） | 作答选项 | 计分 | 来源 |
> |---|---|---|---|---|
> | 0 | 我在科学领域没有接受过专门训练或没有展现过特殊兴趣。 | 勾选 | 0 分 | Carson et al. (2003) |
> | 1 | 我经常阅读科学[[Document|文献]]并自主开展科学小实验。 | 勾选 | 1 分 | 同上 |
> | 2 | 我在中小学或大学举办的科学竞赛中获得过奖励。 | 勾选 | 2 分 | 同上 |
> | 3 | 我在同行评审的学术期刊上独立或合作发表过研究论文。 | 勾选 | 3 分 | 同上 |
> | 4 | 我的科研成果被国家级或国际权威专业媒体报道。 | 勾选 | 4 分 | 同上 |
> | 5 | 我的学术成果获得了国家级学会或政府颁发的科技奖项。 | 勾选 | 7 分 | 同上 |

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | 同上 | 原始英文版（96 题） | $N = 296$ 名哈佛大学学生；效度验证与潜抑制机制研究 | 重测信度 $r = 0.81$；与专家评定产品效度 $r = 0.59$ | 确立 CAQ 优秀的[[Construct Validity|构念效度]]与跨领域预测力 |
> | Yörük & Sen (2022) | 跨语言与修订版 | 56 个独立实证样本元分析（$N = 18,412$） | 信度概化元分析显示均值 $lpha = 0.85$；重测信度 $r = 0.81$ | 证实 CAQ 跨文化与跨群体的测量稳健性 |
> | [[Argument_Runco_2026_CRJ|Runco et al. (2026)]] | 产出测量基准工具 | 52 项二阶元分析；用于区分预测源（$r=0.29$）与效标（$r=0.12$） | 产出/成果维度二阶合并关联系数 $r = 0.12$ | 揭示[[Creativity|创造性]]成就受多重外在环境[[Variable|变量]]制约 |
> | [[Argument_Park_2026_TSC|Park et al. (2026)]] | — | 元分析中 Lu et al. (2023) 以 CAQ 作为主观创造力成就测量报告其与[[Critical Thinking|批判性思维]]倾向的相关 | — | 采用主观创造力测量的相关显著更高（$Q_M(1) = 10.133, p = .002$） |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | 原始 CAQ | 英语（美国/加拿大） | 青年与成人 | 96 题 | 奠定 10 大领域 8 级阶梯成就结构 | 同上 |
> | 中文修订版 | 中文（中国大陆/台湾） | 大学生与科研人员 | 96 题 | 针对本土学术与艺术奖励体系微调题项表述 | 张景焕等 (2012) |

---
title: Course Experience Questionnaire
aliases:
  - CEQ
  - 澳大利亚课程体验问卷
  - 课程体验问卷
  - Course Experience Questionnaire (CEQ)
summary: "由保罗·拉姆斯登（Paul Ramsden）开创、澳大利亚教育研究委员会（ACER）与澳大利亚政府标准化的全国性大学毕业生调查工具，包含优良教学、通用技能及适度工作量等核心分量表。"
type: instrument
instrument_type: questionnaire
part_of: ""
developers:
  - "Ramsden, P."
original_year: "1991"
languages:
  - en
  - zh
item_count: "23–30 items"
administration_mode: self-report
response_format: "5-point Likert scale"
tags:
  - instrument/questionnaire
  - theme/student-experience
  - theme/higher-order-thinking
  - higher-education
related_concepts:
  - "[[Questionnaire]]"
  - "[[Surface and Deep Learning]]"
  - "[[Rote Learning]]"
  - "[[Paradigm]]"
related_theories: []
related_methods: []
related_instruments: []
related_persons: []
related_facts:
  - "[[Australian Council for Educational Research]]"
  - "[[Graduate Outcomes Survey]]"
  - "[[OECD]]"
related_arguments:
  - "[[Argument_Bouckaert_2023_OECD]]"
confidence: high
status: completed
created: 2026-08-27
updated: 2026-08-27
---

# Course Experience Questionnaire

---

## 工具定位

> [!instrument-profile] Course Experience [[Questionnaire]]（CEQ）
> - **工具类型** 全国性大学课程体验与通用技能自陈调查问卷（Standardized Graduate Survey Instrument）。
> - **开发者与年份** Paul Ramsden (1991) / [[Australian Council for Educational Research|澳大利亚教育研究委员会]]（ACER）与澳大利亚联邦政府教育部门。
> - **测量目的** 评估大学毕业生对所学专业教学质量、考核评价方式与通用高阶技能（批判性分析、问题解决、沟通）获得的感知，作为澳大利亚毕业生成果调查（[[Graduate Outcomes Survey|GOS]]）的核心模块。
> - **实施方式** 毕业生毕业 4–6 个月后在线自陈施测（5 点莱克特等级评定）。

---

## 测量构念与维度

> [!construct-table] 课程教学体验与高阶通用技能
> <span class="instrument-dimension-table-marker" aria-hidden="true"></span>
>
> | 维度 | 题项数 | 测量内容 | 计分方式 |
> |---|---|---|---|
> | **通用技能（Generic Skills Scale）** | 6 题 | 评估课程在培养批判性分析、复杂问题解决、书面表达与团队合作等可迁移素养上的有效性。 | 5 点 Likert 均值计分（1–5 分） |
> | **优良教学（Good Teaching Scale）** | 6 题 | 教师讲授清晰度、激发学术兴趣、提供指导性反馈与支持理解的程度。 | 5 点 Likert 均值计分 |
> | **清晰目标与标准（Clear Goals & Standards）** | 4 题 | 课程预期学习成果、考核要求与评分准则的公开透明度。 | 5 点 Likert 均值计分 |
> | **适度工作负荷（Appropriate Workload）** | 4 题 | 课程大纲与考核要求是否合理，避免过度拥挤导致的应试[[Surface and Deep Learning|表层学习]]。 | 5 点 Likert 均值计分 |
> | **适切评价（Appropriate Assessment）** | 3 题 | 考核是否重视高阶理解而非孤立[[Rote Learning|死记硬背]]。 | 5 点 Likert 均值计分 |

---

## 题项与作答方式

> [!instrument-items] 作答规则
> - **题项形式** 莱克特 5 点自陈量表题（从“1-强烈不同意”到“5-强烈同意”）。
> - **反向计分题** 在适度工作量与适切评价维度中设置反向题以控制作答偏差。
> - **通用技能核心题项样例** “本课程提升了我分析问题的主体能力”；“本课程磨炼了我批判性审视证据的习惯”。

---

## 使用该工具的研究

> [!ref-table]- 研究索引
> <span class="instrument-study-table-marker" aria-hidden="true"></span>
>
> | 研究 | 工具版本 | 样本与用途 | 测量属性 | 关键结果 |
> |---|---|---|---|---|
> | [[Argument_Bouckaert_2023_OECD|Bouckaert (2023)]] | CEQ 澳大利亚官方版 | [[OECD]] 报告分析国家级学生调查在高等教育质量监测中的应用[[Paradigm|范式]] | 全国毕业生调查工具 | 指出 CEQ 是全球高等教育运用学生主观感知数据监测通用高阶技能获得的最成熟系统之一 |

---

## 版本与适配

> [!ref-table]- 版本索引
> <span class="instrument-version-table-marker" aria-hidden="true"></span>
>
> | 版本 | 语言与地区 | 目标人群 | 题项数 | 主要变化 | 来源 |
> |---|---|---|---|---|---|
> | CEQ 23 题原始版 | 英语（澳大利亚/英国） | 大学本科毕业生 | 23 题 | 确立教学、技能、目标、负荷与评价 5 个经典分量表 | Ramsden (1991) |
> | CEQ 扩展版（[[Graduate Outcomes Survey|GOS]] 模块） | 英语（全球多国） | 高校毕业生各学段 | 30 题 | 强化学习共同体与学习资源维度，深度整合至澳大利亚毕业生成果调查（GOS） | ACER (2002, 2015) |

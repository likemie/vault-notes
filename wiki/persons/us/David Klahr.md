---
title: David Klahr
aliases:
  - 戴维·克拉尔
  - 大卫·克拉尔
  - Klahr, D.
summary: "美国卡耐基梅隆大学认知心理学与学习科学荣休讲席教授，科学推理与探究技能研究巨擘。提出科学发现双空间搜索模型（SDDS），开创变量控制策略（CVS）经典教学实验；其关于直接讲授与发现学习的对比研究引发了国际学习科学界长达二十年的大论辩"
type: person
nationality: "us"
person_region: "us"
person_related_count: 19
person_related_level: 2
person_related_stars: "⭐⭐"
person_related_color: "#e0e7ff"
born: "1939"
died: ""
lifespan: "1939–至今"
tags:
  - person
  - discipline/cognitive-psychology
  - discipline/learning-sciences
  - theme/science-education
  - theme/scientific-reasoning
related_concepts:
  - "[[Problem Solving]]"
  - "[[Control of Variables Strategy]]"
  - "[[Variable]]"
  - "[[Paradigm]]"
  - "[[Direct Instruction]]"
  - "[[Discovery Learning]]"
  - "[[International Education]]"
  - "[[Hypothesis]]"
  - "[[Metacognition]]"
  - "[[Learning Analytics]]"
  - "[[STEM Education]]"
  - "[[Inquiry-Based Learning]]"
  - "[[Scientific Method]]"
  - "[[Scientific Literacy]]"
related_arguments:
  - "[[Argument_DeJong_2023_ERR]]"
related_methods:
  - "[[Pre-test and Post-test]]"
  - "[[Meta-analysis]]"
related_facts:
  - "[[Australian Philanthropic Services]]"
  - "[[American Educational Research Association]]"
confidence: high
status: active
created: 2026-09-21
updated: 2026-09-21
---

# David Klahr

## 学者概述

> [!def] 学者生平与定位
> 戴维·克拉尔（David Klahr，1939–至今）是美国当代著名认知心理学家、学习科学与科学教育研究巨擘，卡耐基梅隆大学（Carnegie Mellon University, CMU）心理学系荣休讲席教授，美国心理科学协会（[[Australian Philanthropic Services|APS]]）与美国教育研究协会（[[American Educational Research Association|AERA]]）会士。Klahr 的毕生研究聚焦于儿童科学推理、[[Problem Solving|问题解决]]与概念发展的认知心理机制，其与凯文·邓巴（Kevin Dunbar）共同创立了著名的**科学发现双空间搜索模型（Scientific Discovery as Dual Search, SDDS）**，并开创了中小学[[Control of Variables Strategy|变量控制策略]]（Control of [[Variable|variables]] Strategy, CVS）的教学实验[[Paradigm|范式]]。其在 2004 年发表的关于[[Direct Instruction|直接教学]]与[[Discovery Learning|发现学习]]对比的实证研究，成为[[International Education|国际教育]]界二十年来争论教学法优劣的风暴中心（Klahr & Nigam, 2004; [[Argument_DeJong_2023_ERR|De Jong et al., 2023, pp. 4, 10]]）。

---

## 核心理论与方法学贡献

### 1. 科学发现双空间搜索模型（SDDS）
Klahr & Dunbar (1988) 突破传统行为主义框架，将科学探究解析为一个受目标驱动的信息加工系统。该模型将科学家的推理操作解构为在两个相互依存的问题空间中的启发式搜索：
- **[[Hypothesis|假设]]空间搜索（Search in Hypothesis Space）** 学习者调动已有先验知识、类比直觉或先前实验数据，生成、细化并重构待检验的因果假设；
- **实验空间搜索（Search in Experiment Space）** 针对特定假设，规划并设计具体物理或虚拟实验操作（如调整斜坡坡度或小球材质）；
- **证据评估（Evidence Evaluation）** 将实验观察结果与假设预期比对，决定是接受、修正还是彻底推翻既有假设。

### 2. 变量控制策略（CVS）实验范式
Klahr 团队设计了测量儿童科学探究能力的经典斜坡小球物理装置（Ramps Task）。学生需通过操纵斜坡高度、表面粗糙度、起点位置及小球质量，确定小球滑行距离的真正因果[[Variable|变量]]。Klahr 证明，[[Control of Variables Strategy|变量控制策略]]并非儿童自然自发成熟的生物本能，而是一项高度依赖教育干预的程序性[[Metacognition|元认知]]技能（Chen & Klahr, 1999）。

---

## 在直接教学与探究论战中的学术风暴

Klahr 的实证研究构成了[[International Education|国际教育]]学界探究争论中最具戏剧性的篇章：

> [!tension-table] Klahr & Nigam (2004) 实验与两派截然相反的解读
> | 争议维度 | 认知负荷与直接教学派解读（如 Kirschner et al., 2006; Zhang et al., 2022） | 学习科学与当代循证学派重构（[[Argument_DeJong_2023_ERR\|De Jong et al., 2023]]） |
> |---|---|---|
> | **实验基本发现** | 显性讲授 CVS 规则的学生在短期[[Pre-test and Post-test\|后测]]中通过率达 77%，远高于纯自主探索组（仅 23%）。 | 实验确证了显性指导能迅速传授孤立算法规则，但该实验并不构成“[[Direct Instruction\|直接讲授]]全面胜出”的证据。 |
> | **教学程序实质** | 证明由教师掌控全局的直接显性讲授是唯一科学有效的教学路径。 | **Klahr 采用的“直接教学”本身具有高度探究属性** 并非被动听讲，而是包含让学生亲自动手规划实验、主动评估实验成败、并在遭遇混淆时由教师给予即时纠错反馈的高互动探究设计（p. 4）。 |
> | **长效保持反转** | 忽视长期追踪，宣称直接讲授建立了稳固图式。 | **Dean & Kuhn (2007) 证实长效衰减** 在 13 周延迟后测中，直接讲授组遗忘严重（保持率仅 31%），而经历自主探究试错与反思的学生保持率高达 78%–83%（p. 4）。 |
> | **[[Meta-analysis|元分析]]定论** | 断言探究无法有效教授科学[[Variable|变量]]控制。 | **Schwichow et al. (2016) 72 项实验元分析证实** 显性讲授组（$g = 0.58$）与结构化探究组（$g = 0.65$）效应量无统计显著差异，直接讲授无相对优势（pp. 4–5）。 |

### 走出二元对立：多维教学空间模型
值得注意的是，Klahr 本人并未陷入极端的直接讲授原教旨主义。在后期研究中，Koedinger, Booth, & Klahr (2013) 提出了涵盖多种独立维度的综合教学空间矩阵。Klahr 明确指出，教学不应被窄化为“直接教学对决探究”的零和博弈，而应由教师根据具体学科概念、学生准备状态及学习目标阶段，灵活组合显性讲解、例题剖析与自主实验探究（[[Argument_DeJong_2023_ERR|De Jong et al., 2023, p. 10]]）。

---

## 主要著作

- Klahr, D. (2000). *Exploring Science: The Cognition and Development of Discovery Processes*. MIT Press.（科学推理与双空间搜索奠基专著）
- Klahr, D., & Dunbar, K. (1988). Dual space search during scientific reasoning. *Cognitive Science*, 12(1), 1–48.
- Chen, Z., & Klahr, D. (1999). All other things being equal: Acquisition and transfer of the [[Control of Variables Strategy]]. *Child Development*, 70(5), 1098–1120.
- Klahr, D., & Nigam, M. (2004). The equivalence of learning paths in early science instruction: Effects of [[Direct Instruction]] and [[Discovery Learning]]. *Psychological Science*, 15(10), 661–667.
- Koedinger, K. R., Booth, J. L., & Klahr, D. (2013). [[Learning Analytics|Educational Data Mining]] and learning analytics in [[STEM Education|Science, Technology, Engineering, and Mathematics Education]]. In *The Cambridge Handbook of Computing Education Research*.

---

## 相关条目网络

> [!entry-map]
> 
> | 条目 | 类型 | 关系 |
> |:-----|:-----|:-----|
> | [[Control of Variables Strategy]] | 概念 | Klahr 是变量控制策略教学实验设计与认知机制评估的开创者。 |
> | [[Inquiry-Based Learning]] | 概念 | Klahr 的实验深刻触发了学习科学界关于探究教学中指导时机与形式的全面反思。 |
> | [[Direct Instruction]] | 概念 | 其 2004 年论文常被直接讲授倡导派援引为核心证据，但后续研究深化了对其局限的认识。 |
> | [[Discovery Learning]] | 概念 | Klahr 实验中的对照组对比了无指导发现与显性讲授的习得差异。 |
> | [[Scientific Method]] | 概念 | 双空间搜索模型（SDDS）为科学方法的认知心理机制提供了严密的计算表征。 |
> | [[Scientific Literacy]] | 概念 | 掌握变量控制策略是学生形成现代科学素养与实证因果推理的核心门槛。 |
> | [[STEM Education]] | 概念 | Klahr 的实验装置被广泛融入中小学 STEM 跨学科实验与物理力学课程。 |
> | [[Argument_DeJong_2023_ERR\|De Jong et al. (2023)]] | 论证 | 权威综述重新审视 Klahr 实验，揭示其所谓直接教学实为高互动探究，并指出长效保持依赖探究。 |

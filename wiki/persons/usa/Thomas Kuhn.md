...
库恩的科学发展模型包含四个阶段：常规科学（normal science）→ 异常积累（anomalies）→ 危机（crisis）→ 科学革命（scientific revolution）即范式转换（paradigm shift）。新范式取代旧范式成为新的常规科学（[[Argument_Cohen_Manion_Morrison_2011_Routledge|Cohen et al., 2011]], Ch. 1）。

### 不可通约性

库恩指出范式之间具有[[Incommensurability|不可通约性]]（incommensurability）：不同范式之间没有共同的原则、标准或衡量尺度，因此一个人不能同时持有两个截然不同的范式（[[Argument_Cohen_Manion_Morrison_2011_Routledge|Cohen et al., 2011]], Ch. 1）。

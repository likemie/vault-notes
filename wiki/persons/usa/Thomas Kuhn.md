---
title: Thomas Kuhn
aliases:
  - 托马斯·库恩
  - Kuhn
  - T. S. Kuhn
  - Thomas S. Kuhn
summary: "美国科学史家和科学哲学家，以《科学革命的结构》（1962）提出范式概念和范式转换理论，深刻改变了人们对科学发展和研究方法的理解。"
type: person
tags:
  - person/philosopher
  - person/historian-of-science
  - theme/philosophy-of-science
related_concepts:
  - "[[Paradigm]]"
  - "[[Positivism]]"
  - "[[Postpositivism]]"
  - "[[Incommensurability]]"
related_theories:
  - "[[Complexity Theory]]"
related_methods:
  - "[[Qualitative Research]]"
  - "[[Mixed Methods Research]]"
related_persons: []
related_facts: []
related_arguments: []
confidence: high
status: draft
created: 2026-06-12
updated: 2026-06-12
---

# Thomas Kuhn

> [!info]- 自动维护字段
> - `related_*` 由脚本自动维护，AI 不手动填写。
> - 非 Argument 条目不写 YAML `sources` 和正文 `## 来源`。

## 基本信息

托马斯·库恩（Thomas Samuel Kuhn, 1922–1996）是美国科学史家和科学哲学家，对科学哲学、科学社会学和教育研究方法论产生了深远影响。

## 核心贡献

### 范式概念

库恩最具影响力的贡献是其在《科学革命的结构》（*The Structure of Scientific Revolutions*, 1962）中提出的**[[Paradigm|范式]]**（Paradigm）概念。他将范式定义为"一种被接受的模型或模式"（Kuhn, 1962: 23），包含以下多重含义(Cohen et al., 2011, Ch. 1)：

- 一种看待或研究现象的方式
- 一种世界观
- 一种关于什么算作被接受或正确的科学知识或工作方式的观点
- 一种共享的信念体系或原则集合
- 研究共同体的身份标识
- 对研究什么以及如何研究的共识

### 范式转换与科学革命

库恩的核心论点是，科学进步不是通过知识的平稳累积实现的，而是通过间断性的**科学革命**发生的。他的科学发展模式：

1. **常规科学**（normal science）：在一个被接受的范式内解谜
2. **异常积累**（anomalies）：出现现有范式无法解释的现象
3. **危机**（crisis）：异常积累到动摇范式的程度
4. **科学革命**（scientific revolution）：新范式取代旧范式——即**范式转换**（paradigm shift）

> [!example] 历史案例
> - 托勒密地心说 → 哥白尼日心说
> - 牛顿机械宇宙观 → 爱因斯坦相对论宇宙观
> - 价值无涉的[[Positivism|实证主义]]科学观 → [[Postpositivism|后实证主义]]科学观

### 不可通约性

库恩指出范式之间具有**[[Incommensurability|不可通约性]]**（incommensurability）：不同范式之间没有共同的原则、标准或衡量尺度，因此一个人不能同时持有两个截然不同的范式(Cohen et al., 2011, Ch. 1)。

### 范式共同体与权威

库恩将知识领域与学者共同体联系起来，强调一个范式的有效性不仅取决于证据，还取决于它在"权威人士"中所受到的尊重。这意味着科学不仅是认知活动，也是社会和政治活动。

## 对教育研究的影响

自库恩的奠基性工作以来，教育研究中的方法论取向一直被理解为存在于不同的[[Paradigm|范式]]和学者共同体中(Cohen et al., 2011, Ch. 1)。他的工作为理解以下议题提供了框架：

- 量化与[[Qualitative Research|质性研究]]之间的"范式战争"（Gage, 1989）
- [[Mixed Methods Research|混合方法研究]]作为"第三种研究范式"（Johnson and Onwuegbuzie, 2004）
- [[Complexity Theory|复杂性理论]]作为一种"新兴范式"（Morrison, 2008）


## 关键引用

> [!quote]
> 范式是一种"被接受的模型或模式"（Kuhn, 1962: 23），一种共享的信念体系或原则集合，一个研究共同体的身份标识，一种追求知识的方式，对研究什么问题以及如何研究这些问题的共识，典型的问题解决方案，以及一种比其竞争对手更容易被接受的理解。（第1章，p.133）
> Original: an 'accepted model or pattern' (Kuhn, 1962: 23), a shared belief system or set of principles, the identity of a research community, a way of pursuing knowledge, consensus on what problems are to be investigated and how to investigate them, typical solutions to problems, and an understanding that is more acceptable than its rivals.

...
「章节概览」按教材自身的知识推进逻辑组织内容，跟随作者每一步思路，记录概念、理论、证据、案例、表格、图片和结论如何依次展开。建立条目的内容只简单提及，不在教材 Argument 页展开；详细内容进入具体条目。
...
跟随教材自身逻辑，记录知识如何展开。表格必须复刻为 Markdown；只有无法转写的纯图像材料才按 `vault-schema.md` 写图片占位。
```
...
7. 更新或新建教材 Argument：
   ...
   - 表格必须复刻为 Markdown；只有无法转写的纯图像材料才按 `vault-schema.md` 写图片占位。
8. 在 `## 来源` 章节列出来源。
9. 执行脚本运行规则。
10. 当前章节处理完成后停止。
